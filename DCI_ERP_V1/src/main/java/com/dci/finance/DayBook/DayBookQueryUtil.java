...
public class DayBookQueryUtil {
	...
	public static String excelByAcctHead = "select transaction_no as transactionNo,(select emp_name  as createdBy from employee_master where created_by = created_by limit 1), cc.cost_center_name as costcenter,TO_CHAR(ledger_date,'dd/mm/yyyy') as transactionDate, "
...
*/
	 ...
	 * ; //modified at customer entity added entity as union at line no 227 
	 */
	public static final String GET_VIEW_REPORT3_QUERY =
			
			"with a as(	Select distinct on (general_ledger_id) general_ledger_id as  ledgerNo,TO_CHAR(LEDGER_DATE,'DD-MM-YYYY') as ledgerDate,   cash.sub_account_code as sub,(select cost_center_name  from cost_center where cost_center_id::text = GL.cost_center) as costCentername ,"
					...
					+ "						(COALESCE(TC_DEBIT,0)) as tcDebit,(COALESCE(TC_CREDIT,0)) as tcCredit,(COALESCE(BC_DEBIT,0)) as bcDebit, (COALESCE(BC_CREDIT,0)) as bcCredit ,GL.cost_center as costCenter,  get_customer_name_by_custid(GL.SUB_ACCOUNT_CODE) as subGroupName, "
					+ "						ac.acct_head_name as subGroupAddress, "
					/*+ "						case when (cash.sub_account_code is   null OR cash.sub_account_code= '') then  ac.acct_head_name else (select entity_name  from customer_entity  where customer_acct_code = cash.sub_account_code union select entity_name  from entity where supplier_acct_code = cash.sub_account_code limit 1)   end "*/
					+ "						ac.acct_head_name"
					+ "			 "
					+ "				as subGroupAddress1, "
					+ "			 "
					...
					+ "				) "
					+ "			 	select *, "
					+ "			 	case when transactionNo like '%DCI%' then "
					+ "			 	 "
					+ "		 case when null_or_empty(subGroupName::text) =false and bcDebit != 0 "
					+ "			  then  subGroupName   else subGroupAddress1 end "
					+ "			else "
					+ "			 "
					+ "			 case when null_or_empty(subGroupAddress::text) =false and bcDebit != 0 "
					+ "			  then  subGroupAddress   else subGroupAddress1 end "
					+ "			  end "
					+ "			   as subGroupAddress2, "
					+ "			  subGroupAddress,subGroupAddress1 from a    order by ledgerdate, transactionno, bcDebit desc  ;";
...
*/	/*
	...
	public static final String GET_VIEW_REPORT3_QUERY1 = 
			//"select TO_CHAR(LEDGER_DATE,'DD-MM-YYYY') as ledgerDate,TO_CHAR(GL.created_date,'DD-MM-YYYY') as createdDate, string_agg(account_code,' TO ') as accountHeadCode,transaction_no as transactionNo,String_agg(coalesce(GL.narration , ''),' ') as narration,string_agg(distinct CURRENCY_CODE,',') as currency,  em.first_name  as userBy, string_agg(get_customer_name_by_custid(SUB_ACCOUNT_CODE),' TO ') as subGroupName, SUM(COALESCE(BC_DEBIT,0)) as bcDebit, SUM(COALESCE(BC_CREDIT,0)) as bcCredit ,string_agg(COALESCE(case when e.is_college='t' then 'College' when e.is_college='f' then 'Others' end,''),' ') vochuertype,string_agg(ac.acct_head_name,' TO ') as subGroupAddress from general_ledger GL left join entity e on e.supplier_acct_code=SUB_ACCOUNT_CODE left join account_head_master ac on ac.acct_head_code=account_code left join employee_master em on em.emp_id=GL.created_by where Gl.company_id=? and ledger_date between to_date(?,'dd/mm/yyyy') and to_date(?,'dd/mm/yyyy') group by transaction_no,LEDGER_DATE,GL.created_date,em.first_name";
			
			/*"Select distinct on (general_ledger_id) general_ledger_id as  ledgerNo,TO_CHAR(LEDGER_DATE,'DD-MM-YYYY') as ledgerDate, case when (cash.sub_account_code is   null OR cash.sub_account_code= '') then  ac.acct_head_name else (select entity_name  from customer_entity  where customer_acct_code = cash.sub_account_code  limit 1)   end as subGroupAddress, " + 
			"TO_CHAR(GL.created_date,'DD-MM-YYYY') as createdDate,  account_code as accountHeadCode,   transaction_no as transactionNo, coalesce(GL.narration , '') as narration,CURRENCY_CODE as currency,  exchange_rate as exchangeRate,  cash.SUB_ACCOUNT_CODE as subAccountCode,   GL.parent_code as subgroup , "
			+ "SUM(COALESCE(TC_DEBIT,0)) as tcDebit,SUM(COALESCE(TC_CREDIT,0)) as tcCredit,SUM(COALESCE(BC_DEBIT,0)) as bcDebit, SUM(COALESCE(BC_CREDIT,0)) as bcCredit ,GL.cost_center as costCenter, " + "get_customer_name_by_custid(cash.SUB_ACCOUNT_CODE) as subGroupName, "
			+ "ac.acct_head_name as subGroupAddress1,  GL.created_by,em.first_name as userBy from general_ledger GL   left join entity e on  e.supplier_acct_code=SUB_ACCOUNT_CODE  left join account_head_master ac on  ac.acct_head_code=account_code left join cashbank_receipt_dtl  cash on  cash.voucher_no = transaction_no      left join employee_master em on  em.emp_id=GL.created_by " + " where  Gl.company_id=? and ledger_date between to_date(?,'dd/mm/yyyy')  and to_date(?,'dd/mm/yyyy') " + "group by LEDGER_DATE,general_ledger_id,e.entity_name,ac.acct_head_name,em.first_name,cash.SUB_ACCOUNT_CODE";
	*/
			...
			*/	
			// modified line no 304 added union of entity
			"with a as(	Select distinct on (general_ledger_id) general_ledger_id as  ledgerNo,TO_CHAR(LEDGER_DATE,'DD-MM-YYYY') as ledgerDate,   cash.sub_account_code as sub, (select cost_center_name  from cost_center where cost_center_id::text = GL.cost_center) as costCentername, "
			+ "			 			TO_CHAR(GL.created_date,'DD-MM-YYYY') as createdDate,  account_code as accountHeadCode,   transaction_no as transactionNo, coalesce(GL.narration , '') as narration,CURRENCY_CODE as currency,  exchange_rate as exchangeRate,  GL.SUB_ACCOUNT_CODE as subAccountCode,   GL.parent_code as subgroup , "
			+ "			 			(COALESCE(TC_DEBIT,0)) as tcDebit,(COALESCE(TC_CREDIT,0)) as tcCredit,(COALESCE(BC_DEBIT,0)) as bcDebit, (COALESCE(BC_CREDIT,0)) as bcCredit ,GL.cost_center as costCenter,  get_customer_name_by_custid(GL.SUB_ACCOUNT_CODE) as subGroupName, "
			+ "			 			ac.acct_head_name as subGroupAddress, "
