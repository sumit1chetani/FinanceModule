...
public class ManageLocationQueryUtil {
	...
	public static String update_ManageStores_Data = "UPDATE location SET location_name=?,parent_location_id=?,location_type=?,location_incharge=?,location_activity=?,is_scrap_location=?,is_active=?,modified_by=?,modified_date=?,address_id=?,is_parent_address=? WHERE location_id= ?";

	public static final String delete_ManageStoresaddress_data = "DELETE FROM address WHERE address_id = ? ";

	public static final String delete_ManageStores_data = "DELETE FROM location WHERE location_id = ?";

	public static final String sCheckLocationName = "SELECT count(*) FROM location WHERE LOWER(location_name)=LOWER(?) and type='LO'";
	...
	public static final String get_ParentLocation = "WITH RECURSIVE q AS (SELECT location_id ,cast(location_name as varchar(255)) as path FROM location WHERE parent_location_id is null UNION ALL SELECT ic.location_id,cast( q.path ||' / ' || cast(ic.location_name as varchar(255)) as  varchar(255)) FROM location ic JOIN q ON  ic.parent_location_id =  q.location_id where ic.type='LO') SELECT location_id as id,path as text FROM q";

	public static final String get_LocationType = "select def_table_id as id,value as text from def_table where form_field_id= '45'";

	public static final String get_LocationInCharge = "select emp_id as empId,concat(first_name,' ',surname) as locationIncharge from employee_master  employee inner join branch_department bd on bd.branch_id = employee.branch_department_id::text inner join branch b on b.brnch_id::text = bd.branch_id where b.cmpny_id = ?\r\n"  ;
	...
	public static final String getCityedit = "select a.cty_id cityId,a.address_id as addressId ,c.cty_nam city from address a,city c where a.city_id=c.cty_id and  a.address_id= ?";
	...
	public static final String getAssetLocationList = "WITH RECURSIVE q AS (SELECT location_id ,cast(location_name as varchar(255)) as path FROM location WHERE parent_location_id is null  and type='LO' and is_active=true  UNION ALL SELECT ic.location_id,cast( q.path ||' / ' || cast(ic.location_name as varchar(255)) as  varchar(255)) FROM location ic JOIN q  ON  ic.parent_location_id =  q.location_id  where type='LO' ) SELECT location_id as locationId,path as manageName  FROM q order by manageName";
}
