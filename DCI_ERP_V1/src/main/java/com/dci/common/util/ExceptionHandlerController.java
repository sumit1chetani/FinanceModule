...
public class ExceptionHandlerController {
	...
	@ResponseBody
	public BasicResultBean handleIntegrityException(Exception ex) {
		LOG.error("DataIntegrityViolationException handler executed", ex);
		BasicResultBean result = new BasicResultBean();
		result.setMessage("Record can not be deleted - related records exist");
		result.setSuccess(false);
		result.setType("error");
		return result;
	}
