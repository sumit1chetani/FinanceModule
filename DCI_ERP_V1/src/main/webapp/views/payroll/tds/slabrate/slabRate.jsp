		...
		<%@include file="/views/templates/panel-header.jsp"%>
		    	...
		    	<table id="dt_basic" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dt_basic_info">
		         <thead class="dataTables-Main-Head">
		          <tr>
		          <!--  <th class="width_1"></th> -->
		           <th class="sorting width_10" st-sort="financialYear">Financial Year</th>
		           <th class="sorting width_10" st-sort="taxPayerTypeName">Slab Type</th>
		           <th class="sorting width_10" st-sort="rangeFrom">Range From</th>
		           <th class="sorting width_10" st-sort="rangeTo">Range To</th>
		           <th class="sorting width_10" st-sort="rateInPercentage">Percentage</th>
         ...
         <!--   <td cs-select="objVesselMasterItem"></td> -->
           <td>{{slabRateCollection.financialYear}}</td>
			<td>{{slabRateCollection.taxPayerTypeName}}</td>
			<td>{{slabRateCollection.rangeFrom}}</td>
			<td>{{slabRateCollection.rangeTo}}</td>
			<td>{{slabRateCollection.rateInPercentage}}</td>
			
		 <td class=" td-actions text-center">
        <span>
           <security:authorize access="hasRole('${form_code}_${modify}')">   
         <i class="fa  fa-pencil text-success text" data-ng-click="editRow(slabRateCollection.taxRateId)"></i>
         </security:authorize>
        </span>
        <span>
        <security:authorize access="hasRole('${form_code}_${delete}')">
         <i class="fa fa-trash-o text-danger-dker text" data-ng-click="deleteRow(slabRateCollection.taxRateId)"></i>
         </security:authorize>
        </span>
       </td>
          </tr>
