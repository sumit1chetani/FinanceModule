        ...
        <table id="dt_basic"
         ...
         <tbody class="dataTables-Main-Body">
          <tr data-ng-class="$index % 2 == 0? 'even' : 'odd'"
           ng-repeat="objItemWiseReorderLevel in displayedCollection">
           <td cs-select="objItemWiseReorderLevel" class="text-center"></td>
           <td>{{objItemWiseReorderLevel.itemName}}</td>
           <td>{{objItemWiseReorderLevel.itemCategory}}</td>
           <td>{{objItemWiseReorderLevel.itemDescription}}</td>
           <td>{{objItemWiseReorderLevel.itemType}}</td>
           <td>{{objItemWiseReorderLevel.itemCode}}</td>
           <td class=" td-actions text-center"><security:authorize access="hasRole('${form_code}_${modify}')">
             <span> <i class="fa  fa-pencil text-success text"
              data-ng-click="editRow(objItemWiseReorderLevel)"></i>
             </span>
            </security:authorize> <security:authorize access="hasRole('${form_code}_${delete}')">
             <span> <i class="fa fa-trash-o text-danger-dker text"
              data-ng-click="deleteRow(objItemWiseReorderLevel,$index)"></i>
             </span>
