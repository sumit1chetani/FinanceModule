...
(function(ng, undefined) {
    ...
    ng.module('smart-table', []).run([ '$templateCache', function($templateCache) {
        $templateCache.put('template/smart-table/pagination.html', '<nav ng-if="numPages && pages.length >= 2"><ul class="pagination">' + '<li ng-repeat="page in pages" ng-class="{active: page==currentPage}"><a ng-click="selectPage(page)">{{page}}</a></li>' + '</ul></nav>');
    } ]);
    ...
    ng.module('smart-table').controller('stTableController', [ '$scope', '$parse', '$filter', '$attrs', function StTableController($scope, $parse, $filter, $attrs) {
         ...
         * 
         * @param {Function |
         *            String} predicate - function or string which will be used
         *            as predicate for the sorting
         * @param [reverse] -
         ...
         * @param {String}
         *            [predicate] - the property name against you want to check
         *            the match, otherwise it will search on all properties
         */
        this.search = function search(input, predicate) {
            ...
            tableState.pagination.start = 0;
            return this.multiWordSearch();
        };

        /**
         * this will chain the operations of sorting and multi word filtering
         * based on the current table state (sort options, multi word filtering,
         * ect)
         */
        // created by buvana
        this.multiWordSearch = function multiWordSearch() {
            var pagination = tableState.pagination;
            var output;
            if (tableState.search.predicateObject) {
                var input = tableState.search.predicateObject.$;
                if (input != undefined) {
                    var inputAry = input.split(' ');
                    filtered = safeCopy;
                    for ( var value in inputAry) {
                        if (inputAry[value] != '') {
                            var inputObject = new Object();
                            inputObject['$'] = inputAry[value];
                            filtered = filter(filtered, inputObject);
                        }
                    }
                } else {
         ...
         *            the name under which the custom order function is
         *            registered
         */
    ...
    ng.module('smart-table').directive('stSort', [ 'stConfig', '$parse', function(stConfig, $parse) {
        return {
            restrict : 'A',
            require : '^stTable',
            link : function(scope, element, attr, ctrl) {
