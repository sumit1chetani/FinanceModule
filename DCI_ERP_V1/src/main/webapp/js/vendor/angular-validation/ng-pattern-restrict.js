...
angular.module('ngPatternRestrict', []).value('ngPatternRestrictConfig', {
    showDebugInfo : true,
    ...
    return {
        ...
        compile : function uiPatternRestrictCompile() {
            ...
            return function ngPatternRestrictLinking(scope, iElement, iAttrs, ngModelController) {
                ...
                oldValue, // keeping track of the previous value of the
                // element
                caretPosition, // keeping track of where the caret is at to
                // avoid jumpiness
                // housekeeping
                initialized = false, // have we initialized our directive
                // yet?
                eventsBound = false, // have we bound our events yet?
                // functions
                getCaretPosition, // function to get the caret position, set
                // in detectGetCaretPositionMethods
                setCaretPosition; // function to set the caret position, set
                // in detectSetCaretPositionMethods

                // -------------------------------------------------------------------
                // caret position
                function getCaretPositionWithInputSelectionStart() {
                    return iElement[0].selectionStart; // we need to go under
                    // jqlite
                }

                function getCaretPositionWithDocumentSelection() {
                    // create a selection range from where we are to the
                    // beggining
                ...
                function getCaretPositionWithWindowSelection() {
                    var s = window.getSelection(), originalSelectionLength = String(s).length, selectionLength, didReachZero = false, detectedCaretPosition, restorePositionCounter;

                    do {
                        selectionLength = String(s).length;
                        s.modify('extend', 'backward', 'character');
                        // we're undoing a selection, and starting a new one
                        // towards the beggining of the string
                ...
                function setCaretPositionWithWindowSelection(position) {
                    var s = window.getSelection(), selectionLength;
                ...
                // HACK: Opera 12 won't give us a wrong validity status although
                // the input is invalid
                // we can select the whole text and check the selection size
                // Congratulations to IE 11 for doing the same but not returning
                // the selection.
                function getValueLengthThroughSelection(input) {
                    // only do this on opera, since it'll mess up the caret
                    // position
                ...
                function genericEventHandler(evt) {
                    ...
                    // HACK Chrome returns an empty string as value if user
                    // inputs a non-numeric string into a number type input
                    // and this may happen with other non-text inputs soon
                    // enough. As such, if getting the string only gives us an
                    // empty string, we don't have the chance of validating it
                    // against a regex. All we can do is assume it's wrong,
                    // since the browser is rejecting it either way.
                    /*
                     * var newValue = iElement.val(), inputValidity =
                     * iElement.prop("validity"); if (newValue === "" &&
                     * iElement.attr("type") !== "text" && inputValidity &&
                     * inputValidity.badInput) { showDebugInfo("Value cannot be
                     * verified. Should be invalid. Reverting back to:",
                     * oldValue); evt.preventDefault(); revertToPreviousValue(); }
                     * else if (newValue === "" &&
                     * getValueLengthThroughSelection(iElement[0]) !== 0) {
                     * showDebugInfo("Invalid input. Reverting back to:",
                     * oldValue); evt.preventDefault(); revertToPreviousValue(); }
                     * else
                     */
                ...
                function unbindListeners() {
                    ...
                    // keyup: DOM L3 spec, key released (possibly changing
                    // content)

                    iElement.unbind('click', genericEventHandler);
                    // click: DOM L3 spec, mouse clicked and released (possibly
                    // changing content)
