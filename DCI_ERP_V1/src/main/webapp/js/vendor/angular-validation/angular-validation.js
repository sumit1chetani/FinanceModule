...
(function() {
    angular.module('validation.provider', []).provider('$validation', function() {
        ...
        /**
         * Allow user to set a custom Expression, do remember set the default
         * message using setDefaultMsg
         * 
         * @param obj
         * @returns {*}
         */
        ...
        /**
         * Whether show the validation success message You can easily change
         * this to false in your config example:
        ...
        /**
         * Whether show the validation error message You can easily change this
         * to false in your config example: $validationProvider.showErrorMessage =
         * false;
         * 
         * @type {boolean}
         */
        ...
        /**
         * Check form valid, return true checkValid(Form): Check the specific
         * form(Form) valid from angular `$valid`
         * 
         * @param form
         * @returns {boolean}
         */
         ...
         * $get
         * 
         * @returns {{setErrorHTML: *, getErrorHTML: Function, setSuccessHTML: *,
         *          getSuccessHTML: Function, setExpression: *, getExpression:
         *          Function, setDefaultMsg: *, getDefaultMsg: Function,
         *          checkValid: Function, validate: Function, reset: Function}}
         */
...
(function() {
    angular.module('validation.directive', [ 'validation.provider' ]).directive('validator', [ '$injector', function($injector) {

        var $validationProvider = $injector.get('$validation'), $q = $injector.get('$q'), $timeout = $injector.get('$timeout');

        var sharedProperties = $injector.get('sharedProperties');
        ...
        var validFunc = function(element, validMessage, validation, scope, ctrl) {
            var messageElem, messageToShow = validMessage || $validationProvider.getDefaultMsg(validation).success;
            ...
            if ($validationProvider.showSuccessMessage && messageToShow) {
                ...
                messageElem.css('border', '1px solid #ccc');
            } else {
        ...
        var invalidFunc = function(element, validMessage, validation, scope, ctrl) {
            var messageElem, messageToShow = validMessage || $validationProvider.getDefaultMsg(validation).error;
            ...
            if ($validationProvider.showErrorMessage && messageToShow) {
                ...
                sharedObject[element[0].name] = messageToShow;
                messageElem.css('border', '1px solid red');
            } else {
                messageElem.css('display', 'none');
            }
        ...
        var checkValidation = function(scope, element, attrs, ctrl, validation, value) {
            ...
            var validators = validation.slice(0), validatorExpr = validators[0].trim(), paramIndex = validatorExpr.indexOf('='), validator = paramIndex === -1 ? validatorExpr : validatorExpr.substr(0, paramIndex), validatorParam = paramIndex === -1 ? null : validatorExpr.substr(paramIndex + 1), leftValidation = validators.slice(1), successMessage = validator + 'SuccessMessage', errorMessage = validator + 'ErrorMessage', expression = $validationProvider.getExpression(validator), valid = {
                success : function() {
        ...
        return {
            ...
            link : function(scope, element, attrs, ctrl) {
                ...
                /**
                 * Set initial validity to undefined if no boolean value is
                 * transmitted
                 */
                var initialValidity;
                ...
                /**
                 * Set custom initial validity Usage: <input
                ...
                scope.$on(ctrl.$name + 'reset-' + uid, function() {
                    ...
                    /**
                     * clear scope.$watch here when reset status clear the
                     * $watch method to prevent $watch again while reset the
                     * form
                     */
                    watch();
                ...
                (function() {
                    ...
                    scope.$on(ctrl.$name + 'submit-' + uid, function(event, index) {
                        var value = ctrl.$viewValue, isValid = false;
                    ...
                    sharedProperties.setErrorObject(sharedObject);
                    ...
                    /**
                     * Validate watch method This is the default method
                     */
                    scope.$watch('model', function(value) {
                        ...
                        if (ctrl.$pristine && ctrl.$viewValue) {
                            ...
                            // Don't validate form when the input is
                            // clean(pristine)
                            if (scope.messageId)
                                angular.element(document.querySelector('#' + scope.messageId)).html('');
                            else
    ...
    .directive('validationSubmit', [ '$injector', function($injector) {
        ...
        var $validationProvider = $injector.get('$validation'), $timeout = $injector.get('$timeout'), $parse = $injector.get('$parse');
