...
.directive('accordionGroup', [ '$parse', function($parse) {
    return {
        require : '^accordion', // We need this directive to be inside an
                                // accordion
        restrict : 'EA',
        transclude : true, // It transcludes the contents of the directive into
                            // the template
        replace : true, // The element containing the directive will be replaced
                        // with the template
        templateUrl : 'template/accordion/accordion-group.html',
        scope : {
            heading : '@'
        }, // Create an isolated scope and interpolate the heading attribute
            // onto this scope
        controller : function() {
    ...
    return {
        ...
        compile : function(element, attr, transclude) {
            return function link(scope, element, attr, accordionGroupCtrl) {
                ...
                // cloned so that they work in ng-repeat]
    ...
    self.select = function(nextSlide, direction) {
        ...
        if (nextSlide && nextSlide !== self.currentSlide) {
            if ($scope.$currentTransition) {
                ...
                // Timeout so ng-class in template has time to fix classes for
                // finished slide
                $timeout(goNext);
            } else {
                goNext();
        ...
        function goNext() {
            ...
            // If we have a slide to transition from and we have a transition
            // type and we're allowed, go
            if (self.currentSlide && angular.isString(direction) && !$scope.noTransition && nextSlide.$element) {
                // We shouldn't do class manip in here, but it's the same weird
                // thing bootstrap does. need to fix sometime
                nextSlide.$element.addClass(direction);
                var reflow = nextSlide.$element[0].offsetWidth; // force reflow

                // Set all other slides to stop doing their stuff for the new
                // transition
                ...
                // We have to create new pointers inside a closure since next &
                // current will change
 ...
 *          in slides" active="slide.active"> <img ng-src="{{slide.image}}"
 *          style="margin:auto;"> <div class="carousel-caption">
 *          <h4>Slide {{$index}}</h4>
 *          <p>
 *          {{slide.text}}
 *          </p>
 *          </div> </slide> </carousel> <div class="row-fluid"> <div
 *          class="span6">
 *          <ul>
 *          <li ng-repeat="slide in slides"> <button class="btn btn-mini"
 *          ng-class="{'btn-info': !slide.active, 'btn-success': slide.active}"
 *          ng-disabled="slide.active" ng-click="slide.active = true">select</button>
 *          {{$index}}: {{slide.text}} </li>
 *          </ul>
 *          <a class="btn" ng-click="addSlide()">Add Slide</a> </div> <div
 *          class="span6"> Interval, in milliseconds: <input type="number"
 *          ng-model="myInterval"> <br />
 *          Enter a negative number to stop the interval. </div> </div> </div>
 *          </file> <file name="script.js"> function CarouselDemoCtrl($scope) {
 *          $scope.myInterval = 5000; var slides = $scope.slides = [];
 *          $scope.addSlide = function() { var newWidth = 200 + ((slides.length +
 *          (25 * slides.length)) % 150); slides.push({ image:
 *          'http://placekitten.com/' + newWidth + '/200', text:
 *          ['More','Extra','Lots of','Surplus'][slides.length % 4] + ' '
 *          ['Cats', 'Kittys', 'Felines', 'Cutes'][slides.length % 4] }); }; for
 *          (var i=0; i<4; i++) $scope.addSlide(); } </file> <file
 *          name="demo.css"> .carousel-indicators { top: auto; bottom: 15px; }
 *          </file> </example>
 */
...
.directive('slide', [ '$parse', function($parse) {
    return {
        ...
        link : function(scope, element, attrs, carouselCtrl) {
            ...
            // when the scope is destroyed then remove the slide from the
...
.controller('DatepickerController', [ '$scope', '$attrs', 'dateFilter', 'datepickerConfig', function($scope, $attrs, dateFilter, dtConfig) {
    ...
    this.modes = [ {
        ...
        getVisibleDates : function(date, selected) {
            ...
            // This was customised here as per this post:
            // http://stackoverflow.com/questions/22865429/angular-ui-bootstrap-current-date-today-highlight-without-selecting
...
.directive('datepickerPopup', [ '$compile', '$parse', '$document', '$position', 'dateFilter', 'datepickerPopupConfig', 'datepickerConfig', function($compile, $parse, $document, $position, dateFilter, datepickerPopupConfig, datepickerConfig) {
    return {
        ...
        link : function(originalScope, element, attrs, ngModel) {
            var scope = originalScope.$new(), // create a child scope so we
                                                // are not polluting original
                                                // one
            ...
            scope.isOpen = getIsOpen ? getIsOpen(originalScope) : false; // Initial
                                                                            // state
    ...
    var $modalProvider = {
        ...
        $get : [ '$injector', '$rootScope', '$q', '$http', '$templateCache', '$controller', '$modalStack', function($injector, $rootScope, $q, $http, $templateCache, $controller, $modalStack) {
            ...
            $modal.open = function(modalOptions) {
                ...
                // prepare an instance of a modal to be injected into
                // controllers and returned to a caller
                var modalInstance = {
    ...
    return {
        ...
        link : function(scope, element, attrs, paginationCtrl) {
            ...
            paginationCtrl.getPages = function(currentPage, totalPages) {
                ...
                if (isMaxSized) {
                    if (rotate) {
                        // Current page is displayed in the middle of the
                        // visible ones
...
.provider('$tooltip', function() {
    ...
    this.$get = [ '$window', '$compile', '$timeout', '$parse', '$document', '$position', '$interpolate', function($window, $compile, $timeout, $parse, $document, $position, $interpolate) {
        return function $tooltip(type, prefix, defaultTriggerShow) {
             ...
             * 
             * If a trigger is supplied, it is used to show the tooltip;
             * otherwise, it will use the `trigger` option passed to the
             ...
             * option was passed to the `$tooltipProvider.options` method, it
             * will use the mapped trigger from `triggerMap` or the passed
             * trigger if the map is undefined; otherwise, it uses the
             * `triggerMap` value of the show trigger; else it will just use the
             * show trigger.
             */
                ...
                compile : function(tElem, tAttrs) {
                    ...
                    return function link(scope, element, attrs) {
                        ...
                        var positionTooltip = function() {
                            ...
                            // Get the height and width of the tooltip so we can
                            // center it.
                            ttWidth = tooltip.prop('offsetWidth');
                            ttHeight = tooltip.prop('offsetHeight');

                            // Calculate the tooltip's top and left coordinates
                            // to center it with
                            // this directive.
                        ...
                        // Show the tooltip with delay if specified, otherwise
                        // show it immediately
                        ...
                        function show() {
                            ...
                            // If there is a pending remove transition, we must
                            // cancel it, lest the
                            ...
                            // Now we add it to the DOM because need some info
                            ...
                            scope.$digest(); // digest required as $apply is
                                                // not called

                            // Return positioning function as promise callback
                            // for correct
                        ...
                        function hide() {
                            ...
                            // if tooltip is going to be shown after delay, we
                            // must cancel this
                            $timeout.cancel(popupTimeout);

                            // And now we remove it from the DOM. However, if we
                            // have animation, we
                        ...
                        function createTooltip() {
                            // There can only be one tooltip element per
                        ...
                        // if a tooltip is attached to <body> we need to remove
                        // it on
                        // location change as its parent scope will probably not
                        // be destroyed
                        // by the change.
                        if (appendToBody) {
...
.directive('progress', function() {
    ...
    // templateUrl: 'template/progressbar/progress.html' // Works in AngularJS
    // 1.2
    };
})
...
.controller('TabsetController', [ '$scope', function TabsetCtrl($scope) {
    ...
    ctrl.removeTab = function removeTab(tab) {
        ...
        if (tab.active && tabs.length > 1) {
            // If this is the last tab, select the previous tab. else, the next
            // tab.
...
.directive('tab', [ '$parse', function($parse) {
    return {
        ...
        controller : function() {
            // Empty controller so other directives can require being 'under' a
            // tab
        },
            ...
            return function postLink(scope, elm, attrs, tabsetCtrl) {
                ...
                if (attrs.active) {
                    ...
                    scope.$parent.$watch(getActive, function updateActive(value, oldVal) {
                        // Avoid re-initializing scope.active as it is already
                        // initialized
                        // below. (watcher is called async during init with
                        // value ===
                        // oldVal)
                        if (value !== oldVal) {
                            scope.active = !!value;
                ...
                scope.$watch('active', function(active) {
                    // Note this watcher also initializes and assigns
                    // scope.active to the
                ...
                // We need to transclude later, once the content container is
                // ready.
...
.directive('typeahead', [ '$compile', '$parse', '$q', '$timeout', '$document', '$position', 'typeaheadParser', function($compile, $parse, $q, $timeout, $document, $position, typeaheadParser) {
    ...
    return {
        ...
        link : function(originalScope, element, attrs, modelCtrl) {
            ...
            // minimal wait time after last character typed before typehead
            // kicks-in
            var waitTime = originalScope.$eval(attrs.typeaheadWaitMs) || 0;

            // should it restrict model values to the ones selected from the
            // popup only?
            ...
            var getMatchesAsync = function(inputValue) {
                ...
                $q.when(parserResult.source(originalScope, locals)).then(function(matches) {
                    ...
                    // it might happen that several async queries were in
                    // progress if a user were typing fast
                    // but we are interested only in responses that correspond
                    // to the current view value
                    if (inputValue === modelCtrl.$viewValue && hasFocus) {
                        if (matches.length > 0) {
                            ...
                            // opening a window
                            // with matches as a pop-up might be
                            // absolute-positioned and position of an input
            ...
            modelCtrl.$parsers.unshift(function(inputValue) {
                ...
                // ahneo :: new (set input value to empty string if it contains
                // " " string value)
                if (inputValue === " ") {
                    inputValue = "";
                ...
                // ahneo :: after (add new condition to get matches for min
                // search = 0)
                if (minSearch === 0 || inputValue && inputValue.length >= minSearch) {
                    if (waitTime > 0) {
                        if (timeoutPromise) {
                            $timeout.cancel(timeoutPromise);// cancel previous
                                                            // timeout
                        }
            ...
            modelCtrl.$formatters.push(function(modelValue) {
                ...
                if (inputFormatter) {
                    ...
                    // it might happen that we don't have enough info to
                    // properly render input value
                    // we need to check for this situation and simply return
            ...
            scope.select = function(activeIdx) {
                ...
                // return focus to the input element if a mach was selected via
                // a mouse click event
                element[0].focus();
            };

            // bind keyboard events: arrows up(38) / down(40), enter(13) and
            // tab(9), esc(27)
            ...
            // ahneo :: new (bind element to focus event to trigger
            // modelCtrl.$parsers.unshift method)
