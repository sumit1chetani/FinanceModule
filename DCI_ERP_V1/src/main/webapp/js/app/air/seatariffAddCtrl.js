...
app.controller('seatariffAddCtrl', function($scope, $rootScope, $http, $location, logger,
    ...
    $scope.charge ={
    		...
    		carrier:'',
    		isActive : true,
    		chargeDtl:[{id:0,chargeHeads:'',unit:'',currency:'',qty:'1',rate:'',paymentMethod : '',transactionType : '1',buySell : '',note : '',remarks:''}]
    ...
    $scope.cancel = function(){
        $state.go("app.sea.tariff.list",{tenantid:$stateParams.tenantid});
    };  
   ...
   $scope.dropdown();
	...
	$scope.submit=function(){
		...
		if(!$scope.checkundefined(msg)){
			...
			$http.post($stateParams.tenantid+'/app/airtariff/savesea',$scope.charge).success(function(datas) {
				debugger
		...
		function($scope, $timeout, $stateParams, sharedProperties,
		   ...
		   $scope.dropdown();
			...
			$scope.checkValidation = function() {
				...
				if ($scope.checkundefined($scope.charge.date)) {
					msg = msg + "<li>Validity Date :Field is required.</li>";
			...
			$scope.submitupdate = function() {
				...
				if (!$scope.checkundefined(msg)) {
					...
					$http.post($stateParams.tenantid + '/app/airtariff/updatesea',
