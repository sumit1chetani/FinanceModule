...
$scope.getQuotationType=function(){
		...
		$http.post($stateParams.tenantid+'/app/seaquotationApp/modelist').success(function(data) {
			debugger
...
/*$scope.getListMode1=function(){
    $http.get($stateParams.tenantid+'/app/seaquotationApp/modelist').success(function(datas) {
        console.log(datas);
...
$.fn.simulateClick = function() {
	...
	$scope.$watchCollection('quotation.customer', function(newValue, oldValue) {
		if(newValue != "" && newValue != null && newValue != undefined ){
        	...
        	$http.post($stateParams.tenantid+'/app/seaquotationApp/creditDays',$scope.quotation).success(function(datas) {
	               if(datas.success){
			...
			$http.get($stateParams.tenantid+'/app/seaQuotationApproval/getsalesPerson?id='+newValue).success(function(datas) {
	...
	//	$http.get($stateParams.tenantid+'/app/seaquotationApp/getCustomerDay?id='+newValue).success(function(datas) {
	//	console.log(datas);	
		//$scope.serviceParnrDropList=datas.serviceParnrList;
	//	if(datas.success){
	//	logger.logError("Customer has over 45 days due");
...
//	$scope.getpaymentMethod();
	...
	$scope.dropdown=function(){
		$scope.getQuotationType();
		$http.post($stateParams.tenantid+'/app/seaquotationApp/getServicePartnerList').success(function(datas) {
			debugger
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getiataList').success(function(datas) {
			debugger
		...
		/*$http.get($stateParams.tenantid+'/app/seaquotationApp/getcommodity').success(function(datas) {
		...
		$scope.getDropDownListprt = function() {
	        $http.post($stateParams.tenantid+'/app/seaquotationApp/getcommodity').success(function(data){
		...
		/*$http.get($stateParams.tenantid+'/app/seaquotationApp/getBranch').success(function(datas) {
	    ...
	    */// cargo type List ..... 
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getCurrencyList').success(function(datas) {	  
		...
		/*$http.get($stateParams.tenantid+'/app/seaquotationApp/getServicePartnerType').success(function(datas) {	  
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getEmployeeList').success(function(datas) {
...
*/		
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getChargeHeads').success(function(datas) {
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getTerms').success(function(datas) {
		...
		$http.get($stateParams.tenantid+'/app/seaquotationApp/getUnitList').success(function(datas) {
...
//if($scope.quotation.commodityL!=[] && $scope.quotation.commodityL!=null && $scope.quotation.commodityL.length!=0){
	...
	if($scope.quotation.carrier!='' && $scope.quotation.carrier!=null){
            	$http.post($stateParams.tenantid+'/app/seaquotationApp/save',$scope.quotation).success(function(datas) {
				debugger
				  if(datas.success==true){
					  if ($scope.files.length > 0) {
	                        ...
	                        angular.forEach($scope.files, function(files, index) {
	                            ...
	                            $.ajax({
	                                type : "POST",
	                                url : $stateParams.tenantid+"/app/seaquotationApp/saveuploadfile",
	                                data : frmData,
	                                contentType : false,
	                                processData : false,
					  ...
					  logger.logSuccess(datas.message);
  					  $state.go('app.sea.seaQuotationApproval.list',{tenantid:$stateParams.tenantid});
...
//							
							...
							if($scope.files == null || $scope.files == '' || $scope.files == undefined){
								...
								$http.post($stateParams.tenantid + '/app/seaquotationApp/uploaddelete',quotationNo).success(
										function(data) {
							...
							for(var i=0;i<$scope.files.length;i++){
							 ...
							 $.ajax({
                             type : "POST",
                             url : $stateParams.tenantid+"/app/seaquotationApp/updateuploadfile",
                             data : frmData,
                             contentType : false,
                             processData : false,
                             success : function(result) {
					...
					logger.logError("Carrier field is Mandatory!.");
		            	...
		            	logger.logError("Atleast One free days Required");
		            }
			 }else{
		        	logger.logError("Atleast One Row Required");
		        }
	...
	/*$scope.sendmail = function(quotationHdId){
		$http.get($stateParams.tenantid + '/app/seaquotationApp/sendMail?quotationHdId='+quotationHdId)
		.success(function(datas) {
	...
	$scope.printQuot = function(quotationHdId){
        ...
        var url = $stateParams.tenantid+'/app/seaquotationApp/print?quotationHdId=' + quotationHdId;
	 ...
	 $scope.$watch('quotation.quotationDtl[trIndex].unit', function(newValue, oldValue) {
			...
			if(newValue != "" && newValue != null && newValue != undefined ){
				if (Currency != null && Currency != '' && Currency != undefined && service != null && service != '' && service != undefined &&  customer != null && customer != '' && customer != undefined && !isNaN(pol) && !isNaN(pod) && !isNaN(chrgID) && !isNaN(unitID)) {
					$http.post($stateParams.tenantid + '/app/seaquotationApp/checkAmountfromMRG?service='+service+'&customer='+customer+
			    ...
			    $scope.deleteuploadfiles = function(filename) {
			      /*  $scope.tempfiles = [];
			        ...
			        $scope.quotation.files = $scope.tempfilename;*/
			    	 ...
			    	 $http.post($stateParams.tenantid+'/app/seaquotationApp/deleteFiles', filename).success(function(result) {
...
//			$scope.getpaymentMethod();
			...
			$scope.dropdown = function() {
				...
				$http
						.post(
								$stateParams.tenantid
										+ '/app/seaquotationApp/getServicePartnerList')
						.success(
								function(datas) {
									debugger
									 $scope.customerDropList = datas.customerList;
				...
				$http.get(
						$stateParams.tenantid
								+ '/app/seaquotationApp/getiataList')
						.success(function(datas) {
							debugger
							$scope.portList = datas.commonUtilityBean;
				...
				$http
						.get(
								$stateParams.tenantid
										+ '/app/seaquotationApp/getCurrencyList')
						.success(
								function(datas) {
				...
				/*$http
						.get(
								$stateParams.tenantid
										+ '/app/seaquotationApp/getServicePartnerType')
						.success(
								function(datas) {
				...
				$http
						.get(
								$stateParams.tenantid
										+ '/app/seaquotationApp/getEmployeeList')
						.success(
								function(datas) {
									$scope.employeeList = datas.commonUtilityBean;
...
*/				
				...
				$http
						.get(
								$stateParams.tenantid
										+ '/app/seaquotationApp/getChargeHeads')
						.success(
								function(datas) {
									$scope.chargeHeadList = datas.commonUtilityBean;
				...
				$http.get(
						$stateParams.tenantid
								+ '/app/seaquotationApp/getUnitList')
						.success(function(datas) {
							$scope.UnitList = datas.commonUtilityBean;
       ...
       if(cond==true){
    	  ...
    	  // if($scope.quotation.commodityL!=[] && $scope.quotation.commodityL!=null && $scope.quotation.commodityL.length!=0){
        	   if($scope.quotation.carrier!='' && $scope.quotation.carrier!=null){
					...
					$http.post($stateParams.tenantid + '/app/seaquotationApp/update',
			...
			*/ 
			...
			$scope.cancel = function() {
				$state.go('app.sea.seaQuotationApproval.list', {
...
/*
			...
			*/
			...
			$scope.reject = function(quotationNo) {	
		         ...
		         ngDialog.open({
		             scope : $scope,
		             template : 'views/air/seaQuotationApproval/quotationRejectRemarks',
		...
		*/	        if ($scope.checkundefined(chargesDetail.currency)) {
			...
			$scope.approve = function(quotationNo,cosolidated) {	
		         ...
		         ngDialog.open({
		             scope : $scope,
		             template : 'views/air/seaQuotationApproval/quotationApprovalRemarks',
			...
			$scope.submitupdateapprove = function() {
				...
				if (!$scope.checkundefined(msg)) {
					...
					if($scope.quotation.quotationDtl.length>0){
	 var cond=true;var msg="";var j=0,l=0;
		if($scope.quotation.mode!=5){
       ...
       if(cond==true){
    	  ...
    	  // if($scope.quotation.commodityL!=[] && $scope.quotation.commodityL!=null && $scope.quotation.commodityL.length!=0){
        	   if($scope.quotation.carrier!='' && $scope.quotation.carrier!=null){
					...
					$http.post($stateParams.tenantid + '/app/seaquotation/updateapprove',
							$scope.quotation).success(function(datas) {
						debugger
						if (datas.success == true) {
...
app.controller('quotationRejectCtrl', function($stateParams , $scope , $rootScope, $http, $location,logger, $state, $window,ngDialog) {
    debugger;
        ...
        $http.post($stateParams.tenantid+'/app/seaquotationApp/rejectQuotation',quotationNew).success(function(datas) {
			if(datas.success){
				 ngDialog.close();    
    ...
    debugger;
        ...
        $scope.approveQuote = function(quotationNew){
        	...
        	//approve
        		...
        		$http.post($stateParams.tenantid+'/app/seaquotationApp/approveQuotation',quotationNew).success(function(datas) {
        			if(datas.success){
