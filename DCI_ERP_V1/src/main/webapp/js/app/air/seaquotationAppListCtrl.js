...
app.controller('seaquotationListCtrl', function($scope, $timeout, $stateParams,sharedProperties,toaster,
	...
	$scope.getBookingList = function() {
		$http.post($stateParams.tenantid+'/app/seaquotationApp/list').success(function(data) {
			debugger
...
/*	
	//approve Request
	$scope.approveRow = function(QuotHdId) {
	    $location.url($stateParams.tenantid+'/seaQuotationApproval/Approval?QuotHdId=' + QuotHdId);
	};*/
	...
	$scope.add = function() {
        ...
        $state.go('app.sea.seaQuotationApproval.add',{tenantid:$stateParams.tenantid});
    };
    
    $scope.viewGeneralLedgerReportNew = function(quotation) {
    	$http.post($stateParams.tenantid+'/app/seaquotationApp/filterList',quotation).success(function(data) {
    		angular.forEach(data.lQuotationBean, function(row, index) { 
    	...
    	//$http.get($stateParams.tenantid+'/app/seaquotationApp/filterList?mode='+quotation.mode+'&polCode='+quotation.polCode+'&podCode='+quotation.podCode+'&status='+quotation.status+'&fromDate='+quotation.fromDate+'&toDate='+quotation.toDate).success(function(data) {
		debugger
		$scope.rowCollection = data.lQuotationBean;
	});};
	...
	$scope.editQuot = function(QuotHdId) {
		$rootScope.seaquotationListCtrl=$('.rounded').val();
		debugger
			$location.url($stateParams.tenantid+'/seaQuotationApproval/edit?QuotHdId='+QuotHdId);
	...
	$scope.editQuot1 = function(QuotHdId1) {
        ...
        $state.go('app.sea.seaQuotationApproval.Add',{QuotHdId1: $stateParams.QuotHdId1});
...
//		$rootScope.seaquotationListCtrl=$('.rounded').val();
//		debugger
//			$location.url($stateParams.tenantid+'/seaQuotationApproval/view?QuotHdId='+QuotHdId);
//		
//	}
	 ...
	 $scope.printQuot = function(quotationHdId){
		...
		$scope.deleteQ = function(quotationId,quotationNo) {	
	         ...
	         ngDialog.open({
	             scope : $scope,
	             template : 'views/air/seaQuotationApproval/quotationDeleteRemarks',
		 ...
		 $scope.exportExcelnew = function(){
			...
			/*if($scope.bookingReport.salesPersonId!='' && $scope.bookingReport.salesPersonId!=null){*/
		   	 $http.post($stateParams.tenantid+'/app/seaquotationApp/ExportExcel', $scope.quotation).success(function(response) {
	 ...
	 $scope.mailview = function(quotationHdId) {
         ...
         if(quotationHdId =="" || quotationHdId ==undefined){
            // logger.logError("");
             ...
             ngDialog.open({
                 scope : $scope,
                 template : 'views/air/seaQuotationApproval/seaQuotationViewMail',
  ...
  app.controller('seaquotationmailViewCtrl',
	  ...
	  $scope.viewmaildata = function(){
			$http.get($stateParams.tenantid + '/app/seaquotationApp/viewSeaQuotationMail?quotationHdId='+quotationid)
			.success(function(datas) {
				console.log(datas);
	  ...
	  $scope.sendmail = function(quotationHdId){
		  $scope.getuserdetail();
			$http.get($stateParams.tenantid + '/app/seaquotationApp/sendMail?quotationHdId='+quotationid)
			.success(function(datas) {
				//console.log(datas);
    ...
    $scope.deleteQuotation = function(quotationNew){
    	...
    	//approve
    		...
    		$http.post($stateParams.tenantid+'/app/seaquotationApp/delete',quotationNew).success(function(datas) {
    			if(datas.success){
