//invoiceList.js
'use strict';
app.controller('invoiceListController', function($scope, $timeout, $stateParams,sharedProperties,toaster,
    ...
    $scope.add = function() {
        $state.go('app.documentation.invoiceadd',{tenantid:$stateParams.tenantid});
    };
    
    $scope.getList=function(){

    	$http.get($stateParams.tenantid+'/app/invoice/list').success(function(response) {
  ...
  $scope.bulkMail = function(giRowData) {
      ...
      if (invoiceCodes == '') {
          ...
          $http.post($stateParams.tenantid+'/app/invoice/bulkMail', invoiceCodes).success(function(data) {
              if (data.success)
   ...
   $scope.viewInvoice = function(invoiceNo ) {
		$location.url($stateParams.tenantid+'/documentation/invoice/view?invoiceNo='+invoiceNo);
	}
   ...
   $scope.sendMail = function(invoiceNo) { 
	   $rootScope.invoice = invoiceNo;
		   ...
		   $http.get($stateParams.tenantid+'/app/invoice/getEmailList?invoiceno='+invoiceNo).success(function(data) {
   ...
   $scope.datechangeinv = function(invoiceNo) { 
 	  debugger;
 	   $rootScope.invoice = invoiceNo;
...
app.controller('deliveryOrderpopCtrl', function($scope, $timeout, $stateParams,sharedProperties,toaster,
	...
	$http.post($stateParams.tenantid+'/app/invoice/branchlist').success(function(data) {
	 ...
	 $http.post($stateParams.tenantid+'/app/invoice/getdodetails?invoNo=' +$scope.dodata.invoiceNo).success(function(result) {
	 ...
	 $scope.save = function(invoiceListForm,dodata){	
		 ...
		 if (new validationService().checkFormValidity(invoiceListForm)) {	
			 ...
			 $http.post($stateParams.tenantid+'/app/invoice/savedo', $scope.dodata).success(function(data) {
	 ...
	 $scope.print = function(){
		 ...
		 var url = $stateParams.tenantid+'/app/invoice/print?invNo=' + invNo;
...
app.controller('poPopupCtrl', function($scope, $timeout, $stateParams,sharedProperties,toaster,
	...
	$rootScope.invoice;
	 ...
	 $scope.senttomail = function() { 
		    $http.get($stateParams.tenantid+'/app/invoice/sendMail?invoiceno='+  $rootScope.invoice).success(function(data) {
         if (data == true) {
             logger.logSuccess("Mail sent successfully!");
             ngDialog.close();
         }else{
...
app.controller('FreightInvDateCtrl', function($scope, $http,ngDialog,logger,$location,sinvoiceNo,screenName,$timeout,$stateParams) {
    $scope.FreightDateobj = {dateOfIssue:'', invoiceNo:''};
            ...
            if($scope.FreightDateobj.dateOfIssue!=""){
                $http.post($stateParams.tenantid+'/app/provinvoice/updateInvDate',$scope.FreightDateobj).success(function(datas) {
                    if(datas.success == true){
                        logger.logSuccess("Invoice Date Updated SuccessFully!");
                        ngDialog.close();
                    }else{
