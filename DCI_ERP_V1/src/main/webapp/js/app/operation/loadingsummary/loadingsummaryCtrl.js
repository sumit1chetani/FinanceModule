app.controller('loadingsummaryCtrl', function($scope, $rootScope, $http,
...
//			loadingTypeHdr:'';	
		...
		$scope.loadingReport = {
		 ...
		 if(localStorage.getItem('loadingReportObj')!=null && localStorage.getItem('loadingReportObj')!=undefined){
				$scope.loadingReport = JSON.parse(localStorage.getItem('loadingReportObj'));
			}
		...
		$scope.getLoadingSummaryData = function(loadingSummaryForm,loadingReport) {
			localStorage.setItem('loadingReportObj',JSON.stringify($scope.loadingReport));
			if(loadingReport.mode!=null && loadingReport.mode!=''){
				$http.post($stateParams.tenantid + '/app/loadingsummaryreport/viewReport',$scope.loadingReport)
					...
					logger.logError("Mode field is required !");
