...
app.controller('bladdingCtrl', function($scope, $rootScope,ngDialog, $http, $location, logger,
    ...
    $scope.add = function() {
    	 ...
    	 $state.go('app.documentation.outwardbladding.outwardadd',{tenantid:$stateParams.tenantid});
    };
    ...
    if(localStorage.getItem('blSearchObj')!=null && localStorage.getItem('blSearchObj')!=undefined){
 	...
 	//BL View 
  	 ...
  	 $scope.viewBlDetail = function(blNo){
  		ngDialog.close();
  		$location.url($stateParams.tenantid+'/reports/BLDetail?blNo='+blNo+'&from=bl');
  	 };
    ...
    $scope.getBlList = function() {
			if($scope.blSearch.checked==true){
				$scope.checked1=true;
				 $http.post($stateParams.tenantid+'/api/outWard/list1',$scope.blSearch).success(function(datas) {
				        $scope.rowCollection = datas;
				        console.log("$scope.rowCollection1",$scope.rowCollection);
				        }).error(function(datas) {
				    });
			}else{
				 $http.post($stateParams.tenantid+'/api/outWard/list',$scope.blSearch).success(function(datas) {
				        $scope.rowCollection = datas;
...
$scope.getBlList();
    ...
    $scope.editRow = function(rowid) { 
    	 localStorage.setItem('blSearchObj',JSON.stringify($scope.blSearch));
    	$location.url($stateParams.tenantid+'/documentation/outwardBl/edit?blNo='+rowid);       
     }
...
// Consolidated BL Print
	  ...
	  $http.get('/api/BLInventory/getConsolidatedBLNumberCheck?serial_seqNo='+blSerialNo).success(function(datas) {
	   ...
	   $scope.printBLOriginalStationary = function(blNo,location,blSerialNo) {
	        ...
	        $http.get($stateParams.tenantid+'/api/outWard/checkBLSurrenderOrNot?blNo=' +blNo).success(function(result) {
	        ...
	        $http.get($stateParams.tenantid+'/api/billofLading/printcountOrgnlSta?blNo=' +blNo).success(function(result) {
	           if(result.message != null && result.message != ''){
	        	   ...
	        	   var url = $stateParams.tenantid+'/api/outWard/printbloutwardoriginalSta?blNo=' + blNo;
	               if(result.count <3){
 ...
 $scope.printConsolidatedBLOriginalStationary = function(blNo,location,blSerialNo) {
	        ...
	        $http.get($stateParams.tenantid+'/api/billofLading/consoildatedPrintcountOrgnlSta?blNo=' +blNo).success(function(result) {
	           if(result.message != null && result.message != ''){
	        	   ...
	        	   var url = $stateParams.tenantid+'/api/outWard/consolidatedOriginalPrint?blNo=' + blNo;
	               if(result.count <3){
    ...
    $scope.printBLOriginal = function(blNo) {
        ...
        $http.get($stateParams.tenantid+'/api/billofLading/printcountOrgnl?blNo=' +blNo).success(function(result) {
           if(result.message != null && result.message != ''){
        	   ...
        	   var url = $stateParams.tenantid+'/api/outWard/printbloutwardoriginal?blNo=' + blNo;
               if(result.count <7){
            	/*   $http.get('/api/BLInventory/blNoSave?blNo='+blNo+'&location='+location+'&serial_seqNo='+blSerialNo).success(function(datas) {
             	       console.log("blNoSave",datas);
...
//        }
    ...
    $scope.convertSWB = function(blNo) {
        debugger
          $http.get($stateParams.tenantid+'/api/outWard/convertSeawaybill?blNo=' +blNo).success(function(result) {
             if(result.isSuccess){
    ...
    $scope.unlockOgs = function(blNo) {
        debugger
          $http.get($stateParams.tenantid+'/api/outWard/unLockOrginalSta?blNo=' +blNo).success(function(result) {
             if(result.isSuccess){
      ...
      $scope.unlockConsolidatedOgs = function(blNo) {
          debugger
            $http.get($stateParams.tenantid+'/api/outWard/unLockConsolidatedOrginalSta?blNo=' +blNo).success(function(result) {
               if(result.isSuccess){
    ...
    $scope.unlockOg = function(blNo) {
      debugger
        $http.get($stateParams.tenantid+'/api/outWard/unLockOrginal?blNo=' +blNo).success(function(result) {
           if(result.isSuccess){
    ...
    $scope.unlockCopy = function(blNo) {
    	debugger
        $http.get($stateParams.tenantid+'/api/outWard/unLockCopy?blNo=' +blNo).success(function(result) {
           if(result.isSuccess){
    ...
    $scope.unlockCopyWithStationary = function(blNo) {
    	debugger
        $http.get($stateParams.tenantid+'/api/outWard/unLockCopyWithSta?blNo=' +blNo).success(function(result) {
           if(result.isSuccess){
        	   logger.logSuccess("Print limit unlocked successfully - copy!");
           }
           else{
...
//        }
    ...
    $scope.printBLCopyWithStationary = function(blNo) {
       ...
       $http.get($stateParams.tenantid+'/api/billofLading/printcountCopyWithSta?blNo=' +blNo).success(function(result) {
        	...
        	var url = $stateParams.tenantid+'/api/outWard/printbloutwardcopyWithStationary?blNo=' + blNo;
            if((result.count < 3)){
            var wnd = window.open(url, 'Shipping', 'height=400,width=750,top=100,left=100,toolbar=no,scrollbars=yes,fullscreen=no,status=yes,menubar=no,location=no,directories=no,resizable=yes');
            wnd.print();
              } else {
...
//        }
	...
	$scope.reset = function(){
	 ...
	 $scope.exportExcel = function(){
	            ...
	            $http.post($stateParams.tenantid+'/api/outWard/excelExport', $scope.blSearch).success(function(response) {
	                if(response){
	                    debugger;
...
app.controller('deliveryOrderpopCtrl', function($scope, $timeout, $stateParams,sharedProperties,toaster,
	...
	$scope.blNoData = {
	 ...
	 $http.post($stateParams.tenantid+'/api/outWard/popuplist' ,$scope.blNoData).success(function(result) {
	...
	$scope.bl = {
	 ...
	 $scope.saveData = function(blForm1) {
		 if($scope.bl.blCancelreason){
			 	$http.post($stateParams.tenantid+'/api/outWard/cancelBL' ,$scope.bl).success(function(result) {
			 		if (result.isSuccess == true) {
			 			  ngDialog.close();
