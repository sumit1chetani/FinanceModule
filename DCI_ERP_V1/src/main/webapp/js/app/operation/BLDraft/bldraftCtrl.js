...
app.controller('bldraftCtrl', function($scope, $rootScope,ngDialog, $http, $location, logger,
   ...
   $scope.add = function() {
	   localStorage.setItem('blSearchObj',JSON.stringify($scope.blSearch));
    	 $state.go('app.documentation.bldraft.bldraftAdd',{tenantid:$stateParams.tenantid});
    };
    ...
    $scope.getBlList = function() {
			if($scope.blSearch.checked==true){
				$scope.checked1=true;
				 ...
				 $http.post($stateParams.tenantid+'/api/outWard/BLDraftlist',$scope.blSearch).success(function(datas) {
				        $scope.rowCollection = datas;
...
$scope.getBlList();
		...
		$rootScope.CheckBLDraft=function(blDraftDescription,blInvoiceNo){
			...
			if($scope.validation){
					...
					$http.get($stateParams.tenantid+'/api/outWard/approveBFDraft?blDraftDescription='+blDraftDescription+'&blInvoiceNo='+blInvoiceNo+'&blNo='+$scope.DraftBLNO).success(function(datas) {
						if(datas.isSuccess){
							   ngDialog.close();
							   $scope.getBlList();
	...
	/* 
	  $scope.sendBLDraftMailprint = function(blNo) {
			   $http.get($stateParams.tenantid+'/api/outWard/sendMailPrint?blNo='+blNo).success(function(datas) {
...
//        }
    ...
    $scope.printBLCopy = function(blNo) {
      ...
      /*  $http.get($stateParams.tenantid+'/api/billofLading/printcountCopy?blNo=' +blNo).success(function(result) {
    	 ...
    	 $http.get($stateParams.tenantid+'/api/outWard/checkSailingDate?blNo=' +blNo).success(function(result) {
    		 debugger;
             if(!result.isSuccess ){
          	   logger.logError(result.message);
             }else{
            	 var url = $stateParams.tenantid+'/api/outWard/printbldraftcopy?blNo=' + blNo;
...
//        }
	...
	$scope.deleteBLDraft = function(blNo) {
        ...
        ngDialog.openConfirm().then(function() {
            var url =$stateParams.tenantid+'/api/outWard/deleteBLDraft?blNo=' + blNo;
