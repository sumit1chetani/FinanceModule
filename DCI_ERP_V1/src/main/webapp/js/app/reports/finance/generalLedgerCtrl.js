     ...
     //   $('#gl_op_header').remove();
        ...
        if($scope.generalLedger.toDate !='' && $scope.generalLedger.fromDate != '' && $scope.generalLedger.companyCode !='' ){
           ...
           $http.post('app/generalLedger/getGeneralLedgerReport', $scope.generalLedger).success(function(data) {
                if(data.success){                	 
                	 $scope.rowCollection = data.lGeneralLedgerList;
                	 $scope.generalLedger.clbal=data.amt.closingBalance;
                	 $scope.generalLedger.opbal=data.amt.openingBalance;
                     
                     if(data.amt.openingBalance>0){
                    	 $scope.generalLedger.opbaldr=	data.amt.openingBalance;
                    	 $scope.generalLedger.opbalcr = 0;                    	
                     }else{
                    	 $scope.generalLedger.opbalcr=	-data.amt.openingBalance;
                    	 $scope.generalLedger.opbaldr = 0;                    	
                     }
                    ...
                    $('#gl_op_header').removeAttr("class","hidden");                    
  ...
  //Excel PDF	
    ...
    $scope.populateGeneralLedgerGrid = function(sgListData){
        ...
        $("#generalLedgerGrid").jqGrid({
          ...
          /*  colNames : [' ' ,   ''],
            colModel:[
                {name:'subGroupCode',index:'subGroupCode', width:80,hidden:true, align:"left",searchoptions:{sopt:['cn']}}, 
                {name:'transaction_no',index:'transaction_no',  formatter: addLink ,width:80, align:"left",searchoptions:{sopt:['cn']},hidden:false},
               // {name:'tcDebit',index:'tcDebit', width:80,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
               // {name:'tcCredit',index:'tcCredit', width:80,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
                {name:'subGroupName1',index:'subGroupName1' ,width:80, align:"left",searchoptions:{sopt:['cn']},hidden:false},
             //   {name:'bcDebit',index:'tcDebit', width:80,align:"right",hidden:true,  searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
             //   {name:'bcCredit',index:'tcCredit', width:80,align:"right",hidden:true,  searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
             //   {name:'currentTCBalance',index:'currentTCBalance', width:80,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
                {name:'currentBalance',index:'currentBalance', width:80,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}}
             
                ],*/
            colNames:['Date','Particulars','Transaction No','Debit', 'Credit','Balance'],
            
            colModel:[
            		{name:'ledgerDate',index:'ledgerDate', width:200, align:"left",searchoptions:{sopt:['cn']}},           	
                  // {name:'accountHeadCode',index:'accountHeadCode', width:200, align:"left",searchoptions:{sopt:['cn']}},
                   {name:'accountHeadName',index:'accountHeadName', width:200, align:"left",searchoptions:{sopt:['cn']},hidden:false},
                   ...
                   {name:'bcDebit',index:'bcDebit', width:200,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
                 //  {name:'currentTCBalance',index:'currentTCBalance', width:80,align:"right", searchoptions:{sopt:['cn']},hidden:false,formatter:'currency',formatoptions: { thousandsSeparator:','}},
                   {name:'currentBalance',index:'currentBalance', width:200,align:"right", searchoptions:{sopt:['cn']},hidden:true,formatter:'currency',formatoptions: { thousandsSeparator:','}}
               ],
           // pager: "#generalLedgerPage",
            viewrecords: true,
            ignoreCase: true,
            ...
            loadComplete: function () {
                   ...
                   var credit = $grid.jqGrid('getCol', 'bcCredit', false, 'sum');
                   var debit = $grid.jqGrid('getCol', 'bcDebit', false, 'sum');
                   var fclosing = $grid.jqGrid('getCol', 'currentBalance', false, 'sum');
                   var fb = debit + (-fclosing);
                 //  credit = credit.toFixed(2);
                   
                 //  debit = debit.toFixed(2);
                   fclosing = fclosing.toFixed(2);
                  
                  $(this).jqGrid('footerData','set',{transactionNo:"Closing Balance", bcCredit:credit, bcDebit:debit  ,currentBalance: 'Balance ',  currentBalance:fclosing});
                
                //  alert(credit);
                  
                  if($scope.generalLedger.opbal > 0 ){                	  
                	  $scope.generalLedger.currentdr = credit + parseInt($scope.generalLedger.opbal, 10);
                	  $scope.generalLedger.currentcr =  debit;
                  }else{
                	  $scope.generalLedger.currentcr =  debit + parseInt(-$scope.generalLedger.opbal, 10);
                	  $scope.generalLedger.currentdr = credit;                	                  	  
                  }                                  
                 
                 $scope.generalLedger.currentBal = fclosing;  
                 
                 if(((parseInt($scope.generalLedger.currentcr, 10)) - (parseInt($scope.generalLedger.currentdr, 10))) > 0){
                	 if(parseInt($scope.generalLedger.clbal, 10)<0){ $scope.generalLedger.clbal = -(parseInt($scope.generalLedger.clbal, 10));}
                	 $scope.generalLedger.clbaldr = $scope.generalLedger.clbal;
                	 $scope.generalLedger.clbalcr = 0;
                	 $scope.generalLedger.finalcr = parseInt($scope.generalLedger.currentdr, 10) + parseInt($scope.generalLedger.clbal, 10);
                	 $scope.generalLedger.finaldr = parseInt($scope.generalLedger.currentcr, 10);
                 }else{     
                	 if(parseInt($scope.generalLedger.clbal, 10)<0){ $scope.generalLedger.clbal = -(parseInt($scope.generalLedger.clbal, 10));}
                	 $scope.generalLedger.clbalcr = $scope.generalLedger.clbal;
                	 $scope.generalLedger.clbaldr = 0;
                	 $scope.generalLedger.finalcr = parseInt($scope.generalLedger.currentdr, 10);
                	 $scope.generalLedger.finaldr = parseInt($scope.generalLedger.currentcr, 10)+ parseInt($scope.generalLedger.clbal, 10);
                 }
                 
               /*    
            	 var $this = $(this),
            	 credit = $grid.jqGrid('getCol', 'bcCredit', false, 'sum'),
            	 debit = $grid.jqGrid('getCol', 'bcDebit', false, 'sum'), 
                 $footerRow = $(this.grid.sDiv).find("tr.footrow"),
                 localData = $this.jqGrid("getGridParam", "data"),
                 totalRows = localData.length,
                 currentBalance = fclosing,
                 openingBalance = 0,
                 closingBalance = 0,
                 $newFooterRow,
                 i;

             $newFooterRow = $(this.grid.sDiv).find("tr.myfootrow"); $newFooterRow1 = $(this.grid.sDiv).find("tr.myfootrow");
             ...
             $this.jqGrid("footerData", "set", {transactionNo: "Opening Balance", currentBalance: 0});
             
             // calculate the value for the second footer row
             for (i = 0; i < totalRows; i++) { 
            	 currentBalance += parseInt(localData[i].fclosing, 10);            	             	 
             }
             $newFooterRow.find(">td[aria-describedby=" + this.id + "_transactionNo]").text("Current Balance"); 
	         $newFooterRow.find(">td[aria-describedby=" + this.id + "_currentBalance]").text($.fmatter.util.NumberFormat(currentBalance, $.jgrid.formatter.number));
	         */
