    	...
    	app.controller('salaryfixationCtrl1', function($scope,$stateParams, $rootScope, $http,validationService, $location,ngDialog, logger, utilsService,$state,sharedProperties,$window) {
      ...
      /*  $scope.checkEditTrue= function(paycomponenetList){
            debugger
...
debugger
            if (new validationService().checkFormValidity(salaryfixationListForm)) {
                console.log("employeePaycomponentId" + employeePaycomponenet.employeeId);
                if ($scope.employeePaycomponenet.departmentId != '' && $scope.employeePaycomponenet.employeeId != '') {
                    $http.get("payroll/payroll/employeepaycom/list3?employeeId=" + employeePaycomponenet.employeeId).success(function(response) {
                        console.log("response is");
                        console.log(response);
                        $scope.renamingDateValue(response.employeeComponentList);
                        $scope.checkingLargeDate($scope.employeePayComponentList);
                       // $scope.checkEditTrue($scope.employeePayComponentList);
                    ...
                    $http.get("payroll/payroll/employeepaycom/list3?employeeId=" + $scope.employeePaycomponenet.employeeId).success(function(response) {
                        console.log("response is");
                        console.log(response);
                        $scope.renamingDateValue(response.employeeComponentList);
                        $scope.checkingLargeDate($scope.employeePayComponentList);
                ...
                toaster.pop('error', "Please fill the required fields", logger.getErrorHtmlNew(salaryfixationListForm.$validationSummary), 5000, 'trustedHtml');
            }
        }
        ...
        $scope.viewRow = function(employeeId, fromdate) {
            ...
            var companyId = $scope.employeePaycomponenet.companyId;
            var branchId = $scope.employeePaycomponenet.branchId;
            $location.url($stateParams.tenantid+'/payroll/salaryfixation/view?employeeId=' + $scope.employeePaycomponenet.employeeId + '&fromdate=' + fromdate + '&departmentId=' + departmentId + '&branchId=' + branchId + '&companyId=' + companyId);
        }
...
$location.url($stateParams.tenantid+'/payroll/salaryfixation/getValue?departmentId=' + departmentId + '&employeeId=' + employeeId + '&branchId=' + branchId + '&companyId=' + companyId);
       ...
       /* $scope.exportSampleExcel = function() {
            ...
            $http.post('payroll/payroll/employeepaycom/exportSampleExcel', resultBean).success(function(data) {
                ...
                if (data.success == true) {
                    ...
                    $("#btnRowDivId").bind('click', function() {
            	    });
            	    $('#btnRowDivId').simulateClick('click');
...
*/
  /*      $.fn.simulateClick = function() {
        ...
        $scope.exportSampleExcel = function() {
        	...
        	$scope.employeePaycomponenet.Show = true;
        	logger.logSuccess("File Exported Successfully");
        	$('#btnRowDivIdsamp').append('<a id="captainMsglink" href="assets/docs/Sample_Employee_Salary_Upload_FileNew.xls" class="control-label" download="Sample_Employee_Salary_Upload_File.xls"></a>');
        	$("#captainMsglink").bind('click', function() {
        	});
        	$('#captainMsglink').simulateClick('click');
        	} else {
