    'use strict';
   	app.controller('ptslabgenerationCtrl', function($stateParams,$scope, $state, $http, ngDialog, logger, $location, $controller, $injector, sharedProperties, toaster, $rootScope, validationService,utilsService, $window,$timeout) {
        ...
        $scope.update = function() {
            if ($scope.rowCollection != undefined && $scope.rowCollection != null && $scope.rowCollection != '') {
                ...
                if ($scope.rowCollection.length > 0) {
                    ...
                    $http.post("payroll/tds/ptslab/insertEmployeePTList", $scope.ptList).success(function(response) {
                        if (response) {
                            logger.logSuccess("Employee PT Slab List Updated Successfully");
                            if ($scope.ptslabList.companyId != undefined && $scope.ptslabList.companyId != null && $scope.ptslabList.companyId != '' && $scope.ptslabList.departmentId != undefined && $scope.ptslabList.departmentId != null && $scope.ptslabList.departmentId != '' && $scope.ptslabList.branchId != undefined && $scope.ptslabList.branchId != null && $scope.ptslabList.branchId != '' && $scope.ptslabList.financialYear != undefined && $scope.ptslabList.financialYear != null && $scope.ptslabList.financialYear != '') {
                                ...
                                var resultBean = {
                                    ...
                                    departmentId : $scope.ptslabList.departmentId,
        ...
        $scope.validate = function(ptslabgenerationForm) {
            ...
            if (new validationService().checkFormValidity(ptslabgenerationForm)) {
                ...
                var departmentId = $scope.ptslabList.departmentId;
        ...
        $scope.exportExcel = function() {
            ...
            $http.post('payroll/tds/ptslab/exportExcel', resultBean).success(function(data) {
                if (isValid) {
                    if (data.success == true) {
                        $scope.ptslabList.Show = true;
                        logger.logSuccess("File Exported Successfully");
                        $('#btnRowDivId').append('<a id="captainMsglink" href="tempdoc/Employee_Profession_Tax.xls" class="control-label" download="Employee_Profession_Tax.xls"></a>');
