'use strict';
app.controller('SeapurchaseInvoiceAddCtrl', function($scope, $rootScope,sharedProperties, 
       ...
       $scope.getDropdownvalue = function() {
           ...
           //new  
    	   $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getJobList').success(function(datas) {
           ...
           /*$http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getCustomerList').success(function(datas) {
               $scope.customerList = datas;
               }).error(function(datas) {
           });           
           $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getPolList').success(function(datas) {
               $scope.polList = datas;
               }).error(function(datas) {
           });
           $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getPodList').success(function(datas) {
               $scope.podList = datas;
               }).error(function(datas) {
           });
           $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getOriginList').success(function(datas) {
               $scope.originList = datas;
               }).error(function(datas) {
           });
           $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getDestinationList').success(function(datas) {
   	            ...
   	            if(datas){
   	                logger.logError("Job Order Closed Pls Contact IT Support");
                    	...
                    	if (newValue != '' && newValue != undefined) {
            				$http.post($stateParams.tenantid + '/app/seapurchaseinvoice/dropDownList?jobNo='+newValue).success(function(datas) {
            					console.log(datas);
       ...
       $scope.$watch('purchaseInvoiceData.partyNo', function(newValue, oldValue) {
           ...
           if(newValue!="" && newValue!=undefined && newValue!=null){
               ...
               $http.get($scope.tenant+'/app/seapurchaseinvoice/getJobDetails?partyNo='+parseInt(newValue) +'&jobNo=' + parseInt($scope.purchaseInvoiceData.jobNo) +'&status=' + $scope.tempDrfatStatus).success(function(data){
                       ...
                       $scope.purchaseInvoiceData.partyCountryId=data.partyCountryId;
                       ...
                       if(tempPurchaseInvoiceDtl.length > 0){
                    	   $scope.purchaseInvoiceData.bankDtl=bankId.toString();
                       }
                       ...
                       /*for(var j=0;j< tempPurchaseInvoiceDtl.length;j++){
                    	   for(var k=0;k< $scope.purchaseInvoiceData.purchaseInvoiceDetail.length;k++){
                			  
                    		   if(tempPurchaseInvoiceDtl[j].jobChargeDtlBin == $scope.purchaseInvoiceData.purchaseInvoiceDetail[k].jobChargeDtlBin ){
                    			   $scope.purchaseInvoiceData.purchaseInvoiceDetail[k]['select']=true;
                    			   $scope.purchaseInvoiceData.purchaseInvoiceDetail[k].taxPrct=tempPurchaseInvoiceDtl[j].taxPrct;
                    			   $scope.purchaseInvoiceData.purchaseInvoiceDetail[k].taxAmount=tempPurchaseInvoiceDtl[j].taxAmount;
                        	   }
                    	   }
...
$scope.calAmt = function(tables) {
           ...
           $scope.purchaseInvoiceData.totalAmount = totalAmount.toFixed(2);
       ...
       $scope.calculateTotAmount = function(tables) {
        ...
        $scope.calTaxAmount = function() {
			...
			for (var i = 0; i <= $scope.purchaseInvoiceData.purchaseInvoiceDetail.length; i++) {
				...
				$scope.purchaseInvoiceData.purchaseInvoiceDetail[i].taxAmount=amt;
			}
		}
       ...
       if(purchaseInvoiceNo == undefined || purchaseInvoiceNo == null || purchaseInvoiceNo ==""){
           ...
           $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/edit?purchaseInvoiceNo='+parseInt(purchaseInvoiceNo)).success(function(data) {
               $scope.isEdit1 = true;
               ...
               bankId=data.bankDtl;
               $scope.purchaseInvoiceData.jobNo= data.jobNo.toString();
               $scope.purchaseInvoiceData.partyNo= data.partyNo.toString();
               $scope.purchaseInvoiceData.status=data.status.toString();
               $scope.purchaseInvoiceData.customerId=data.customerId.toString();
               $scope.purchaseInvoiceData.polId=data.polId.toString();    
               $scope.purchaseInvoiceData.podId=data.podId.toString(); 
               $scope.purchaseInvoiceData.originId=data.originId.toString(); 
               ...
               $scope.purchaseInvoiceData.bankId=data.bankId;
               $scope.purchaseInvoiceData.partyCountryId=data.partyCountryId;
               $scope.purchaseInvoiceData.hbl=data.hbl.toString();
               $scope.purchaseInvoiceData.mbl=data.mbl.toString();
               $scope.purchaseInvoiceData.draftSave=false;
               tempPurchaseInvoiceDtl=data.purchaseInvoiceDetail;
               ...
               for(var j=0;j< tempPurchaseInvoiceDtl.length;j++)  {
    			   ...
    			   $scope.purchaseInvoiceData.purchaseInvoiceDetail[j].conType=tempPurchaseInvoiceDtl[j].conType.toString();
...
$scope.onSubmitDraft = function(purchaseInvoiceForm,purchaseInvoiceData) {
       ...
       $scope.save = function(){
    	   ...
    	   console.log("$scope.purchaseInvoiceData:::::::::SAVE:::::::::::::::::");
       console.log($scope.purchaseInvoiceData);
       $scope.approvedData=[];
       $scope.rejectData=[];
       ...
       if($scope.approvedData.length >0){
              $http.post($stateParams.tenantid+'/app/seapurchaseinvoice/savePurInv', $scope.purchaseInvoiceData).success(function(result) {
               if (result.success) {
                   console.log(result)
                   $location.path($stateParams.tenantid+"/invoice/sea/seapurchaseinvoice/PurchaseInvoiceList");
 ...
 $scope.onSubmitDraft1 = function(purchaseInvoiceForm,purchaseInvoiceData) {
  ...
  // save Draft//
     ...
     $scope.printUsdFormDraft = function(purchaseInvoiceForm,purchaseInvoiceData) {
       ...
       $scope.printUsd = function(){
           ...
           if($scope.approvedData.length >0){
        	      ...
        	      $http.post($stateParams.tenantid+'/app/seapurchaseinvoice/savePurInvPreview', $scope.purchaseInvoiceData).success(function(result) {
                      if (result.success) {
                          
                  
                         var url = $stateParams.tenantid+'/app/seapurchaseinvoice/printusdandInr?purInvNo=' + result.purchaseInvoiceNo1 +'&printNumber='+$scope.purchaseInvoiceData.printNumber+ '&currSymbol=' +$scope.purchaseInvoiceData.currSymbol;
     ...
     app.controller('purchaseInvoiceTaxCtrl', function($scope, $rootScope,sharedProperties, 
    	 ...
    	 $scope.rowCollection=[];
     	$http.get($stateParams.tenantid + '/app/seasalesinvoice/getTaxpurList?chargeHead=' + chargeHeadId + '&jobNo=' + jobNo + '&partyNo=' + partyNo).success(
     			function(response) {
...
app.controller('seaPurchaseInvoiceViewCtrl', function($scope, $rootScope,sharedProperties, 
    ...
    // fetching edit details
        ...
        $scope.edit=true;
        $http.get($stateParams.tenantid+'/app/seapurchaseinvoice/getViewData?purInvNo='+purchaseInvoiceNo).success(function(data) {
            debugger;
 ...
 // invoiceCancel
    ...
    $scope.printPurchaseInvoice= function(puchaseInvoiceNo){
	    ...
	    var url = $stateParams.tenantid+'/app/seapurchaseinvoice/printfinalusdandInr?purInvNo=' + puchaseInvoiceNo +'&printNumber='+$scope.purchaseInvoiceData.printNumber+ '&currSymbol=' +$scope.purchaseInvoiceData.currSymbol;
        var wnd = $window.open(url, 'Shipping', 'height=700,width=800,toolbar=no,scrollbars=yes,fullscreen=no,status=yes,menubar=no,location=no,directories=no,resizable=yes');
        wnd.print();    
     }
    ...
    $scope.quickLinkMethd=function(qulkVal){
		if(qulkVal!='Select'){
			...
			$http.post($stateParams.tenantid + '/app/seapurchaseinvoice/getQuickLinkId?category='+qulkVal+'&prchsInvNo='+$scope.purchaseInvoiceData.purchaseInvoiceSno).success(function(datas) {
