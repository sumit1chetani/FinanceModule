'use strict';
app.controller('salesInvoiceListCtrl', function($scope, $rootScope, $http, $location, logger, $log, ngDialog, 
    ...
    $http.get($stateParams.tenantid + '/app/salesinvoice/getlist').success(
			function(response) {
				console.log(response);
				$scope.rowCollection = response.salesInvoiceList;
			});
    
    $http.get($stateParams.tenantid + '/app/salesinvoice/getDraftList').success(
			function(response) {
		        ...
		        $http.get($stateParams.tenantid+'/jobOrderMonthClose/getInvoiceDate?mode=' + $scope.modeType +'&invoiceId='+ salesSNo +'&invoiceType='+ $scope.invoiceType).success(function(datas) {
		            if(datas){
		                logger.logError("Job Order Closed Pls Contact IT Support");
	                	 ...
	                	 $location.path($stateParams.tenantid+"/invoice/salesinvoice/salesInvoiceDraftEdit/"+salesSNo)
	                 }
		        })
   };
 ...
 // invoice Draft Delete
  ...
  $scope.add = function() {
      $scope.isAdd  = true;
      $location.path($stateParams.tenantid+"/invoice/salesinvoice/salesInvoiceAdd");
  ...
  $scope.viewDraft = function(limit, offset) {
      $state.go('app.finance.seasalesinvoiceDraft',{tenantid:$stateParams.tenantid});
  };
 ...
 // Redirecting Page For View Functionality
    $scope.view = function(purchaseInvoiceNo,index) {
        $scope.isAdd  = false;
        $location.path($stateParams.tenantid+"/invoice/salesinvoice/salesInvoiceView/"+purchaseInvoiceNo);
    };
    ...
    /*  $scope.reversePIN = function(reverseInvoiceNo){
        ...
        $scope.clickTestInvoiceMail = function(salesSNO) {
            ngDialog.openConfirm().then(function() {
              //  var url = $stateParams.tenantid+'/app/purchaseinvoice/bulkPrint?fromPurInvoiceNo=' + fromPurInvoiceNo+"&toPurInvoiceNo="+toPurInvoiceNo;

                var myURL = $stateParams.tenantid+'/app/salesinvoice/sendMail?purchaseInvoiceNo=' + salesSNO;
                $http({
                    method : 'post',
                    url : myURL,
        ...
        if(quickLinkIdList!='' && quickLinkIdList != undefined){
        	 ...
        	 $http.get($stateParams.tenantid + '/app/salesinvoice/getlist').success(function(response) {
        					$scope.rowCollection = response.salesInvoiceList;
...
app.controller('airsalesInvoiceListDraftCtrl', function($scope, $rootScope, $http, $location, logger, $log, ngDialog, 
        $modal, utilsService, sharedProperties, $state,$window,$controller,$stateParams) {
    ...
    $scope.backToList = function() {
		$state.go('app.finance.salesinvoice',
				{
