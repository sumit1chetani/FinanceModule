app.controller('purchaseOrderAddCtrl', function($scope, $state, $http, ngDialog, logger, 
            $location, $controller, $injector, sharedProperties, toaster, $rootScope,$stateParams,
    ...
    $scope.cancel = function(){
        $state.go("app.finance.purchase.purchaseOrder.list");
    };
    ...
    $scope.loadVendorAddress = function(){
        var foundItemVen = $filter('filter')($scope.vendorList, { id:  $scope.purchaseOrder.vendorId })[0];
            ...
            $scope.purchaseOrder.paymentTerms= $scope.vendorList[indexVen].paymentTerms;
            }else{
    ...
    $scope.getListValue = function(data,addQuoteStatus){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].isSelected){
                ...
                $scope.purchaseOrder.freight = data[i].frieght;
                if($scope.purchaseOrder.paymentTerms == 0 || $scope.purchaseOrder.paymentTerms == null 
            ...
            if(isDetailSelected){ngDialog.close();}else{logger.logError("Please select an item to add purchase Order.");} 
...
app.controller('purchaseOrderRequestAddCtrl', function($scope, $http, ngDialog, logger, requestCode, $injector, 
