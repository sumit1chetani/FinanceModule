app.controller('bookingCtrl', function($scope, $window, $rootScope, $location, $http, $filter, logger, $log, ngDialog, $modal, utilsService, $state, $timeout,$stateParams) {
    $scope.polList = [];
    $scope.podList = [];
    $scope.payerList = [];
    $scope.customerList = [];
    ...
    $scope.getReprtHeader = function() {
        ...
        $http.get($stateParams.tenantid+'/app/bookingreport/getReprtHeader').success(function(datas) {
            console.log("report header")
            console.log(datas)
            $scope.messageHeaderList = datas.getBookingReportHeaderList;
            ...
            $("#pod").multiselect({
                ...
                onChange : function(element, checked) {
                    ...
                    if ($scope.podList.length > 0) {
                        console.log($scope.booking.lpod);
                        $scope.booking.pod = '';
                        angular.forEach($scope.booking.lpod, function(item, key) {
                            console.log("pod")
                            console.log(item);
                            if (ct == "") {
                                ct = item.id;
                            ...
                            $scope.booking.pod = ct;
                            console.log("pod selected")
                            console.log($scope.booking.pod)
                        });
                    }
            ...
            $("#pol").multiselect({
                ...
                onChange : function(element, checked) {
                    ...
                    if ($scope.polList.length > 0) {
                        console.log($scope.booking.lpol);
                        $scope.booking.pol = '';
                        angular.forEach($scope.booking.lpol, function(item, key) {
                            console.log("pol")
                            console.log(item);
                            if (ct == "") {
                                ct = item.id;
                            ...
                            $scope.booking.pol = ct;
                            console.log(" pol selected")
                            console.log($scope.booking.pol)
                        });
                    }
            ...
            $("#aod").multiselect({
                ...
                onChange : function(element, checked) {
                    ...
                    if ($scope.aodList.length > 0) {
                        console.log($scope.booking.laod);
                        $scope.booking.aod = '';
                        angular.forEach($scope.booking.laod, function(item, key) {
                            console.log("aod")
                            console.log(item);
                            if (ct == "") {
                                ct = item.id;
                            ...
                            $scope.booking.aod = ct;
                            console.log("aod selected")
                            console.log($scope.booking.aod)
                        });
                    }
            ...
            $("#aol").multiselect({
                ...
                onChange : function(element, checked) {
                    ...
                    if ($scope.aolList.length > 0) {
                        console.log($scope.booking.laol);
                        $scope.booking.aol = '';
                        angular.forEach($scope.booking.laol, function(item, key) {
                            console.log("aol")
                            console.log(item);
                            if (ct == "") {
                                ct = item.id;
                            ...
                            $scope.booking.aol = ct;
                            console.log(" aol selected")
                            console.log($scope.booking.aol)
                        });
                    }
			  	        ...
			  	        $("#service").multiselect({
			  	            ...
			  	            onChange : function(element, checked) {
			  	                ...
			  	                if ($scope.servicePartnerTypelist.length > 0) {
			  	                    ...
			  	                    angular.forEach($scope.booking.lservice, function(item, key) {
			  	                        if (cc == "") {
			  	                            cc = item.id;
			  	                        } else {
			  	                            cc += "," + item.id;
			  	                        }
			  	                        $scope.booking.service = cc;
			  	                    });
			  	                }
						  	        ...
						  	        $("#term").multiselect({
						  	            ...
						  	            onChange : function(element, checked) {
						  	                ...
						  	                if ($scope.TermList.length > 0) {
						  	                    $scope.booking.term = '';
						  	                    angular.forEach($scope.booking.lterm, function(item, key) {
						  	                        if (cc == "") {
						  	                            cc = item.id;
						  	                        } else {
						  	                            cc += "," + item.id;
						  	                        }
						  	                        $scope.booking.term = cc;
						  	                    });
						  	                }
	        ...
	        $("#sales").multiselect({
	            ...
	            onChange : function(element, checked) {
	                ...
	                if ($scope.salesTypeList.length > 0) {
	                    $scope.booking.sales = '';
	                    angular.forEach($scope.booking.lsales, function(item, key) {
	                        if (cc == "") {
	                            cc = item.id;
	                        } else {
	                            cc += "," + item.id;
	                        }
	                        $scope.booking.sales = cc;
	                    });
	                }
        ...
        $("#status").multiselect({
            ...
            onChange : function(element, checked) {
                ...
                if ($scope.salesTypeList.length > 0) {
                    ...
                    angular.forEach($scope.booking.lstatus, function(item, key) {
                        if (cc == "") {
                            cc = item.id;
                        } else {
                            cc += "," + item.id;
                        }
                        $scope.booking.status = cc;
                    });
                }
...
/*    $timeout(function() { // You might need this timeout to be sure its run
  	        ...
  	        $("#branch").multiselect({
  	            ...
  	            onChange : function(element, checked) {
  	                ...
  	                if ($scope.branchList.length > 0) {
  	                    ...
  	                    angular.forEach($scope.booking.lbranch, function(item, key) {
  	                        if (cc == "") {
  	                            cc = item.id;
  	                        } else {
  	                            cc += "," + item.id;
  	                        }
  	                        $scope.booking.branch = cc;
  	                    });
  	                }
  ...
  /*  $scope.creditCategorylist = [ {
        ...
        $("#customerType").multiselect({
            ...
            onChange : function(element, checked) {
                ...
                if ($scope.customerTypelist.length > 0) {
                    console.log($scope.booking.lCustomerType);
                    $scope.booking.customerType = '';
                    angular.forEach($scope.booking.lCustomerType, function(item, key) {
                        ...
                        $scope.booking.customerType = ct;
                        console.log(" customer selected")
                        console.log($scope.booking.customerType)
                    });
                }
        ...
        $("#category").multiselect({
            ...
            onChange : function(element, checked) {
                ...
                if ($scope.categoryWiseList.length > 0) {
                    $scope.booking.category = '';
                    angular.forEach($scope.booking.lcategory, function(item, key) {
                        if (c == "") {
                            c = item.id;
                        } else {
                            c += "," + item.id;
                        }
                        $scope.booking.category = c;
                    });
                }
    ...
    // function call
    $scope.onSearch = function(booking) {
        console.log("delivery order  search");
        console.log(booking);

        $http.post($stateParams.tenantid+"/app/bookingreport/getReport", booking).success(function(response) {

            $scope.rowCollection = response.lbookingReportlist;
            console.log("final result")
            console.log($scope.rowCollection)
            $scope.excelList = response.lbookingReportlist;
...
/*    // excel
    ...
    // excel export
    $scope.excel = function(booking) {

        var groupHeaderSelectedList = $filter('filter')($scope.messageHeaderList, {
            visible : true
        }, true);
        var objWholeData = {
            'lbookingReportlist' : $scope.excelList,
            'lbookingReportheaderlist' : groupHeaderSelectedList,
            'BookingReportBean' : booking
        };
        ...
        $http.post($stateParams.tenantid+"/app/bookingreport/generateExcel", objWholeData).success(function(response) {

            $('#exportXl').append('<a id="tbExcelExport" stype="display:none" href="filePath/Booking.xls" download="Booking.xls"></a>');
