'use strict';
app.controller('prepaidExpensesCtrl', function($scope, $rootScope,$filter , $controller,$http, $location, logger, $log, ngDialog, $modal, utilsService, sharedProperties, $state,$stateParams) {
    ...
    $scope.getListDetail=function(){
        $http.get($stateParams.tenantid+'/app/prepaidExpenses/list').success(function(datas){
            console.log(datas);
            $scope.rowCollection = datas.lPrepaidExpensesBean;
    ...
    $scope.approve=function(table){
       ...
       if($scope.tablerow.length>0){
           ngDialog.open({
               ...
               controller : $controller('prepaidExpensesApproveCtrl', {
                   $scope : $scope,
                   sVoucherNo: '21',
    ...
    if(window.localStorage.getItem('prepaidexp')==$scope.currentURL){
         alert('window ' + $scope.currentURL + ' is already opened');
        // window.focus();
         //window.open($rootScope.currentURL,'_self').close();
         setTimeout(window.close(),5000);
     }else{
         window.localStorage.setItem('prepaidexp', $scope.currentURL);
     }
    $(window).unload(function(){
        localStorage.removeItem('prepaidexp');
        });
    ...
    $scope.excel=function(){
      // alert("hi");
            ...
            $scope.selectinterest={
                    lPrepaidExpensesBean:obj,
                    filename:"PrepaidExpanses"+day+""+mon+""+yr+""+n+""+s+""+ms
            }
            
            
            $scope.filename="PrepaidExpanses"+day+""+mon+""+yr+""+n+""+s+""+ms+".xlsx";
            
           
            
            $http.post($stateParams.tenantid+'/app/prepaidExpenses/generateExcel',$scope.selectinterest).success(function(data) {
                console.log(data);
...
app.controller('prepaidExpensesApproveCtrl', function($scope, $http,ngDialog,logger,$location,sVoucherNo,screenName,$timeout,$stateParams) {
    ...
    $scope.reverseConfirm = function(){
        ...
        if($scope.screenNames=="approvalDialog"){
            if($scope.prepaidExp.approvalDate!=""){
                var obj={
                        lPrepaidExpensesBean:$scope.tablerow,
                        approvalDate:$scope.prepaidExp.approvalDate
                }
                $http.post($stateParams.tenantid+'/app/prepaidExpenses/createJvForPrepaidExpenses', obj).success(function(result) {
                if(result.success) {
                    logger.logSuccess("Approved successfully!");
                    $scope.closeCBPReverseDialog();
                } else {
