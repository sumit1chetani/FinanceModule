...
app.controller('servicepartnerCtrl', function($scope, $rootScope,ngDialog, $http, $location, logger,
    ...
    $scope.add = function() {
    	 $state.go('app.master.servicepartner.add',{tenantid:$stateParams.tenantid});
    };
    $scope.getList=function(){
    	var name = 'mascustomer';
    $http.get($stateParams.tenantid+'/app/master/servicepartner/list?name='+name).success(function(datas) {
        console.log(datas);
    ...
    $scope.servicePartner = {
      ...
      $scope.deleteRow = function(rowid) {
            ...
            ngDialog.openConfirm().then(function() {
                var myURL = $stateParams.tenantid+'/app/master/servicepartner/delete?servicePartnerId='+rowid;
                $http({
                    method : 'post',
                    url : myURL,
                    data : rowid,
    ...
    $scope.editRow = function(rowid) {    	
    	$location.url($stateParams.tenantid+'/master/servicepartner/edit?rowid='+rowid);       
     }
    
    $scope.view = function(rowid) {
    	$location.url($stateParams.tenantid+'/master/servicepartner/view?rowid='+rowid); 
     }
...
console.log($scope.rowCollection);
        ...
        $http.get($stateParams.tenantid+"/app/master/servicepartner/generateExcel").success(function(response) {
        	console.log(response);
            $('#exportXl').append('<a id="tbExcelExport" stype="display:none" href="filePath/Customer.xls" download="Customer.xls"></a>');
		...
		for (var i = 0; i < fName.length; i++) {
			if (fName[i] == "xls" || fName[i] == "xlsx") {
				...
				$
						.ajax({
							type : "POST",
							url : $stateParams.tenantid+'/app/master/servicepartner/uploadfile',
							data : frmData,
							contentType : false,
							processData : false,
...
//									}
    ...
    $http.post($stateParams.tenantid+'/app/master/servicepartner/createLogin?rowid='+rowid).success(function(data) {
    	if(data.success){
