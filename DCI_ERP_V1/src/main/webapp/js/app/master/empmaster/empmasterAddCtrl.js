'use strict';
app.controller('empmasterAddCtrl', function($scope,$stateParams,$controller, ngDialog,$filter, $timeout, toaster, $injector,
		$rootScope, $http, $location, logger, utilsService, $state, sharedProperties, $window, validationService) {
    ...
    $http.get($stateParams.tenantid+'/app/commonUtility/getBankList').success(function(datas) {
    ...
    $http.post($stateParams.tenantid+'/app/master/vendor/dropDownList').success(function(data) {
       	if(data){
       ...
       /* $scope.editRow = function(employeeId, fromdate) {
            debugger
            var departmentId = $scope.employeePaycomponenet.departmentId;
            var companyId = $scope.employeePaycomponenet.companyId;
            var branchId = $scope.employeePaycomponenet.branchId;
            $location.url($stateParams.tenantid+"/payroll/salaryfixation/edit?employeeId=" + $scope.employeePaycomponenet.employeeId + '&fromdate=' + fromdate + '&departmentId=' + departmentId + '&branchId=' + branchId + '&companyId=' + companyId);
        }*/
  ...
  //employeee changes
    ...
    $scope.viewRow = function(empId){
    $scope.EmployeeMasterData.empId =empId;
    $window.viewRow = $scope.EmployeeMasterData;
    var employeeId=empId;
    //$window.open('/#/cordelia/payroll/salaryfixation/list');
    /*$http
	.get(
			$stateParams.tenantid
					+ "payroll/payroll/employeepaycom/list?employeeId="
					+ empId)
	.success(
			function(response) {
			       
			       angular.forEach(response.employeeComponentList, function(value, key) {        
			           var date =new date();
			       ...
			       if(response.employeeComponentList.length>0){
			    	   var  fromDate =response.employeeComponentList[0].fromdate.split("-");
			    	     fromDate =  fromDate[2]+'/' +  fromDate[1] + '/'+ fromDate[0];
			       $window.open('/#/cordelia/payroll/salaryfixation/edit?employeeId='
					+ response.employeeComponentList[0].employeeId + '&fromdate=' + fromDate
					+ '&departmentId=' + response.employeeComponentList[0].departmentId
					+ '&branchId=' + response.employeeComponentList[0].branchId + '&companyId='
	...
	.post(
			$stateParams.tenantid
					+ "payroll/payroll/salarycomfixation/list",$scope.EmployeeMasterData)
	.success(
			function(response) {
			       
			       /*angular.forEach(response.empPayComList, function(value, key) {        
			           var date =new date();
                        if(date<value.fromdate)
			    		  {
			    		  date=value.fromdate;
			       ...
			       if(response.empPayComList.length>0){
			    	   /*var  fromDate =response.employeeComponentList[0].fromdate.split("-");
			    	     fromDate =  fromDate[2]+'/' +  fromDate[1] + '/'+ fromDate[0];*/
			       $window.open('/#/cordelia/payroll/salarycompfixation/edit?employeeId='
					+ response.empPayComList[0].empId + '&fromdate=' + response.empPayComList[0].fromdate) 
			    	  
			
			  
			}else{
				$window.open('/#/cordelia/payroll/salarycompfixation/list');
			}
			});
};
...
/*
    $scope.viewRow = function(employeeId) {
    ...
    $scope.EmployeeMasterDataPayRollBank = {
        ...
        var tmpEmployeeMasterDataPayRollBank = angular.copy($scope.EmployeeMasterDataPayRollBank);
        ...
        $scope.EmployeeMasterDataAircliams = {
            ...
            var tmpEmployeeMasterDataAircliams = angular.copy($scope.EmployeeMasterDataAircliams);
            ...
            $scope.EmployeeMasterDataSettle = {
                ...
                var tmpEmployeeMasterDataSettle = angular.copy($scope.EmployeeMasterDataSettle);
        ...
        $scope.EmployeeMasterDatapassport = {
            ...
            var tmpEmployeeMasterDatapassport = angular.copy($scope.EmployeeMasterDatapassport);
            
            
            $scope.EmployeeMasterDataAssets = {
                ...
                var tmpEmployeeMasterDataAssets = angular.copy($scope.EmployeeMasterDataAssets);
    ...
    $scope.tabs = [ {
        ...
        title : 'Passport Release Request',
    ...
    $scope.callDialogEmergency = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template : 'views/master/empmaster/employeeMasterEmergency',
    ...
    $scope.callDialogNomination = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template : 'views/master/empmaster/employeeNomination',
    ...
    $scope.callDialogReference = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeReference',
    ...
    $scope.callDialogProbation = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeProbationAdd',
    ...
    $scope.callDialogDocument = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeDocumentsAdd',
    ...
    $scope.callDialogFamily = function($scope,isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeMasterFamily',
    ...
    $scope.callDialogMerits = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeMasterMerits',
    ...
    $scope.callDialogEducation = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template : 'views/master/empmaster/employeeMasterEducation',
    ...
    $scope.callDialogExperiance = function($scope, isFunction, employeeEdit, employeeId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template :'views/master/empmaster/employeeExperianceAdd',
    ...
    $scope.callDialog = function($scope, designationId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowData) {
        ngDialog.open({
            scope : $scope,
            template : 'views/master/empmaster/multiPhoneNo',
    ...
    $scope.callDialogMobile = function($scope, designationId, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, rowDatas) {
        ngDialog.open({
            scope : $scope,
            template : 'views/hrms/master/employeeMaster/multiMobileNo',
...
*/
    ...
    $scope.getBranchList = function(companyCode) {
        ...
        if (companyCode != "") {
            ...
            $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/getBranchList1',comp).success(function(datas) {
...
debugger
        if ($scope.EmployeeMasterData.doj != undefined && $scope.EmployeeMasterData.relieveDate != "") {
...
//                $scope.EmployeeMasterData.firstName1 = result.editList[0].grade.toString();
               ...
               // $scope.getBranchList($scope.EmployeeMasterData.companyCode);
                ...
                $scope.EmployeeMasterData.altreportMangrId = result.editList[0].altreportMangrId;
              ...
              //  $scope.getReportDesig($scope.EmployeeMasterData.reportMangrId);
                ...
                var altreportMangrName = result.editList[0].altreportMangrId+ " - "+ result.editList[0].altreportManagerName;
                $scope.employeeDuplicateEdit = [ {
                    ...
                    columnName : 'Alternative Reporting AUthority',
                    oldValue : altreportMangrName,
                    newValue : '',
                    active : false,
                    index : 11
                },
              ...
              /*  if ($scope.EmployeeMasterData.status == "true") {
                ...
                var filePhoto;
                if ($scope.EmployeeMasterData.uploadPhoto != undefined) {
                	filePhoto = $scope.EmployeeMasterData.uploadPhoto;
                } else {
                	filePhoto = "";
                }
...
//                $('#image-holder').show();
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editPayRoll?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDataPayRollBank = result.objEmployeeMasterAddressBean;
                    }
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editFormReview?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDataforms = result.objEmployeeMasterAddressBean;
                    }
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editAssets?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDataAssets = result.objEmployeeMasterAddressBean;
                    }
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editPassportReqeuest?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDatapassport = result.objEmployeeMasterAddressBean;
                    }
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editAirClaim?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDataAircliams = result.objEmployeeMasterAddressBean;
                    }
                ...
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/editSettlement?empId=' + editEmployeeID).success(function(result) {
                    $scope.isEdit = true;
                    if( result.objEmployeeMasterAddressBean!=undefined &&  result.objEmployeeMasterAddressBean!=null &&  result.objEmployeeMasterAddressBean!=''){
                        $scope.EmployeeMasterDataSettle = result.objEmployeeMasterAddressBean;
                    }
                });
    ...
    $scope.saveProfile = function(form, EmployeeMasterData) {
        ...
        if (new validationService().checkFormValidity(form)) {
            ...
            if (flag == true && flag1==true) {
                ...
                var altreportMangrName = $("#altreportMangrId option:selected").text();
...
//                    	$scope.EmployeeMasterData.firstName = $scope.EmployeeMasterData.firstName1 + " "+$scope.EmployeeMasterData.firstName;
                    	if(EmployeeMasterData.firstName1 != undefined && EmployeeMasterData.firstName1 != "" && EmployeeMasterData.firstName1 != null)
{
                        ...
                        logger.logError("Please Fill the First Name !");
    ...
    $scope.updateProfileDet = function(form, EmployeeMasterData) {
        ...
        if (new validationService().checkFormValidity(form)) {
            ...
            if (flag == true && flag1==true) {
                ...
                var altreportMangrId = $scope.EmployeeMasterData.altreportMangrId;
                var altreportMangrName = $("#altreportMangrId option:selected").text();
                var columnAltReportingAuthority = "Alternate Reporting Authority";
                angular.forEach($scope.employeeDuplicateEdit, function(value, key) {
                    ...
                    if (value.columnName == columnAltReportingAuthority) {
                        if (value.oldValue != altreportMangrName) {
                            value.active = true;
                            value.newValue = altreportMangrName;
                        }
                    }
                });
                ...
                if($scope.EmployeeMasterData.esic!=''&& $scope.EmployeeMasterData.esic!=null && $scope.EmployeeMasterData.esic!=undefined && $scope.EmployeeMasterData.epfNo!=''&&$scope.EmployeeMasterData.epfNo!=null && $scope.EmployeeMasterData.epfNo!=undefined){
                    if($scope.EmployeeMasterData.esic!=$scope.EmployeeMasterData.epfNo){
//                    	EmployeeMasterData.firstName = EmployeeMasterData.firstName1 + " "+EmployeeMasterData.firstName;
                    	if(EmployeeMasterData.firstName1 != undefined && EmployeeMasterData.firstName1 != "" && EmployeeMasterData.firstName1 != null)
                    	{
...
//                	EmployeeMasterData.firstName = EmployeeMasterData.firstName1 + " "+EmployeeMasterData.firstName;
    ...
    $scope.savePayBank = function(frmPayBank, EmployeeMasterDataPayRollBank) {
        EmployeeMasterDataPayRollBank.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/savePayBank', EmployeeMasterDataPayRollBank).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.savefrmAssets = function(frmAssets, EmployeeMasterDataAssets) {
    	EmployeeMasterDataAssets.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/savefrmAssets', EmployeeMasterDataAssets).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
                        $scope.isfrmAssets = true;
                        $scope.isOnLoad = true;
                        logger.logSuccess("Saved Succesfully!");
                    } else {
                        logger.logError("Not Saved!");
    ...
    $scope.saveAirClaim = function(frmPayBank, EmployeeMasterDataAircliams) {
    	EmployeeMasterDataAircliams.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/saveAirClaim', EmployeeMasterDataAircliams).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
                        $scope.isAirClaim = true;
                        $scope.isOnLoad = true;
                        logger.logSuccess("Saved Succesfully!");
                    } else {
                        logger.logError("Not Saved!");
    ...
    $scope.saveSettle = function(frmSettle, EmployeeMasterDataSettle) {
    	EmployeeMasterDataSettle.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/saveSettle', EmployeeMasterDataSettle).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
    ...
    //SAVE PASSPORT
    $scope.savePassport = function(frmPassport, EmployeeMasterDatapassport) {
    	EmployeeMasterDatapassport.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/savePassport', EmployeeMasterDatapassport).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.saveFormReview = function(frmReview, EmployeeMasterDataforms) {
    	EmployeeMasterDataforms.empId = $scope.employeeId;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/saveFormReview', EmployeeMasterDataforms).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result.success == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.downloadDoc = function(EmployeeMasterDataDoc,fileName) {
        if (EmployeeMasterDataDoc.uploadDoc == "" || EmployeeMasterDataDoc.uploadDoc == null) {
            logger.logError("File Not Found..!");
        } else {
            debugger;
            $('#exportPath').bind('click', function() {
            });
            $('#exportPath').simulateClick('click');
            logger.logSuccess("Downloaded successfully!");
        }
    }
    ...
    $scope.updatePersonalInfo = function(frmPersonalInfo, EmployeeMasterDataPersonal) {
    	EmployeeMasterDataPersonal.empId = $scope.employeeId;
        if (new validationService().checkFormValidity(frmPersonalInfo)) {
            var isPanNo=true,isMBl=true;
            ...
            toaster.pop('error', "Please fill the required fields", logger.getErrorHtmlNew(frmPersonalInfo.$validationSummary), 5000, 'trustedHtml');
        }
    };
    ...
    $scope.updatePayBank = function(frmPayBank, EmployeeMasterDataPayRollBank) {
        EmployeeMasterDataPayRollBank.empId = $scope.employeeEdit;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updatePayBank', EmployeeMasterDataPayRollBank).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    editEmployeeID = EmployeeMasterDataPayRollBank.empId;
    ...
    $scope.updatefrmAssets = function(frmAssets, EmployeeMasterDataAssets) {
    	EmployeeMasterDataAssets.empId = $scope.employeeEdit;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updatefrmAssets', EmployeeMasterDataAssets).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.updateAirClaim = function(frmAirClaim, EmployeeMasterDataAircliams) {
    	EmployeeMasterDataAircliams.empId = $scope.employeeEdit;
    	EmployeeMasterDataAircliams.airadulttickAmt=parseInt(EmployeeMasterDataAircliams.airadulttickAmt);
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updateAirClaim', EmployeeMasterDataAircliams).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.updateSettle = function(frmSettle, EmployeeMasterDataSettle) {
    	EmployeeMasterDataSettle.empId = $scope.employeeEdit;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updateSettle', EmployeeMasterDataSettle).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result == true) {
                        $scope.employeeId = result.empId;
                        $scope.frmSettle = true;
                        $scope.isOnLoad = true;
                        logger.logSuccess("Updated Succesfully!");
                    } else {
                        logger.logError("Not Updated!");
    ...
    $scope.updatePassport = function(frmPassport, EmployeeMasterDatapassport) {
    	EmployeeMasterDatapassport.empId = $scope.employeeEdit;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updatePassport', EmployeeMasterDatapassport).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result == true) {
                        $scope.employeeId = result.empId;
    ...
    $scope.updateFormReview = function(frmReview, EmployeeMasterDataforms) {
    	EmployeeMasterDataforms.empId = $scope.employeeEdit;
      
                $http.post($stateParams.tenantid+'/hrms/master/employeeAdminMaster/updateformReview', EmployeeMasterDataforms).success(function(result) {
                    console.log("result is" + result);
                    console.log(result);
                    if (result == true) {
                        $scope.employeeId = result.empId;
                        $scope.isformsreview = true;
                        $scope.isOnLoad = true;
                        logger.logSuccess("Updated Succesfully!");
                    } else {
                        logger.logError("Not Updated!");
    ...
    $scope.updateDocumentDet = function(frmPersonalDetails, EmployeeMasterDataPerDet) {
        ...
        if(EmployeeMasterDataPerDet.issuedDate!=undefined && EmployeeMasterDataPerDet.issuedDate!=null && EmployeeMasterDataPerDet.issuedDate!=''){
            if($scope.copyPasIssueDate !=EmployeeMasterDataPerDet.issuedDate){
               var fromDate = EmployeeMasterDataPerDet.issuedDate;
               var dobDate = $scope.EmployeeMasterData.dob;
               dobDate = dobDate.split("/");
               dobDate = new Date(dobDate[2], dobDate[1] - 1, dobDate[0]);
               fromDate = fromDate.split("/");
        ...
        }if(EmployeeMasterDataPerDet.expiryDate!=undefined && EmployeeMasterDataPerDet.expiryDate!=null && EmployeeMasterDataPerDet.expiryDate!=''){
           var isExp =true;
           var fromDate = EmployeeMasterDataPerDet.expiryDate;
           fromDate = fromDate.split("/");
           fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
           
           if($scope.copyPasExpDate !=EmployeeMasterDataPerDet.expiryDate){
               if (fromDate > todayDate ) {
                   if(EmployeeMasterDataPerDet.issuedDate!=undefined && EmployeeMasterDataPerDet.issuedDate!=null && EmployeeMasterDataPerDet.issuedDate!=''){
                       var IssueDate =EmployeeMasterDataPerDet.issuedDate;
                       IssueDate = IssueDate.split("/");
                       IssueDate = new Date(IssueDate[2], IssueDate[1] - 1, IssueDate[0]);
        ...
        } if(EmployeeMasterDataPerDet.licenseIssuedDate!=undefined && EmployeeMasterDataPerDet.licenseIssuedDate!=null && EmployeeMasterDataPerDet.licenseIssuedDate!=''){
            if($scope.copylinExpDate !=EmployeeMasterDataPerDet.licenseIssuedDate){
                var dobDate = $scope.EmployeeMasterData.dob;
                dobDate = dobDate.split("/");
                dobDate = new Date(dobDate[2], dobDate[1] - 1, dobDate[0]);
                var fromDate = EmployeeMasterDataPerDet.licenseIssuedDate;
                fromDate = fromDate.split("/");
                fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
                if (fromDate < todayDate && fromDate > dobDate) {
                } else {
         ...
         }if(EmployeeMasterDataPerDet.licenseexpiryDate!=undefined && EmployeeMasterDataPerDet.licenseexpiryDate!=null && EmployeeMasterDataPerDet.licenseexpiryDate!=''){
            var isExp1 =true;
            var fromDate = EmployeeMasterDataPerDet.licenseexpiryDate;
            fromDate = fromDate.split("/");
            fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
            var dobDate = $scope.EmployeeMasterData.dob;
            dobDate = dobDate.split("/");
            dobDate = new Date(dobDate[2], dobDate[1] - 1, dobDate[0]);
            if($scope.copylinIssueDate !=EmployeeMasterDataPerDet.licenseexpiryDate){
                if (fromDate > todayDate ) {
                    if(EmployeeMasterDataPerDet.licenseIssuedDate!=undefined && EmployeeMasterDataPerDet.licenseIssuedDate!=null &&EmployeeMasterDataPerDet.licenseIssuedDate!=''){
                        var IssueDate = EmployeeMasterDataPerDet.licenseIssuedDate;
                        IssueDate = IssueDate.split("/");
                        IssueDate = new Date(IssueDate[2], IssueDate[1] - 1, IssueDate[0]);
         ...
         }if(EmployeeMasterDataPerDet.renewalDate!=undefined && EmployeeMasterDataPerDet.renewalDate!=null && EmployeeMasterDataPerDet.renewalDate!=''){
             var isExp1 =true;
             var fromDate = EmployeeMasterDataPerDet.renewalDate;
             fromDate = fromDate.split("/");
             fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
             if($scope.copylinReDate !=EmployeeMasterDataPerDet.renewalDate){
                 if (fromDate > todayDate ) {
                     if(EmployeeMasterDataPerDet.licenseexpiryDate!=undefined && EmployeeMasterDataPerDet.licenseexpiryDate!=null && EmployeeMasterDataPerDet.licenseexpiryDate!=''){
                         var expDate = EmployeeMasterDataPerDet.licenseexpiryDate;
                         expDate = expDate.split("/");
                         expDate = new Date(expDate[2], expDate[1] - 1, expDate[0]);
                         if (fromDate < expDate) {
                         } else {
         ...
         if(EmployeeMasterDataPerDet.visaIssuedDate!=undefined && EmployeeMasterDataPerDet.visaIssuedDate!=null && EmployeeMasterDataPerDet.visaIssuedDate!=''){
             if($scope.copyvisaIssueDate !=EmployeeMasterDataPerDet.visaIssuedDate){
                 var dobDate = $scope.EmployeeMasterData.dob;
                 dobDate = dobDate.split("/");
                 dobDate = new Date(dobDate[2], dobDate[1] - 1, dobDate[0]);
                 var fromDate =EmployeeMasterDataPerDet.visaIssuedDate;
                 fromDate = fromDate.split("/");
                 fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
                 if (fromDate < todayDate && fromDate > dobDate) {
                 } else {
          ...
          }if(EmployeeMasterDataPerDet.visaExpiryDate!=undefined && EmployeeMasterDataPerDet.visaExpiryDate!=null && EmployeeMasterDataPerDet.visaExpiryDate!=''){
             var isExp2 =true;
             var fromDate = EmployeeMasterDataPerDet.visaExpiryDate;
             fromDate = fromDate.split("/");
             fromDate = new Date(fromDate[2], fromDate[1] - 1, fromDate[0]);
             if($scope.copyvisaExpDate !=EmployeeMasterDataPerDet.visaExpiryDate){
                 if (fromDate > todayDate ) {
                     if(EmployeeMasterDataPerDet.visaIssuedDate!=undefined && EmployeeMasterDataPerDet.visaIssuedDate!=null && EmployeeMasterDataPerDet.visaIssuedDate!=''){
                         var IssueDate = EmployeeMasterDataPerDet.visaIssuedDate;
                         IssueDate = IssueDate.split("/");
                         IssueDate = new Date(IssueDate[2], IssueDate[1] - 1, IssueDate[0]);
                         if (fromDate > IssueDate) {
                         } else {
    ...
    //new upload by jayanthi 
    
    $rootScope.uploadImgLink = function(element) {
	    $scope.imgfile = element.files[0];
	    /*$rootScope.uploadImg();*/
	}
    ...
    $scope.$watch('EmployeeMasterData.salarybreakup', function(newVal, oldVal) {
        var newValue = $scope.EmployeeMasterData.salarybreakup;
        var arr = String(newValue).split("");
        var oldValue;
        if (arr.length == undefined)
            return;
            ...
            $scope.EmployeeMasterData.salarybreakup = oldValue;
            logger.logError("Number Only Allowed");
        }
    });
...
app.controller('employeeEmergencyAdminAddCtrl', function($scope,$stateParams, isFunction, employeeEdit, employeeId, $state, $http,
		ngDialog, $controller, logger, $injector, $location, sharedProperties, toaster, $rootScope, $filter, validationService, rowData) {
...
app.controller('employeeMasterDocumentAdminAddCtrl', function($scope,$stateParams, isFunction, employeeEdit, employeeId, $state, $http, ngDialog, $controller, logger, $injector, $location, sharedProperties, toaster, $rootScope, $filter, validationService, rowData) {
    ...
    $rootScope.uploadDocumentFiles = function() {
        ...
        //frmData.append("fileName1",$scope.tempDoc.name);

        // $scope.nomineePhoto=frmData;
        $.ajax({
            type : "POST",
            url : $stateParams.tenantid+"/hrms/master/employeeAdminMaster/uploadfileNew",
            data : frmData,
            contentType : false,
            processData : false,
            success : function(result) {
