...
app.controller('leaseAgreementListCtrl', function($scope, $rootScope, $http, $location, logger, utilsService, ngDialog, $state,sharedProperties,$window,$stateParams) {
    ...
    $scope.add = function() {
        $state.go('app.eqc.leaseagreementtype.add',{tenantid:$stateParams.tenantid});
    };
    ...
    $scope.editRow = function(rowid) {   
    	...
    	$location.url($stateParams.tenantid+'/eqc/leaseagreementtype/edit?code='+rowid);    
     }
    
 $scope.viewRow = function(rowid) {   
    	
    	$location.url($stateParams.tenantid+'/eqc/leaseagreementtype/view?code='+rowid);    
     }
