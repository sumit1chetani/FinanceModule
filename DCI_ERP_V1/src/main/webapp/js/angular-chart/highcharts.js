...
(function(root, factory) {
...
}(typeof window !== 'undefined' ? window : this, function(win) { // eslint-disable-line
    // no-undef
    // encapsulated variables
    var UNDEFINED, doc = win.document, math = Math, mathRound = math.round, mathFloor = math.floor, mathCeil = math.ceil, mathMax = math.max, mathMin = math.min, mathAbs = math.abs, mathCos = math.cos, mathSin = math.sin, mathPI = math.PI, deg2rad = mathPI * 2 / 360,

    // some variables
    userAgent = (win.navigator && win.navigator.userAgent) || '', isOpera = win.opera, isMS = /(msie|trident|edge)/i.test(userAgent) && !isOpera, docMode8 = doc && doc.documentMode === 8, isWebKit = !isMS && /AppleWebKit/.test(userAgent), isFirefox = /Firefox/.test(userAgent), isTouchDevice = /(Mobile|Android|Windows Phone)/.test(userAgent), SVG_NS = 'http://www.w3.org/2000/svg', hasSVG = doc && doc.createElementNS && !!doc.createElementNS(SVG_NS, 'svg').createSVGRect, hasBidiBug = isFirefox && parseInt(userAgent.split('Firefox/')[1], 10) < 4, // issue
    // #38
    ...
    }, charts = [], chartCount = 0, PRODUCT = 'Highcharts', VERSION = '4.2.5',

    // some constants for frequently used strings
    DIV = 'div', ABSOLUTE = 'absolute', RELATIVE = 'relative', HIDDEN = 'hidden', PREFIX = 'highcharts-', VISIBLE = 'visible', PX = 'px', NONE = 'none', M = 'M', L = 'L', numRegex = /^[0-9]+$/, NORMAL_STATE = '', HOVER_STATE = 'hover', SELECT_STATE = 'select', marginNames = [ 'plotTop', 'marginRight', 'marginBottom', 'plotLeft' ],
    ...
    /**
     * An animator object. One instance applies to one property (attribute or
     * style prop) on one element.
     * 
     * @param {object}
     *            elem The element to animate. May be a DOM element or a
     *            Highcharts SVGElement wrapper.
     * @param {object}
     *            options Animation options, including duration, easing, step
     *            and complete.
     * @param {object}
     *            prop The property to animate.
     */
    ...
    Fx.prototype = {
        ...
        dSetter : function() {
            ...
            if (now === 1) { // land on the final path without adjustment
        ...
        run : function(from, to, unit) {
            var self = this, timer = function(gotoEnd) {
         ...
         * 
         * @param {Boolean}
         *            gotoEnd Whether to go to then endpoint of the animation
         *            after abort
         * @returns {Boolean} True if animation continues
         */
        step : function(gotoEnd) {
            var t = +new Date(), ret, done, options = this.options, elem = this.elem, complete = options.complete, duration = options.duration, curAnim = options.curAnim, i;

            if (elem.attr && !elem.element) { // #2616, element including flag
                // is destroyed
                ret = false;
        ...
        initPath : function(elem, fromD, toD) {
            ...
            isArea = elem.isArea, positionFactor = isArea ? 2 : 1, sixify = function(arr) { // in
                // splines
                // make
                // move
                // points
            ...
            // If shifting points, prepend a dummy point to the end path. For
            // areas,
            ...
            if (start.length) {
                ...
                while (start.length < endLength) {
                    ...
                    // Pull out the slice that is going to be appended or
                    // inserted. In a line graph,
                    // the positionFactor is 1, and the last point is sliced
                    // out. In an area graph,
                    // the positionFactor is 2, causing the middle two points to
                    // be sliced out, since
                    // an area path starts at left, follows the upper path then
                    // turns and follows the
                    // bottom back.
                    ...
                    // Now insert the slice, either in the middle (for areas) or
                    // at the end (for lines)
    ...
    dateFormat = function(format, timestamp, capitalize) {
        ...
        var date = new Date(timestamp - getTZOffset(timestamp)), key, // used
        // in
        // for
        // constuct
        // below
        // get the basic time values
        hours = date[getHours](), day = date[getDay](), dayOfMonth = date[getDate](), month = date[getMonth](), fullYear = date[getFullYear](), lang = defaultOptions.lang, langWeekdays = lang.weekdays, shortWeekdays = lang.shortWeekdays,

        // List all format keys. Custom formats can be added from the outside.
        replacements = extend({

            // Day
            'a' : shortWeekdays ? shortWeekdays[day] : langWeekdays[day].substr(0, 3), // Short
            // weekday,
            // like
            // 'Mon'
    ...
    function normalizeTickInterval(interval, multiples, magnitude, allowDecimals, preventExceed) {
        ...
        for (i = 0; i < multiples.length; i++) {
            ...
            if ((preventExceed && retInterval * magnitude >= interval) || // only
            // allow
            // tick
            // amounts
            // smaller
     ...
     * @param {String}
     *            decimalPoint The decimal point, defaults to the one given in
     *            the lang options
     * @param {String}
    ...
    Highcharts.numberFormat = function(number, decimals, decimalPoint, thousandsSep) {
        ...
        var lang = defaultOptions.lang, origDec = (number.toString().split('.')[1] || '').length, decimalComponent, strinteger, thousands, absNumber = Math.abs(number), ret;

        if (decimals === -1) {
            decimals = Math.min(origDec, 20); // Preserve decimals. Not huge
            // numbers (#3793).
     ...
     * 
     * @param {Number}
     *            pos Current position, ranging from 0 to 1
     */
     ...
     */
    offset = function(el) {
        var docElem = doc.documentElement, box = el.getBoundingClientRect();
    ...
    /**
     * Stop running animation. A possible extension to this would be to stop a
     * single property, when we want to continue animating others. Then assign
     * the prop to the timer in the Fx.run method, and check for the prop here.
     * This would be an improvement in all cases where we stop the animation
     * from .attr. Instead of stopping everything, we can just stop the actual
    ...
    removeEvent = function(el, type, fn) {
        ...
        var events, hcEvents = el.hcEvents, index;
    ...
    fireEvent = function(el, type, eventArguments, defaultFunction) {
        var e, hcEvents = el.hcEvents, events, len, i, fn;
        ...
        if (doc.createEvent && (el.dispatchEvent || el.fireEvent)) {
            ...
            // Attach a simple preventDefault function to skip default handler
            // if called.
            // The built-in defaultPrevented property is not overwritable
            // (#5112)
            ...
            // If the type is not set, we're running a custom event (#2297). If
            // it is set,
            // we're running a browser event, and setting it will cause en error
            // in
            // IE8 (#2465).
    ...
    animate = function(el, params, opt) {
        var start, unit = '', end, fx, args, prop;
        ...
        win.jQuery.fn.highcharts = function() {
            ...
            if (this[0]) { // this[0] is the renderTo div
                ...
                if (args[0]) {
                    ...
                    isString(args[0]) ? args.shift() : 'Chart' // Constructor
                    // defaults to
                    // Chart
    ...
    /**
     * Compatibility section to add support for legacy IE. This can be removed
     * if old IE support is not needed.
     */
     ...
     **************************************************************************/
    defaultOptions = {
        colors : [ '#7cb5ec'/*light blue*/, '#434348'/*light black*/, '#90ed7d'/*light green*/, '#f7a35c'/*light orange*/, '#8085e9'/*violate*/, '#f15c80'/*pink*/, '#e4d354'/*yellow green*/, '#2b908f'/*green*/, '#f45b5b'/*light red*/, '#91e8e1'/*sky blue*/ ],
    ...
    Color.prototype = {
        ...
        // Collection of parsers. This can be extended from the outside by
        // pushing parsers
    ...
    SVGElement.prototype = {
         ...
         *            params
         * @param {Number}
         *            options Options include duration, easing, step and
         *            complete
         * @param {Function}
        ...
        colorGradient : function(color, prop, elem) {
            var renderer = this.renderer, colorObject, gradName, gradAttr, radAttr, gradients, gradientObject, stops, stopColor, stopOpacity, radialReference, n, id, key = [], value;
            ...
            if (gradName) {
                ...
                if (gradName === 'radialGradient' && radialReference && !defined(gradAttr.gradientUnits)) {
                    radAttr = gradAttr; // Save the radial attributes for
                    // updating
         ...
         * element and apply strokes to the copy.
         * 
        ...
        applyTextShadow : function(textShadow) {
            ...
            // IE10 and IE11 report textShadow in elem.style even though it
            // doesn't work. Check
            // this again with new IE release. In exports, the rendering is
            // passed to PhantomJS.
            supports = forExport || (elem.style.textShadow !== UNDEFINED && !isMS);

            // When the text shadow is set to contrast, use dark stroke for
            ...
            // Firefox does not tolerate this,
            ...
            /*
             * Selective side-by-side testing in supported browser
             * (http://jsfiddle.net/highcharts/73L1ptrh/) if
             * (elem.textContent.indexOf('2.') === 0) {
             * elem.style['text-shadow'] = 'none'; supports = false; } //
             */

            // No reason to polyfill, we've got native support
            if (supports) {
                this.css(styles); // Apply altered textShadow or textRendering
                // workaround
            } else {
                ...
                each(textShadow.split(/\s?,\s?/g), function(textShadow) {
                    ...
                    // Approximately tune the settings to the text-shadow
                    // behaviour
                    ...
                    if (strokeWidth) {
                        each(tspans, function(tspan, y) {
                            ...
                            // Let the first line start at the correct X
                            // position
                            if (y === 0) {
        ...
        attr : function(hash, val, complete) {
            var key, value, element = this.element, hasSetSymbolSize, ret = this, skipAttr, setter;
         ...
         * @param {Function}
         *            setter The setter function, inherited from the parent
         *            wrapper
         * @returns {undefined}
         */
        ...
        crisp : function(rect) {
            ...
            var wrapper = this, key, attribs = {}, normalizer, strokeWidth = wrapper.strokeWidth || 0;
        ...
        setRadialReference : function(coordinates) {
            ...
            // On redrawing objects with an existing gradient, the gradient
            // needs
        ...
        getBBox : function(reload, rot) {
            ...
            renderer = wrapper.renderer, width, height, rotation, rad, element = wrapper.element, styles = wrapper.styles, textStr = wrapper.textStr, textShadow, elemStyle = element.style, toggleTextShadowShim, cache = renderer.cache, cacheKeys = renderer.cacheKeys, cacheKey;
            ...
            if (textStr !== UNDEFINED) {
                ...
                // Since numbers are monospaced, and numerical labels appear a
                // lot in a chart,
                // we assume that a label of n characters has the same bounding
                // box as others
                // of the same length.
            ...
            if (!bBox) {
                ...
                if (element.namespaceURI === SVG_NS || renderer.forExport) {
                    try { // Fails in Firefox if the container has display:
                        ...
                        // When the text shadow shim is used, we need to hide
                        // the fake shadows
                        ...
                        // Workaround for #3842, Firefox reporting wrong
        ...
        dashstyleSetter : function(value) {
            ...
            // If "inherit", like maps in IE, assume 1 (#4981). With HC5 and the
            // new strokeWidth
            ...
            if (value) {
                value = value.replace('shortdashdotdot', '3,1,1,1,1,1,').replace('shortdashdot', '3,1,1,1').replace('shortdot', '1,1,').replace('shortdash', '3,1,').replace('longdash', '8,3,').replace(/dot/g, '1,3,').replace('dash', '4,3,').replace(/,$/, '').split(','); // ending
                // comma
        ...
        titleSetter : function(value) {
            ...
            titleNode.appendChild(doc.createTextNode((String(pick(value), '')).replace(/<[^>]*>/g, '') // #3276,
            // #3895
            ));
        },
        ...
        zIndexSetter : function(value, key) {
            ...
            if (defined(value)) {
                element.zIndex = value; // So we can read it for other elements
                // in the group
                value = +value;
            ...
            // Insert according to this and other elements' zIndex. Before
            // .add() is called,
    ...
    SVGElement.prototype['stroke-widthSetter'] = SVGElement.prototype.strokeSetter = function(value, key, element) {
        ...
        if (this.stroke && this['stroke-width']) {
            ...
            SVGElement.prototype.fillSetter.call(this, this.stroke, 'stroke', element); // use
            // prototype
            // as
            // instance
            // may
    ...
    SVGRenderer.prototype = {
        ...
        init : function(container, width, height, style, forExport, allowHTML) {
            ...
            renderer.url = (isFirefox || isWebKit) && doc.getElementsByTagName('base').length ? win.location.href.replace(/#.*?$/, '') // remove
            // the
            // hash
        ...
        buildText : function(wrapper) {
            var textNode = wrapper.element, renderer = this, forExport = renderer.forExport, textStr = pick(wrapper.textStr, '').toString(), hasMarkup = textStr.indexOf('<') !== -1, lines, childNodes = textNode.childNodes, styleRegex, hrefRegex, wasTooLong, parentX = attr(textNode, 'x'), textStyles = wrapper.styles, width = wrapper.textWidth, textLineHeight = textStyles && textStyles.lineHeight, textShadow = textStyles && textStyles.textShadow, ellipsis = textStyles && textStyles.textOverflow === 'ellipsis', i = childNodes.length, tempParent = width && !wrapper.added && this.box, getLineHeight = function(tspan) {
            ...
            if (!hasMarkup && !textShadow && !ellipsis && textStr.indexOf(' ') === -1) {
                ...
                if (tempParent) {
                    tempParent.appendChild(textNode); // attach it to the DOM
                    // to read offset width
                }
                ...
                each(lines, function buildTextLines(line, lineNo) {
                    var spans, spanNo = 0;
                    line = line.replace(/^\s+|\s+$/g, '') // Trim to prevent
                    // useless/costly
                    // process on the
                    // spaces (#5258)
                    ...
                    each(spans, function buildTextSpans(span) {
                        if (span !== '' || spans.length === 1) {
                            ...
                            if (hrefRegex.test(span) && !forExport) { // Not
                                // for
                                // export
                                // -
                                // #1529
                            ...
                            if (span !== ' ') {
                                ...
                                if (!spanNo) { // first span in a line, align
                                    // it to the left
                                ...
                                if (!spanNo && lineNo) {
                                    ...
                                    // Set the line height based on the font
                                    // size of either
                                ...
                                /*
                                 * if (width) { renderer.breakText(wrapper,
                                 * width); }
                                 */
                                ...
                                if (width) {
                                    ...
                                    hasWhiteSpace = spans.length > 1 || lineNo || (words.length > 1 && textStyles.whiteSpace !== 'nowrap'), tooLong, actualWidth, rest = [], dy = getLineHeight(tspan), softLineNo = 1, rotation = wrapper.rotation, wordStr = span, // for
                                    // ellipsis
                                    cursor = wordStr.length, // binary search
                                    // cursor
                                    bBox;

                                    while ((hasWhiteSpace || ellipsis) && (words.length || rest.length)) {
                                        wrapper.rotation = 0; // discard
                                        // rotation when
                                        // computing box
                                        bBox = wrapper.getBBox(true);
                                        actualWidth = bBox.width;

                                        // Old IE cannot measure the actualWidth
                                        // for SVG elements (#2314)
                                        ...
                                        // For ellipsis, do a binary search for
                                        // the correct string length
                                        if (wasTooLong === undefined) {
                                            wasTooLong = tooLong; // First
                                            // time
                                        }
                                        if (ellipsis && wasTooLong) {
                                            ...
                                            // Looping down, this is the first
                                            // word sequence that is not too
                                            // long,
                                            // so we can move on to build the
                                            // next line.
                ...
                if (tempParent) {
                    tempParent.removeChild(textNode); // attach it to the DOM
                    // to read offset width
                }
        ...
        /*
         * breakText: function (wrapper, width) { var bBox = wrapper.getBBox(),
         * node = wrapper.element, textLength = node.textContent.length, pos =
         * mathRound(width * textLength / bBox.width), // try this position
         * first, based on average character width increment = 0, finalPos;
         * 
         * if (bBox.width > width) { while (finalPos === undefined) { textLength =
         * node.getSubStringLength(0, pos);
         * 
         * if (textLength <= width) { if (increment === -1) { finalPos = pos; }
         * else { increment = 1; } } else { if (increment === 1) { finalPos =
         * pos - 1; } else { increment = -1; } } pos += increment; } }
         * console.log('width', width, 'stringWidth', node.getSubStringLength(0,
         * finalPos)) },
         */
        ...
        symbol : function(symbol, x, y, width, height, options) {
            ...
            imageRegex = /^url\((.*?)\)$/, imageSrc, imageSize, centerImage;
        ...
        symbols : {
            ...
            callout : function(x, y, w, h, options) {
                ...
                if (anchorX && anchorX > w && anchorY > y + safeDistance && anchorY < y + h - safeDistance) { // replace
                    // right
                    // side
                    path.splice(13, 3, 'L', x + w, anchorY - halfDistance, x + w + arrowLength, anchorY, x + w, anchorY + halfDistance, x + w, y + h - r);
                } else if (anchorX && anchorX < 0 && anchorY > y + safeDistance && anchorY < y + h - safeDistance) { // replace
                    // left
                    // side
                    path.splice(33, 3, 'L', x, anchorY + halfDistance, x - arrowLength, anchorY, x, anchorY - halfDistance, x, y + r);
                } else if (anchorY && anchorY > h && anchorX > x + safeDistance && anchorX < x + w - safeDistance) { // replace
                    // bottom
                    path.splice(23, 3, 'L', anchorX + halfDistance, y + h, anchorX, y + h + arrowLength, anchorX - halfDistance, y + h, x + r, y + h);
                } else if (anchorY && anchorY < 0 && anchorX > x + safeDistance && anchorX < x + w - safeDistance) { // replace
                    // top
        ...
        fontMetrics : function(fontSize, elem) {
            ...
            if (!fontSize && elem && win.getComputedStyle) {
                ...
                fontSize = style && style.fontSize; // #4309, the style doesn't
                // exist inside a hidden
                // iframe in Firefox
            }
        ...
        label : function(str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
            ...
            updateBoxSize = function() {
                ...
                bBox = (width === undefined || height === undefined || wrapper.styles.textAlign) && defined(text.textStr) && text.getBBox(); // #3295
                // &&
                // 3514
                // box
                // failure
                ...
                if (needsBox) {
                    ...
                    if (!box) {
                        ...
                        if (!box.isImg) { // #4324, fill "none" causes it to
                            // be ignored by mouse events in IE
                            box.attr('fill', NONE);
                        }
                        box.add(wrapper);
                    }
            ...
            updateTextPadding = function() {
                var styles = wrapper.styles, textAlign = styles && styles.textAlign, x = paddingLeft + padding, y;
            ...
            wrapper.onAdd = function() {
                text.add(wrapper);
                wrapper.attr({
                    text : (str || str === 0) ? str : '', // alignment is
                    // available now //
                    // #3295: 0 not
                    // rendered if given
                    // as a value
            ...
            wrapper.alignSetter = function(value) {
                ...
                if (value !== alignFactor) {
                    alignFactor = value;
                    if (bBox) { // Bounding box exists, means we're dynamically
                        // changing
                        wrapper.attr({
                            x : wrapperX
                        }); // #5134
            ...
            return extend(wrapper, {
                /**
                 * Pick up some properties and apply them to the text instead of
                 * the wrapper
                 */
                css : function(styles) {
                    if (styles) {
                        var textStyles = {};
                        styles = merge(styles); // create a copy to avoid
    ...
    extend(SVGElement.prototype, {
        ...
        /**
         * VML and useHTML method for calculating the bounding box based on
         * offsets
         * 
         * @param {Boolean}
         *            refresh Whether to force a fresh value from the DOM or to
        ...
        htmlGetBBox : function() {
            ...
            // faking getBBox in exported SVG in legacy IE (is this a duplicate
            // of the fix for #1079?)
        ...
        htmlUpdateTransform : function() {
            ...
            if (elem.tagName === 'SPAN') {
                ...
                var rotation = wrapper.rotation, baseline, textWidth = pInt(wrapper.textWidth), whiteSpace = styles && styles.whiteSpace, currentTextTransform = [ rotation, align, elem.innerHTML, wrapper.textWidth, wrapper.textAlign ].join(',');
                ...
                if (currentTextTransform !== wrapper.cTT) { // do the
                    ...
                    // Update textWidth
                    if (elem.offsetWidth > textWidth && /[ \-]/.test(elem.textContent || elem.innerText)) { // #983,
                        // #1254
                        css(elem, {
                            width : textWidth + PX,
                            display : 'block',
                ...
                // element (#1249)
                if (isWebKit) {
                    baseline = elem.offsetHeight; // assigned to baseline for
                    // lint purpose
                }
    ...
    extend(SVGRenderer.prototype, {
        /**
         * Create HTML text node. This is used by the VML renderer as well as
         * the SVG renderer through the useHTML option.
         * 
         * @param {String}
         *            str
         * @param {Number}
        ...
        html : function(str, x, y) {
            var wrapper = this.createElement('span'), element = wrapper.element, renderer = wrapper.renderer, isSVG = renderer.isSVG, addSetters = function(element, style) {
            ...
            wrapper.xSetter = wrapper.ySetter = wrapper.alignSetter = wrapper.rotationSetter = function(value, key) {
                if (key === 'align') {
                    key = 'textAlign'; // Do not overwrite the SVGElement.align
                    // method. Same as VML.
                }
                wrapper[key] = value;
                wrapper.htmlUpdateTransform();
                ...
                wrapper.add = function(svgGroupWrapper) {
                    ...
                    if (svgGroupWrapper) {
                        ...
                        if (!htmlGroup) {
                            ...
                            // Read the parent chain into an array and read from
                            // top down
                            ...
                            // Ensure dynamically updating position when any
                            ...
                            each(parents.reverse(), function(parentGroup) {
                                ...
                                // Create a HTML div and append it to the parent
                                // div to emulate
                                ...
                                // #5075
                                }, htmlGroup || container); // the top group is
                                // appended to
                                // container
                                ...
                                // Set listeners to update the HTML div's
                                // position whenever the SVG group
                                // position is changed
                                extend(parentGroup, {
                ...
                return extend(clipRect, {
                    ...
                    updateClipping : function() {
                        each(clipRect.members, function(member) {
                            if (member.element) { // Deleted series, like in
                                // stock/members/series-remove
                                // demo. Should be removed
                                // from members, but this
                                // will do.
    ...
    Tick.prototype = {
        ...
        addLabel : function() {
            ...
            // prepare CSS
            // css = width && { width: mathMax(1, mathRound(width - 2 *
            // (labelOptions.padding || 10))) + PX };
            ...
            if (!defined(label)) {
                ...
                tick.labelLength = label && label.getBBox().width; // Un-rotated
                // length
                tick.rotation = 0; // Base value to detect change for new calls
                // to getBBox
        ...
        handleOverflow : function(xy) {
            var axis = this.axis, pxPos = xy.x, chartWidth = axis.chart.chartWidth, spacing = axis.chart.spacing, leftBound = pick(axis.labelLeft, mathMin(axis.pos, spacing[3])), rightBound = pick(axis.labelRight, mathMax(axis.pos + axis.len, chartWidth - spacing[1])), label = this.label, rotation = this.rotation, factor = {
                ...
                center : 0.5,
                right : 1
            }[axis.labelAlign], labelWidth = label.getBBox().width, slotWidth = axis.getSlotWidth(), modifiedSlotWidth = slotWidth, xCorrection = factor, goRight = 1, leftPos, rightPos, textWidth, css = {};

            // Check if the label overshoots the chart spacing box. If it does,
            // move it.
            ...
            if (!rotation) {
                ...
                // If the label width exceeds the available space, set a text
                // width to be
                // picked up below. Also, if a width has been set before, we
                // need to set a new
                // one because the reported labelWidth will be limited by the
                // box (#3938).
                if (labelWidth > modifiedSlotWidth || (axis.autoRotation && label.styles.width)) {
                    textWidth = modifiedSlotWidth;
                }

                // Add ellipsis to prevent rotated labels to be clipped against
    ...
    Highcharts.PlotLineOrBand.prototype = {
        ...
        renderLabel : function(optionsLabel, path, isBand, zIndex) {
            var plotLine = this, label = plotLine.label, renderer = plotLine.axis.chart.renderer, attribs, xs, ys, x, y;
    ...
    Axis.prototype = {
        ...
        init : function(chart, userOptions) {
            ...
            axis.side = userOptions.side || (axis.horiz ? (axis.opposite ? 0 : 2) : // top
            // :
            // bottom
            ...
            axis.labelFormatter = options.labels.formatter || axis.defaultLabelFormatter; // can
            // be
            // overwritten
            // by
            // dynamic
        ...
        setOptions : function(userOptions) {
            this.options = merge(this.defaultOptions, this.isXAxis ? {} : this.defaultYAxisOptions, [ this.defaultTopAxisOptions, this.defaultRightAxisOptions, this.defaultBottomAxisOptions, this.defaultLeftAxisOptions ][this.side], merge(defaultOptions[this.coll], // if
            // set
            // in
            // setOptions
            // (#1053)
                ...
                if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
                    ...
                    if (axis.isXAxis) {
                        ...
                        if (xData.length) {
                            ...
                            // To prevent performance hit, we only do this after
                            // we have already
                            // found seriesDataMin because in most cases all
                            // data is valid. #5234.
                            seriesDataMin = arrayMin(xData);
                            if (!isNumber(seriesDataMin) && !(seriesDataMin instanceof Date)) { // Date
                                // for
                                // #5010
                                xData = grep(xData, function(x) {
                                    return isNumber(x);
                                });
                                seriesDataMin = arrayMin(xData); // Do it
                                // again
                                // with
                                // valid
                                // data
                        ...
                        // used, the min and max are
                        // always 0 and 100. If seriesDataMin and seriesDataMax
                        // is null, then series
        ...
        translate : function(val, backwards, cvsCoord, old, handleLog, pointPlacement) {
            ...
            sign = 1, cvsOffset = 0, localA = old ? axis.oldTransA : axis.transA, localMin = old ? axis.oldMin : axis.min, returnValue, minPixelPadding = axis.minPixelPadding, doPostTranslate = (axis.isOrdinal || axis.isBroken || (axis.isLog && handleLog)) && axis.lin2val;
        ...
        getPlotLinePath : function(value, lineWidth, old, force, translatedValue) {
            ...
            /**
             * Check if x is between a and b. If not, either move to a/b or
             * skip, depending on the force parameter.
             */
        ...
        getMinorTickPositions : function() {
            var axis = this, options = axis.options, tickPositions = axis.tickPositions, minorTickInterval = axis.minorTickInterval, minorTickPositions = [], pos, i, pointRangePadding = axis.pointRangePadding || 0, min = axis.min - pointRangePadding, // #1498
            max = axis.max + pointRangePadding, // #1498
            range = max - min, len;

            // If minor ticks get too dense, they are hard to read, and may
            // cause long running script. So we don't draw them.
            ...
            if (minorTickPositions.length !== 0) { // don't change the
                // extremes, when there is
                // no minor ticks
                axis.trimTicks(minorTickPositions, options.startOnTick, options.endOnTick); // #3652
                // #3743
                // #1498
            }
        ...
        setAxisTranslation : function(saveOld) {
            ...
            if (isXAxis || hasCategories || pointRange) {
                if (linkedParent) {
                    ...
                    each(axis.series, function(series) {
                        var seriesPointRange = hasCategories ? 1 : (isXAxis ? pick(series.options.pointRange, closestPointRange, 0) : (axis.axisPointRange || 0)), // #2806
                        pointPlacement = series.options.pointPlacement;

                        pointRange = mathMax(pointRange, seriesPointRange);

                        if (!axis.single) {
                            // minPointOffset is the value padding to the left
                            // of the axis in order to make
                            ...
                            // Determine the total padding needed to the length
                            // of the axis to make room for the
                ...
                ordinalCorrection = axis.ordinalSlope && closestPointRange ? axis.ordinalSlope / closestPointRange : 1; // #988,
                // #1853
        ...
        setTickInterval : function(secondPass) {
            ...
            if (isLog) {
                ...
                // it
                // was too aggressive for #4360 because of conversion back to
                // lin,
            ...
            // Hook for Highstock Scroller. Consider combining with
            // beforePadding.
            ...
            // the data extreme and the padded extreme land on either side of
            // the threshold. For example,
            // a series of [0, 1, 2, 3] would make the yAxis add a tick for -1
            // because of the
            // default minPadding and startOnTick options. This is prevented by
            // the softThreshold
            // option.
            ...
            if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
                ...
                axis.tickInterval = pick(tickIntervalOption, this.tickAmount ? ((axis.max - axis.min) / mathMax(this.tickAmount - 1, 1)) : undefined, categories ? // for
                // categoried
                // axis,
                // 1 is
                // default,
            ...
            if (!isDatetimeAxis && !isLog && !tickIntervalOption) {
                ...
                // in the order of
                // thousands, chances are we are dealing with years. Don't allow
                // decimals. #3363.
            ...
            if (!this.tickAmount && this.len) { // Color axis with disabled
                // legend has no length
        ...
        /**
         * Now we have computed the normalized tickInterval, get the tick
         * positions
         */
        setTickPositions : function() {
            ...
            this.tickPositions = tickPositions = tickPositionsOption && tickPositionsOption.slice(); // Work
            // on a
            // copy
            // (#1565)
            if (!tickPositions) {
                ...
                // Run the tick positioner callback, that allows modifying auto
                // tick positions.
                if (tickPositioner) {
        ...
        /**
         * Handle startOnTick and endOnTick by either adapting to padding
         * min/max or rounded min/max
         */
        ...
        alignToOthers : function() {
            var others = {}, // Whether there is another axis to pair with
            // this one
            hasOther, options = this.options;

            if (this.chart.options.chart.alignTicks !== false && options.alignTicks !== false) {
                each(this.chart[this.coll], function(axis) {
                    var otherOptions = axis.options, horiz = axis.horiz, key = [ horiz ? otherOptions.left : otherOptions.top, otherOptions.width, otherOptions.height, otherOptions.pane ].join(',');
        ...
        getTickAmount : function() {
            ...
            if (!tickAmount && this.alignToOthers()) {
                // Add 1 because 4 tick intervals require 5 ticks (including
                // first and last)
                tickAmount = mathCeil(this.len / tickPixelInterval) + 1;
            }

            // For tick amounts of 2 and 3, compute five ticks and remove the
            // intermediate ones. This
            // prevents the axis from adding ticks that are too far away from
            // the data extremes.
            if (tickAmount < 4) {
        ...
        adjustTickAmount : function() {
            ...
            if (currentTickAmount < tickAmount) {
                ...
                // We have too many ticks, run second pass to try to reduce
                // ticks
            ...
            if (defined(finalTickAmt)) {
                ...
                while (i--) {
                    if ((finalTickAmt === 3 && i % 2 === 1) || // Remove every
                    // other tick
                    (finalTickAmt <= 2 && i > 0 && i < len - 1) // Remove all
                    // but first and
                    // last
                    ) {
        ...
        setScale : function() {
            ...
            // is there new data?
            each(axis.series, function(series) {
                if (series.isDirtyData || series.isDirty || series.xAxis.isDirty) { // when
                    // x
                    // axis
                    // is
                    // dirty,
        ...
        getExtremes : function() {
            var axis = this, isLog = axis.isLog, lin2log = axis.lin2log;
        ...
        getThreshold : function(threshold) {
            var axis = this, isLog = axis.isLog, lin2log = axis.lin2log, realMin = isLog ? lin2log(axis.min) : axis.min, realMax = isLog ? lin2log(axis.max) : axis.max;

            // With a threshold of null, make the columns/areas rise from the
            // top or bottom
            // depending on the value, assuming an actual threshold of 0
            // (#4233).
            if (threshold === null) {
         ...
         * 
         * @param {String}
         *            prefix 'tick' or 'minorTick'
         * @returns {Array} An array of tickLength and tickWidth
         */
        tickSize : function(prefix) {
            var options = this.options, tickLength = options[prefix + 'Length'], tickWidth = pick(options[prefix + 'Width'], prefix === 'tick' && this.isXAxis ? 1 : 0); // X
            // axis
            // defaults
            // to 1
        ...
        /**
         * Prevent the ticks from getting so close we can't draw the labels. On
         * a horizontal axis, this is handled by rotating the labels, removing
         * ticks and adding ellipsis. On a vertical axis remove ticks and add
         * ellipsis.
         */
        unsquish : function() {
            var labelOptions = this.options.labels, horiz = this.horiz, tickInterval = this.tickInterval, newTickInterval = tickInterval, slotSize = this.len / (((this.categories ? 1 : 0) + this.max - this.min) / tickInterval), rotation, rotationOption = labelOptions.rotation, labelMetrics = this.labelMetrics(), step, bestScore = Number.MAX_VALUE, autoRotation,
            ...
            if (horiz) {
                ...
                if (autoRotation) {
                    ...
                    // which gives the best score. The
                    // best score is that with the lowest number of steps and a
                    // rotation closest to horizontal.
        ...
        /**
         * Get the general slot width for this axis. This may change between the
         * pre-render (from Axis.getOffset) and the final tick rendering and
         * placement (#5086).
         */
        getSlotWidth : function() {
            ...
            ((this.staggerLines || 1) * chart.plotWidth) / slotCount) || (!horiz && ((marginLeft && (marginLeft - chart.spacing[3])) || chart.chartWidth * 0.33)); // #1580,
            // #1931

        },

        /**
         * Render the axis labels and determine whether ellipsis or rotation
         * need to be applied
         */
        renderUnsquish : function() {
            var chart = this.chart, renderer = chart.renderer, tickPositions = this.tickPositions, ticks = this.ticks, labelOptions = this.options.labels, horiz = this.horiz, slotWidth = this.getSlotWidth(), innerWidth = mathMax(1, mathRound(slotWidth - 2 * (labelOptions.padding || 5))), attr = {}, labelMetrics = this.labelMetrics(), textOverflowOption = labelOptions.style.textOverflow, css, labelLength = 0, label, i, pos;
            ...
            if (this.autoRotation) {
                ...
                // Apply rotation only if the label is too wide for the slot,
                // and
                    ...
                    while (!horiz && i--) {
                        ...
                        if (label) {
                            ...
                            if (label.styles.textOverflow === 'ellipsis') {
                                ...
                                // Set the correct width in order to read the
                                // bounding box height (#4678, #5034)
            ...
            // Add ellipsis if the label length is significantly longer than
            // ideal
            if (attr.rotation) {
                css = {
            ...
            each(tickPositions, function(pos) {
                ...
                if (label) {
                    label.attr(attr); // This needs to go before the CSS in
                    // old IE (#4502)
                    if (css) {
        ...
        getOffset : function() {
            ...
            labelOffsetPadded, opposite = axis.opposite, axisOffset = chart.axisOffset, clipOffset = chart.clipOffset, clip, directionFactor = [ -1, 1, 1, -1 ][side], n, textAlign, axisParent = axis.axisParent, // Used
            // in
            // color
            // axis
            ...
            if (!axis.axisGroup) {
                ...
                }).addClass(PREFIX + axis.coll.toLowerCase() + '-labels').add(axisParent);
            }
            ...
            // Decide the clipping needed to keep the graph inside the plot area
            // and axis lines
            clip = options.offset ? 0 : mathFloor(options.lineWidth / 2) * 2; // #4308,
            // #4371
        ...
        getTitlePosition : function() {
            ...
            offAxis = (horiz ? axisTop + this.height : axisLeft) + (horiz ? 1 : -1) * // horizontal
            // axis
            // reverses
            // the
            // margin
         ...
         */
        render : function() {
            var axis = this, chart = axis.chart, renderer = chart.renderer, options = axis.options, isLog = axis.isLog, lin2log = axis.lin2log, isLinked = axis.isLinked, tickPositions = axis.tickPositions, axisTitle = axis.axisTitle, ticks = axis.ticks, minorTicks = axis.minorTicks, alternateBands = axis.alternateBands, stackLabelOptions = options.stackLabels, alternateGridColor = options.alternateGridColor, tickmarkOffset = axis.tickmarkOffset, lineWidth = options.lineWidth, linePath, hasRendered = chart.hasRendered, slideInTicks = hasRendered && isNumber(axis.oldMin), showAxis = axis.showAxis, animation = animObject(renderer.globalAnimation), from, to;
            ...
            if (axis.hasData() || isLinked) {
                ...
                if (alternateGridColor) {
                    each(tickPositions, function(pos, i) {
                        ...
                        if (i % 2 === 0 && pos < axis.max && to <= axis.max + (chart.polar ? -tickmarkOffset : tickmarkOffset)) { // #2248,
                            // #4660
            ...
            each([ ticks, minorTicks, alternateBands ], function(coll) {
                var pos, i, forDestruction = [], delay = animation.duration, destroyInactiveItems = function() {
         ...
         * 
         * @param {Object}
         *            e The event arguments from the modified pointer event
         * @param {Object}
         *            point The Point object
         */
        drawCrosshair : function(e, point) {

            var path, options = this.crosshair, pos, attribs, categorized, strokeWidth;
    ...
    Axis.prototype.getTimeTicks = function(normalizedInterval, min, max, startOfWeek) {
        ...
        if (defined(min)) { // #1300
            ...
            count * mathFloor(minDate.getMilliseconds() / count)); // #3652,
            // #3654
            ...
            minYear = minDate[getFullYear]();
            var time = minDate.getTime(), minMonth = minDate[getMonth](), minDateDate = minDate[getDate](), variableDayLength = !useUTC || !!getTimezoneOffset, // #4951
            localTimezoneOffset = (timeUnits.day + (useUTC ? getTZOffset(minDate) : minDate.getTimezoneOffset() * 60 * 1000)) % timeUnits.day; // #950,
            // #3359
    ...
    Axis.prototype.normalizeTimeTickInterval = function(tickInterval, unitsOption) {
        ...
        ], [ 'second', [ 1, 2, 5, 10, 15, 30 ] ], [ 'minute', [ 1, 2, 5, 10, 15, 30 ] ], [ 'hour', [ 1, 2, 3, 4, 6, 8, 12 ] ], [ 'day', [ 1, 2 ] ], [ 'week', [ 1, 2 ] ], [ 'month', [ 1, 2, 3, 4, 6 ] ], [ 'year', null ] ], unit = units[units.length - 1], // default
        // unit
        // is
        // years
        ...
        // get the count
        count = normalizeTickInterval(tickInterval / interval, multiples, unit[0] === 'year' ? mathMax(getMagnitude(tickInterval / interval), 1) : 1 // #1913,
        // #2360
        );
    ...
    Axis.prototype.getLogTickPositions = function(interval, min, max, minor) {
        var axis = this, options = axis.options, axisLength = axis.len, lin2log = axis.lin2log, log2lin = axis.log2lin,
        ...
        if (interval >= 0.5) {
            ...
            for (i = roundedMin; i < max + 1 && !break2; i++) {
                ...
                for (j = 0; j < len && !break2; j++) {
                    ...
                    if (pos > min && (!minor || lastPos <= max) && lastPos !== UNDEFINED) { // #1670,
                        // lastPos
                        // is
                        // #3113
    ...
    Tooltip.prototype = {
        ...
        getAnchor : function(points, mouseEvent) {
            ...
            if (!ret) {
                ...
                ret = [ inverted ? chart.plotWidth - plotY : plotX, this.shared && !inverted && points.length > 1 && mouseEvent ? mouseEvent.chartY - plotTop : // place
                // shared
                // tooltip
                // next
                // to
        ...
        getPosition : function(boxWidth, boxHeight, point) {
            ...
            var chart = this.chart, distance = this.distance, ret = {}, h = point.h || 0, // #4117
            swapped, first = [ 'y', chart.chartHeight, boxHeight, point.plotY + chart.plotTop, chart.plotTop, chart.plotTop + chart.plotHeight ], second = [ 'x', chart.chartWidth, boxWidth, point.plotX + chart.plotLeft, chart.plotLeft, chart.plotLeft + chart.plotWidth ],
        ...
        defaultFormatter : function(tooltip) {
            ...
            // build the header
            s = [ tooltip.tooltipFooterHeaderFormatter(items[0]) ]; // #3397:
            // abstraction
            // to enable
            // formatting
            // of footer
            ...
            // footer
            s.push(tooltip.tooltipFooterHeaderFormatter(items[0], true)); // #3397:
            // abstraction
            // to
            // enable
            // formatting
        ...
        updatePosition : function(point) {
            ...
            // do the move
            this.move(mathRound(pos.x), mathRound(pos.y || 0), // can be
            // undefined
            // (#3977)
        ...
        getXDateFormat : function(point, options, xAxis) {
            var xDateFormat, dateTimeLabelFormats = options.dateTimeLabelFormats, closestPointRange = xAxis && xAxis.closestPointRange, n, blank = '01-01 00:00:00.000', strpos = {
                ...
                hour : 6,
                day : 3
            }, date, lastN = 'millisecond'; // for sub-millisecond data, #4223
            ...
            if (closestPointRange) {
                ...
                for (n in timeUnits) {
                    ...
                    // If the point is placed every day at 23:59, we need to
                    // show
        ...
        /**
         * Format the footer/header of the tooltip #3397: abstraction to enable
         * formatting of footer and header
         */
         ...
         * returning one entry for each item, abstracting this functionality
         * allows to easily overwrite and extend it.
         */
    ...
    Pointer.prototype = {
        ...
        runPointActions : function(e) {
            ...
            var pointer = this, chart = pointer.chart, series = chart.series, tooltip = chart.tooltip, shared = tooltip ? tooltip.shared : false, followPointer, hoverPoint = chart.hoverPoint, hoverSeries = chart.hoverSeries, i, distance = [ Number.MAX_VALUE, Number.MAX_VALUE ], // #4511
            anchor, noSharedTooltip, stickToHoverSeries, directTouch, kdpoints = [], kdpoint = [], kdpointT;

            // For hovering over the empty parts of the plot area (hoverSeries
            // is undefined).
            // If there is one series with point tracking (combo chart), don't
            // go to nearest neighbour.
            ...
            // If it has a hoverPoint and that series requires direct touch
            // (like columns, #3899), or we're on
            // a noSharedTooltip series among shared tooltip series (#4546), use
            // the hoverPoint . Otherwise,
            // search the k-d tree.
            stickToHoverSeries = hoverSeries && (shared ? hoverSeries.noSharedTooltip : hoverSeries.directTouch);
            if (stickToHoverSeries && hoverPoint) {
                kdpoint = [ hoverPoint ];

                // Handle shared tooltip or cases where a series is not yet
                // hovered
            } else {
                ...
                each(kdpoints, function(p) {
                    if (p) {
                        // Store both closest points, using point.dist and
                        // point.distX comparisons (#4645):
                        each([ 'dist', 'distX' ], function(dist, k) {
                            if (isNumber(p[dist])) {
                                var
                                // It is closer than the reference point
                                isCloser = p[dist] < distance[k],
                                // It is equally close, but above the reference
                                // point (#4679)
            ...
            // Remove points with different x-positions, required for shared
            // tooltip and crosshairs (#4645):
            if (shared) {
                i = kdpoints.length;
                while (i--) {
            ...
            // Refresh tooltip for kdpoint if new hover point or tooltip was
            // hidden // #3926, #4200
            ...
            // Crosshair. For each hover point, loop over axes and draw cross if
            // that point
            // belongs to the axis (#4927).
            each(shared ? kdpoints : [ pick(hoverPoint, kdpoint[1]) ], function(point) { // #5269
                each(chart.axes, function(axis) {
                    // In case of snap = false, point is undefined, and we draw
                    // the crosshair anyway (#5066)
        ...
        drag : function(e) {
            ...
            // If the device supports both touch and mouse (like IE11), and we
            // are touch-dragging
                ...
                if (this.hasDragged || hasPinched) {
                    ...
                    each(chart.axes, function(axis) {
                        if (axis.zoomEnabled && defined(axis.min) && (hasPinched || pointer[{
                            ...
                            var horiz = axis.horiz, minPixelPadding = e.type === 'touchend' ? axis.minPixelPadding : 0, // #1207,
                            // #3075
                            ...
                            selectionData[axis.coll].push({
                                axis : axis,
                                min : mathMin(selectionMin, selectionMax), // for
                                // reversed
                                // axes
        ...
        onContainerMouseLeave : function(e) {
            ...
            if (chart && (e.relatedTarget || e.toElement)) { // #4886, MS
                // Touch end
                // fires
                // mouseleave
                // but with no
        ...
        onTrackerMouseOut : function(e) {
            ...
            !this.inClass(relatedTarget, PREFIX + 'tooltip') && !this.inClass(relatedTarget, PREFIX + 'series-' + series.index)) { // #2499,
                // #4465
                series.onMouseOut();
            }
        },
    ...
    extend(Highcharts.Pointer.prototype, {
        ...
        pinchTranslateDirection : function(horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch, forcedScale) {
            var chart = this.chart, xy = horiz ? 'x' : 'y', XY = horiz ? 'X' : 'Y', sChartXY = 'chart' + XY, wh = horiz ? 'width' : 'height', plotLeftTop = chart['plot' + (horiz ? 'Left' : 'Top')], selectionWH, selectionXY, clipXY, scale = forcedScale || 1, inverted = chart.inverted, bounds = chart.bounds[horiz ? 'h' : 'v'], singleTouch = pinchDown.length === 1, touch0Start = pinchDown[0][sChartXY], touch0Now = touches[0][sChartXY], touch1Start = !singleTouch && pinchDown[1][sChartXY], touch1Now = !singleTouch && touches[1][sChartXY], outOfBounds, transformScale, scaleKey, setScale = function() {
                if (!singleTouch && mathAbs(touch0Start - touch1Start) > 20) { // Don't
                    // zoom
                    // if
                    // fingers
                    // are
            ...
            selectionXY = clipXY; // the clip position (x or y) is altered if
            // out of bounds, the selection position is
            // not
            ...
            if (outOfBounds) {
                ...
                // Modify the touchNow position in order to create an elastic
                // drag movement. This indicates
                // to the user that the chart is responsive but can't be dragged
                // further.
        ...
        pinch : function(e) {
            ...
            // Don't initiate panning until the user has pinched. This prevents
            // us from
            ...
            } else if (pinchDown.length) { // can be 0 when releasing, if
                // touchend fires first
        ...
        touch : function(e, start) {
            var chart = this.chart, hasMoved, pinchDown;
            ...
            if (e.touches.length === 1) {
                ...
                if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop) && !chart.openMenu) {
                    ...
                    // Android fires touchmove events after the touchstart even
                    // if the
                    // finger hasn't moved, or moved only a pixel or two. In iOS
                    // however,
                    // the touchmove doesn't fire unless the finger moves more
                    // than ~4px.
                    // So we emulate this behaviour in Android by checking how
                    // much it
                    ...
                    // Hide the tooltip on touching outside the plot area
                    // (#1203)
                    this.reset();
                }
    ...
    Legend.prototype = {
        ...
        positionCheckboxes : function(scrollOffset) {
            var alignAttr = this.group.alignAttr, translateY, clipHeight = this.clipHeight || this.legendHeight, titleHeight = this.titleHeight;
        ...
        renderItem : function(item) {
            ...
            if (!li) { // generate it once, later move it
                ...
                item.legendItem = li = renderer.text('', ltr ? symbolWidth + symbolPadding : -symbolPadding, legend.baseline || 0, useHTML).css(merge(item.visible ? itemStyle : itemHiddenStyle)) // merge
                // to
                // prevent
                // modifying
                // original
        ...
        /**
         * Adjust the chart margins by reserving space for the legend on only
         * one side of the chart. If the position is set to a corner, top or
         * bottom is reserved for horizontal legends and left or right for
         * vertical ones.
         */
        adjustMargins : function(margin, spacing) {
            ...
            // the side
            alignment = options.align.charAt(0) + options.verticalAlign.charAt(0) + options.layout.charAt(0); // #4189
            // -
            // use
            // charAt(x)
            // notation
            ...
            if (this.display && !options.floating) {
                ...
                each([ /(lth|ct|rth)/, /(rtv|rm|rbv)/, /(rbh|cb|lbh)/, /(lbv|lm|ltv)/ ], function(alignments, side) {
                    if (alignments.test(alignment) && !defined(margin[side])) {
                        // Now we have detected on which side of the chart we
                        // should reserve space for the legend
        ...
        scroll : function(scrollBy, animation) {
            ...
            if (currentPage > 0) {
                ...
                this.down.attr({
                    x : 18 + this.pager.getBBox().width, // adjust to text
                    // width
    ...
    var LegendSymbolMixin = Highcharts.LegendSymbolMixin = {
        ...
        drawRectangle : function(legend, item) {
            ...
            legend.symbolWidth, symbolHeight, legend.options.symbolRadius || 0).attr({
                zIndex : 3
            }).add(item.legendGroup);
     ...
     * 
     * @param {String|Object}
     *            renderTo The DOM element to render to, or its id
     * @param {Object}
     *            options
     * @param {Function}
    ...
    Chart.prototype = {
        ...
        redraw : function(animation) {
                ...
                each(axes, function(axis) {
                    ...
                    if (axis.extKey !== key) { // #821, #4452
                        ...
                        afterRedraw.push(function() { // prevent a recursive
                            ...
                            // chart.redraw()
                            // (#1119)
                            fireEvent(axis, 'afterSetExtremes', extend(axis.eventArgs, axis.getExtremes())); // #747,
                            // #751
            ...
            each(series, function(serie) {
                if (serie.isDirty && serie.visible && (!serie.isCartesian || serie.xAxis)) { // issue
                    // #153
                    serie.redraw();
                }
            });
        ...
        layOutTitles : function(redraw) {
            var titleOffset = 0, title = this.title, subtitle = this.subtitle, options = this.options, titleOptions = options.title, subtitleOptions = options.subtitle, requiresDirtyBox, renderer = this.renderer, spacingBox = this.spacingBox;
        ...
        getChartSize : function() {
            ...
            chart.chartWidth = mathMax(0, widthOption || chart.containerWidth || 600); // #1393,
            // 1460
        ...
        getContainer : function() {
            var chart = this, container, options = chart.options, optionsChart = options.chart, chartWidth, chartHeight, renderTo = chart.renderTo, indexAttrName = 'data-highcharts-chart', oldChartIndex, Ren, containerId = 'highcharts-' + idCounter++;
        ...
        getAxisMargins : function() {
            ...
            var chart = this, axisOffset = chart.axisOffset = [ 0, 0, 0, 0 ], // top,
            // right,
            // bottom,
            // left
            margin = chart.margin;
         ...
         */
        reflow : function(e) {
            var chart = this, optionsChart = chart.options.chart, renderTo = chart.renderTo, width = optionsChart.width || getStyle(renderTo, 'width'), height = optionsChart.height || getStyle(renderTo, 'height'), target = e ? e.target : win;
            ...
            if (!chart.hasUserSize && !chart.isPrinting && width && height && (target === win || target === doc)) { // #1093
                if (width !== chart.containerWidth || height !== chart.containerHeight) {
                    ...
                    // When called from window.resize, e is set, else it's
                    // called directly (#2224)
                    chart.reflowTimeout = syncTimeout(function() {
                        if (chart.container) { // It may have been destroyed in
                            // the meantime (#1257)
        ...
        setSize : function(width, height, animation) {
            var chart = this, chartWidth, chartHeight, renderer = chart.renderer, globalAnimation;
        ...
        render : function() {
            ...
            tempHeight = chart.plotHeight = chart.plotHeight - 21; // 21 is the
            // most
            // common
            // correction
            // for X
            ...
            redoVertical = tempHeight / chart.plotHeight > 1.05; // Height is
            // more
            // sensitive
            ...
            if (redoHorizontal || redoVertical) {
                ...
                each(axes, function(axis) {
                    if ((axis.horiz && redoHorizontal) || (!axis.horiz && redoVertical)) {
                        axis.setTickInterval(true); // update to reflect the new
                        // margins
                    }
                });
        ...
        isReadyToRender : function() {
            ...
            if ((!hasSVG && (win == win.top && doc.readyState !== 'complete')) || (useCanVG && !win.canvg)) { // eslint-disable-line
                // eqeqeq
                if (useCanVG) {
        ...
        onload : function() {
            ...
            each([ this.callback ].concat(this.callbacks), function(fn) {
                if (fn && chart.index !== undefined) { // Chart destroyed in
                    // its own callback
                    // (#3600)
    ...
    Point.prototype = {
        ...
        optionsToObject : function(options) {
            ...
            if (isNumber(options) || options === null) {
                ...
                while (j < valueCount) {
                    if (!keys || options[i] !== undefined) { // Skip
                        // undefined
                        // positions for
                        // keys
        ...
        destroyElements : function() {
            var point = this, props = [ 'graphic', 'dataLabel', 'dataLabelUpper', 'connector', 'shadowGroup' ], prop, i = 6;
            while (i--) {
                prop = props[i];
                if (point[prop]) {
        ...
        firePointEvent : function(eventType, eventArgs, defaultFunction) {
            ...
            if (eventType === 'click' && seriesOptions.allowPointSelect) {
                defaultFunction = function(event) {
                    ...
                    // Shift for Opera
                    if (point.select) { // Could be destroyed by prior event
                        // handlers (#2911)
     ...
     *                   arrays. - First, series.options.data contains all the
     *                   original config options for each point whether added by
     *                   options or methods like series.addPoint. - Next,
     *                   series.data contains those values converted to points,
     *                   but in case the series data length exceeds the
     *                   cropThreshold, or if the data is grouped, series.data
     *                   doesn't contain all the points. It only contains the
     *                   points that have been created on demand. - Then there's
     *                   series.points that contains all currently visible point
     *                   objects. In case of cropping, the cropped-away points
     *                   are not part of this array. The series.points array
     *                   starts at series.cropStart compared to series.data and
     *                   series.options.data. If however the series data is
     *                   grouped, these can't be correlated one to one. -
     *                   series.xData and series.processedXData contain clean x
     *                   values, equivalent to series.data and series.points. -
     *                   series.yData and series.processedYData contain clean x
     *                   values, equivalent to series.data and series.points.
     * 
     * @param {Object}
     *            chart
    ...
    Series.prototype = {
        ...
        bindAxes : function() {
            ...
            each(series.axisTypes || [], function(AXIS) { // repeat for xAxis
                // and yAxis

                each(chart[AXIS], function(axis) { // loop through the chart's
                    // axis objects
        ...
        updateParallelArrays : function(point, i) {
            ...
            // Apply the method specified in i with the following arguments as
            // arguments
            function(key) {
        ...
        setOptions : function(itemOptions) {
            ...
            // otherwise, default
            // type options like column.animation would be overwritten by the
            // general option.
            // But issues have been raised here (#3881), and the solution may be
            // to distinguish
        ...
        getColor : function() {
            if (this.options.colorByPoint) {
                this.options.color = null; // #4359, selected slice got
                // series.color even when
                // colorByPoint was set.
            } else {
        ...
        processData : function(force) {
            ...
            isCartesian = series.isCartesian, xExtremes, val2lin = xAxis && xAxis.val2lin, isLog = xAxis && xAxis.isLog, min, max;
        ...
        cropData : function(xData, yData, min, max) {
            var dataLength = xData.length, cropStart = 0, cropEnd = dataLength, cropShoulder = pick(this.cropShoulder, 1), // line-type
            // series
            // need
            // one
            // point
        ...
        getExtremes : function(yData) {
            var xAxis = this.xAxis, yAxis = this.yAxis, xData = this.processedXData, yDataLength, activeYData = [], activeCounter = 0, xExtremes = xAxis.getExtremes(), // #2117,
            // need
            // to
            // compensate
            // for
        ...
        getValidPoints : function(points, insideOnly) {
            var chart = this.chart;
            return grep(points || this.points || [], function isValidPoint(point) { // #3916,
                // #5029
        ...
        setClip : function(animation) {
            ...
            if (!clipRect) {
                ...
                if (animation) {
                    ...
                    chart[sharedClipKey + 'm'] = markerClipRect = renderer.clipRect(-99, // include
                    // the
                    // width
                    // of
                    // the
        ...
        getAttribs : function() {
            ...
            if (seriesOptions.marker) { // line, spline, area, areaspline,
                ...
                // if no hover negativeColor is given, brighten the normal
                // negativeColor
            ...
            if (!turboThreshold || i < turboThreshold || hasPointSpecificOptions) {
                while (i--) {
                    ...
                    if (zones.length) {
                        ...
                        point.color = point.fillColor = zoneColor = pick(threshold.color, series.color); // #3636,
                        // #4267,
                        // #4430
                        // -
                        // inherit
                    ...
                    if (hasPointSpecificOptions) {
                        ...
                        if (!seriesOptions.marker || (point.negative && !pointStateOptionsHover.fillColor && !stateOptionsHover.fillColor)) { // column,
                            // bar,
                            // point
                            // or
                            // negative
                        ...
                        // Color is explicitly set to null or undefined (#1288,
                        // #4068)
                        if (normalOptions.hasOwnProperty('color') && !normalOptions.color) {
                            delete normalOptions.color;
                        }

                        // When zone is set, but series.states.hover.color is
                        // not set, apply zone color on hover, #4670:
        ...
        destroy : function() {
            ...
            for (prop in series) {
                if (series[prop] instanceof SVGElement && !series[prop].survive) { // Survive
                    // provides
                    // a
                    // hook
                    // for
        ...
        getGraphPath : function(points, nullsAsZeroes, connectCliffs) {
            var series = this, options = series.options, step = options.step, reversed, graphPath = [], gap;
            ...
            each(points, function(point, i) {
                ...
                var plotX = point.plotX, plotY = point.plotY, lastPoint = points[i - 1], pathToPoint; // the
                // path
                // to
                // this
                // point
                ...
                if (point.isNull && !defined(nullsAsZeroes) && i > 0) {
                    ...
                    } else if (series.getPointSpline) { // generate the spline
                        // as defined in the
                        // SplineSeries object
         ...
         */
        drawGraph : function() {
            var series = this, options = this.options, props = [ [ 'graph', options.lineColor || this.color, options.dashStyle ] ], lineWidth = options.lineWidth, roundCap = options.linecap !== 'square', graphPath = (this.gappedPath || this.getGraphPath).call(this), fillColor = (this.fillGraph && this.color) || NONE, // polygon
            // series
            // use
            // filled
            // graph
            ...
            each(props, function(prop, i) {
                ...
                if (graph) {
                    ...
                    series[graphKey] = series.chart.renderer.path(graphPath).attr(attribs).add(series.group).shadow((i < 2) && options.shadow); // add
                    // shadow
                    // to
                    // normal
                    // series
         ...
         */
        redraw : function() {
            var series = this, chart = series.chart, wasDirty = series.isDirty || series.isDirtyData, // cache
            // it
            // here
            // as
            // it
        ...
        buildKDTree : function() {
            ...
            function startRecursive() {
                series.kdTree = _kdtree(series.getValidPoints(null, !series.directTouch // For
                // line-type
                // series
                // restrict
                // to
    ...
    StackItem.prototype = {
        ...
        render : function(group) {
            var options = this.options, formatOption = options.format, str = formatOption ? format(formatOption, this) : options.formatter.call(this); // format
            // the
            // text
            // in
            // the
            ...
            if (this.label) {
                ...
                this.label = this.axis.chart.renderer.text(str, null, null, options.useHTML) // dummy
                // positions,
                // actual
                // position
                // updated
        ...
        setOffset : function(xOffset, xWidth) {
            var stackItem = this, axis = stackItem.axis, chart = axis.chart, inverted = chart.inverted, reversed = axis.reversed, neg = (this.isNegative && !reversed) || (!this.isNegative && reversed), // #4056
            y = axis.translate(axis.usePercentage ? 100 : this.total, 0, 0, 0, 1), // stack
            // value
            // translated
            // mapped
            // to
    ...
    Axis.prototype.buildStacks = function() {
        var axisSeries = this.series, series, reversedStacks = pick(this.options.reversedStacks, true), len = axisSeries.length, i;
        if (!this.isXAxis) {
    ...
    Series.prototype.setStackedPoints = function() {
        ...
        for (i = 0; i < yDataLength; i++) {
            ...
            if (y !== null) {
                ...
                // In area charts, if there are multiple points on the same X
                // value, let the
        ...
        each([ stackKey, '-' + stackKey ], function(key) {
            ...
            while (i--) {
                ...
                if (pointExtremes) {
                    ...
                    pointExtremes[0] = correctFloat(pointExtremes[0] * totalFactor); // Y
                    // bottom
                    // value
                    pointExtremes[1] = correctFloat(pointExtremes[1] * totalFactor); // Y
                    // value
    ...
    /**
     * Get stack indicator, according to it's x-value, to determine points with
     * the same x-value
     */
        ...
        update : function(options, redraw, animation, runEvent) {
            ...
            function update() {
                ...
                if (isObject(options) && !isArray(options)) {
                    // Defer the actual redraw until getAttribs has been called
                    // (#3260)
                ...
                // Record the options to options.data. If there is an object
                // from before,
    ...
    extend(Series.prototype, {
        ...
        addPoint : function(options, redraw, shift, animation) {
            var series = this, seriesOptions = series.options, data = series.data, graph = series.graph, area = series.area, chart = series.chart, names = series.xAxis && series.xAxis.names, currentShift = (graph && graph.shift) || 0, shiftShapes = [ 'graph', 'area' ], dataOptions = seriesOptions.data, point, isInTheMiddle, xData = series.xData, i, x;
            ...
            if (area) {
                area.isArea = true; // needed in animation, both with and
                // without shift
            }
        ...
        update : function(newOptions, redraw) {
            ...
            // If we're changing type or zIndex, create new groups (#3380,
            // #3404)
    ...
    extend(Axis.prototype, {
        ...
        update : function(newOptions, redraw) {
            ...
            this.destroy(true);
            this._addedPlotLB = this.chart._labelPanes = UNDEFINED; // #1611,
            // #2887,
            // #4314
    ...
    var AreaSeries = extendClass(Series, {
        ...
        getStackPoints : function() {
            var series = this, segment = [], keys = [], xAxis = this.xAxis, yAxis = this.yAxis, stack = yAxis.stacks[this.stackKey], pointMap = {}, points = this.points, seriesIndex = series.index, yAxisSeries = yAxis.series, seriesLength = yAxisSeries.length, visibleSeries, upOrDown = pick(yAxis.options.reversedStacks, true) ? 1 : -1, i, x;
            ...
            if (this.options.stacking) {
                ...
                // Sort the keys (#1651)
                for (x in stack) {
                    if (stack[x].total !== null) { // nulled after switching
                        // between grouping and not
                        // (#1651, #2336)
                        keys.push(x);
                    }
                }
                ...
                each(keys, function(x, idx) {
                    ...
                    if (pointMap[x] && !pointMap[x].isNull) {
                        ...
                        each([ -1, 1 ], function(direction) {
                            var nullName = direction === 1 ? 'rightNull' : 'leftNull', cliffName = direction === 1 ? 'rightCliff' : 'leftCliff', cliff = 0, otherStack = stack[keys[idx + direction]];

                            // If there is a stack next to this one, to the left
                            // or to the right...
                            if (otherStack) {
                                i = seriesIndex;
                                while (i >= 0 && i < seriesLength) { // Can
                                    // go
                                    // either
                                    // up or
                                    // down,
                                    ...
                                    if (!stackPoint) {
                                        ...
                                        if (i === seriesIndex) {
                                            ...
                                            // series below this one, we need to
                                            // substract the missing values
                        ...
                        // There is no point for this X value in this series, so
                        // we
                        // insert a dummy point in order for the areas to be
                        // drawn
                        // correctly.
                    } else {

                        // Loop down the stack to find the series below this one
                        // that has
                        // a value (#1991)
                        i = seriesIndex;
        ...
        getGraphPath : function(points) {
            var getGraphPath = Series.prototype.getGraphPath, graphPath, options = this.options, stacking = options.stacking, yAxis = this.yAxis, topPath,
            // topPoints = [],
            ...
            addDummyPoints = function(i, otherI, side) {
                var point = points[i], stackedValues = stacking && stacks[point.x].points[seriesIndex], nullVal = point[side + 'Null'] || 0, cliffVal = point[side + 'Cliff'] || 0, top, bottom, isNull = true;
            ...
            for (i = 0; i < points.length; i++) {
                ...
                if (!isNull || connectNulls) {
                    ...
                    if (!(isNull && !stacking && connectNulls)) { // Skip null
                        // point
                        // when
                        // stacking
                        // is false
            ...
            graphPath = getGraphPath.call(this, graphPoints, false, connectNulls); // TODO:
            // don't
            // set
            // leftCliff
            // and
        ...
        drawGraph : function() {
            ...
            var series = this, areaPath = this.areaPath, options = this.options, zones = this.zones, props = [ [ 'area', this.color, options.fillColor ] ]; // area
            // name,
            // main
            // color,
            // fill
            ...
            each(props, function(prop) {
                var areaKey = prop[0], area = series[areaKey], attr;
    ...
    var SplineSeries = extendClass(Series, {
        ...
        getPointSpline : function(points, point, i) {
            var smoothing = 1.5, // 1 means control points midway between
            // points, 2 means 1/3 from the point, 3 is
            // 1/4 etc
            denom = smoothing + 1, plotX = point.plotX, plotY = point.plotY, lastPoint = points[i - 1], nextPoint = points[i + 1], leftContX, leftContY, rightContX, rightContY, ret;
            ...
            if (lastPoint && !lastPoint.isNull && nextPoint && !nextPoint.isNull) {
                ...
                if (leftContY > lastY && leftContY > plotY) {
                    ...
                    rightContY = 2 * plotY - leftContY; // mirror of left
                    // control point
             ...
             * this.chart.plotLeft, leftContY + this.chart.plotTop, 2) .attr({
             * stroke: 'red', 'stroke-width': 1, fill: 'none' }) .add();
             * this.chart.renderer.path(['M', leftContX + this.chart.plotLeft,
             * leftContY + this.chart.plotTop, 'L', plotX + this.chart.plotLeft,
             * plotY + this.chart.plotTop]) .attr({ stroke: 'red',
             * 'stroke-width': 1 }) .add();
             * this.chart.renderer.circle(rightContX + this.chart.plotLeft,
             * rightContY + this.chart.plotTop, 2) .attr({ stroke: 'green',
             * 'stroke-width': 1, fill: 'none' }) .add();
             * this.chart.renderer.path(['M', rightContX + this.chart.plotLeft,
             * rightContY + this.chart.plotTop, 'L', plotX +
             * this.chart.plotLeft, plotY + this.chart.plotTop]) .attr({ stroke:
             * 'green', 'stroke-width': 1 }) .add(); } //
             */
            ...
            lastPoint.rightContX = lastPoint.rightContY = null; // reset for
            // updating
            // series later
            return ret;
        }
    ...
    defaultPlotOptions.column = merge(defaultSeriesOptions, {
        ...
        softThreshold : false,
        startFromThreshold : true, // false doesn't work well:
        // http://jsfiddle.net/highcharts/hz8fopan/14/
    ...
    var ColumnSeries = extendClass(Series, {
        ...
        cropShoulder : 0,
        directTouch : true, // When tooltip is not shared, this series (and
        // derivatives) requires direct touch/hover. KD-tree
        // does not apply.
        trackerGroups : [ 'group', 'dataLabelsGroup' ],
        negStacks : true, // use separate negative stacks, unlike area stacks
        // where a negative
        ...
        getColumnMetrics : function() {
            ...
            var series = this, options = series.options, xAxis = series.xAxis, yAxis = series.yAxis, reversedXAxis = xAxis.reversed, stackKey, stackGroups = {}, columnCount = 0;
            ...
            if (options.grouping === false) {
                ...
                each(series.chart.series, function(otherSeries) {
                    var otherOptions = otherSeries.options, otherYAxis = otherSeries.yAxis, columnIndex;
                    if (otherSeries.type === series.type && otherSeries.visible && yAxis.len === otherYAxis.len && yAxis.pos === otherYAxis.pos) { // #642,
                        // #2086
            ...
            xAxis.len // #1535
            ), groupPadding = categoryWidth * options.groupPadding, groupWidth = categoryWidth - 2 * groupPadding, pointOffsetWidth = groupWidth / columnCount, pointWidth = mathMin(options.maxPointWidth || xAxis.len, pick(options.pointWidth, pointOffsetWidth * (1 - 2 * options.pointPadding))), pointPadding = (pointOffsetWidth - pointWidth) / 2, colIndex = (series.columnIndex || 0) + (reversedXAxis ? 1 : 0), // #1251,
            // #3737
         ...
         */
        translate : function() {
            var series = this, chart = series.chart, options = series.options, borderWidth = series.borderWidth = pick(options.borderWidth, series.closestPointRange * series.xAxis.transA < 2 ? 0 : 1 // #3635
            ), yAxis = series.yAxis, threshold = options.threshold, translatedThreshold = series.translatedThreshold = yAxis.getThreshold(threshold), minPointLength = pick(options.minPointLength, 5), metrics = series.getColumnMetrics(), pointWidth = metrics.width, seriesBarW = series.barW = mathMax(pointWidth, 1 + 2 * borderWidth), // postprocessed
            // for
            // border
            // width
            ...
            // tightly, so we allow individual
            // columns to have individual sizes. When pointPadding is greater,
            // we strive for equal-width
            // columns (#2694).
            ...
            each(series.points, function(point) {
                ...
                safeDistance = 999 + mathAbs(yBottom), plotY = mathMin(mathMax(-safeDistance, point.plotY), yAxis.len + safeDistance), // Don't
                // draw
                // too
                // far
                // outside
                ...
                if (mathAbs(barH) < minPointLength) {
                    if (minPointLength) {
                        ...
                        translatedThreshold - (up ? minPointLength : 0); // #1485,
                        // #4051
                    }
                }
    ...
    var PiePoint = extendClass(Point, {
         ...
         * 
         * @param {Boolean}
         *            vis Whether to show the slice or not. If undefined, the
         ...
         * @param {Boolean}
         *            redraw Whether to redraw the chart. True by default.
         */
    ...
    var PieSeries = {
            ...
            if (!init) {
                each(points, function(point) {
                    ...
                    if (graphic) {
                        // start values
                        graphic.attr({
                            r : point.startR || (series.center[3] / 2), // animate
                            // from
                            // inner
                            // radius
                            // (#779)
        ...
        translate : function(positions) {
            ...
            for (i = 0; i < len; i++) {
                ...
                point.labelPos = [ positions[0] + radiusX + mathCos(angle) * labelDistance, // first
                // break
                // of
                // connector
                positions[1] + radiusY + mathSin(angle) * labelDistance, // a/a
                positions[0] + radiusX + mathCos(angle) * connectorOffset, // second
                // break,
                // right
                // outside
                // pie
        ...
        drawPoints : function() {
            ...
            graphic,
            // group,
            shadow = series.options.shadow, shadowGroup, pointAttr, shapeArgs, attr;
    ...
    Series.prototype.drawDataLabels = function() {
        ...
        var series = this, seriesOptions = series.options, cursor = seriesOptions.cursor, options = seriesOptions.dataLabels, points = series.points, pointOptions, generalOptions, hasRendered = series.hasRendered || 0, str, dataLabelsGroup, defer = pick(options.defer, true), renderer = series.chart.renderer;
        ...
        if (options.enabled || series._hasPointLabels) {
            ...
            each(points, function(point) {
                ...
                pointOptions = point.dlOptions || (point.options && point.options.dataLabels); // dlOptions
                // is
                // used
                // in
                // treemaps
                enabled = pick(pointOptions && pointOptions.enabled, generalOptions.enabled) && point.y !== null; // #2282,
                // #4641
                ...
                if (dataLabel && !enabled) {
                    ...
                    // disabled
                    // in the point options, or if they fall outside the plot
                    // area.
                } else if (enabled) {
                    ...
                    if (dataLabel) {
                        ...
                        dataLabel = point.dataLabel = renderer[rotation ? 'text' : 'label']( // labels
                        // don't
                        // support
                        // rotation
                        str, 0, -9999, options.shape, null, null, options.useHTML).attr(attr).css(extend(style, moreStyle)).add(dataLabelsGroup).shadow(options.shadow);
    ...
    Series.prototype.alignDataLabel = function(point, dataLabel, options, alignTo, isNew) {
        var chart = this.chart, inverted = chart.inverted, plotX = pick(point.plotX, -9999), plotY = pick(point.plotY, -9999), bBox = dataLabel.getBBox(), baseline = chart.renderer.fontMetrics(options.style.fontSize).b, rotation = options.rotation, normRotation, negRotation, align = options.align, rotCorr, // rotation
        // correction
        // Math.round for rounding errors (#2683), alignTo to allow column
        // labels (#2700)
        visible = this.visible && (point.series.forceDL || chart.isInsidePlot(plotX, mathRound(plotY), inverted) || (alignTo && chart.isInsidePlot(plotX, inverted ? alignTo.x + 1 : alignTo.y + alignTo.height - 1, inverted))), alignAttr, // the
        // final
        // position;
    ...
    if (seriesTypes.pie) {
        seriesTypes.pie.prototype.drawDataLabels = function() {
            ...
            while (i--) {
                ...
                while (!labelHeight && points[j]) { // #1569
                    labelHeight = points[j] && points[j].dataLabel && (points[j].dataLabel.getBBox().height || 21); // 21
                    // is
                    // for
                    // #968
                    j++;
                ...
                if (distanceOption > 0) {
                    ...
                    for (j = 0; j < length; j++) {
                        ...
                        if (slotIndex < j && slots[j] !== null) { // cluster
                            ...
                            // top
                            slotIndex = j;
                        } else if (slotsLength < length - j + slotIndex && slots[j] !== null) { // cluster
                            // at
                            // the
                            // bottom
                ...
                if (outside && connectorWidth) {
                    each(this.points, function(point) {
                        ...
                        if (dataLabel && dataLabel._pos && point.visible) {
                            ...
                            x = dataLabel.connX;
                            y = dataLabel.connY;
                            connectorPath = softConnector ? [ M, x + (labelPos[6] === 'left' ? 5 : -5), y, // end
                            // of
                            // the
                            // string
                            // at
                            ...
                            2 * labelPos[2] - labelPos[4], 2 * labelPos[3] - labelPos[5], labelPos[2], labelPos[3], // second
                            // break
                            L, labelPos[4], labelPos[5] // base
                            ] : [ M, x + (labelPos[6] === 'left' ? 5 : -5), y, // end
                            // of
                            // the
                            // string
                            // at
        ...
        seriesTypes.pie.prototype.verifyDataLabelOverflow = function(overflow) {
            ...
            if (centerOption[1] !== null) { // Fixed center
                ...
                newSize = mathMax(mathMin(newSize, center[2] - overflow[0] - overflow[2] // vertical
                // overflow
                ), minSize);
    ...
    if (seriesTypes.column) {
        ...
        seriesTypes.column.prototype.alignDataLabel = function(point, dataLabel, options, alignTo, isNew) {
            var inverted = this.chart.inverted, series = point.series, dlBox = point.dlBox || point.shapeArgs, // data
            // label
            // box
            // for
            // alignment
            below = pick(point.below, point.plotY > pick(this.translatedThreshold, series.yAxis.len)), // point.below
            // is
            // used
            // in
            // range
            // series
            inside = pick(options.inside, !!this.options.stacking), // draw it
            // inside
            // the box?
            overshoot;
    ...
    /**
     * Highcharts module to hide overlapping data labels. This module is
     * included in Highcharts.
     */
    (function(H) {
        var Chart = H.Chart, each = H.each, pick = H.pick, addEvent = H.addEvent;

        // Collect potensial overlapping data labels. Stack labels probably
        // don't need to be
        // considered because they are usually accompanied by data labels that
        // lie inside the columns.
        Chart.prototype.callbacks.push(function(chart) {
            function collectAndHide() {
                var labels = [];

                each(chart.series, function(series) {
                    var dlOptions = series.options.dataLabels, collections = series.dataLabelCollections || [ 'dataLabel' ]; // Range
                    // series
                    // have
                    // two
                    // collections
        ...
        /**
         * Hide overlapping labels. Labels are moved and faded in and out on
         * zoom to provide a smooth visual imression.
         */
        Chart.prototype.hideOverlappingLabels = function(labels) {

            var len = labels.length, label, i, j, label1, label2, isIntersecting, pos1, pos2, parent1, parent2, padding, intersectRect = function(x1, y1, w1, h1, x2, y2, w2, h2) {
            ...
            for (i = 0; i < len; i++) {
                ...
                for (j = i + 1; j < len; ++j) {
                    ...
                    if (label1 && label2 && label1.placed && label2.placed && label1.newOpacity !== 0 && label2.newOpacity !== 0) {
                        ...
                        parent1 = label1.parentGroup; // Different panes have
                        // different positions
                        parent2 = label2.parentGroup;
                        padding = 2 * (label1.box ? 0 : label1.padding); // Substract
                        // the
                        // padding
                        // if
                        // no
            ...
            each(labels, function(label) {
                ...
                if (label) {
                    ...
                    if (label.oldOpacity !== newOpacity && label.placed) {
                        ...
                        // Make sure the label is completely hidden to avoid
                        // catching clicks (#4362)
                        if (newOpacity) {
                            label.show(true);
                        } else {
                            complete = function() {
    ...
    var TrackerMixin = Highcharts.TrackerMixin = {
        ...
        drawTrackerGraph : function() {
            ...
            /*
             * for (i = 0; i < singlePoints.length; i++) { singlePoint =
             * singlePoints[i]; trackerPath.push(M, singlePoint.plotX - snap,
    ...
    extend(Chart.prototype, {
        ...
        pan : function(e, panning) {
            ...
            each(panning === 'xy' ? [ 1, 0 ] : [ 1 ], function(isX) { // xy is
                // used
                // in
                // maps
                ...
                chart[mouseDown] = mousePos; // set new reference for next
                // run
            });
    ...
    extend(Point.prototype, {
         ...
         * @param {Boolean}
         *            byProximity Falsy for kd points that are closest to the
         *            mouse, or to actually hovered points. True for other
         *            points in shared tooltip.
         */
        ...
        setState : function(state, move) {
            var point = this, plotX = mathFloor(point.plotX), // #4586
            ...
            if (point.graphic) {
                ...
                point.graphic.attr(merge(pointAttr, radius ? { // new symbol
                    // attributes
                    // (#507, #612)
                    x : plotX - radius,
                    y : plotY - radius,
                ...
                // Zooming in from a range with no markers to a range with
                // markers
                if (stateMarkerGraphic) {
                    stateMarkerGraphic.hide();
                }
            } else {
                // if a graphic is not applied to each point in the normal
                // state, create a shared
                // graphic for the hover state
                if (state && markerStateOptions) {
                    radius = markerStateOptions.radius;
                    newSymbol = pointMarker.symbol || series.symbol;

                    // If the point has another symbol than the previous one,
                    // throw away the
            ...
            if (haloOptions && haloOptions.size) {
                ...
                halo.attr(extend({
                    ...
                    'zIndex' : -1
                // #4929, IE8 added halo above everything
         ...
         * 
         * @param {Number}
         *            size The radius of the circular halo
         * @returns {Array} The path definition
         */
        haloPath : function(size) {
            var series = this.series, chart = series.chart, plotBox = series.getPlotBox(), inverted = chart.inverted, plotX = Math.floor(this.plotX);
    ...
    extend(Series.prototype, {
        ...
        onMouseOut : function() {
            ...
            chart.hoverSeries = null; // #182, set to null before the mouseOut
            // event fires
        ...
        setState : function(state) {
            ...
            if (series.state !== state) {
                ...
                if (graph && !graph.dashstyle) { // hover is turned off for
                    ...
                    // use attr because animate will cause any other animation
                    // on the graph to stop
                    graph.attr(attribs);
