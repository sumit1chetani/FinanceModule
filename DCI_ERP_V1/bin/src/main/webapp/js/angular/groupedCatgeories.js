...
(function(factory) {
    ...
    // #74, since Highcharts 4.1.10 HighchartsAdapter is only provided by the
    ...
    axisProto.render = function() {
        ...
        if (!grid) {
            grid = axis.labelsGrid = axis.chart.renderer.path().attr({
                // #58: use tickWidth/tickColor instead of lineWidth/lineColor:
                strokeWidth : tickWidth, // < 4.0.3
                'stroke-width' : tickWidth, // 4.0.3+ #30
                stroke : options.tickColor || '' // for styled mode
                                                    // (tickColor === undefined)
            }).add(axis.axisGroup);
    ...
    axisProto.groupSize = function(level, position) {
        var positions = this.labelsSizes, direction = this.directionFactor, groupedOptions = this.options.labels.groupedOptions ? this.options.labels.groupedOptions[level - 1] : false, userXY = 0;
    ...
    tickProto.addGroupedLabels = function(category) {
        ...
        while (tick) {
            if (depth > 0 && !category.tick) {
                ...
                label = chart.renderer.text(name, 0, 0, useHTML).attr(mergedAttrs).css(mergedCSS).add(axis.labelGroup);
    ...
    tickProto.render = function(index, old, opacity) {
        ...
        // render grid for "normal" categories (first-level), render left grid
        // line only for the first category
        if (isFirst) {
