(function() {
    ...
    /*
     * AngularJS Toaster Version: 0.4.9
     * 
     * Copyright 2013 Jiri Kavulak. All Rights Reserved. Use, reproduction,
     * distribution, and modification of this code is subject to the terms and
     * conditions of the MIT license, available at
     * http://www.opensource.org/licenses/mit-license.php
     * 
     * Author: Jiri Kavulak Related to project of John Papa and Hans Fjällemark
     */

    angular.module('toaster', [ 'ngAnimate' ]).constant('toasterConfig', {
        ...
        'time-out' : 50000, // Set timeOut and extendedTimeout to 0 to make it
        // sticky
        'icon-classes' : {
            error : 'toast-error',
            info : 'toast-info',
    ...
    }).service('toaster', [ '$rootScope', 'toasterConfig', '$timeout', function($rootScope, toasterConfig, $timeout) {
    ...
    }).directive('toasterContainer', [ '$parse', '$rootScope', '$interval', '$sce', 'toasterConfig', 'toaster', 'toasterRegisterEvents', function($parse, $rootScope, $interval, $sce, toasterConfig, toaster, toasterRegisterEvents) {
        return {
            replace : true,
            restrict : 'EA',
            ...
            link : function(scope, elm, attrs) {
                ...
                scope.$on("$destroy", function() {
                    if (scope.deregClearToasts)
                        scope.deregClearToasts();
                    if (scope.deregNewToast)
                        scope.deregNewToast();
                    scope.deregClearToasts = null;
                    scope.deregNewToast = null;
                });
                ...
                function addToast(toast) {
                    ...
                    // Set the toast.bodyOutputType to the default if it isn't
                    // set
            ...
            template : '<div  id="toast-container" ng-class="[config.position, config.animation]">' + '<div style=" overflow-y: auto !important" ng-repeat="toaster in toasters" class="toast" ng-class="toaster.type" ng-click="click(toaster)" ng-mouseover="stopTimer(toaster)"  ng-mouseout="restartTimer(toaster)">' + '<button class="toast-close-button" ng-show="config.closeButton" ng-click="click(toaster, true)">&times;</button>' + '<div ng-class="config.title"><h4><b>{{toaster.title}}</b></h4></div>' + '<div ng-class="config.message" ng-switch on="toaster.bodyOutputType">' + '<div ng-switch-when="trustedHtml"><div class="scrollme" ng-scrollbar rebuild-on="rebuild:me"><div ng-bind-html="toaster.html"></div></div></div>' + '<div ng-switch-when="template"><div ng-include="toaster.bodyTemplate"></div></div>' + '<div ng-switch-when="templateWithData"><div ng-include="toaster.bodyTemplate"></div></div>' + '<div ng-switch-default >{{toaster.body}}</div>' + '</div>' + '</div>' + '</div>'
        };
    } ]);
})(window, document);