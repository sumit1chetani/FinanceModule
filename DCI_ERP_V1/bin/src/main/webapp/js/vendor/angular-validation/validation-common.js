 ...
 * 
 * @author: Ghislain B.
 * @desc: angular-validation common functions used by both the Directive &
 *        Service
 * 
 */
angular.module('ghiscoding.validation').factory('validationCommon', [ '$rootScope', '$translate', 'validationRules', function($rootScope, $translate, validationRules) {
    // global variables of our object (start with _var), these variables are
    // shared between the Directive & Service
    var _bFieldRequired = false; // by default we'll consider our field not
    // required, if validation attribute calls
    // it, then we'll start validating
    var _INACTIVITY_LIMIT = 1000; // constant of maximum user inactivity time
    // limit, this is the default cosntant but
    // can be variable through typingLimit
    // variable
    var _formElements = []; // Array of all Form Elements, this is not a DOM
    // Elements, these are custom objects defined as {
    // fieldName, elm, attrs, ctrl, isValid, message }
    var _globalOptions = { // Angular-Validation global options, could be
        // define by scope.$validationOptions or by
        // validationService.setGlobalOptions()
        resetGlobalOptionsOnRouteChange : true
    // do we want to reset the Global Options on a route change? True by default
    };
    var _remotePromises = []; // keep track of promises called and running
    // when using the Remote validator
    var _validationSummary = []; // Array Validation Error Summary

    // watch on route change, then reset some global variables, so that we don't
    // carry over other controller/view validations
    $rootScope.$on("$routeChangeStart", function(event, next, current) {
        if (_globalOptions.resetGlobalOptionsOnRouteChange) {
            _globalOptions = {
                displayOnlyLastErrorMsg : false, // reset the option of
                // displaying only the last
                // error message
                preValidateFormElements : false, // reset the option of
                // pre-validate all form
                // elements, false by
                // default
            ...
            _formElements = []; // array containing all form elements, valid or
            // invalid
            _validationSummary = []; // array containing the list of invalid
            // fields inside a validationSummary
        }
    });
    ...
    var validationCommon = function(scope, elm, attrs, ctrl) {
        ...
        // as not required, if validation
        // attribute calls it, then we'll start
        // validating
        this.validators = [];
        ...
        if (!!scope && !!scope.$validationOptions) {
            _globalOptions = scope.$validationOptions; // save the global
            // options
        }
        ...
        if (!!scope && (!!_globalOptions.isolatedScope || !!_globalOptions.scope)) {
            this.scope = _globalOptions.isolatedScope || _globalOptions.scope; // overwrite
            // original
            // scope
            // (isolatedScope/scope
            // are
            // equivalent
            // arguments)
            _globalOptions = mergeObjects(scope.$validationOptions, _globalOptions); // reuse
            // the
            // validationOption
            // from
            // original
        ...
        // if the resetGlobalOptionsOnRouteChange doesn't exist, make sure to
        ...
        // only the angular-validation Directive can possibly reach this
        // condition with all properties filled
        // on the other hand the angular-validation Service will `initialize()`
        // function to initialize the same set of variables
    ...
    validationCommon.prototype.arrayFindObject = arrayFindObject; // search an
    // object
    // inside an
    // array of
    // objects
    validationCommon.prototype.defineValidation = defineValidation; // define
    // our
    // validation
    // object
    validationCommon.prototype.getFormElementByName = getFormElementByName; // get
    // the
    // form
    // element
    // custom
    ...
    // it's
    // name
    validationCommon.prototype.getFormElements = getFormElements; // get the
    // array of
    // form
    // elements
    // (custom
    // objects)
    validationCommon.prototype.getGlobalOptions = getGlobalOptions; // get the
    // global
    // options
    // used by
    // all
    ...
    // the
    // validationService)
    validationCommon.prototype.isFieldRequired = isFieldRequired; // return
    // boolean
    // knowing
    // if the
    // current
    // field is
    // required
    validationCommon.prototype.initialize = initialize; // initialize current
    // object with passed
    // arguments
    validationCommon.prototype.mergeObjects = mergeObjects; // merge 2
    // javascript
    // objects,
    // Overwrites obj1's
    // values with
    ...
    // priority over
    // Object1)
    validationCommon.prototype.removeFromValidationSummary = removeFromValidationSummary; // remove
    // an
    // element
    // from
    // the
    // $validationSummary
    validationCommon.prototype.removeFromFormElementObjectList = removeFromFormElementObjectList; // remove
    // named
    // items
    // from
    // formElements
    // list
    validationCommon.prototype.setDisplayOnlyLastErrorMsg = setDisplayOnlyLastErrorMsg; // setter
    // on
    // the
    // behaviour
    // of
    ...
    // error
    // message
    validationCommon.prototype.setGlobalOptions = setGlobalOptions; // set
    // global
    // options
    // used by
    // all
    ...
    // the
    // validationService)
    validationCommon.prototype.updateErrorMsg = updateErrorMsg; // update on
    // screen an
    // error message
    // below current
    // form element
    ...
    function defineValidation() {
        ...
        // debounce (alias of typingLimit) timeout after user stop typing and
        ...
        // get the rules(or validation), inside directive it's named
        // (validation), inside service(rules)
        var rules = self.validatorAttrs.rules || self.validatorAttrs.validation;

        // We first need to see if the validation holds a custom user regex, if
        // it does then deal with it first
        // So why deal with it separately? Because a Regex might hold pipe '|'
        // and so we don't want to mix it with our regular validation pipe
        if (rules.indexOf("pattern=/") >= 0) {
            ...
            // rewrite the rules so that it doesn't contain any regular
            // expression
            // we simply remove the pattern so that it won't break the
            // Angular-Validation since it also use the pipe |
            rules = rules.replace('pattern=' + pattern, 'pattern');
        }
        // DEPRECATED, in prior version of 1.3.34 and less, the way of writing a
        // regular expression was through regex:/.../:regex
        // this is no longer supported but is still part of the code so that it
        // won't break for anyone using previous way of validating
        // Return string will have the complete regex pattern removed but we
        // will keep ':regex' so that we can still loop over it
            ...
            // rewrite the rules so that it doesn't contain the regex: ...
            // :regex ending
            // we simply remove it so that it won't break if there's a pipe |
            // inside the actual regex
            rules = rules.replace(matches[0], 'regex:');
        }

        // at this point it's safe to split with pipe (since regex was
        ...
        if (validations) {
            ...
            for (var i = 0, ln = validations.length; i < ln; i++) {
                // params split will be:: [0]=rule, [1]=ruleExtraParams OR
                // altText, [2] altText
                var params = validations[i].split(':');

                // check if user provided an alternate text to his validator
                // (validator:alt=Alternate Text)
     ...
     * obj1 When both object have the same property, the Object2 will higher
     * priority over Object1 (basically that property will be overwritten inside
     * Object1)
     * 
     * @param obj1
     * @param obj2
    ...
    function removeFromFormElementObjectList(elmName) {
        var index = arrayFindObjectIndex(_formElements, 'fieldName', elmName); // find
        // index
        // of
        // object
        // in
    ...
    function removeFromValidationSummary(elmName, validationSummaryObj) {
        var self = this;
        var form = getElementParentForm(elmName, self); // find the parent form
        // (only found if it has
        // a name)
        var vsObj = validationSummaryObj || _validationSummary;

        var index = arrayFindObjectIndex(vsObj, 'field', elmName); // find
        // index of
        // object in
        // our array
        ...
        index = arrayFindObjectIndex(_validationSummary, 'field', elmName); // find
        // index
        // of
        // object
        // in
        ...
        if (!!form) {
            ...
            // we need to find only the errors of current form and them into the
            // current scope form object
            form.$validationSummary = arrayFindObjects(_validationSummary, 'formName', form.$name);
        }

        // overwrite the ControllerAs alias if it was passed in the global
        // options
    ...
    /**
     * Setter on the behaviour of displaying only the last error message of each
     * element. By default this is false, so the behavior is to display all
     * error messages of each element.
     * 
     * @param boolean
     *            value
     */
    ...
    function setGlobalOptions(options) {
        ...
        // merge both attributes but 2nd object (attrs) as higher priority, so
        // that for example debounce property inside `attrs` as higher priority
        // over `validatorAttrs`
        // so the position inside the mergeObject call is very important
        _globalOptions = mergeObjects(_globalOptions, options); // save in
        // global
    ...
    /**
     * in general we will display error message at the next element after our
     * input as <span class="validation validation-inputName text-danger"> but
     * in some cases user might want to define which DOM id to display error (as
    ...
    function updateErrorMsg(message, attrs) {
        ...
        // In addition we need to set validatorAttrs, as they are defined as
        // attrs on obj.
        if (!!attrs && attrs.obj) {
            self = attrs.obj;
        ...
        // get the name attribute of current element, make sure to strip dirty
        // characters, for example remove a <input name="options[]"/>, we need
        // to strip the "[]"
        var elmInputName = elmName.replace(/[|&;$%@"<>()+,\[\]\{\}]/g, '');
        var errorElm = null;

        // find the element which we'll display the error message, this element
        // might be defined by the user with 'validationErrorTo'
        if (!!self.validatorAttrs && self.validatorAttrs.hasOwnProperty('validationErrorTo')) {
            // validationErrorTo can be used in 3 different ways: with '.'
            // (element error className) or with/without '#' (element error id)
        ...
        // 2. validationErrorTo has been mistyped, and if mistyped, use regular
        // functionality
        if (!errorElm || errorElm.length === 0) {
            // most common way, let's try to find our <span
            // class="validation-inputName">
        ...
        // invalid & isDirty, display the error message... if <span> not exist
        // then create it, else udpate the <span> text
        if (!!attrs && !attrs.isValid && (isSubmitted || self.ctrl.$dirty || self.ctrl.$touched)) {
            ...
            errorElm.html(''); // element is pristine or no validation applied,
            // error message has to be blank
        }
    }

    /**
     * Validate function, from the input value it will go through all validators
     * (separated by pipe) that were passed to the input element and will
     * validate it. If field is invalid it will update the error text of the
     * span/div element dedicated for that error display.
     * 
     * @param string
     *            value: value of the input field
     * @param bool
     *            showError: do we want to show the error or hide it (false is
     *            useful for adding error to $validationSummary without
    ...
    function validate(strValue, showError) {
        ...
        // to make proper validation, our element value cannot be an undefined
        // variable (we will at minimum make it an empty string)
        // For example, in some particular cases "undefined" returns always True
        // on regex.test() which is incorrect especiall on max_len:x
        if (typeof strValue === "undefined") {
            strValue = '';
        }

        // get some common variables
        var elmName = (!!self.ctrl && !!self.ctrl.$name) ? self.ctrl.$name : (!!self.attrs && !!self.attrs.name) ? self.attrs.name : self.elm.attr('name');
        ...
        for (var j = 0, jln = self.validators.length; j < jln; j++) {
            ...
            // the AutoDetect type is a special case and will detect if the
            // given value is of type numeric or not.
            // then it will rewrite the conditions or regex pattern, depending
            // on type found
            ...
            // there is multiple type that can influence how the value will be
            // validated

            if (validator.type === "conditionalDate") {
                var isWellFormed = isValid = false;

                // 1- make sure Date is well formed (if it's already a Date
                // object then it's already good, else check that with Regex)
                if ((strValue instanceof Date)) {
                    isWellFormed = true;
                } else {
                    // run the Regex test through each iteration, if required
                    // (\S+) and is null then it's invalid automatically
                ...
                // 2- date is well formed, then go ahead with conditional date
                // check
                if (isWellFormed) {
                    // For Date comparison, we will need to construct a Date
                    // Object that follows the ECMA so then it could work in all
                    // browser
                    // Then convert to timestamp & finally we can compare both
                    // dates for filtering
                    var dateType = validator.dateType; // date type (ISO, EURO,
                    // US-SHORT, US-LONG)
                    var timestampValue = (strValue instanceof Date) ? strValue : parseDate(strValue, dateType).getTime(); // our
                    // input
                    // value
                    // parsed
                    // into
                    ...
                    if (validator.params.length == 2) {
                        // this is typically a "between" condition, a range of
                        // number >= and <=
            ...
            // it might be a match input checking
                ...
                // get the element 'value' ngModel to compare to (passed as
                // params[0], via an $eval('ng-model="modelToCompareName"')
                ...
                // if element to compare against has a friendlyName or if
                // matching 2nd argument was passed, we will use that as a new
                // friendlyName
                // ex.: <input name='input1' friendly-name='Password1'/> :: we
                // would use the friendlyName of 'Password1' not input1
                // or <input name='confirm_pass'
                // validation='match:input1,Password2' /> :: we would use
                // Password2 not input1
            ...
            // it might be a remote validation, this should return a promise
            // with the result as a boolean or a { isValid: bool, message: msg }
            else if (validator.type === "remote") {
                if (!!strValue && !!showError) {
                    self.ctrl.$processing = true; // $processing can be use in
                    // the DOM to display a
                    // remote processing message
                    // to the user
                    ...
                    if (fname.indexOf(".") === -1) {
                        ...
                        // function name might also be declared with the
                        // Controller As alias, for example: vm.customRemote()
                        // split the name and flatten it to find it inside the
                        // scope
                    ...
                    // if we already have previous promises running, we might
                    // want to abort them (if user specified an abort function)
                    if (_remotePromises.length > 1) {
                        while (_remotePromises.length > 0) {
                            ...
                            if (typeof previousPromise.abort === "function") {
                                previousPromise.abort(); // run the abort if
                                // user declared it
                            }
                        }
                    }
                    _remotePromises.push(promise); // always add to beginning
                    // of array list of promises

                    if (!!promise && typeof promise.then === "function") {
                        self.ctrl.$setValidity('remote', false); // make the
                        // field
                        // invalid
                        // before
                        // processing
                        ...
                        (function(altText) {
                            promise.then(function(result) {
                                ...
                                _remotePromises.pop(); // remove the last
                                // promise from array
                                // list of promises

                                self.ctrl.$processing = false; // finished
                                // resolving, no
                                // more pending
                                var errorMsg = message + ' '; // use the
                                // global error
                                // message
                                ...
                                if (isValid === false) {
                                    ...
                                    // is field is invalid and we have an error
                                    // message given, then add it to
                                    // validationSummary and display error
                                    addToValidationAndDisplayError(self, formElmObj, errorMsg, false, showError);
                                }
                                if (isValid === true && isFieldValid === true) {
                                    // if field is valid from the remote check
                                    // (isValid) and from the other validators
                                    // check (isFieldValid)
                                    // clear up the error message and make the
                                    // field directly as Valid with $setValidity
                                    // since remote check arrive after all other
                                    // validators check
            ...
            // or finally it might be a regular regex pattern checking
                ...
                // a 'disabled' element should always be valid, there is no need
                // to validate it
                if (!!self.elm.prop("disabled") || !!self.scope.$eval(elmAttrNgDisabled)) {
                    isValid = true;
                } else {
                    // before running Regex test, we'll make sure that an input
                    // of type="number" doesn't hold invalid keyboard chars, if
                    // true skip Regex
                    if (typeof strValue === "string" && strValue === "" && self.elm.prop('type').toUpperCase() === "NUMBER") {
                        isValid = false;
                        // message = $translate.instant("INVALID_KEY_CHAR");
                    } else {
                        // run the Regex test through each iteration, if
                        // required (\S+) and is null then it's invalid
                        // automatically
                        regex = new RegExp(validator.pattern);
            ...
            // not required and not filled is always valid & 'disabled',
            // 'ng-disabled' elements should always be valid
            ...
            if (!isValid) {
                ...
                // run $translate promise, use closures to keep access to all
                // necessary variables
                (function(formElmObj, isValid, validator) {
                    ...
                    $translate(msgToTranslate).then(function(translation) {
                        // if user is requesting to see only the last error
                        // message, we will use '=' instead of usually
                        // concatenating with '+='
                        // then if validator rules has 'params' filled, then
                        // replace them inside the translation message (foo{0}
                        // {1}...), same syntax as String.format() in C#
                        ...
                        // error caught:
                        // alternate text might not need translation if the user
                        // sent his own custom message or is already translated
    ...
    function addToFormElementObjectList(elm, attrs, ctrl, scope) {
        ...
        var form = getElementParentForm(elmName, {
            scope : scope
        }); // find the parent form (only found if it has a name)
        ...
        var index = arrayFindObjectIndex(_formElements, 'fieldName', elm.attr('name')); // find
        // index
        // of
        // object
        // in
    ...
    /**
     * Will add error to the validationSummary and also display the error
    ...
    function addToValidationAndDisplayError(self, formElmObj, message, isFieldValid, showError) {
        ...
        // change the Form element object boolean flag from the `formElements`
        // variable, used in the `checkFormValidity()`
        if (!!formElmObj) {
        ...
        if (!!self.validatorAttrs.preValidateFormElements || !!_globalOptions.preValidateFormElements) {
            // make the element as it was touched for CSS, only works in
            // AngularJS 1.3+
    ...
    function addToValidationSummary(self, message) {
        ...
        // (only found if it has
        // a name)
        var index = arrayFindObjectIndex(_validationSummary, 'field', elmName); // find
        // index
        // of
        // object
        // in
        ...
        if (index >= 0 && message === '') {
            ...
            // if error already exist then refresh the error object inside the
            // array, else push it to the array
            if (index >= 0) {
        ...
        // also save it inside the ControllerAs alias if it was passed in the
        // global options
        if (!!_globalOptions && !!_globalOptions.controllerAs) {
            _globalOptions.controllerAs.$validationSummary = _validationSummary;
    ...
    /**
     * Quick function to find an object inside an array by it's given field name
     * and value, return the object found or null
     * 
     * @param Array
     *            sourceArray
     * @param string
     *            searchId: search property id
     * @param string
    ...
    /**
     * Quick function to find all object(s) inside an array of objects by it's
     * given field name and value, return array of object found(s) or empty
     * array
     * 
     * @param Array
     *            sourceArray
    ...
    /**
     * Quick function to find an object inside an array by it's given field name
     * and value, return the index position found or -1
     * 
     * @param Array
     *            sourceArray
     * @param string
    ...
    function getElementParentForm(elmName, self) {
        ...
        for (var i = 0; i < forms.length; i++) {
            ...
            if (!!form && !!form.name) {
                parentForm = (!!_globalOptions && !!_globalOptions.controllerAs && form.name.indexOf('.') >= 0) ? explodedDotNotationStringToObject(form.name, self.scope) : self.scope[form.name];

                if (!!parentForm) {
                    if (typeof parentForm.$name === "undefined") {
                        parentForm.$name = form.name; // make sure it has a
                        // $name, since we use
                        // that variable later
                        // on
                    }
        ...
        // falling here with a form name but without a form object found in the
        // scope is often due to isolate scope
        // we can hack it and define our own form inside this isolate scope, in
        // that way we can still use something like:
        // isolateScope.form1.$validationSummary
        if (!!form && !!form.name) {
            var obj = {
                $name : form.name,
    ...
    /**
     * Parse a date from a String and return it as a Date Object to be valid for
     * all browsers following ECMA Specs Date type ISO (default), US, UK,
     * Europe, etc... Other format could be added in the switch case
     * 
     * @param String
     *            dateStr: date String
     * @param String
     *            dateType: date type (ISO, US, etc...)
     * @return object date
     */
    function parseDate(dateStr, dateType) {
        ...
        // Parse using the date type user selected, (separator could be dot,
        // slash or dash)
        ...
        case 'EUROPE': // UK, Europe format is: dd/mm/yy hh:mm:ss
            ...
            day = dateSplit[0];
            month = dateSplit[1];
            year = (parseInt(dateSplit[2]) < 50) ? ('20' + dateSplit[2]) : ('19' + dateSplit[2]); // below
            // 50
            // we'll
            // consider
            // that
        ...
        case 'US-SHORT': // US short format is: mm/dd/yy hh:mm:ss OR
            ...
            break;
        case 'ISO':
        default: // ISO format is: yyyy-mm-dd hh:mm:ss (separator could be
            // dot, slash or dash: ".", "/", "-")
            dateSubStr = dateStr.substring(0, 10);
        ...
        // Note that, in JavaScript, months run from 0 to 11, rather than 1 to
        // 12!
        return new Date(year, month - 1, day, hour, min, sec);
    }

    /**
     * From a date substring split it by a given separator and return a split
     * array
     * 
     * @param string
     *            dateSubStr
    ...
    /**
     * Test values with condition, I have created a switch case for all possible
     * conditions.
     * 
     * @param string
     *            condition: condition to filter with
     * @param any
     *            value1: 1st value to compare, the type could be anything
     *            (number, String or even Date)
     * @param any
     *            value2: 2nd value to compare, the type could be anything
     *            (number, String or even Date)
    ...
    /**
     * Override javascript trim() function so that it works accross all browser
     * platforms
     */
    ...
    /**
     * Override javascript format() function to be the same as the effect as the
     * C# String.Format Input: "Some {0} are showing {1}".format("inputs",
     * "invalid"); Output: "Some inputs are showing invalid"
     * 
     * @param string
     * @param replacements
     */
    ...
    /**
     * Override javascript String.format() function to be the same as the effect
     * as the C# String.Format Input: String.format("Some {0} are showing {1}",
     * "inputs", "invalid"); Output: "Some inputs are showing invalid"
     * 
     * @param string
     * @param replacements
     */
