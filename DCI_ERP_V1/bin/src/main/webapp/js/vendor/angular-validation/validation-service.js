 ...
 * 
 * @author: Ghislain B.
 * @desc: angular-validation service definition Provide a way to
 *        programmatically create validators and validate a form directly from
 *        within the controller. This Service is totally independant from the
 *        Directive, it could be used separately but the minimum it needs is the
 *        `validation-rules.js` file.
 */
angular.module('ghiscoding.validation').service('validationService', [ '$interpolate', '$timeout', 'validationCommon', function($interpolate, $timeout, validationCommon) {
    ...
    var validationService = function(globalOptions) {
        ...
        this.commonObj = new validationCommon(); // Object of
    ...
    validationService.prototype.addValidator = addValidator; // add a
    // Validator to
    // current
    // element
    validationService.prototype.checkFormValidity = checkFormValidity; // check
    // the
    // form
    // validity
    // (can
    ...
    // both
    // Directive/Service)
    validationService.prototype.removeValidator = removeValidator; // remove a
    // Validator
    // from an
    // element
    validationService.prototype.resetForm = resetForm; // reset the form (reset
    // it to Pristine and
    // Untouched)
    validationService.prototype.setDisplayOnlyLastErrorMsg = setDisplayOnlyLastErrorMsg; // setter
    // on
    // the
    // behaviour
    // of
    ...
    // error
    // message
    validationService.prototype.setGlobalOptions = setGlobalOptions; // set
    // and
    // initialize
    // global
    // options
    ...
    // all
    // validators
    validationService.prototype.clearInvalidValidatorsInSummary = clearInvalidValidatorsInSummary; // clear
    // clearInvalidValidatorsInSummary
    ...
    /**
     * Add a validator on a form element, the argument could be passed as 1
     * single object (having all properties) or 2 to 3 string arguments
     * 
     * @param mixed
     *            var1: could be a string (element name) or an object
     *            representing the validator
     * @param string
     *            var2: [optional] validator rules
     * @param string
     *            var3: [optional] friendly name
     */
    function addValidator(var1, var2, var3) {
        var self = this;
        var attrs = {};

        // find if user provided 2 string arguments else it will be a single
        ...
        // get the scope from the validator or from the global options
        // (validationAttrs)
        var scope = (!!attrs.scope) ? attrs.scope : self.validationAttrs.scope;

        // find the DOM element & make sure it's a filled object before going
        // further
        ...
        // when using dynamic elements, we might have encounter unparsed or
        // uncompiled data, we need to get Angular result with $interpolate
        ...
        if (!!self.validationAttrs.isolatedScope) {
            var tempValidationOptions = scope.$validationOptions || null; // keep
            // global
            // validationOptions
            scope = self.validationAttrs.isolatedScope; // rewrite original
            // scope
            if (!!tempValidationOptions) {
                scope.$validationOptions = tempValidationOptions; // reuse the
                // validationOption
                // from
                // original
                // scope
        ...
        // if it become disabled then skip validation else it becomes enable
        // then we need to revalidate it
        watchNgDisabled(self, scope, attrs);

        // if DOM element gets destroyed, we need to cancel validation, unbind
        // onBlur & remove it from $validationSummary
        ...
        var watcherHandler = scope.$watch(attrs.elmName, function(newVal, oldVal) {
            // when previous value was set and new value is not, this is most
            // probably an invalid character entered in a type input="text"
            // we will still call the `.validate()` function so that it shows
            // also the possible other error messages
            if (newVal === undefined && oldVal !== undefined) {
                $timeout.cancel(self.timer);
                self.commonObj.ctrl.$setValidity('validation', self.commonObj.validate('', true));
                return;
            }
            // from the DOM element, find the Angular controller of this element
            // & add value as well to list of attribtues
        ...
        // save the watcher inside an array in case we want to deregister it
        // when removing a validator
        _watchers.push({
            elmName : attrs.elmName,
    ...
    /**
     * Check the form validity (can be called by an empty validationService and
     * used by both Directive/Service) Loop through Validation Summary and if
     * any errors found then display them and return false on current function
     * 
     * @param object
     *            Angular Form or Scope Object
    ...
    function checkFormValidity(obj) {
        ...
        // loop through $validationSummary and display errors when found on each
        // field
    ...
    /**
     * Remove all objects in validationsummary and matching objects in
     * FormElementList. This is for use in a wizard type setting, where you
     * 'move back' to a previous page in wizard. In this case you need to remove
     * invalid validators that will exist in 'the future'.
     * 
     * @param object
     *            Angular Form or Scope Object
     */
    function clearInvalidValidatorsInSummary(obj) {
        ...
        // Loop on list of names. Cannot loop on obj.$validationSummary as you
        // are removing objects from it in the loop.
    ...
    /**
     * Remove a validator and also any withstanding error message from that
     * element
     * 
     * @param object
     *            Angular Form or Scope Object
     * @param array/string
     *            of element name(s) (name attribute)
     * @return object self
     */
    function removeValidator(obj, args) {
        ...
        // Note: removeAttr() will remove validation attribute from the DOM (if
        // defined by Directive), but as no effect when defined by the Service
        // removeValidator() 2nd argument could be passed an Array or a string
        // of element name(s)
    ...
    /**
     * Reset a Form, reset all input element to Pristine, Untouched & remove
     * error dislayed (if any)
     * 
     * @param object
     *            Angular Form or Scope Object
     * @param bool
     *            empty also the element values? (True by default)
     */
    function resetForm(obj, args) {
        ...
        // get all Form input elements and loop through all of them to set them
        // Pristine, Untouched and also remove errors displayed
        ...
        if (formElements instanceof Array) {
            for (var i = 0, ln = formElements.length; i < ln; i++) {
                ...
                // if yes, then run removeValidator() and since that already
                // removes message & make input valid, no need to run the
                // $setUntouched() and $setPristine()
                // else make the field $setUntouched() and $setPristine()
                if (!!shouldRemoveValidator) {
                    removeValidator(obj, {
                        self : self,
                        formElmObj : formElmObj
                    });
                } else {
                    // make the element as it was touched for CSS, only works in
                    // AngularJS 1.3+
    ...
    /**
     * Validator function to attach to the element, this will get call whenever
     * the input field is updated and is also customizable through the
     * (typing-limit) for which inactivity this.timer will trigger validation.
     * 
     * @param object
     *            self
     * @param string
    ...
    function attemptToValidate(self, value, typingLimit) {
        // get the waiting delay time if passed as argument or get it from
        // common Object
        ...
        // pre-validate without any events just to pre-fill our
        // validationSummary with all field errors
        // passing false as 2nd argument for not showing any errors on screen
        self.commonObj.validate(value, false);

        // if field is not required and his value is empty, cancel validation
        // and exit out
        ...
        // if a field holds invalid characters which are not numbers inside an
        // `input type="number"`, then it's automatically invalid
        // we will still call the `.validate()` function so that it shows also
        // the possible other error messages
        ...
        // onKeyDown event is the default of Angular, no need to even bind it,
        // it will fall under here anyway
        // in case the field is already pre-filled, we need to validate it
        // without looking at the event binding
        if (typeof value !== "undefined") {
            // Make the validation only after the user has stopped activity on a
            // field
            // everytime a new character is typed, it will cancel/restart the
            // timer & we'll erase any error mmsg
            self.commonObj.updateErrorMsg('');
            $timeout.cancel(self.timer);
    ...
    function cancelValidation(obj, formElmObj) {
        ...
        // unbind onBlur handler (if found) so that it does not fail on a
        // non-required element that is now dirty & empty
     ...
     *            self
     * @param object
     *            formElmObj: form element object
     * @param object
     *            validationSummary
     */
    function removeWatcherAndErrorMessage(self, formElmObj, validationSummary) {
        var scope = !!self.commonObj.scope ? self.commonObj.scope : !!formElmObj.scope ? formElmObj.scope : null;
        ...
        if (!!foundWatcher) {
            foundWatcher.watcherHandler(); // deregister the watch by calling
            // his handler
        }

        // make the validation cancelled so it won't get called anymore in the
        // blur eventHandler
        formElmObj.isValidationCancelled = true;
        ...
        // now to remove any errors, we need to make the element untouched,
        // pristine and remove the validation
        // also remove it from the validationSummary list and remove any
        // displayed error
        if (typeof formElmObj.ctrl.$setUntouched === "function") {
    ...
    function watchNgDisabled(self, scope, attrs) {
        scope.$watch(function() {
            return (typeof attrs.elm.attr('ng-disabled') === "undefined") ? null : scope.$eval(attrs.elm.attr('ng-disabled')); // this
            // will
            // evaluate
            // attribute
            // value
            ...
            // get current ctrl of element & re-initialize to use current
            // element
            attrs.ctrl = angular.element(attrs.elm).controller('ngModel');
            self.commonObj.initialize(scope, attrs.elm, attrs, attrs.ctrl);
            ...
            // use a timeout so that the digest of removing the `disabled`
            // attribute on the DOM is completed
            // because commonObj.validate() checks for both the `disabled` and
            // `ng-disabled` attribute so it basically fails without the
            // $timeout because of the digest
            $timeout(function() {
                if (disabled) {
                    ...
                    // Re-Validate the input when enabled (without displaying
                    // the error)
                    var value = attrs.ctrl.$viewValue || '';

                    // re-initialize to use current element & validate without
                    // delay (without displaying the error)
                    self.commonObj.initialize(scope, attrs.elm, attrs, attrs.ctrl);
            ...
            // these cannot be done inside the $timeout, when doing it would
            // cancel validation for all element because of the delay
            if (disabled) {
                // Turn off validation when element is disabled & remove from
                // validationSummary (seems I need to remove from summary inside
                // $timeout and outside)
                // make the element as it was untouched for CSS, only works in
                // AngularJS 1.3+
