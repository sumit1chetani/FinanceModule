(function(Handsontable) {
    ...
    function HandsontableFormula() {
        ...
        var afterChange = function(changes, source) {
            ...
            if (source === 'edit' || source === 'undo' || source === 'autofill') {
                ...
                changes.forEach(function(item) {
                    ...
                    // if changed value, all references cells should be
                    // recalculated
        ...
        var beforeAutofillInsidePopulate = function(index, direction, data, deltas, iterators, selected) {
            ...
            var r = index.row, c = index.col, value = data[r][c], delta = 0, rlength = data.length, // rows
            ...
            if (value[0] === '=') { // formula
                ...
                if (rlength >= 2 || clength >= 2) {
                    ...
                    var newValue = instance.plugin.helper.number(value), ii, start;
        ...
        var afterRemoveRow = function(row, amount, auto) {
            ...
            var items = instance.plugin.matrix.getRefItemsToRow(row), changes = [];
        ...
        var afterCreateRow = function(row, amount, auto) {
            ...
            var direction = (selectedRow >= row) ? 'before' : 'after', items = instance.plugin.matrix.getRefItemsToRow(row), counter = 1, changes = [];

            items.forEach(function(id) {
                var item = instance.plugin.matrix.getItem(id), formula = instance.plugin.utils.changeFormula(item.formula, 1, {
                    row : row
        ...
        var afterCreateCol = function(col) {
            ...
            var items = instance.plugin.matrix.getRefItemsToColumn(col), counter = 1, direction = (selectedCol >= col) ? 'before' : 'after', changes = [];
