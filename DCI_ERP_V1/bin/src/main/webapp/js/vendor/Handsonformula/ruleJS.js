var ruleJS = (function(root) {
    ...
    var Matrix = function() {
         ...
         * 
         * @type {{id: string, formula: string, value: string, error: string,
         *       deps: Array, formulaEdit: boolean}}
         */
        var item = {
            id : '',
            formula : '',
            ...
            instance.matrix.data.forEach(function(item) {
                if (item.deps) {
                    var deps = item.deps.filter(function(cell) {
                        ...
                        var alpha = instance.utils.getCellAlphaNum(cell).alpha, num = instance.utils.toNum(alpha);
        ...
        this.updateElementItem = function(element, props) {
            var id = element.getAttribute('id'), item = instance.matrix.getItem(id);
        ...
        var recalculateElementDependencies = function(element) {
            var allDependencies = instance.matrix.getElementDependencies(element), id = element.getAttribute('id');
        ...
        var calculateElementFormula = function(formula, element) {
            ...
            var parsed = parse(formula, element), value = parsed.result, error = parsed.error, nodeName = element.nodeName.toUpperCase();
        ...
        var registerElementInMatrix = function(element) {
            ...
            var id = element.getAttribute('id'), formula = element.getAttribute('data-formula');
        ...
        this.depsInFormula = function(item) {
            ...
            var formula = item.formula, deps = item.deps;
     ...
     * utils methods
     * 
     * @type {{isArray: isArray, toNum: toNum, toChar: toChar, cellCoords:
     *       cellCoords}}
     */
    var utils = {
        /**
        ...
        changeRowIndex : function(cell, counter) {
            var alphaNum = instance.utils.getCellAlphaNum(cell), alpha = alphaNum.alpha, col = alpha, row = parseInt(alphaNum.num + counter, 10);
        ...
        changeColIndex : function(cell, counter) {
            var alphaNum = instance.utils.getCellAlphaNum(cell), alpha = alphaNum.alpha, col = instance.utils.toChar(parseInt(instance.utils.toNum(alpha) + counter, 10)), row = alphaNum.num;

            if (!col || col.length === 0) {
                col = 'A';
            }

            var fixedCol = alpha[0] === '$' || false, fixedRow = alpha[alpha.length - 1] === '$' || false;
        ...
        changeFormula : function(formula, delta, change) {
            ...
            return formula.replace(/(\$?[A-Za-z]+\$?[0-9]+)/g, function(match) {
                var alphaNum = instance.utils.getCellAlphaNum(match), alpha = alphaNum.alpha, num = alphaNum.num;
         ...
         * 
         * @param {Object}
         *            startCell ex.: {row:1, col: 1}
         * @param {Object}
         *            endCell ex.: {row:10, col: 1}
         * @param {Function=}
         *            callback
    ...
    var helper = {
        ...
        cellValue : function(cell) {
            var value, fnCellValue = instance.custom.cellValue, element = this, item = instance.matrix.getItem(cell);

            // check if custom cellValue fn exists
            if (instance.utils.isFunction(fnCellValue)) {

                var cellCoords = instance.utils.cellCoords(cell), cellId = instance.utils.translateCellCoords({
                    row : element.row,
                    col : element.col
                });
        ...
        cellRangeValue : function(start, end) {
            var fnCellValue = instance.custom.cellValue, coordsStart = instance.utils.cellCoords(start), coordsEnd = instance.utils.cellCoords(end), element = this;
            ...
            if (instance.utils.isFunction(fnCellValue)) {
                ...
                var cellId = instance.utils.translateCellCoords({
                    row : element.row,
                    col : element.col
                });
