...
/**
 * Array.filter() shim by Trevor Menagh (https://github.com/trevmex) with some
 * modifications
 */
...
/*
 * Copyright 2012 The Polymer Authors. All rights reserved. Use of this source
...
if (typeof WeakMap === 'undefined') {
  (function() {
    ...
    /*
     * IE8 does not support Date.now() but IE8 compatibility mode in IE9 and
     * IE10 does. M$ deserves a high five for this one :)
     */
 ...
 * @description
 * 
 * After Handsontable is constructed, you can modify the grid behavior using the
 ...
 * 
 * @example ```js // all following examples assume that you constructed
 *          Handsontable like this var exampleContainer1 =
 *          document.getElementById('example1'); var ht = new
 *          Handsontable(exampleContainer1, options); // now, to use
 *          setDataAtCell method, you can either: ht.setDataAtCell(0, 0, 'new
 *          value'); ```
 * 
 * Alternatively, you can call the method using jQuery wrapper (__obsolete__,
 * requires initialization using our jQuery guide
 * @example ```js $('#example1').handsontable('setDataAtCell', 0, 0, 'new
 *          value'); ``` ---
 */
Handsontable.Core = function (rootElement, userSettings) {
  ...
  this.guid = 'ht_' + Handsontable.helper.randomString(); // this is the
                                                            // namespace for
                                                            // global events

  if (!this.rootElement.id || this.rootElement.id.substring(0, 3) === "ht_") {
    this.rootElement.id = this.guid; // if root element does not have an id,
                                        // assign a random id
  }
  priv = {
    cellSettings: [],
  ...
  grid = {
     ...
     * 
     * @param {String}
     *            action Possible values: "insert_row", "insert_col",
     ...
     *            amount
     * @param {String}
     *            [source] Optional. Source of hook runner.
     * @param {Boolean}
     *            [keepEmptyRows] Optional. Flag for preventing deletion of
     *            empty rows.
     */
    alter: function (action, index, amount, source, keepEmptyRows) {
      ...
      switch (action) {
        case "insert_row":
          ...
          if (delta) {
            if (selection.isSelected() && priv.selRange.from.row >= index) {
              ...
              selection.refreshBorders(); // it will call render and prepare
                                            // methods
            }
          }
          break;
        ...
        case "insert_col":
          // //column order may have changes, so we need to translate the
            // selection column index -> source array index
          ...
          if (delta) {
            ...
            if(Array.isArray(instance.getSettings().colHeaders)){
              ...
              spliceArray.length += delta; // inserts empty (undefined)
                                            // elements at the end of an array
              Array.prototype.splice.apply(instance.getSettings().colHeaders, spliceArray); // inserts
                                                                                            // empty
                                                                                            // (undefined)
                                                                                            // elements
                                                                                            // into
        ...
        case "remove_col":
          ...
          for(var row = 0, len = datamap.getAll().length; row < len; row++){
            if(row in priv.cellSettings){  // if row hasn't been rendered it
                                            // wouldn't have cellSettings
      ...
      if (!keepEmptyRows) {
        grid.adjustRowsAndCols(); // makes sure that we did not add rows that
                                    // will be removed in next refresh
      }
    },
    ...
    adjustRowsAndCols: function () {
      ...
      // instance.countRows() > priv.settings.minRows) &&
        // (priv.settings.minSpareRows && emptyRows >
        // priv.settings.minSpareRows)); emptyRows--) {
      // datamap.removeRow();
      // }
      // }
      ...
      // instance.countCols() > priv.settings.minCols) &&
        // (priv.settings.minSpareCols && emptyCols >
        // priv.settings.minSpareCols)); emptyCols--) {
      // datamap.removeCol();
      // }
      // }
     ...
     * 
     * @param {Object}
     *            start Start selection position
     * @param {Array}
     *            input 2d array
     * @param {Object}
     *            [end] End selection position (only for drag-down mode)
     * @param {String}
     ...
     * @param {Array}
     *            deltas array
     * @returns {Object|undefined} ending td in pasted area (only if any cell
     *          was changed)
     */
  ...
  this.selection = selection = { // this public assignment is only temporary
    ...
    /**
     * Sets inProgress to `true`. This enables onSelectionEnd and
     * onSelectionEndByProp to function as desired.
     */
    begin: function () {
    ...
    /**
     * Sets inProgress to `false`. Triggers onSelectionEnd and
     ...
     *            coords
     * @param {Boolean}
     *            [scrollToCell=true] If `true`, viewport will be scrolled to
     *            range end
     * @param {Boolean}
     *            [keepEditorOpened] If `true`, cell editor will be still opened
     *            after changing selection range
     */
    ...
    /**
     * Sets selection end cell relative to current selection end cell (if
     * possible).
     */
    ...
    /**
     * Returns `true` if currently there is a selection on screen, `false`
     * otherwise.
     * 
     * @returns {Boolean}
     */
  ...
  function ValidatorsQueue() { // moved this one level up so it can be used in
                                // any function here. Probably this should be
  ...
  function validateChanges(changes, source, callback) {
    ...
    for (var i = changes.length - 1; i >= 0; i--) {
      if (changes[i] === null) {
        ...
        // column order may have changes, so we need to translate physical col
        // index (stored in datasource) to logical (displayed to user)
        ...
        if (cellProperties.type === 'numeric' && typeof changes[i][3] === 'string') {
          if (changes[i][3].length > 0 && (/^-?[\d\s]*(\.|\,)?\d*$/.test(changes[i][3]) || cellProperties.format )) {
            ...
            // this input in format XXXX.XX is likely to come from paste. Let's
            // parse it using international rules
          ...
          instance.validateCell(changes[i][3], cellProperties, (function (i, cellProperties) {
            return function (result) {
              ...
              if (result === false && cellProperties.allowInvalid === false) {
                ...
                cellProperties.valid = true;  // we cancelled the change, so
                                                // cell value is still valid
                --i;
              }
    ...
    function resolve() {
      ...
      if (changes.length) {
        ...
        if (typeof beforeChangeResult === 'function') {
          ...
          changes.splice(0, changes.length); // invalidate all changes
                                                // (remove everything from
                                                // array)
        }
      }
        callback(); // called when async validators are resolved and
                    // beforeChange was not async
    }
  }
  ...
  /**
     ...
     * @private
     * @param {Array}
     *            changes Array in form of [row, prop, oldValue, newValue]
     * @param {String}
     *            source String that identifies how this change will be
     *            described in changes array (useful in onChange callback)
     */
  ...
  this.validateCell = function (value, cellProperties, callback, source) {
    ...
    if (typeof validator == 'function') {
      ...
      // To provide consistent behaviour, validation should be always
        // asynchronous
  ...
  /**
     * @description Set new value to a cell. To change many cells at once, pass
     *              an array of `changes` in format `[[row, col, value], ...]`
     *              as the only parameter. `col` is the index of __visible__
     *              column (note that if columns were reordered, the current
     *              order will be used). `source` is a flag for
     *              before/afterChange events. If you pass only array of changes
     *              then `source` could be set as second parameter.
     * 
     * @param {Number|Array}
     *            row or array of changes in format `[[row, col, value], ...]`
     * @param {Number|String}
     *            col or source String
     * @param {String}
     *            value
     * @param {String}
     *            [source] String that identifies how this change will be
  ...
  /**
     ...
     * property (e.g. `[0, 'first.name', 'Jennifer']`).
     * 
     * @param {Number|Array}
     *            row or array of changes in format `[[row, prop, value], ...]`
     * @param {String}
     *            prop or source String
     * @param {String}
     *            value
     * @param {String}
     *            [source] String that identifies how this change will be
  ...
  /**
     * Returns `true` if current Handsontable instance is listening to keyboard
  ...
  /**
     ...
     * 
     * @param {Boolean}
     *            [revertOriginal] If != `true`, edited data is saved. Otherwise
  ...
  /**
     ...
     * Use `endRow`, `endCol` when you want to cut input when certain row is
     * reached. Optional `source` parameter (default value "populateFromArray")
     * is used to identify this call in the resulting events (beforeChange,
     * afterChange). Optional `populateMethod` parameter (default value
     * "overwrite", possible values "shift_down" and "shift_right") has the same
     * effect as pasteMethod option (see Options page)
     * 
     * @since 0.9.0
     * @param {Number}
     *            row Start row
     ...
     *            input 2d array
     * @param {Number}
     *            [endRow] End row (use when you want to cut input when certain
     *            row is reached)
     * @param {Number}
     *            [endCol] End column (use when you want to cut input when
     *            certain column is reached)
     * @param {String}
     *            [source="populateFromArray"]
     * @param {String}
     *            [method="overwrite"]
     * @param {String}
     *            direction edit (left|right|up|down)
     * @param {Array}
     *            deltas array
     * @returns {Object|undefined} ending td in pasted area (only if any cell
     *          was changed)
  ...
  this.populateFromArray = function (row, col, input, endRow, endCol, source, method, direction, deltas) {
    ...
    if (!(typeof input === 'object' && typeof input[0] === 'object')) {
      throw new Error("populateFromArray parameter `input` must be an array of arrays"); // API
                                                                                            // changed
                                                                                            // in
                                                                                            // 0.9-beta2,
                                                                                            // let's
  ...
  /**
     * Adds/removes data from the column. This function works is modelled after
     * Array.splice. Parameter `col` is the index of column in which do you want
     * to do splice. Parameter `index` is the row index at which to start
     * changing the array. If negative, will begin that many elements from the
     * end. Parameter `amount`, is the number of old array elements to remove.
     * If the amount is 0, no elements are removed. Fourth and further
     * parameters are the `elements` to add to the array. If you don't specify
     * any elements, spliceCol simply removes elements from the array.
     * {@link Handsontable.DataMap#spliceCol}
     * 
     * @since 0.9-beta2
     * @param {Number}
     *            col Index of column in which do you want to do splice.
     * @param {Number}
     *            index Index at which to start changing the array. If negative,
     *            will begin that many elements from the end
     * @param {Number}
     *            amount An integer indicating the number of old array elements
     *            to remove. If amount is 0, no elements are removed
     * @param {*}
     *            [elements] The elements to add to the array. If you don't
     *            specify any elements, spliceCol simply removes elements from
     *            the array
     */
  ...
  /**
     ...
     * Array.splice. Parameter `row` is the index of row in which do you want to
     * do splice. Parameter `index` is the column index at which to start
     * changing the array. If negative, will begin that many elements from the
     * end. Parameter `amount`, is the number of old array elements to remove.
     * If the amount is 0, no elements are removed. Fourth and further
     * parameters are the `elements` to add to the array. If you don't specify
     ...
     * @since 0.11
     * @param {Number}
     *            row Index of column in which do you want to do splice.
     * @param {Number}
     *            index Index at which to start changing the array. If negative,
     *            will begin that many elements from the end
     * @param {Number}
  ...
  /**
     * Return index of the currently selected cells as an array `[startRow,
  ...
  /**
     ...
     * 
     * @since 0.11
     * @returns {WalkontableCellRange} Returns `undefined` if there is no
     *          selection.
     */
  ...
  this.loadData = function (data) {
    if (typeof data === 'object' && data !== null) {
      if (!(data.push && data.splice)) { // check if data is array. Must use
                                            // duck-type check so Backbone
                                            // Collections also pass it
  ...
  /**
     * Return the current data object (the same that was passed by `data`
     * configuration option or `loadData` method). Optionally you can provide
     * cell range `row`, `col`, `row2`, `col2` to get only a fragment of grid
     * data.
     * 
     * @param {Number}
     *            [r] From row
  ...
  /**
     * Get value of selected range. Each column is separated by tab, each row is
  ...
  /**
     * Get schema provided by constructor settings or if it doesn't exist return
     * schema based on data structure on the first row.
     * 
     * @since 0.13.2
     * @returns {Object}
     */
...
// instance.rootElement.addClass(settings.className);
  ...
  /**
     ...
     * 
     * @param {String}
     *            action See grid.alter for possible values: `"insert_row"`,
     ...
     *            amount
     * @param {String}
     *            [source] Source of hook runner
     * @param {Boolean}
     *            [keepEmptyRows] Flag for preventing deletion of empty rows
     * 
     * @example Insert new row(s) above the row at given `index`. If index is
     *          `null` or `undefined`, the new row will be added after the
     *          current last row. Default `amount` equals 1. ```js var
     *          exampleContainer = document.getElementById('example'); var hot =
     *          new Handsontable(exampleContainer); hot.alter('insert_row', 10);
     *          ```
     * 
     * Insert new column(s) before the column at given `index`. If index is
     * `null` or `undefined`, the new column will be added after the current
     * last column. Default `amount` equals 1 ```js var exampleContainer =
     * document.getElementById('example'); var hot = new
     * Handsontable(exampleContainer); hot.alter('insert_col', 10); ```
     * 
     * Remove the row(s) at given `index`. Default `amount` equals 1 ```js var
     * exampleContainer = document.getElementById('example'); var hot = new
     * Handsontable(exampleContainer); hot.alter('remove_row', 10); ```
     * 
     * Remove the column(s) at given `index`. Default `amount` equals 1 ```js
     * var exampleContainer = document.getElementById('example'); var hot = new
     * Handsontable(exampleContainer); hot.alter('remove_col', 10); ```
     */
  ...
  /**
     ...
     * Returns `null` if the TD is not rendered on screen (probably because that
     * part of table is not visible).
     * 
     * @param {Number}
     *            row
     * @param {Number}
  ...
  /**
     ...
     * {@link Handsontable.DataMap#colToProp}
     * 
     * @param {Number}
     *            col Column index
     * @returns {String}
  ...
  /**
     ...
     * {@link Handsontable.DataMap#propToCol}
     * 
     * @param {String}
     *            prop
     * @returns {Number}
  ...
  /**
     * @description Return cell value at `row`, `col`. `row` and `col` are the
     *              __visible__ indexes (note that if columns were reordered or
     *              sorted, the current order will be used).
     * 
     * @param {Number}
     *            row
     * @param {Number}
  ...
  /**
     * @description Returns array of column values from the data source. `col`
     *              is the __visible__ index of the column.
     * 
     * @since 0.9-beta2
     * @param {Number}
     *            col
  ...
  /**
     * Given the object property name (e.g. `'first.name'`), returns array of
  ...
  /**
     * Returns array of column values from the data source. `col` is the index
     * of the row in the data source.
     * 
     * @since 0.11.0-beta3
     * @param {Number}
     *            col
  ...
  /**
     * @description Returns a single row of the data (array or object, depending
     *              on what you have). `row` is the __visible__ index of the
     *              row.
     * 
     * @param {Number}
     *            row
  ...
  /**
     * Remove `key` property object from cell meta data corresponding to params
     * `row`, `col`.
     * 
     * @param {Number}
     *            row
  ...
  this.getCellMeta = function (row, col) {
    ...
    cellProperties = priv.cellSettings[row][col]; // retrieve cellProperties
                                                    // from cache
    ...
    Handsontable.helper.extend(cellProperties, expandType(cellProperties)); // for
                                                                            // `type`
                                                                            // added
                                                                            // in
                                                                            // beforeGetCellMeta
    ...
    if (cellProperties.cells) {
      ...
      if (settings) {
        ...
        Handsontable.helper.extend(cellProperties, expandType(settings)); // for
                                                                            // `type`
                                                                            // added
                                                                            // in
                                                                            // cells
  ...
  /**
     * If displayed rows order is different than the order of rows stored in
     * memory (i.e. sorting is applied) we need to translate logical (stored)
     * row index to physical (displayed) index.
     * 
     * @param {Number}
  ...
  /**
     * If displayed columns order is different than the order of columns stored
     * in memory (i.e. column were moved using manualColumnMove plugin) we need
     * to translate logical (stored) column index to physical (displayed) index.
     * 
     * @private
     * @param {Number}
  ...
  /**
     * Validates all cells using their validator functions and calls callback
     * when finished. Does not render the view.
     * 
     * @param {Function}
     *            callback
     */
  ...
  /**
     * Returns array of row headers (if they are enabled). If param `row` given,
     * return header at given row as string.
     * 
     * @param {Number}
     *            [row]
  ...
  /**
     * Returns information of this table is configured to display column
     * headers.
     * 
     * @since 0.11
     * @returns {Boolean}
  ...
  this.hasColHeaders = function () {
    if (priv.settings.colHeaders !== void 0 && priv.settings.colHeaders !== null) { // Polymer
                                                                                    // has
                                                                                    // empty
                                                                                    // value
                                                                                    // =
  ...
  /**
     * Return array of column headers (if they are enabled). If param `col`
     * given, return header at given column as string
     * 
     * @param {Number}
     *            [col] Column index
     * @returns {Array|String}
  ...
  this._getRowHeightFromSettings= function (row) {
    /*
     * inefficient var cellProperties = instance.getCellMeta(0, row); var height =
     * cellProperties.height; if (height === void 0 || height ===
     * priv.settings.height) { height = cellProperties.rowHeights; }
     */
  ...
  /**
     * Return number of rendered rows (including rows partially or fully
  ...
  /**
     * Return number of visible rows (rendered rows that fully fit inside
     * viewport).
     * 
     * @returns {Number} Returns -1 if table is not visible
     */
  ...
  /**
     * Returns number of empty rows. If the optional ending parameter is 1,
     * returns number of empty rows at the bottom of the table.
     * 
     * @param {Boolean}
     *            [ending] If `true`, will only count empty rows at the end of
     *            the data source
  ...
  /**
     * Returns number of empty columns. If the optional `ending` parameter is
     * `true`, returns number of empty columns at right hand edge of the table.
     * 
     * @param {Boolean}
     *            [ending] If `true`, will only count empty columns at the end
     *            of the data source row
  ...
  /**
     ...
     * @param {Number}
     *            row Row index
     * @returns {Boolean} Return `true` if the row at the given `row` is empty,
     *          `false` otherwise.
     */
  ...
  /**
     ...
     * @param {Number}
     *            col Column index
     * @returns {Boolean} Return `true` if the column at the given `col` is
  ...
  /**
     * Select cell `row`, `col` or range finishing at `endRow`, `endCol`. By
     * default, viewport will be scrolled to selection.
     * 
     * @param {Number}
     *            row
     * @param {Number}
  ...
  this.selectCell = function (row, col, endRow, endCol, scrollToCell) {
    ...
    if (document.activeElement && document.activeElement !== document.documentElement && document.activeElement !== document.body) {
      document.activeElement.blur(); // needed or otherwise prepare won't
                                        // focus the cell. selectionSpec tests
                                        // this (should move focus to selected
                                        // cell)
    }
    instance.listen();
    if (typeof endRow === "undefined") {
  ...
  /**
     * Select cell `row`, `prop` or range finishing at `endRow`, `endProp`. By
     * default, viewport will be scrolled to selection.
     * 
     * @param {Number}
     *            row
  ...
  this.destroy = function () {
    ...
    if (instance.view) { // in case HT is destroyed before initialization has
                            // finished
    ...
    for (var i in instance) {
      if (instance.hasOwnProperty(i)) {
        ...
        // it should not be necessary but this prevents a memory leak side
        // effects that show itself in Jasmine tests
        else if (i !== "guid") {
          instance[i] = null;
        }
      }
    ...
    // it should not be necessary but this prevents a memory leak side effects
    // that show itself in Jasmine tests
    priv = null;
    datamap = null;
    grid = null;
    selection = null;
  ...
  /**
     ...
     * {@link Handsontable.EditorManager#getActiveEditor}
     * 
     * @returns {Object}
     */
  ...
  /**
     * Sets timeout. Purpose of this method is to clear all known timeouts when
 ...
 * 
 * Constructor options are applied using an object literal passed as a first
 * argument to the Handsontable constructor.
 * 
 * @example ```js var exampleContainer = document.getElementById('example1');
 *          var hot = new Handsontable(exampleContainer, { data: myArray, width:
 *          400, height: 300 }) ```
 * 
 * --- ## Cascading configuration
 * 
 * Handsontable 0.9 and newer is using *Cascading Configuration*, which is fast
 * way to provide configuration options for whole table, its columns and
 * particular cells.
 * 
 * Consider the following example: ```js var exampleContainer =
 * document.getElementById('example'); var hot = new
 * Handsontable(exampleContainer, { readOnly: true, columns: [ {readOnly:
 * false}, {}, {} ], cells: function (row, col, prop) { var cellProperties = {};
 * 
 * if (row === 0 && col === 0) { cellProperties.readOnly = true; }
 * 
 * return cellProperties; } }); ```
 * 
 * The above notation will result in all TDs being *read only*, except for first
 * column TDs which will be *editable*, except for the TD in top left corner
 * which will still be *read only*.
 * 
 ...
 * `handsontable(container, {option: "value"})` and `updateSettings` method.
 * 
 * ##### 2. Columns
 * 
 * Configuration options that are provided using second-level object
 * `handsontable(container, {columns: {option: "value"}]})`
 * 
 * ##### 3. Cells
 * 
 * Configuration options that are provided using second-level function
 * `handsontable(container, {cells: function: (row, col, prop){ }})`
 * 
 * --- ## Architecture performance
 * 
 * The Cascading Configuration model is based on prototypical inheritance. It is
 * much faster and memory efficient compared to the previous model that used
 * jQuery extend. See: http://jsperf.com/extending-settings.
 * 
 * --- __Important notice:__ In order for the data separation to work properly,
 * make sure that each instance of Handsontable has a unique `id`.
 */
var DefaultSettings = function () {};

DefaultSettings.prototype = {
  /**
     * @description Initial data source that will be bound to the data grid __by
     *              reference__ (editing data grid alters the data source). Can
     *              be Array of Array, Array of Objects or Function.
     * 
     * See [Understanding binding as
     * reference](http://handsontable.com/demo/understanding_reference.html).
     * 
     * @type {Array|Function}
     * @default undefined
     */
  ...
  /**
     * @description Defines the structure of a new row when data source is an
     *              object. Default like the first data row Array or Object.
     * 
     * See
     * [demo/datasources.html](http://handsontable.com/demo/datasources.html)
     * for examples.
     * 
     * @type {Object}
     * @default undefined
  ...
  /**
     ...
     * 
     * __Notice:__ This option only has effect in Handsontable constructor and
     * only if `data` option is not provided
     * 
     * @type {Number}
     * @default 5
     */
  ...
  /**
     * Setting `true` or `false` will enable or disable the default row headers
     * (1, 2, 3). You can also define an array `['One', 'Two', 'Three', ...]` or
     * a function to define the headers. If a function is set the index of the
     * row is passed as a parameter.
     * 
     * @type {Boolean|Array|Function}
     * @default null
     * @example ... // as boolean rowHeaders: true, ...
     * 
     * ... // as array rowHeaders: [1, 2, 3], ...
     * 
     * ... // as function rowHeaders: function(index) { return index + ': AB'; },
     * ...
     */
  rowHeaders: null,

  /**
     * Setting `true` or `false` will enable or disable the default column
     * headers (A, B, C). You can also define an array `['One', 'Two', 'Three',
     * ...]` or a function to define the headers. If a function is set the index
     * of the column is passed as a parameter.
     * 
     * @type {Boolean|Array|Function}
     * @default null
     * @example ... // as boolean colHeaders: true, ...
     * 
     * ... // as array colHeaders: ['A', 'B', 'C'], ...
     * 
     * ... // as function colHeaders: function(index) { return index + ': AB'; },
     * ...
     */
  colHeaders: null,

  /**
     * Defines column widths in pixels. Accepts number, string (that will be
     * converted to number), array of numbers (if you want to define column
     * width separately for each column) or a function (if you want to set
     * column width dynamically on each render).
     * 
  ...
  /**
     * @description Defines the cell properties and data binding for certain
     *              columns.
     * 
     * __Notice:__ Using this option sets a fixed number of columns (options
     * `startCols`, `minCols`, `maxCols` will be ignored).
     * 
     * See
     * [demo/datasources.html](http://handsontable.com/demo/datasources.html)
     * for examples.
     ...
     * @type {Array}
     * @default undefined
     * @example ... var exampleContainer = document.getElementById('example');
     *          var hot = new Handsontable(exampleContainer, { columns: [ { //
     *          column options for the first column type: 'numeric', format:
     *          '0,0.00 $' }, { // column options for the second column type:
     *          'text', readOnly: true } ] }); ...
     */
  columns: void 0,

  /**
     * @description Defines the cell properties for given `row`, `col`, `prop`
     *              coordinates. Any constructor or column option may be
     *              overwritten for a particular cell (row/column combination),
     *              using `cell` array passed to the Handsontable constructor.
     *              Or using `cells` function property to the Handsontable
     *              constructor.
     * 
     * @type {Function}
     * @default undefined
     * @example ```js ... var exampleContainer =
     *          document.getElementById('example'); var hot = new
     *          Handsontable(exampleContainer, { cells: function (row, col,
     *          prop) { var cellProperties = {};
     * 
     * if (row === 0 && col === 0) { cellProperties.readOnly = true; }
     * 
     * return cellProperties; } }); ... ```
  ...
  /**
     * Any constructor or column option may be overwritten for a particular cell
     * (row/column combination), using `cell` array passed to the Handsontable
     * constructor.
     * 
     * @type {Array}
     * @default []
     * @example ... var exampleContainer = document.getElementById('example');
     *          var hot = new Handsontable(exampleContainer, { cell: [ {row: 0,
     *          col: 0, readOnly: true} ] }); ...
     */
  cell: [],

  /**
     * @description If `true`, enables Comments plugin, which enables applying
     *              cell comments through the context menu (configurable with
     *              context menu keys commentsAddEdit, commentsRemove).
     * 
     * To initialize Handsontable with predefined comments, provide cell
     * coordinates and comment texts in form of an array.
     * 
     * See [Comments](http://handsontable.com/demo/comments.html) demo for
     * examples.
     * 
     * @since 0.11.0
     * @type {Boolean|Array}
     * @default false
     * @example ... var exampleContainer = document.getElementById('example');
     *          var hot = new Handsontable(exampleContainer, { comments: [{row:
     *          1, col: 1, comment: "Test comment"}] }); ...
     */
  comments: false,

  /**
     * @description If `true`, enables Custom Borders plugin, which enables
     *              applying custom borders through the context menu
     *              (configurable with context menu key borders).
     * 
     * To initialize Handsontable with predefined custom borders, provide cell
     * coordinates and border styles in form of an array.
     * 
     * See [Custom Borders](http://handsontable.com/demo/custom_borders.html)
     * demo for examples.
     * 
     * @since 0.11.0
     * @type {Boolean|Array}
     * @default false
     * @example ... var exampleContainer = document.getElementById('example');
     *          var hot = new Handsontable(exampleContainer, { customBorders:
     *          [{range: {from: {row: 1, col: 1}, to: {row: 3, col: 4}}, left:
     *          {}, right: {}, top: {}, bottom: {}}], }); ... // or ... var
     *          exampleContainer = document.getElementById('example'); var hot =
     *          new Handsontable(exampleContainer, { customBorders: [{row: 2,
     *          col: 2, left: {width: 2, color: 'red'}, right: {width: 1, color:
     *          'green'}, top: '', bottom: ''}], }); ...
     */
  customBorders: false,

  /**
     * Minimum number of rows. At least that many of rows will be created during
     * initialization.
     * 
     * @type {Number}
     * @default 0
  ...
  /**
     * Minimum number of columns. At least that many of columns will be created
  ...
  /**
     * When set to 1 (or more), Handsontable will add a new row at the end of
     * grid if there are no more empty rows.
     * 
     * @type {Number}
     * @default 0
     */
  ...
  /**
     * When set to 1 (or more), Handsontable will add a new column at the end of
     * grid if there are no more empty columns.
     * 
     * @type {Number}
     * @default 0
     */
  ...
  /**
     * Enables the fill handle (drag-down and copy-down) functionality, which
     * shows the small rectangle in bottom right corner of the selected area,
     * that let's you expand values to the adjacent cells.
     * 
     * Possible values: `true` (to enable in all directions), `"vertical"` or
     * `"horizontal"` (to enable in one direction), `false` (to disable
     * completely). Setting to `true` enables the fillHandle plugin.
     * 
     * @type {Boolean|String}
     * @default true
     */
  ...
  /**
     * Allows to specify the number of rows fixed (aka freezed) on the top of
     * the table.
     * 
     * @type {Number}
     * @default 0
  ...
  /**
     * Allows to specify the number of columns fixed (aka freezed) on the left
     * side of the table.
     * 
     * @type {Number}
     * @default 0
  ...
  /**
     * If `true`, mouse click outside the grid will deselect the current
     * selection.
     * 
     * @type {Boolean}
     * @default true
  ...
  /**
     * If `true`, <kbd>ENTER</kbd> begins editing mode (like Google Docs). If
     * `false`, <kbd>ENTER</kbd> moves to next row (like Excel) and adds new
     * row if necessary. <kbd>TAB</kbd> adds new column if necessary.
     * 
     * @type {Boolean}
     * @default true
     */
  ...
  /**
     * Defines cursor move after <kbd>ENTER</kbd> is pressed (<kbd>SHIFT</kbd> +
     * <kbd>ENTER</kbd> uses negative vector). Can be an object or a function
     * that returns an object. The event argument passed to the function is a
     * DOM Event object received after a <kbd>ENTER</kbd> key has been pressed.
     * This event object can be used to check whether user pressed <kbd>ENTER</kbd>
  ...
  /**
     * Defines cursor move after <kbd>TAB</kbd> is pressed (<kbd>SHIFT</kbd> +
     * <kbd>TAB</kbd> uses negative vector). Can be an object or a function
     * that returns an object. The event argument passed to the function is a
     * DOM Event object received after a <kbd>TAB</kbd> key has been pressed.
     * This event object can be used to check whether user pressed <kbd>TAB</kbd>
  ...
  /**
     * If `true`, pressing <kbd>TAB</kbd> or right arrow in the last column
     * will move to first column in next row
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * If `true`, pressing <kbd>ENTER</kbd> or down arrow in the last row will
     * move to first row in next column
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * Maximum number of rows than can be copied to clipboard using <kbd>CTRL</kbd> +
     * <kbd>C</kbd>.
     * 
     * @type {Number}
     * @default 1000
  ...
  /**
     * Maximum number of columns than can be copied to clipboard using <kbd>CTRL</kbd> +
     * <kbd>C</kbd>.
     * 
     * @type {Number}
     * @default 1000
  ...
  /**
     * Defines paste (<kbd>CTRL</kbd> + <kbd>V</kbd>) behavior. Default value
     * `"overwrite"` will paste clipboard value over current selection. When set
     * to `"shift_down"`, clipboard data will be pasted in place of current
     * selection, while all selected cells are moved down. When set to
     * `"shift_right"`, clipboard data will be pasted in place of current
  ...
  /**
     * @description Turn on saving the state of column sorting, columns
     *              positions and columns sizes in local storage.
     * 
     * You can save any sort of data in local storage in to preserve table state
     * between page reloads. In order to enable data storage mechanism,
     * `persistentState` option must be set to `true` (you can set it either
     * during Handsontable initialization or using the `updateSettings` method).
     * When `persistentState` is enabled it exposes 3 hooks:
     * 
     * __persistentStateSave__ (key: String, value: Mixed) * Saves value under
     * given key in browser local storage.
     * 
     * __persistentStateLoad__ (key: String, valuePlaceholder: Object) * Loads
     * `value`, saved under given key, form browser local storage. The loaded
     * `value` will be saved in `valuePlaceholder.value` (this is due to
     * specific behaviour of `PluginHooks.execute()` method). If no value have
     * been saved under key `valuePlaceholder.value` will be `undefined`.
     * 
     * __persistentStateReset__ (key: String) * Clears the value saved under
     * `key`. If no `key` is given, all values associated with table will be
     * cleared.
     * 
     * __Note:__ The main reason behind using `persistentState` hooks rather
     * than regular LocalStorage API is that it ensures separation of data
     * stored by multiple Handsontable instances. In other words, if you have
     * two (or more) instances of Handsontable on one page, data saved by one
     * instance won't be accessible by the second instance. Those two instances
     * can store data under the same key and no data would be overwritten.
     * 
     * __Important:__ In order for the data separation to work properly, make
     * sure that each instance of Handsontable has a unique `id`.
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * @description [Column
     *              stretching](http://handsontable.com/demo/scroll.html) mode.
     *              Possible values: `"none"`, `"last"`, `"all"`.
     * 
     * @type {String}
     * @default 'none'
     */
  ...
  /**
     * When set to `true`, the table is rerendered when it is detected that it
     * was made visible in DOM.
     * 
     * @type {Boolean}
     * @default true
     */
  ...
  /**
     * If set to `true`, cells will accept value that is marked as invalid by
     * cell `validator`, with a background color automatically applied using CSS
     * class `htInvalid`. If set to `false`, cells will not accept invalid
     * value.
     * 
     * @type {Boolean}
     * @default true
  ...
  /**
     * When set to an non-empty string, displayed as the cell content for empty
     * cells.
     * 
     * @type {Boolean|String}
     * @default false
  ...
  /**
     * String or rendering function. String may be one of the following
     * predefined values: `autocomplete`, `checkbox`, `text`, `numeric`.
     * Function will receive the following arguments: `function(instance, TD,
     * row, col, prop, value, cellProperties) {}` You can map your own function
     * to a string like this: `Handsontable.cellLookup.renderer.myRenderer =
     * myRenderer;`
     * 
  ...
  /**
     * Setting to `true` enables selecting just a fragment of the text within a
     * single cell or between adjacent cells.
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * @description Make cell [read
     *              only](http://handsontable.com/demo/readonly.html).
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * @description Setting to true enables the search plugin (see
     *              [demo](http://handsontable.com/demo/search.html)).
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * @description Shortcut to define combination of cell renderer and editor
     *              for the column.
     * 
  ...
  /**
     * @description Make cell copyable (pressing <kbd>CTRL</kbd> + <kbd>C</kbd>
     *              on your keyboard moves its value to system clipboard).
     * 
     * __Note:__ this setting is `false` by default for cells with type
     * `password`.
     * 
     * @type {Boolean}
     * @default true
  ...
  /**
     * String or rendering function. String may be one of the following
     * predefined values: `autocomplete`, `checkbox`, `text`, `date`,
     * `handsontable`. Function will receive the following arguments:
     * `function(instance, td, row, col, prop, value, cellProperties) {}` You
     * can map your own function to a string like this:
     * `Handsontable.cellLookup.renderer.myEditor = myEditor;`
     * 
     * @type {String|Function}
     * @default 'text'
     */
  ...
  /**
     * @description Autocomplete definitions. See
     *              [demo/autocomplete.html](http://handsontable.com/demo/autocomplete.html)
  ...
  /**
     * Setting to true enables the debug mode, currently used to test the
     * correctness of the row and column header fixed positioning on a layer
     * above the master table.
     * 
     * @type {Boolean}
     * @default false
     */
  ...
  /**
     * When set to `true`, the text of the cell content is wrapped if it does
     * not fit in the fixed column width.
     * 
     * @type {Boolean}
     * @default true
     * @since 0.11.0
  ...
  /**
     * @description Defines if the right-click context menu should be enabled.
     *              Context menu allows to create new row or column at any place
     *              in the grid. Possible values: `true` (to enable basic
     *              options), `false` (to disable completely) or array of any
     *              available strings: `["row_above", "row_below", "col_left",
     *              "col_right", "remove_row", "remove_col", "undo", "redo",
     *              "sep1", "sep2", "sep3"]`.
     * 
     * See
     * [demo/contextmenu.html](http://handsontable.com/demo/contextmenu.html)
     * for examples.
     * 
     * @type {Boolean|Array}
     * @default undefined
  ...
  /**
     * @description Turn on [Column
     *              sorting](http://handsontable.com/demo/sorting.html).
     * 
     * @type {Boolean}
     * @default undefined
     */
  ...
  /**
     * @description Turn on [Manual column
     *              move](http://handsontable.com/demo/column_move.html), if set
     *              to a boolean or define initial column order, if set to an
  ...
  /**
     * @description Turn on [Manual column
     *              resize](http://handsontable.com/demo/column_resize.html), if
     *              set to a boolean or define initial column resized widths, if
     *              set to an array of numbers.
     * 
     * @type {Boolean|Array}
     * @default undefined
     */
  ...
  /**
     * @description Turn on [Manual row
     *              move](http://handsontable.com/demo/column_move.html), if set
     *              to a boolean or define initial row order, if set to an array
     *              of row indexes.
     * 
     * @type {Boolean|Array}
     * @default undefined
  ...
  /**
     * @description Turn on [Manual row
     *              resize](http://handsontable.com/demo/column_resize.html), if
     *              set to a boolean or define initial row resized heights, if
     *              set to an array of numbers.
     * 
     * @type {Boolean|Array}
     * @default undefined
  ...
  /**
     * @description Setting to true or array enables the mergeCells plugin,
     *              which enables the merging of the cells. (see
     *              [demo](http://handsontable.com/demo/merge_cells.html)). You
     *              can provide the merged cells on the pageload if you feed the
     *              mergeCells option with an array.
     * 
     * @type {Boolean|Array}
     * @default false
     */
  ...
  /**
     * Number of rows to be prerendered before and after the viewport is
     * changed.
     * 
     * @type {Number}
     * @default 10
  ...
  /**
     * Number of columns to be prerendered before and after the viewport is
     * changed.
     * 
     * @type {Number}
     * @default 10
  ...
  /**
     * @description If `true`, enables Grouping plugin, which enables applying
     *              expandable row and column groups. To initialize Handsontable
     *              with predefined groups, provide row or column group start
     *              and end coordinates in form of an array.
     * 
     * See [Grouping](http://handsontable.com/demo/grouping.html) demo for
     * examples.
     * 
     * @type {Boolean|Array}
     * @default undefined
     * @since 0.11.4
     * @example ... // as boolean groups: true, ...
     * 
     * ... // as array groups: [{cols: [0, 2]}, {cols: [5, 15], rows: [0, 5]}],
     * ...
     */
  groups: void 0,
  ...
  /**
     ...
     * After you determine if the input is valid, execute `callback(true)` or
     * `callback(false)` to proceed with the execution. In function, `this`
     * binds to cellProperties.
     * 
  ...
  /**
     * @description Disable visual cells selection.
     * 
     * Possible values: `true` - Disables any type of visual selection (current
     * and area selection), `false` - Enables any type of visual selection. This
     * is default value. `current` - Disables to appear only current selected
     * cell. `area` - Disables to appear only multiple selected cells.
     * 
     * @type {Boolean|String|Array}
     * @default false
     * @since 0.13.2
     * @example ... // as boolean disableVisualSelection: true, ...
     * 
     * ... // as string ('current' or 'area') disableVisualSelection: 'current',
     * ...
     * 
     * ... // as array disableVisualSelection: ['current', 'area'], ...
     */
      ...
      'delete': function (key) {
        if (canBeAnArrayMapKey(key)) {
          ...
          map.weakMap['delete'](key);  // Delete must be called using square
                                        // bracket notation, because IE8 does
                                        // not handle using `delete` with dot
                                        // notation
        }
      }
    };
...
/**
 * DOM helper optimized for maximum performance It is recommended for
 * Handsontable plugins and renderers, because it is much faster than jQuery
 * 
 * @type {Object}
 */
if (!window.Handsontable) {
  var Handsontable = {}; // required because Walkontable test suite uses this
                            // class directly
}
Handsontable.Dom = {};
...
/**
 * Goes up the DOM tree (including given element) until it finds an element that
 * matches the nodes or nodes name. This method goes up through web components.
 * 
 * @param {HTMLElement}
 *            element Element from which traversing is started
 * @param {Array}
 *            nodes Array of elements or Array of elements name
 * @param {HTMLElement}
 *            [until]
 ...
 *            Child element
 * @param {Object|string}
 *            parent Parent element OR selector of the parent element. If
 *            classname provided, function returns true for the first occurance
 *            of element with that class.
 * @returns {boolean}
 */
...
/**
 * Counts index of element within its parent WARNING: for performance reasons,
 * assumes there are only element nodes (no text nodes). This is true for
 * Walkotnable Otherwise would need to check for nodeType or use
...
if (document.documentElement.classList) {
  ...
  /**
     ...
     *            element
     * @param {String|Array}
     *            className Class name as string or array of strings
     */
...
/**
 * Remove childs function WARNING - this doesn't unload events and data attached
 * by jQuery http://jsperf.com/jquery-html-vs-empty-vs-innerhtml/9
 * http://jsperf.com/jquery-html-vs-empty-vs-innerhtml/11 - no siginificant
 * improvement with Chrome remove() method
 * 
 * @param element
 * @returns {void}
 */
...
Handsontable.Dom.isVisible = function (elem) {
  ...
  while (Handsontable.Dom.polymerUnwrap(next) !== document.documentElement) { // until
                                                                                // <html>
                                                                                // reached
    if (next === null) { // parent detached from DOM
      ...
      if (next.host) { // this is Web Components Shadow DOM
        ...
        // according to spec, should be if (next.ownerDocument !==
        // window.document), but that doesn't work yet
        if (next.host.impl) { // Chrome 33.0.1723.0 canary (2013-11-29) Web
                                // Platform features disabled
          return Handsontable.Dom.isVisible(next.host.impl);
        }
        else if (next.host) { // Chrome 33.0.1723.0 canary (2013-11-29) Web
...
/**
 * Returns elements top and left offset relative to the document. Function is
 * not compatible with jQuery offset.
 * 
 * @param {HTMLElement}
 *            elem
...
Handsontable.Dom.offset = function (elem) {
  ...
  if (this.hasCaptionProblem() && elem.firstChild && elem.firstChild.nodeName === 'CAPTION') {
    // fixes problem with Firefox ignoring <caption> in TABLE offset (see also
    // Handsontable.Dom.outerHeight)
  ...
  if (lastElem && lastElem.style.position === 'fixed') {
    // if(lastElem !== document.body) { //faster but does gives false positive
    // in Firefox
...
Handsontable.eventManager = function (instance) {
  ...
  function extendEvent(event) {
    ...
    if (Handsontable.Dom.isWebComponentSupportedNatively()) {
      ...
      // Polymer doesn't support `event.target` property properly we must
  ...
  /**
     ...
     * @param {Function}
     *            callback
     * @returns {Function} Returns function which you can easily call to remove
     *          that event
     */
...
Handsontable.TableView = function (instance) {
  ...
  if(originalStyle) {
    instance.rootElement.setAttribute('data-originalstyle', originalStyle); // needed
                                                                            // to
                                                                            // retrieve
                                                                            // original
                                                                            // style
...
// instance.rootElement.addClass('handsontable');
  ...
  this.eventManager.addEventListener(instance.rootElement,'mousedown', function (event) {
    if (!that.isTextSelectionAllowed(event.target)) {
      ...
      window.focus(); // make sure that window that contains HOT is active.
                        // Important when HOT is in iframe.
    }
  });
  ...
  this.eventManager.addEventListener(document.documentElement, 'mouseup', function (event) {
    if (instance.selection.isInProgress() && event.which === 1) { // is left
                                                                    // mouse
                                                                    // button
      instance.selection.finish();
    }
  ...
  this.eventManager.addEventListener(document.documentElement, 'mousedown',function (event) {
    ...
    // immediate click on "spreader" means click on the right side of vertical
    // scrollbar
    if (next !== instance.view.wt.wtTable.holder) {
      while (next !== document.documentElement) {
        if (next === null) {
          ...
          // click on something that was a row but now is detached (possibly
            // because your click triggered a rerender)
          return;
        }
  ...
  this.eventManager.addEventListener(table, 'selectstart', function (event) {
    ...
    // selectstart is IE only event. Prevent text from being selected when
    // performing drag down in IE8
    event.preventDefault();
  });
  ...
  var walkontableConfig = {
    ...
    onCellMouseDown: function (event, coords, TD, wt) {
      ...
      if (!event.isImmediatePropagationStopped()) {
        ...
        if (event.button === 2 && instance.selection.inInSelection(coords)) { // right
                                                                                // mouse
                                                                                // button
          // do nothing
        }
    ...
    /*
     * onCellMouseOut: function (/*event, coords, TD* /) { if (isMouseDown &&
     * that.settings.fragmentSelection === 'single') { clearTextSelection();
     * //otherwise text selection blinks during multiple cells selection } },
     */
      ...
      if (coords.row >= 0 && coords.col >= 0) { // is not a header
        if (isMouseDown) {
          /*
             ...
             * clearTextSelection(); //otherwise text selection blinks during
  ...
  this.eventManager.addEventListener(that.wt.wtTable.spreader, 'mousedown', function (event) {
    if (event.target === that.wt.wtTable.spreader && event.which === 3) { // right
                                                                            // mouse
                                                                            // button
                                                                            // exactly
                                                                            // on
...
Handsontable.TableView.prototype.beforeRender = function (force) {
  if (force) { // force = did Walkontable decide to do full render
    Handsontable.hooks.run(this.instance, 'beforeRender', this.instance.forceFullRender); // this.instance.forceFullRender
                                                                                            // =
                                                                                            // did
                                                                                            // Handsontable
                                                                                            // request
...
Handsontable.TableView.prototype.onDraw = function (force) {
  if (force) { // force = did Walkontable decide to do full render
    Handsontable.hooks.run(this.instance, 'afterRender', this.instance.forceFullRender); // this.instance.forceFullRender
                                                                                            // =
                                                                                            // did
                                                                                            // Handsontable
                                                                                            // request
...
Handsontable.TableView.prototype.appendRowHeader = function (row, TH) {
  ...
  if (row > -1) {
    ...
    Handsontable.Dom.fastInnerText(SPAN, String.fromCharCode(160)); // workaround
                                                                    // for
                                                                    // https://github.com/handsontable/handsontable/issues/1946
  }
...
/**
 * Given a element's left position relative to the viewport, returns maximum
 * element width until the right edge of the viewport (before scrollbar)
 * 
 * @param {Number}
 *            leftOffset
 * @return {Number}
...
/**
 * Given a element's top position relative to the viewport, returns maximum
 * element height until the bottom edge of the viewport (before scrollbar)
 * 
 * @param {Number}
 *            topOffset
 * @return {Number}
...
/**
 * Utility to register editors and common namespace for keeping reference to all
 * editor classes
 */
...
(function(Handsontable){
  ...
  /**
     * @param {Object}
     *            instance Instance of Handsontable
     * @param {*}
     *            priv
     * @param {*}
     *            selection
  ...
  Handsontable.EditorManager = function(instance, priv, selection){
    ...
    function onKeyDown(event) {
      ...
      // catch CTRL but not right ALT (which in some systems triggers
        // ALT+CTRL)
      ...
      switch (event.keyCode) {
        ...
        case keyCodes.TAB:
          ...
          if (event.shiftKey) {
            selection.transformStart(-tabMoves.row, -tabMoves.col); // move
                                                                    // selection
                                                                    // left
          }
          else {
            selection.transformStart(tabMoves.row, tabMoves.col, true); // move
                                                                        // selection
                                                                        // right
                                                                        // (add
                                                                        // a new
        ...
        case keyCodes.F2: /* F2 */
          ...
          event.preventDefault(); // prevent Opera from opening Go to Page
                                    // dialog
          break;
...
/**
 * Utility to register renderers and common namespace for keeping reference to
 * all renderers classes
 */
(function (Handsontable) {
  'use strict';
...
/**
 * Checks if value of n is a numeric one http://jsperf.com/isnan-vs-isnumeric/4
 * 
 * @param n
 * @returns {boolean}
 */
...
/**
 * Generates a random hex string. Used as namespace for Handsontable instance
 * events.
 * 
...
/**
 * Inherit without without calling parent constructor, and setting
 * `Child.prototype.constructor` to `Child` instead of `Parent`. Creates
 * temporary dummy function to call it as constructor. Described in ticket:
 * https://github.com/handsontable/handsontable/pull/516
 * 
 * @param {Object}
 *            Child child class
 * @param {Object}
 ...
 * @param {Object}
 *            extension An object containing additional properties to merge into
 *            the target
 */
...
/**
 * Perform deep clone of an object WARNING! Only clones JSON properties. Will
 * cause error when `obj` contains a function, Date, etc
 * 
 * @param {Object}
 *            obj An object that will be cloned
 * @return {Object}
 */
...
/**
 * Determines if the given DOM element is an input field. Notice: By 'input' we
 * mean input, textarea and select nodes
 * 
 * @param element -
 *            DOM element
 * @returns {boolean}
...
/**
 * Determines whether given object is a plain Object. Note: String and Array are
 * not plain Objects
 * 
 * @param {*}
 *            obj
...
/**
 * Factory that produces a function for searching methods (or any properties)
 * which could be defined directly in table configuration or implicitly, within
 * cell type definition.
 * 
 * For example: renderer can be defined explicitly using "renderer" property in
 * column configuration or it can be defined implicitly using "type" property.
 * 
 * Methods/properties defined explicitly always takes precedence over those
 * defined through "type".
 * 
 * If the method/property is not found in an object, searching is continued
 * recursively through prototype chain, until it reaches the Object.prototype.
 * 
 * 
 * @param methodName
 *            {String} name of the method/property to search (i.e. 'renderer',
 *            'validator', 'copyable')
 * @param allowUndefined
 *            {Boolean} [optional] if false, the search is continued if
 *            methodName has not been found in cell "type"
 * @returns {Function}
 */
Handsontable.helper.cellMethodLookupFactory = function (methodName, allowUndefined) {
  ...
  return function cellMethodLookup (row, col) {
    ...
    return (function getMethodFromProperties(properties) {
      ...
      if (!properties){
      ...
      else if (properties.hasOwnProperty(methodName) && properties[methodName] !== void 0) { // check
                                                                                                // if
                                                                                                // it
                                                                                                // is
                                                                                                // own
      ...
      } else if (properties.hasOwnProperty('type') && properties.type) { // check
                                                                            // if
                                                                            // it
                                                                            // is
                                                                            // own
        ...
        if (type.hasOwnProperty(methodName)) {
          ...
          return; // method does not defined in type (eg. validator), returns
                    // undefined
        }
...
Handsontable.helper.isMobileBrowser = function (userAgent) {
  ...
  //
  /*
     * var type = type != void 0 ? type.toLowerCase() : '' , result;
     * switch(type) { case '': result =
     * (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera
     * Mini/i.test(navigator.userAgent)); return result; break; case 'ipad':
     * return navigator.userAgent.indexOf('iPad') > -1; break; case 'android':
     * return navigator.userAgent.indexOf('Android') > -1; break; case
     * 'windows': return navigator.userAgent.indexOf('IEMobile') > -1; break;
     * default: throw new Error('Invalid isMobileBrowser argument'); break; }
     */
};
...
(function (Handsontable) {
  ...
  /**
     * Utility class that gets and saves data from/to the data source using
     * mapping of columns numbers to object property names
     * 
     * @todo refactor arguments of methods getRange, getText to be numbers (not
     *       objects)
  ...
  /**
     ...
     * 
     * @param {Number}
     *            [index] Index of the row before which the new row will be
     *            inserted
  ...
  /**
     ...
     * 
     * @param {Number}
     *            [index] Index of the column before which the new column will
     *            be inserted
     * @param {Number}
     *            [amount]
  ...
  /**
     ...
     * 
     * @param {Number}
     *            [index] Index of the row to be removed. If not provided, the
     *            last row will be removed
     * @param {Number}
     *            [amount] Amount of the rows to be removed. If not provided,
  ...
  Handsontable.DataMap.prototype.removeRow = function (index, amount) {
    ...
    // We have to map the physical row ids to logical and than perform removing
    // with (possibly) new row id
  ...
  /**
     ...
     * 
     * @param {Number}
     *            [index] Index of the column to be removed. If not provided,
     *            the last column will be removed
     * @param {Number}
     *            [amount] Amount of the columns to be removed. If not provided,
     *            one column will be removed
     */
  ...
  /**
     ...
     */
  Handsontable.DataMap.prototype.spliceCol = function (col, index, amount/*
                                                                             * ,
                                                                             * elements...
                                                                             */) {
  ...
  /**
     ...
     * 
     * @param {Number}
     *            row Index of row in which do you want to do splice
     * @param {Number}
     *            index Index at which to start changing the array. If negative,
     *            will begin that many elements from the end
     * @param {Number}
     ...
     */
  Handsontable.DataMap.prototype.spliceRow = function (row, index, amount/*
                                                                             * ,
                                                                             * elements...
                                                                             */) {
    var elements = 4 <= arguments.length ? [].slice.call(arguments, 3) : [];
  ...
  Handsontable.DataMap.prototype.get = function (row, prop) {
    ...
    if (typeof prop === 'string' && prop.indexOf('.') > -1) {
      ...
      /**
         * allows for interacting with complex structures, for example d3/jQuery
         * getter/setter properties:
         * 
         * {columns: [{ data: function(row, value){ if(arguments.length === 1){
         * return row.property(); } row.property(value); } }]}
         */
  ...
  /**
     * Returns single value from the data array (intended for clipboard copy to
  ...
  /**
     * This ridiculous piece of code maps rows Id that are present in table data
     * to those displayed for user. The trick is, the physical row id (stored in
     * settings.data) is not necessary the same as the logical (displayed) row
  ...
  /**
     ...
     *            [start] Start selection position
     * @param {Object}
     *            [end] End selection position
     * @param {Number}
     *            destination Destination of datamap.get
     * @returns {Array}
     */
  ...
  /**
     * Return data as copyable text (tab separated columns intended for
     * clipboard copy to an external application).
     * 
     * @param {Object}
     *            [start] Start selection position
     * @param {Object}
 ...
 * @param {Element}
 *            TD Table cell where to render
 * @param {Number}
 *            row
 * @param {Number}
 *            col
 * @param {String|Number}
 *            prop Row object property name
 * @param value
 *            Value to render (remember to escape unsafe HTML before inserting
 *            to DOM!)
 * @param {Object}
 *            cellProperties Cell properties (shared by cell renderer and
 *            editor)
 */
(function (Handsontable) {
  'use strict';
  ...
  var TextRenderer = function (instance, TD, row, col, prop, value, cellProperties) {
    ...
    if (cellProperties.rendererTemplate) {
      ...
      Handsontable.Dom.fastInnerText(TD, escaped); // this is faster than
                                                    // innerHTML. See:
  ...
  // Handsontable.TextRenderer = TextRenderer; //Left for backward
    // compatibility
...
(function (Handsontable) {
  ...
  clonableARROW.appendChild(document.createTextNode(String.fromCharCode(9660))); // workaround
                                                                                    // for
                                                                                    // https://github.com/handsontable/handsontable/issues/1946
// this is faster than innerHTML. See:
// https://github.com/handsontable/handsontable/wiki/JavaScript-&-DOM-performance-tips
  ...
  /**
     ...
     *            prop Row object property name
     * @param value
     *            Value to render (remember to escape unsafe HTML before
     *            inserting to DOM!)
     * @param {Object}
     *            cellProperties Cell properites (shared by cell renderer and
     *            editor)
     */
  var AutocompleteRenderer = function (instance, TD, row, col, prop, value, cellProperties) {

    var WRAPPER = clonableWRAPPER.cloneNode(true); // this is faster than
                                                    // createElement
    var ARROW = clonableARROW.cloneNode(true); // this is faster than
                                                // createElement
    ...
    if (!TD.firstChild) { // http://jsperf.com/empty-node-if-needed
      ...
      TD.appendChild(document.createTextNode(String.fromCharCode(160))); // workaround
                                                                            // for
                                                                            // https://github.com/handsontable/handsontable/issues/1946
      // this is faster than innerHTML. See:
        // https://github.com/handsontable/handsontable/wiki/JavaScript-&-DOM-performance-tips
...
(function (Handsontable) {
  ...
  var CheckboxRenderer = function (instance, TD, row, col, prop, value, cellProperties) {
    ...
    Handsontable.Dom.empty(TD); // TODO identify under what circumstances this
                                // line can be removed
    ...
    if (value === cellProperties.checkedTemplate || value === Handsontable.helper.stringify(cellProperties.checkedTemplate)) {
      ...
      Handsontable.Dom.fastInnerText(TD, '#bad value#'); // this is faster
                                                            // than innerHTML.
                                                            // See:
                                                            // https://github.com/handsontable/handsontable/wiki/JavaScript-&-DOM-performance-tips
    }
    ...
    if (cellProperties.readOnly) {
      ...
      eventManager.addEventListener(INPUT,'mousedown',function (event) {
        Handsontable.helper.stopPropagation(event);
        // event.stopPropagation(); //otherwise can confuse cell mousedown
        // handler
      });

      eventManager.addEventListener(INPUT,'mouseup',function (event) {
        Handsontable.helper.stopPropagation(event);
        // event.stopPropagation(); //otherwise can confuse cell dblclick
        // handler
      });
...
(function (Handsontable) {
  ...
  var NumericRenderer = function (instance, TD, row, col, prop, value, cellProperties) {
    if (Handsontable.helper.isNumeric(value)) {
      ...
      value = numeral(value).format(cellProperties.format || '0'); // docs:
                                                                    // http://numeraljs.com/
  ...
  Handsontable.NumericRenderer = NumericRenderer; // Left for backward
                                                    // compatibility with
                                                    // versions prior 0.10.0
...
(function (Handsontable) {
  ...
  BaseEditor.prototype.saveValue = function (val, ctrlDown) {
    ...
    // if ctrl+enter and multiple cells selected, behave like Excel (finish
    // editing and apply to all cells)
    if (ctrlDown) {
  ...
  BaseEditor.prototype.beginEditing = function(initialValue, event){
    ...
    // only rerender the selections (FillHandle should disappear when
    // beginediting is triggered)
    this.instance.view.render();
  };
...
(function(Handsontable){
  ...
  var onBeforeKeyDown =  function onBeforeKeyDown(event){
    ...
    keyCodes = Handsontable.helper.keyCode;
    ctrlDown = (event.ctrlKey || event.metaKey) && !event.altKey; // catch
                                                                    // CTRL but
                                                                    // not right
                                                                    // ALT
                                                                    // (which in
    ...
    if (event.keyCode === 17 || event.keyCode === 224 || event.keyCode === 91 || event.keyCode === 93) {
      // when CTRL or its equivalent is pressed and cell is edited, don't
        // prepare selectable text in textarea
      event.stopImmediatePropagation();
      return;
    }
    ...
    switch (event.keyCode) {
      ...
      case keyCodes.ENTER:
        ...
        if ((ctrlDown && !isMultipleSelection) || event.altKey) { // if
                                                                    // ctrl+enter
                                                                    // or
                                                                    // alt+enter,
                                                                    // add new
      ...
      case keyCodes.V:
        if(ctrlDown){
          event.stopImmediatePropagation(); // CTRL+A, CTRL+C, CTRL+V, CTRL+X
                                            // should only work locally when
                                            // cell is edited (not in table
                                            // context)
        }
        break;
      ...
      case keyCodes.HOME:
      case keyCodes.END:
        event.stopImmediatePropagation(); // backspace, delete, home, end
                                            // should only work locally when
                                            // cell is edited (not in table
                                            // context)
        break;
  ...
  TextEditor.prototype.open = function(){
    this.refreshDimensions(); // need it instantly, to prevent
                                // https://github.com/handsontable/handsontable/issues/348
  ...
  TextEditor.prototype.close = function(){
    ...
    if (document.activeElement === this.TEXTAREA) {
      this.instance.listen(); // don't refocus the table if user focused some
                                // cell outside of HT on purpose
    }
...
// this.TD = this.instance.getCell(this.row, this.col);
    ...
    if (!this.TD) {
      // TD is outside of the viewport. Otherwise throws exception when
        // scrolling the table while a cell is edited
      return;
    }
    // var $td = $(this.TD); //because old td may have been scrolled out with
    // scrollViewport
    ...
    // TODO: Refactor this to the new instance.getCell method (from #ply-59),
    // after 0.12.1 is released
    switch(editorSection) {
      case 'top':
    ...
    var width = Handsontable.Dom.innerWidth(this.TD) - 8  // $td.width()
      , maxWidth = this.instance.view.maximumVisibleElementWidth(cellLeftOffset) - 10 // 10
                                                                                        // is
                                                                                        // TEXTAREAs
                                                                                        // padding

      , height = this.TD.scrollHeight + 1
      , maxHeight = this.instance.view.maximumVisibleElementHeight(cellTopOffset) - 2; // 10
                                                                                        // is
                                                                                        // TEXTAREAs
                                                                                        // border
                                                                                        // and
    ...
    this.autoResize.init(this.TEXTAREA, {
      ...
      maxHeight: maxHeight, // TEXTAREA should never be wider than visible part
                            // of the viewport (should not cover the scrollbar)
      minWidth: Math.min(width, maxWidth),
      maxWidth: maxWidth // TEXTAREA should never be wider than visible part
                            // of the viewport (should not cover the scrollbar)
    }, true);
...
(function (Handsontable) {
  ...
  // this.beginningEditorWidth =
    // Handsontable.Dom.outerWidth(this.editorContainer);
  ...
  // //if(currentScaleRatio > this.scaleRatio + 0.2 || currentScaleRatio <
    // this.scaleRatio - 0.2) {
  // if(currentScaleRatio != this.scaleRatio) {
  // this.editorContainer.style["zoom"] = (1 - ((currentScaleRatio *
    // this.scaleRatio) - this.scaleRatio)) * 100 + "%";
  // }
  //
  // };
...
(function (Handsontable) {
  ...
  HandsontableEditor.prototype.prepare = function (td, row, col, prop, value, cellProperties) {
    ...
    var options = {
      ...
      afterOnCellMouseDown: function () {
        ...
        if (value !== void 0) { // if the value is undefined then it means we
                                // don't want to set the value
  ...
  var onBeforeKeyDown = function (event) {
    ...
    var innerHOT = editor.htEditor.getInstance(); // Handsontable.tmpHandsontable(editor.htContainer,
                                                    // 'getInstance');
  ...
  HandsontableEditor.prototype.finishEditing = function (isCancelled, ctrlDown) {
    if (this.htEditor.isListening()) { // if focus is still in the HOT editor
      ...
      // if (Handsontable.tmpHandsontable(this.htContainer,'isListening')) {
        // //if focus is still in the HOT editor
    // if (this.$htContainer.handsontable('isListening')) { //if focus is still
    // in the HOT editor
    ...
    // var value = this.$htContainer.handsontable('getInstance').getValue();
      ...
      // var value =
        // Handsontable.tmpHandsontable(this.htContainer,'getInstance').getValue();
      if (value !== void 0) { // if the value is undefined then it means we
                                // don't want to set the value
        this.setValue(value);
      }
...
(function (Handsontable) {
  ...
  SelectEditor.prototype.prepare = function(){
    ...
    for (var option in keys){
      if (options.hasOwnProperty(keys[option])){
        var optionElement = document.createElement('OPTION');
        optionElement.value = keys[option];
        Handsontable.Dom.fastInnerHTML(optionElement, options[keys[option]]);
...
console.log(preparedOptions)
  ...
  SelectEditor.prototype.open = function () {
    var width = Handsontable.Dom.outerWidth(this.TD); // important - group
                                                        // layout reads together
                                                        // for better
                                                        // performance
 ...
 * 
 * @param {*}
 *            value - Value of edited cell
 * @param {*}
 *            callback - Callback called with validation result
 */
...
Handsontable.DateValidator = function (value, callback) {
  ...
  var isValidDate = moment(new Date(value)).isValid(), // is value a valid date
                                                        // string
    isValidFormat = moment(value, this.dateFormat || dateEditor.defaultDateFormat, true).isValid(); // is
                                                                                                    // it
                                                                                                    // in
                                                                                                    // the
                                                                                                    // specified
 ...
 *            value - Value of edited cell
 * @param {*}
 *            calback - Callback called with validation result
 */
...
/**
 * Cell type is just a shortcut for setting bunch of cellProperties (used in
 * getCellMeta)
 */

Handsontable.mobileBrowser = Handsontable.helper.isMobileBrowser();  // check
                                                                        // if
                                                                        // viewed
                                                                        // on a
                                                                        // mobile
...
Handsontable.DateCell = {
  ...
  renderer: Handsontable.renderers.AutocompleteRenderer // displays small gray
                                                        // arrow on right side
                                                        // of the cell
};
...
Handsontable.DropdownCell = {
  ...
  renderer: Handsontable.renderers.AutocompleteRenderer, // displays small
                                                            // gray arrow on
                                                            // right side of the
                                                            // cell
  validator: Handsontable.AutocompleteValidator
...
// here setup the friendly aliases that are used by cellProperties.renderer and
...
/**
 * autoResize - resizes a DOM element to the width and height of another DOM
 * element
 * 
 * Copyright 2014, Marcin Warpechowski Licensed under the MIT license
 */
      ...
      for (r = 0, rLen = rows.length; r < rLen; r += 1) {
        ...
        for (c = 0, cLen = rows[r].length; c < cLen; c += 1) {
          ...
          if (multiline && c === 0) {
            ...
            if (multiline && (countQuotes(rows[r][0]) & 1)) { // & 1 is a
                                                                // bitwise way
                                                                // of performing
                                                                // mod 2
              multiline = false;
    ...
    stringify: function (arr) {
      ...
      for (r = 0, rLen = arr.length; r < rLen; r += 1) {
        ...
        for (c = 0; c < cLen; c += 1) {
          ...
          else if (val === null || val === void 0) { // void 0 resolves to
                                                        // undefined
            str += '';
          }
          else {
...
/**
 * Creates a textarea that stays hidden on the page and gets focused when user
 * presses CTRL while not having a form input focused. In future we may
 * implement a better driver when better APIs are available.
 * 
 * @constructor
 * @private
 */
...
CopyPasteClass.prototype.init = function () {
  ...
  if (document.getElementById('CopyPasteDiv')) {
    ...
    this.elTextarea.onpaste = function(event) {
      if ('WebkitAppearance' in document.documentElement.style) { // chrome
                                                                    // and
                                                                    // safari
...
CopyPasteClass.prototype.onKeyDown = function(event) {
  ...
  if (isCtrlDown) {
    // this is needed by fragmentSelection in Handsontable. Ignore copypaste.js
    // behavior if fragment of cell text is selected
...
(function (jsonpatch) {
    ...
    function observe(obj, callback) {
        ...
        if (Object.observe) {
            observer = function (arr) {
                // This "refresh" is needed to begin observing new object
                // properties
    ...
    // Dirty check if obj is different from mirror, generate patches and update
    // mirror
...
/**
 * @description Handsontable events are the common interface that function in 2
 *              ways: as __callbacks__ and as __plugin hooks__.
 * 
 * @constructor Handsontable.Hooks
 * @example // __Using events as callbacks:__ ...
 *          $('div#example1').handsontable({ afterChange: function(changes,
 *          source) { $.ajax({ url: "save.php", data: change }); } }); ... //
 *          __Using events as plugin hooks:__ ... $('#example1').handsontable({
 *          'myPlugin': true });
 * 
 * $('#example2').handsontable({ 'myPlugin': false }); // global hook
 * Handsontable.PluginHooks.add('afterChange', function() {
 * if(this.getSettings().myPlugin) { // function body - will only run in
 * #example1 } }); // local hook (has same effect as a callback)
...
Handsontable.PluginHookClass = (function () {
  ...
  var Hooks = function () {
    return {
      ...
      /**
         * @description Callback fired before Handsontable instance is
         *              initiated.
         * 
      ...
      /**
         ...
         * @param {Boolean}
         *            isForced If true rendering was triggered by a change of
         *            settings or data; or false if rendering was triggered by
         *            scrolling or moving selection.
         */
      beforeRender: [],
      ...
      /**
         ...
         * @param {Array}
         *            coords WalkontableCellCoords array.
         */
      beforeSetRangeEnd: [],
      ...
      /**
         * Callback fired before one or more cells is changed. Its main purpose
         * is to alter changes silently before input.
         * 
         * @example // To disregard a single change, set changes[i] to null or
         *          remove it from array using changes.splice(i, 1). ...
         *          $('div#example1').handsontable({ beforeChange:
         *          function(changes, source) { // [[row, prop, oldVal, newVal],
         *          ...] changes[0] = null; } }); ... // To alter a single
         *          change, overwrite the desired value to changes[i][3]. ...
         *          $('div#example1').handsontable({ beforeChange:
         *          function(changes, source) { // [[row, prop, oldVal, newVal],
         *          ...] changes[0][1] = 10; } }); ... // To cancel all edit,
         *          return false from the callback or set array length to 0
         *          (changes.length = 0). ... $('div#example1').handsontable({
         *          beforeChange: function(changes, source) { // [[row, prop,
         *          oldVal, newVal], ...] return false; } }); ...
         * 
         * @event Handsontable.Hooks#beforeChange
         * @param {Array}
         *            changes 2D array containing information about each of the
         *            edited cells.
         * @param {String}
         *            source The name of a source of changes.
         */
      beforeChange: [],
      ...
      /**
         ...
         * @param {Number}
         *            index Index of starter column.
         * @param {Number}
         *            amount Amount of columns to be removed.
         */
      beforeRemoveCol: [],
      ...
      /**
         * @description A plugin hook executed before validator function, only
         *              if validator function is defined. This can be used to
         *              manipulate value of changed cell before it is applied to
         *              the validator function.
         * 
         * __Notice:__ this will not affect values of changes. This will change
         * value ONLY for validation!
         * 
      ...
      /**
         ...
         * @param {Object}
         *            start Object containing information about first filled
         *            cell: `{row: 2, col: 0}`
         * @param {Object}
         *            end Object containing information about last filled cell:
         *            `{row: 4, col: 1}`
         * @param {Array}
         *            data 2D array containing information about fill pattern:
         *            `[["1", "Ted"], ["1", "John"]]`
         */
      beforeAutofill: [],

      /**
         * Callback fired before keydown event is handled. It can be used to
         * overwrite default key bindings. Caution - in your `beforeKeyDown`
         * handler you need to call `event.stopImmediatePropagation()` to
      ...
      /**
         * Callback fired after new data is loaded (by `loadData` method) into
      ...
      /**
         ...
         * @param {Boolean}
         *            isForced Is `true` if rendering was triggered by a change
         *            of settings or data; or `false` if rendering was triggered
         *            by scrolling or moving selection.
         */
      afterRender : [],
      ...
      /**
         * @description Callback fired after one or more cells is changed. Its
         *              main use case is to save the input.
         * 
         * __Note:__ For performance reasons, the `changes` array is null for
         * `"loadData"` source.
         * 
         * @event Handsontable.Hooks#afterChange
         * @param {Array}
         *            changes 2D array containing information about each of the
         *            edited cells `[[row, prop, oldVal, newVal], ...]`
         * @param {String}
         *            source Is one of the strings: `"alter", "empty", "edit",
         *            "populateFromArray", "loadData", "autofill", "paste"`.
         */
      afterChange : [],

      /**
         * @description A plugin hook executed after validator function, only if
         *              validator function is defined. Validation result is the
         *              first parameter. This can be used to determinate if
         *              validation passed successfully or not.
         * 
      ...
      /**
         ...
         * @param {Number}
         *            index Represents the index of first newly created row in
         *            the data source array.
         * @param {Number}
         *            amount Number of newly created rows in the data source
         *            array.
         */
      afterCreateRow: [],
      ...
      /**
         ...
         * @param {Number}
         *            index Represents the index of first newly created column
         *            in the data source array.
         * @param {Number}
         *            amount Number of newly created columns in the data source
         *            array.
         */
      afterCreateCol: [],
      ...
      /**
         * Callback fired while one or more cells are being selected (on mouse
         * move).
         * 
      ...
      /**
         * The same as above, but data source object property name is used
         ...
         *            r Selection start row
         * @param {String}
         *            p Selection start data source object property
         * @param {Number}
         *            r2 Selection end row
         * @param {String}
         *            p2 Selection end data source object property
         */
      ...
      /**
         * Callback fired after clicking on a cell or row/column header. In case
         * the row/column header was clicked, the index is negative. For example
         * clicking on the row header of cell (0, 0) results with
         * `afterOnCellMouseDown` called with coords `{row: 0, col: -1}`.
         * 
      ...
      /**
         * Callback fired after hovering a cell or row/column header with the
         * mouse cursor. In case the row/column header was hovered, the index is
         * negative. For example clicking on the row header of cell (0, 0)
         * results with `afterOnCellMouseOver` called with coords `{row: 0, col:
         * -1}`.
         * 
  ...
  /**
     * Get hook bucket based on Handsontable instance or if instance is
     * `undefined` get global hook bucked.
     * 
  ...
  PluginHookClass.prototype.add = function (key, fn, instance) {
    ...
    if (Array.isArray(fn)) {
      ...
      if (bucket[key].indexOf(fn) == -1) {
        bucket[key].push(fn); // only add a hook if it has not already been
                                // added (adding the same hook twice is now
                                // silently ignored)
      }
    }
    return this;
  ...
  PluginHookClass.prototype._runBucket = function (bucket, instance, key, p1, p2, p3, p4, p5, p6) {
    ...
    // http://jsperf.com/call-vs-apply-for-a-plugin-architecture
    if (handlers) {
  ...
  /**
     * Registers a hook name (adds it to the list of the known hook names). Used
     * by plugins. It is not neccessary to call, register, but if you use it,
     * your plugin hook will be used returned by getRegistered (which itself is
     * used in the demo http://handsontable.com/demo/callbacks.html)
     * 
     * @memberof Handsontable.Hooks#
     * @function register
     * @private
...
Handsontable.PluginHooks = Handsontable.hooks; // in future move this line to
                                                // legacy.js
...
function HandsontableColumnSorting() {
  ...
  this.sort = function () {
    ...
    instance.sortingEnabled = false; // this is required by translateRow
                                        // plugin hook
    ...
    instance.sortingEnabled = true; // this is required by translateRow plugin
                                    // hook
  };
...
(function (Handsontable) {
  ...
  function ContextMenu(instance, customOptions) {
    ...
    this.markSelected = function (label) {
      return "<span class='selected'>" + String.fromCharCode(10003) + "</span>" + label; // workaround
                                                                                            // for
                                                                                            // https://github.com/handsontable/handsontable/issues/1946
    };
  ...
  ContextMenu.prototype.bindMouseEvents = function () {
    ...
    function contextMenuOpenListener(event) {
      ...
      // if event is from hot-table we must get web component element not
        // element inside him
  ...
  ContextMenu.prototype.show = function (menu, items) {
    ...
    // ContextMenu is not detected HotTableEnv correctly because is injected
    // outside hot-table
 ...
 * 
 * Has 2 UI components: - handle - the draggable element that sets the desired
 * position of the column - guide - the helper guide that shows the desired
 * position as a vertical guide
 * 
 * Warning! Whenever you make a change in this file, make an analogous change in
 * manualRowMove.js
 * 
 * @constructor
 */
...
function HandsontableManualColumnMove() {
  ...
  function setupHandlePosition(TH) {
    ...
    var col = this.view.wt.wtTable.getCoords(TH).col; // getCoords returns
  ...
  this.afterRemoveCol = function (index, amount) {
    ...
    var rmindx,
      ...
      // We have removed columns, we also need to remove the indicies from
        // manual column array
      rmindx = colpos.splice(index, amount);

      // We need to remap manualColPositions so it remains constant linear
        // from 0->ncols
    ...
    this.afterCreateCol = function (index, amount) {
      ...
      if (index >= colpos.length) {
        ...
        // We have added columns, we also need to add new indicies to
        // manualcolumn position array
 ...
 * 
 * Has 2 UI components: - handle - the draggable element that sets the desired
 * width of the column - guide - the helper guide that shows the desired width
 * as a vertical guide
 * 
 * Warning! Whenever you make a change in this file, make an analogous change in
 * manualRowResize.js
...
(function (Handsontable) {
  function HandsontableManualColumnResize() {
    ...
    var bindEvents = function () {
      ...
      eventManager.addEventListener(window, 'mousemove', function (e) {
        if (pressed) {
          ...
          newSize = setManualSize(currentCol, currentWidth); // save col
                                                                // width
    ...
    var setManualSize = function (col, width) {
      ...
      /**
         * We need to run col through modifyCol hook, in case the order of
         * displayed columns is different than the order in data source. For
         * instance, this order can be modified by manualColumnMove plugin.
         */
      col = Handsontable.hooks.run(instance, 'modifyCol', col);
 ...
 * 
 * Has 2 UI components: - handle - the draggable element that sets the desired
 * height of the row - guide - the helper guide that shows the desired height as
 * a horizontal guide
 * 
 * Warning! Whenever you make a change in this file, make an analogous change in
 * manualRowResize.js
...
(function (Handsontable) {
  function HandsontableManualRowResize() {
    ...
    function setupHandlePosition(TH) {
      ...
      var row = this.view.wt.wtTable.getCoords(TH).row; // getCoords returns
                                                        // WalkontableCellCoords
      ...
      eventManager.addEventListener(instance.rootElement, 'mousedown', function (e) {
        if (Handsontable.Dom.hasClass(e.target, 'manualRowResizer')) {
          ...
          if (autoresizeTimeout == null) {
            autoresizeTimeout = setTimeout(function () {
              if (dblclick >= 2) {
                setManualSize(currentRow, null); // double click sets auto
                                                    // row size
                instance.forceFullRender = true;
                instance.view.render(); // updates all
...
(function HandsontableObserveChanges() {
  ...
  function runHookForOperation(rawPatches){
    ...
    /**
     * Removing or adding column will produce one patch for each table row. This
     * function leaves only one patch for each column add/remove operation
     */
    ...
    /**
     * If observeChanges uses native Object.observe method, then it produces
     * patches for length property. This function removes them.
     */
...
(function(Handsontable){
  ...
  /**
     ...
     * 
     * @return {Boolean} Return `true` if undo can be performed, `false`
     *         otherwise
     */
  ...
  /**
     ...
     * 
     * @return {Boolean} Return `true` if redo can be performed, `false`
     *         otherwise.
     */
  ...
  Handsontable.UndoRedo.CreateRowAction.prototype.undo = function (instance, undoneCallback) {
    ...
    if(this.index >= rowCount && this.index - minSpareRows < rowCount) {
      this.index -= minSpareRows; // work around the situation where the
                                    // needed row was removed due to an 'undo'
                                    // of a made change
    }
...
(function(Handsontable){
  ...
  function init(){
    ...
    if(pluginEnabled){
      if(!instance.undoRedo){
        /**
         * Instance of Handsontable.UndoRedo Plugin
         * {@link Handsontable.UndoRedo}
         * 
         * @alias undoRedo
         * @memberof! Handsontable.Core#
         * @type {UndoRedo}
  ...
  function onBeforeKeyDown(event){
    ...
    if(ctrlDown){
      if (event.keyCode === 89 || (event.shiftKey && event.keyCode === 90)) { // CTRL
                                                                                // + Y
                                                                                // or
                                                                                // CTRL
                                                                                // +
...
/**
 * Plugin used to scroll Handsontable by selecting a cell and dragging outside
 * of visible viewport
 * 
 * @constructor
 * @private
...
MergeCells.prototype.mergeRange = function (cellRange) {
  ...
  mergeParent.rowspan = bottomRight.row - topLeft.row + 1; // TD has rowspan ==
                                                            // 1 by default.
                                                            // rowspan == 2
                                                            // means spread over
                                                            // 2 cells
...
MergeCells.prototype.modifyTransform = function (hook, currentSelectedRange, delta) {
  ...
  if (hook == 'modifyTransformStart') {
    ...
    var currentPosition = new WalkontableCellCoords(currentSelectedRange.highlight.row, currentSelectedRange.highlight.col)
      , mergedParent = this.mergedCellInfoCollection.getInfo(currentPosition.row, currentPosition.col)// if
                                                                                                        // current
                                                                                                        // position's
                                                                                                        // parent
                                                                                                        // is a
    ...
    if (mergedParent) { // only merge selected
      ...
      if (!mergeRange.includes(this.lastDesiredCoords)) {
        this.lastDesiredCoords = new WalkontableCellCoords(null, null); // reset
                                                                        // outdated
                                                                        // version
                                                                        // of
                                                                        // lastDesiredCoords
    ...
    if (nextParentIsMerged) { // skipping the invisible cells in the merge
                                // range
    ...
    for (var i = 0, mergesLength = this.mergedCellInfoCollection.length; i < mergesLength; i++) {
      ...
      if (mergedRange.isEqual(currentSelectedRange)) { // only the merged range
                                                        // is selected
        ...
        if (sharedBorders.indexOf('top') > -1) { // if range shares a border
                                                    // with the merged section,
                                                    // change range direction
                                                    // accordingly
      ...
      if (currentSelectedRange.includesRange(mergedRange) && (mergedRange.includes(nextPosition) || withinRowspan || withinColspan)) { // if
                                                                                                                                        // next
                                                                                                                                        // step
                                                                                                                                        // overlaps
                                                                                                                                        // a
...
var afterInit = function () {
  ...
  if (instance.mergeCells) {
    /**
     * Monkey patch WalkontableTable.prototype.getCell to return TD for merged
     * cell parent if asked for TD of a cell that is invisible due to the merge.
     * This is not the cleanest solution but there is a test case for it (merged
     * cells scroll) so feel free to refactor it!
     */
...
/**
 * While selecting cells with keyboard or mouse, make sure that rectangular area
 * is expanded to the extent of the merged cell
 * 
 * @param coords
 */
var beforeSetRangeEnd = function (coords) {

  this.lastDesiredCoords = null; // unset lastDesiredCoords when selection is
                                    // changed with mouse
  var mergeCellsSetting = this.getSettings().mergeCells;
  if (mergeCellsSetting) {
    var selRange = this.getSelectedRange();
    selRange.highlight = new WalkontableCellCoords(selRange.highlight.row, selRange.highlight.col); // clone
                                                                                                    // in
                                                                                                    // case
                                                                                                    // we
                                                                                                    // will
...
/**
 * Returns correct coordinates for merged start / end cells in selection for
 * area borders
 * 
 * @param corners
 * @param className
    ...
    for (var c = 0; c < colCount; c++) {
      ...
      if (mergeParent) {
        if (mergeParent.row < calc.startRow) {
          ...
          return afterViewportRowCalculatorOverride.call(this, calc); // recursively
                                                                        // search
                                                                        // upwards
        }
      }
    ...
    for (var r = 0; r < rowCount; r++) {
      ...
      if (mergeParent) {
        if (mergeParent.col < calc.startColumn) {
          ...
          return afterViewportColumnCalculatorOverride.call(this, calc); // recursively
                                                                            // search
                                                                            // upwards
        }
      }
...
// var bordersArray = {},
  ...
  /***************************************************************************
     ...
     * @param row
     * @param col
     * @returns {{className: *, border: *, row: *, col: *, top: {hide: boolean},
     *          right: {hide: boolean}, bottom: {hide: boolean}, left: {hide:
     *          boolean}}}
     */
 ...
 * 
 * Has 2 UI components: - handle - the draggable element that sets the desired
 * position of the row - guide - the helper guide that shows the desired
 * position as a horizontal guide
 * 
 * Warning! Whenever you make a change in this file, make an analogous change in
 * manualRowMove.js
 * 
...
/**
 * This plugin provides "drag-down" and "copy-down" functionalities, both
 * operated using the small square in the right bottom of the cell selection.
 * 
 * "Drag-down" expands the value of the selected cells to the neighbouring cells
 * when you drag the small square in the corner.
 * 
 * "Copy-down" copies the value of the selection to all empty cells below when
...
(function (Handsontable) {
  ...
  /**
     * Selects cells down to the last row in the left column, then fills down to
     * that cell
     * 
...
var Grouping = function (instance) {
  ...
  /**
     * group definition
     * 
     * @type {{id: String, level: Number, rows: Array, cols: Array, hidden:
     *       Number}}
     */
  var item = {
    id: '',
  ...
  /**
     ...
     * 
     * @param {Array}
     *            cells [{row:0, col:0}, {row:0, col:1}, {row:1, col:2}]
     * @returns {Array}
     */
  ...
  /**
     * Check if the next cell of the dimension (row / column) contains a group
     * at the same level
     * 
     * @param dimension
  ...
  /**
     * Check if the previous cell of the dimension (row / column) contains a
  ...
  /**
     * Check if the provided index is at the beginning of the group indicator
     * line
     * 
     * @param dimension
     * @param index
  ...
  var isCollapsed = function (currentPosition) {
    ...
    if (currentPosition.col === null) { // if col is set to null, check only
                                        // rows
      return false;
    }
  ...
  return {
     ...
     * 
     * @param {Object}
     *            settings, could be an array of objects [{cols: [0,1,2]},
     *            {cols: [3,4,5]}, {rows: [0,1]}]
     */
    init: function () {
...
// this.render();
    ...
    /**
     * Create a renderer for additional row/col headers, acting as group
     * indicators
     * 
    ...
    groupIndicatorsFactory: function (renderersArr, direction) {
      ...
      var makeGroupIndicatorsForLevel = function () {
        var directionClassname = direction.charAt(0).toUpperCase() + direction.slice(1); // capitalize
                                                                                            // the
                                                                                            // first
                                                                                            // letter
        ...
        return function (index, elem, level) { // header rendering function
          ...
          if (index > -1 && (groupsLevelsList[index] && groupsLevelsList[index].indexOf(level) > -1)) {
            ...
            if (isFirstIndexOfTheLine(dataType, index, level, currentGroupId)) { // add
                                                                                    // a
                                                                                    // little
                                                                                    // thingy
                                                                                    // and
            ...
            if (isLastIndexOfTheLine(dataType, index, level, currentGroupId)) { // add
                                                                                // [+]/[-]
                                                                                // button
                                                                                // at
                                                                                // the
      ...
      if (counters[dataType] > 0) {
        for (var i = 0; i < levels[dataType] + 1; i++) { // for each level of
                                                            // col groups add a
                                                            // header renderer
    ...
    toggleGroupVisibility: function (event, coords, TD) {
      if (Handsontable.Dom.hasClass(event.target, classes.expandButton) ||
        ...
        if (element.className.indexOf(classes.levelTrigger) > -1) { // show
                                                                    // levels
                                                                    // below,
                                                                    // hide all
                                                                    // above
        ...
        // add the expander button to a dummy spare row/col, if no longer needed
        // -> remove it
        /* jshint -W038 */
        type = type || levelType;
    ...
    modifySelectionFactory: function (position) {
      ...
      var currentlySelected
        ...
        , nextVisible = function (direction, currentPosition) { // updates delta
                                                                // to skip to
                                                                // the next
                                                                // visible cell
          var updateDelta = 0;
...
var init = function () {
  ...
  if (groupingSetting) {
    ...
    if (!instance.getSettings().rowHeaders) { // force using rowHeaders --
                                                // needs to be changed later
      headerUpdates.rowHeaders = true;
    }
    if (!instance.getSettings().colHeaders) { // force using colHeaders --
                                                // needs to be changed later
...
(function (Handsontable) {
  ...
  ContextMenuCopyPaste.prototype.setupZeroClipboard = function (cmInstance) {
    ...
    for (var i = 0, ilen = data.length; i < ilen; i++) { // find position of
                                                            // 'copy' option
      /* jshint -W083 */
      if (data[i].key === 'copy') {
  ...
  ContextMenuCopyPaste.prototype.bindEvents = function () {
    ...
    // Workaround for 'current' and 'zeroclipboard-is-hover' classes being stuck
    // when moving the cursor over the context menu
  ...
  /**
     * Initialize plugin
     * 
     * @returns {boolean} Returns false if ZeroClipboard is not properly
     *          included
     */
...
(function (Handsontable) {
  function ManualColumnFreeze(instance) {
    ...
    /**
     * Modifies the default Context Menu entry list to consist 'freeze/unfreeze
     * this column' entries
     * 
     * @param {Object}
     *            defaultOptions
    ...
    /**
     * Checks whether 'manualColumnPositions' array needs creating and/or
     * initializing
     * 
     * @param {Number}
     *            [col]
     ...
     *            col
     * @param {Number}
     *            actualCol column index of the currently selected cell
     * @param {Number|null}
     *            returnCol suggested return slot for the unfreezed column (can
     *            be null)
     * @param {String}
    ...
    /**
     * Unfreeze the given column (remove it from fixed columns and bring to it's
     * previous position)
     * 
     * @param {Number}
     *            col
    ...
    function onModifyCol(col) {
      if (this.manualColumnPositionsPluginUsages.length > 1) { // if another
                                                                // plugin is
                                                                // using
...
/**
 * Creates an overlay over the original Walkontable instance. The overlay
 * renders the clone of the original Walkontable and (optionally) implements
 * behavior needed for native horizontal and vertical scrolling
 */
function WalkontableOverlay() {}

/*
 * Possible optimizations: [x] don't rerender if scroll delta is smaller than
 * the fragment outside of the viewport [ ] move .style.top change before
 * .draw() [ ] put .draw() in requestAnimationFrame [ ] don't rerender rows that
 * remain visible after the scroll
 */
...
/**
 * WalkontableCellCoords holds cell coordinates (row, column) and few metiod to
 * validate them and retrieve as an array or an object TODO: change interface to
 * WalkontableCellCoords(row, col) everywhere, remove those unnecessary setter
...
/**
 * Returns boolean information if given set of coordinates is valid in context
 * of a given Walkontable instance
 * 
 * @param instance
 * @returns {boolean}
 */
...
/**
 * Returns boolean information if this cell coords are the same as cell coords
...
/**
 * A cell range is a set of exactly two WalkontableCellCoords (that can be the
 * same or different)
 */

function WalkontableCellRange(highlight, from, to) {
  this.highlight = highlight; // this property is used to draw bold border
                                // around a cell where selection was started and
                                // to edit the cell when you press Enter
  this.from = from; // this property is usually the same as highlight, but in
                    // Excel there is distinction - one can change highlight
                    // within a selection
  this.to = to;
}
...
/**
 * Returns boolean information if given cell coords is within `from` and `to`
 * cell coords of this range
 * 
 * @param {WalkontableCellCoords}
 *            cellCoords
 * @returns {boolean}
...
/**
 * Returns true if tested range overlaps with the range. Range A is considered
 * to to be overlapping with range B if intersection of A and B or B and A is
 * not empty.
 * 
 * @param testedRange
 * @returns {boolean}
...
/**
 * Adds a cell to a range (only if exceeds corners of the range). Returns
 * information if range was expanded
 * 
 * @param {WalkontableCellCoords}
 *            cellCoords
 * @returns {boolean}
...
/**
 * Runs a callback function against all cells in the range. You can break the
 * iteration by returning false in the callback function
 * 
 * @param callback
 *            {Function}
 */
...
function WalkontableColumnStrategy(instance, containerSizeFn, sizeAtIndex, strategy) {
  ...
  // negative value means the last cell is fully visible and there is some
    // space left for stretching
...
WalkontableColumnStrategy.prototype.stretch = function () {
  ...
  if (this.strategy === 'all') {
    if (this.remainingSize < 0) {
      ...
      while (i < this.cellCount - 1) { // "i < this.cellCount - 1" is needed
                                        // because last cellSize is adjusted
                                        // after the loop
    ...
    if (this.remainingSize < 0 && containerSize !== Infinity) { // Infinity is
                                                                // with native
                                                                // scroll when
                                                                // the table is
                                                                // wider than
...
function Walkontable(settings) {
  ...
  this.guid = 'wt_' + walkontableRandomString(); // this is the namespace for
                                                    // global events
 ...
 * 
 * @param fastDraw
 *            {Boolean} When TRUE, try to refresh only the positions of borders
 *            without rerendering the data. It will only work if
 *            WalkontableTable.draw() does not force rendering anyway
 * @returns {Walkontable}
 */
Walkontable.prototype.draw = function (fastDraw) {
  this.drawInterrupted = false;
  if (!fastDraw && !Handsontable.Dom.isVisible(this.wtTable.TABLE)) {
    this.drawInterrupted = true; // draw interrupted because TABLE is not
                                    // visible
    return;
  }
 ...
 * topmost overlay layer, if not set or set to false, returns TD from the master
 * table.
 * 
 * @param {WalkontableCellCoords}
 *            coords
...
Walkontable.prototype.getSetting = function (key, param1, param2, param3, param4) {
  return this.wtSettings.getSetting(key, param1, param2, param3, param4); // this
                                                                            // is
                                                                            // faster
                                                                            // than
                                                                            // .apply
...
/**
 * A overlay that renders ALL available rows & columns positioned on top of the
 * original Walkontable instance and all other overlays. Used for debugging
 * purposes to see if the other overlays (that render only part of the rows &
...
function WalkontableDebugOverlay(instance) {
  ...
  /*
     * var that = this; var lastX = 0; var lastY = 0; var overlayContainer =
     * that.clone.wtTable.holder.parentNode;
     * 
     * var eventManager = Handsontable.eventManager(instance);
     * 
     * eventManager.addEventListener(document.body, 'mousemove', function
     * (event) { if (!that.instance.wtTable.holder.parentNode) { return;
     * //removed from DOM } if ((event.clientX - lastX > -5 && event.clientX -
     * lastX < 5) && (event.clientY - lastY > -5 && event.clientY - lastY < 5)) {
     * return; //ignore minor mouse movement } lastX = event.clientX; lastY =
     * event.clientY; Handsontable.Dom.addClass(overlayContainer,
     * 'wtDebugHidden'); Handsontable.Dom.removeClass(overlayContainer,
     * 'wtDebugVisible'); clearTimeout(this.lastTimeout); this.lastTimeout =
     * setTimeout(function () { Handsontable.Dom.removeClass(overlayContainer,
     * 'wtDebugHidden'); Handsontable.Dom.addClass(overlayContainer,
     * 'wtDebugVisible'); }, 1000); });
     */
}
...
function WalkontableEvent(instance) {
  ...
  // /**
  // * Update touch event target - if user taps on resize handle 'hit area',
    // update the target to the cell itself
  // * @param event
  // */
  /*
     * var adjustTapTarget = function (event) { var currentSelection ,
     * properTarget;
     * 
     * if(Handsontable.Dom.hasClass(event.target,'SelectionHandle')) {
     * if(that.instance.selections[0].cellRange) { currentSelection =
     * that.instance.selections[0].cellRange.highlight;
     * 
     * properTarget = that.instance.getCell(currentSelection, true); } }
     * 
     * if(properTarget) { Object.defineProperty(event,'target',{ value:
     * properTarget }); }
     * 
     * return event; };
     */
  ...
  var onTouchStart = function (event) {
    ...
    // },200);
      ...
      // Prevent cell selection when scrolling with touch event - not the best
    ...
    // eventManager.removeEventListener(that.instance.wtTable.holder,
  ...
  /*
     * var lastMouseOut; var onMouseOut = function (event) { if
     * (that.instance.hasSetting('onCellMouseOut')) { var TABLE =
     * that.instance.wtTable.TABLE; var TD =
     * Handsontable.Dom.closest(event.target, ['TD', 'TH'], TABLE); if (TD && TD
     * !== lastMouseOut && Handsontable.Dom.isChildOf(TD, TABLE)) { lastMouseOut =
     * TD; if (TD.nodeName === 'TD') {
     * that.instance.getSetting('onCellMouseOut', event,
     * that.instance.wtTable.getCoords(TD), TD); } } } };
     */
  ...
  var onTouchEnd = function (event) {
    ...
    // eventManager.removeEventListener(that.instance.wtTable.holder, "mouseup",
    // onMouseUp);
  };
...
WalkontableEvent.prototype.parentCell = function (elem) {
  ...
  if (TD && Handsontable.Dom.isChildOf(TD, TABLE)) {
    ...
    cell.coords = this.instance.selections.current.cellRange.highlight; // selections.current
                                                                        // is
                                                                        // current
                                                                        // selected
                                                                        // cell
    ...
    if (this.instance.selections.area.cellRange) {
      cell.coords = this.instance.selections.area.cellRange.to; // selections.area
                                                                // is area
                                                                // selected
                                                                // cells
      cell.TD = this.instance.wtTable.getCell(cell.coords);
...
/**
 * Generates a random hex string. Used as namespace for Walkontable instance
 * events.
 * 
 * @return {String} - 16 character random string: "92b1bfc74ec4"
 */
...
WalkontableOverlays.prototype.syncScrollPositions = function (e, fakeScrollValue) {
  ...
  if (target === master || target === document) {
    ...
    // if scrolling the master table - populate the scroll values to both top
    // and left overlays
    ...
    // if scrolling the top overlay - populate the horizontal scroll to the
    // master table
    ...
    // if scrolling the left overlay - populate the vertical scroll to the
    // master table
...
WalkontableHorizontalScrollbarNative.prototype.resetFixedPosition = function () {
  ...
  elem.style.width = Handsontable.Dom.outerWidth(this.clone.wtTable.TABLE) + 4 + 'px';// + 4
                                                                                        // +
                                                                                        // 'px';
};
...
WalkontableHorizontalScrollbarNative.prototype.applyToDOM = function () {
  ...
  this.fixedContainer.style.width = headerSize + this.sumCellSizes(0, total) + 'px';// + 4
                                                                                    // +
                                                                                    // 'px';
 ...
 * @param sourceCol
 *            {Number}
 * @param beyondRendered
 *            {Boolean} if TRUE, scrolls according to the bottom edge (top edge
 *            is by default)
 */
...
/**
 * Each Walkontable clone requires it's own border for every selection. This
 * method creates and returns selection borders per instance
 * 
 * @param {Walkontable}
 *            instance
...
/**
 * If selection range from or to property equals oldCoords, replace it with
 * newCoords. Return boolean information about success
 * 
 * @param {WalkontableCellCoords}
 *            oldCoords
 * @param {WalkontableCellCoords}
...
function WalkontableSettings(instance, settings) {
  ...
  this.defaults = {
    ...
    columnWidth: function (col) {
      return; // return undefined means use default size for the rendered cell
                // content
    },
...
WalkontableSettings.prototype.getSetting = function (key, param1, param2, param3, param4) {
  if (typeof this.settings[key] === 'function') {
    return this.settings[key](param1, param2, param3, param4); // this is
                                                                // faster than
                                                                // .apply -
                                                                // https://github.com/handsontable/handsontable/wiki/JavaScript-&-DOM-performance-tips
  }
  else if (param1 !== void 0 && Array.isArray(this.settings[key])) { // perhaps
                                                                        // this
                                                                        // can
                                                                        // be
                                                                        // removed,
...
function WalkontableTable(instance, table) {
  ...
  if (!parent || parent.nodeType !== 1 || !Handsontable.Dom.hasClass(parent, 'wtHolder')) {
    ...
    if (parent) {
      parent.insertBefore(spreader, this.TABLE); // if TABLE is detached
                                                    // (e.g. in Jasmine test),
                                                    // it has no parentNode so
                                                    // we cannot attach holder
                                                    // to it
    }
  ...
  if (!parent || parent.nodeType !== 1 || !Handsontable.Dom.hasClass(parent, 'wtHolder')) {
    ...
    if (parent) {
      parent.insertBefore(hider, this.spreader); // if TABLE is detached
                                                    // (e.g. in Jasmine test),
                                                    // it has no parentNode so
                                                    // we cannot attach holder
                                                    // to it
  ...
  if (!parent || parent.nodeType !== 1 || !Handsontable.Dom.hasClass(parent, 'wtHolder')) {
    ...
    if (parent) {
      parent.insertBefore(holder, this.hider); // if TABLE is detached (e.g. in
                                                // Jasmine test), it has no
                                                // parentNode so we cannot
                                                // attach holder to it
    }
 ...
 * 
 * @param fastDraw
 *            {Boolean} If TRUE, will try to avoid full redraw and only update
 *            the border positions. If FALSE or UNDEFINED, will perform a full
 *            redraw
...
WalkontableTable.prototype.draw = function (fastDraw) {
  ...
  if (!fastDraw) {
    ...
    if (!this.isWorkingOnClone()) {
      // in case we only scrolled without redraw, update visible rows
        // information in oldRowsCalculator
 ...
 * @param {WalkontableCellCoords}
 *            coords
 * @returns {Object} HTMLElement on success or {Number} one of the exit codes on
 *          error: -1 row before viewport -2 row after viewport
 */
 ...
 * @param row
 * @returns {Object} HTMLElement on success or {Number} one of the exit codes on
 *          error: null table doesn't have row headers
 */
...
/**
 * Checks if any of the row's cells content exceeds its initial height, and if
 * so, returns the oversized height
 * 
 * @param {Number}
 *            sourceRow
 * @returns {Number}
...
WalkontableTableRenderer.prototype.render = function () {
  ...
  if (!this.wtTable.isWorkingOnClone()) {
    ...
    if (workspaceWidth !== this.instance.wtViewport.getWorkspaceWidth()) {
      // workspace width changed though to shown/hidden vertical scrollbar.
        // Let's reapply stretching
      this.instance.wtViewport.containerWidth = null;
...
WalkontableTableRenderer.prototype.renderRows = function (totalRows, rowsToRender, columnsToRender) {
  ...
  while (sourceRowIndex < totalRows && sourceRowIndex >= 0) {
    ...
    if (TR.firstChild) {
      // if I have 2 fixed columns with one-line content and the 3rd column
        // has a multiline content, this is the way to make sure that the
        // overlay will has same row height
...
WalkontableTableRenderer.prototype.resetOversizedRow = function (sourceRow) {
  if (this.instance.wtViewport.oversizedRows && this.instance.wtViewport.oversizedRows[sourceRow]) {
    this.instance.wtViewport.oversizedRows[sourceRow] = void 0;  // void 0 is
                                                                    // faster
                                                                    // than
                                                                    // delete,
                                                                    // see
...
/**
 * Check if any of the rendered rows is higher than expected, and if so, cache
 * them
 */
...
/**
 * Check if column header for the specified column is higher than expected, and
 * if so, cache it
 * 
 * @param col
...
WalkontableTableRenderer.prototype.renderCells = function (sourceRowIndex, TR, columnsToRender) {
  ...
  for (var visibleColIndex = 0; visibleColIndex < columnsToRender; visibleColIndex++) {
    ...
    // If the number of headers has been reduced, we need to replace excess TH
    // with TD
    if (TD.nodeName == 'TH') {
...
WalkontableTableRenderer.prototype.renderRowHeaders = function (row, TR) {
  for (var TH = TR.firstChild, visibleColIndex = 0; visibleColIndex < this.rowHeaderCount; visibleColIndex++) {
    ...
    // If the number of row headers increased we need to create TH or replace an
    // existing TD node with TH
    if (!TH) {
...
/**
 * Adjust the number of COL and TH elements to match the number of columns and
 * headers that need to be rendered
 */
...
/**
 * Adjusts the number of COL elements to match the number of columns that need
 * to be rendered
 */
...
/**
 * Adjusts the number of TH elements in THEAD to match the number of headers and
 * columns that need to be rendered
 */
 ...
 * 
 * @param TR
 *            {HTMLElement} Table row in question
 * @param desiredCount
 *            {Number} The desired number of TDs in the TR
 */
...
WalkontableViewport.prototype.getWorkspaceHeight = function () {
  ...
  if (trimmingContainer === window) {
    ...
    var height = (elemHeight > 0 && trimmingContainer.clientHeight > 0) ? trimmingContainer.clientHeight : Infinity; // returns
                                                                                                                        // height
                                                                                                                        // without
                                                                                                                        // DIV
                                                                                                                        // scrollbar
...
WalkontableViewport.prototype.getWorkspaceWidth = function () {
  ...
  if (trimmingContainer === window && totalColumns > 0 && this.sumColumnWidths(0, totalColumns - 1) > width) {
    // in case sum of column widths is higher than available stylesheet width,
    // let's assume using the whole window
...
WalkontableViewport.prototype.getContainerFillWidth = function() {
  ...
  // while(mainContainer.parentNode != document.body &&
    // mainContainer.parentNode != null &&
    // mainContainer.className.indexOf('handsontable') === -1) {
...
WalkontableViewport.prototype.getWorkspaceActualWidth = function () {
  return Handsontable.Dom.outerWidth(this.instance.wtTable.TABLE) ||
    ...
    Handsontable.Dom.outerWidth(this.instance.wtTable.THEAD); // IE8 reports 0
                                                                // as <table>
                                                                // offsetWidth;
};
    ...
    if (rowHeaders.length) {
      ...
      for (var i = 0, ilen = rowHeaders.length; i < ilen; i++) {
        if (TH) {
          ...
          this.rowHeaderWidth += 50; // yes this is a cheat but it worked
                                        // like that before, just taking
                                        // assumption from CSS instead of
                                        // measuring. TODO: proper fix
        }
      }
    }
    else {
...
/**
 * Creates: - rowsRenderCalculator (before draw, to qualify rows for rendering) -
 * rowsVisibleCalculator (after draw, to measure which rows are actually
 * visible)
 * 
...
/**
 * Creates: - columnsRenderCalculator (before draw, to qualify columns for
 * rendering) - columnsVisibleCalculator (after draw, to measure which columns
 ...
 * determine what rows and cols should be rendered)
 * 
 * @param fastDraw
 *            {Boolean} If TRUE, will try to avoid full redraw and only update
 *            the border positions. If FALSE or UNDEFINED, will perform a full
...
WalkontableViewport.prototype.createRenderCalculators = function (fastDraw) {
  ...
  this.rowsVisibleCalculator = null; // delete temporarily to make sure that
                                        // renderers always use
                                        // rowsRenderCalculator, not
...
/**
 * Creates rowsVisibleCalculator and columnsVisibleCalculator (after draw, to
 * determine what are the actually visible rows and columns)
 */
...
/**
 * Returns information whether proposedRowsVisibleCalculator viewport is
 * contained inside rows rendered in previous draw (cached in
 * rowsRenderCalculator)
 * 
 * Returns TRUE if all proposed visible rows are already rendered (meaning:
 * redraw is not needed) Returns FALSE if at least one proposed visible row is
 * not already rendered (meaning: redraw is needed)
 * 
 * @returns {Boolean}
 */
...
/**
 * Returns information whether proposedColumnsVisibleCalculator viewport is
 * contained inside column rendered in previous draw (cached in
 * columnsRenderCalculator)
 * 
 * Returns TRUE if all proposed visible columns are already rendered (meaning:
 * redraw is not needed) Returns FALSE if at least one proposed visible column
 * is not already rendered (meaning: redraw is needed)
 * 
 * @returns {Boolean}
 */
...
function WalkontableViewportColumnsCalculator (width, scrollOffset, totalColumns, columnWidthFn, overrideFn, onlyFullyVisible, stretchH) {
  ...
  /**
     * Get stretched column width based on stretchH (all or last) setting passed
     * in handsontable instance.
     * 
     * @param {Number}
     *            column
     * @param {Number}
...
/**
 * Viewport calculator constructor. Calculates indexes of rows to render OR rows
 * that are visible. To redo the calculation, you need to create a new
 * calculator.
 * 
 * Object properties: this.scrollOffset - position of vertical scroll (in px)
 * this.startRow - index of the first rendered/visible row (can be overwritten
 * using overrideFn) this.startPosition - position of the first rendered/visible
 * row (in px) this.endRow - index of the last rendered/visible row (can be
 * overwritten using overrideFn) this.count - number of rendered/visible rows
 * 
 * @param height -
 ...
 * @param rowHeightFn -
 *            function that returns the height of the row at a given index (in
 *            px)
 * @param overrideFn -
 *            function that changes calculated this.startRow, this.endRow (used
 *            by mergeCells.js plugin)
 * @param onlyFullyVisible
 *            {bool} - if TRUE, only startRow and endRow will be indexes of rows
 *            that are FULLY in viewport
 * @constructor
 * @private
 */
function WalkontableViewportRowsCalculator(height, scrollOffset, totalRows, rowHeightFn, overrideFn, onlyFullyVisible) {
  ...
  // If the estimation has reached the last row and there is still some space
...
WalkontableTopOverlay.prototype.sumCellSizes = function (from, length) {
  var sum = 0;
  while (from < length) {
    sum += this.instance.wtTable.getRowHeight(from) || this.instance.wtSettings.settings.defaultRowHeight; // TODO
                                                                                                            // optimize
                                                                                                            // getSetting,
                                                                                                            // because
                                                                                                            // this
...
/*
 * ! numeral.js version : 1.5.3 author : Adam Draper license : MIT
...
(function() {
    ...
    numeral.validate = function(val, culture) {
        ...
        try {
            // check if the culture is understood by numeral. if not, default it
            // to current language
