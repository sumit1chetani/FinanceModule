app.controller('profitAndLossJobSalesPersonwiseController', function($scope, $window, $rootScope, $location, $http, $filter, logger, $log, ngDialog, $modal, utilsService, $state, $timeout,$stateParams,
    ...
    $http.get($stateParams.tenantid+'/app/airquotation/getEmployeeValue?empId=' + $scope.empId).success(function(datas) {
  ...
  /*  $scope.creditCategorylist = [ {
    ...
    $scope.onSearch = function(joborder,profitLossForm) {
    	...
    	if (new validationService().checkFormValidity(profitLossForm)) {
            ...
            $http.post($stateParams.tenantid+"/app/joborderreportsalespersonwise/getReport", joborder).success(function(response) {
...
/*    // excel
    ...
    $scope.excel = function(joborder) {
        ...
        $http.post($stateParams.tenantid+"/app/joborderreportsalespersonwise/generateExcel", objWholeData).success(function(response) {
