//define([ 'payroll/payroll/payroll' ], function(module) {
    'use strict';
    //module.registerController('employeeFinalSettlementCtrl', function($scope, $state, $http, ngDialog, logger, $location, $controller, $injector, sharedProperties, toaster, $rootScope, validationService) {
   	app.controller('employeeFinalSettlementCtrl', function($stateParams,$scope, $state, $http, ngDialog, logger, $location, validationService,$controller, $injector, sharedProperties, toaster, $rootScope,utilsService, $window,$timeout) {
        ...
        $scope.submit = function(employeeFinalSettlementForm) {
            if (new validationService().checkFormValidity(employeeFinalSettlementForm)) {
                ...
                toaster.pop('error', "Please fill the required fields", logger.getErrorHtmlNew(employeeFinalSettlementForm.$validationSummary), 5000, 'trustedHtml');
            }
        }
        ...
        $scope.getListValue = function(employeeId) {
            ...
            $http.post("payroll/payroll/finalsettlement/list", resultbean).success(function(datas) {
                if (datas.fullFinalSettlementList.length > 0) {
                    ...
                    $scope.grandTotal = 0;
                    $scope.loadTable();
                    if (datas.fullFinalSettlementList[0].status == "Pending") {
                        $scope.isList = true;
                    } else {
                    ...
                    $scope.employeeFinalSettlement.status = datas.fullFinalSettlementList[0].status;
                    $scope.employeeFinalSettlement.tables[0].storeTableRow = datas.fullFinalSettlementList[0].detailList;

                    $scope.calculateTotal();
                } else {
                    $scope.employeeFinalSettlement.tables = [];
                    $scope.employeeFinalSettlement.status = "Pending";
                    $scope.loadTable();
                    $scope.isList = true;
                    $scope.totalDebit = 0;
                    $scope.totalCredit = 0;
        ...
        $scope.save = function(employeeFinalSettlementForm) {
            if (new validationService().checkFormValidity(employeeFinalSettlementForm)) {
                ...
                $http.post("payroll/payroll/finalsettlement/save", resultbean).success(function(datas) {
                    if (datas == true) {
                        $scope.getListValue($scope.employeeFinalSettlement.employeeId);
        ...
        $scope.checkAll = function(rowCollection, checkBox) {
            ...
            angular.forEach($scope.employeeFinalSettlement.tables[0].storeTableRow, function(row) {
        ...
        $scope.removeRow = function(table) {
            ...
            angular.forEach(table.storeTableRow, function(storeTableRow, index) {
                if (storeTableRow.select == false || storeTableRow.select == undefined || storeTableRow.select == "") {
