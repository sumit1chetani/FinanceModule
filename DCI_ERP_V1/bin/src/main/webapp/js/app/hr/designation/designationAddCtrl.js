'use strict';
app.controller('designationAddCtrl', function($scope, $rootScope, $http, $location, logger, utilsService, $state, sharedProperties, $window,validationService,toaster) {
    ...
    $scope.getDropdownvalue = function() {
        $http.get('app/designation/getDesignation').success(function(datas) {
...
if (new validationService().checkFormValidity($scope.designationAddForm)) {
                ...
                $http.post('app/designation/add', addRowData).success(function(result) {
                    console.log("result" + result);
                    if (result) {
                        logger.logSuccess("Saved successfully!");
                        $state.go('app.master.employee.designation');
                    } else {
...
$scope.update = function(designationAddForm, designationData, designationValidateData) { 
    if (new validationService().checkFormValidity($scope.designationAddForm)) {
            ...
            $http.post('app/designation/update', updateRowData).success(function(result) {
                if (result) {
    ...
    $http.post('app/designation/edit',desgnCode).success(function(result) {
