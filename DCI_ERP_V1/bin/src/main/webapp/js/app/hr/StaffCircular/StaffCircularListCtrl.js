 ...
 *///define([ 'hrms/hr/hr' ], function(module) {
    'use strict';
    app.Controller('staffcircularListCtrl', function($scope, $state, $http, ngDialog, logger, $location, $controller, $injector, sharedProperties, toaster, $rootScope, validationService) {
        ...
        $scope.isUpload = true;
        $scope.add = function() {
            $state.go("app.hr.staffcircular.add");
        }

        $scope.editRow = function(staffCircularId) {
            $location.path('hr/staffcircular/edit/' + staffCircularId);
        }
