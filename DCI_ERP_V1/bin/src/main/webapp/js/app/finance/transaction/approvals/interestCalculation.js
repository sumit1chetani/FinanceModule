'use strict';
app.controller('interestCalculationCtrl', function($scope, $rootScope,$filter, $http, $location, logger, $log, ngDialog, $modal, utilsService, sharedProperties, $state,$controller,$stateParams) {
    ...
    $scope.getListDetail=function(){
        $http.get($stateParams.tenantid+'/app/interestCalculation/list').success(function(datas){
          //  console.log(datas);
            $scope.rowCollection = datas.lLoanList;
    ...
    if(window.localStorage.getItem('interest')==$scope.currentURL){
         alert('window ' + $scope.currentURL + ' is already opened');
         window.focus();
         //window.open($rootScope.currentURL,'_self').close();
         window.close();
     }else{
         window.localStorage.setItem('interest', $scope.currentURL);
     }
    ...
    $scope.approve=function(table){
       ...
       if($scope.tablerow.length>0){
           ngDialog.open({
               ...
               controller : $controller('interestCalculationApproveCtrl', {
                   $scope : $scope,
                   sVoucherNo: '21',
    ...
    $scope.excel=function(){
     //  alert("hi");
            ...
            $scope.selectinterest={
                    lLoanList:obj,
                    filename:"InterestCalculation"+day+""+mon+""+yr+""+n+""+s+""+ms
            }
            
            
            $scope.filename="InterestCalculation"+day+""+mon+""+yr+""+n+""+s+""+ms+".xlsx";
            
           
            
            $http.post($stateParams.tenantid+'/app/interestCalculation/generateExcel',$scope.selectinterest).success(function(data) {
                console.log(data);
...
app.controller('interestCalculationApproveCtrl', function($scope, $http,ngDialog,logger,$location,sVoucherNo,screenName,$timeout,$stateParams) {
    ...
    $scope.reverseConfirm = function(){
        ...
        if($scope.screenNames=="approvalDialog"){
            if($scope.prepaidExp.approvalDate!=""){
                var obj={
                        lLoanList:$scope.tablerow,
                        approvalDate:$scope.prepaidExp.approvalDate
                }
                $http.post($stateParams.tenantid+'/app/interestCalculation/createJvForLoanInterest', obj).success(function(result) {
                if(result.success) {
