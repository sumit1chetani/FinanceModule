    ...
    app.factory('QUOTATION', function($http, $q, logger) {
        ...
        QUOTATION.getQuotationList = function(obj){
            ...
            $http.post($stateParams.tenantid+'/report/auditTrial/quotation/purQuotList', obj).success(function(resultBean) {
            if (resultBean.success = true) {
   ...
   app.controller('PurQuotAuditTrialCtrl', function($scope, $timeout,$rootScope, $http, logger, $log, ngDialog, $modal, $location, $filter, utilsService, QUOTATION, $state, sharedProperties,$stateParams) {
        $scope.customerList=[];
        $scope.quotationList=[];
        $scope.rowCollection = [];
        $scope.quotation = {           
        ...
        $scope.exportData = function(contInvoice) {
            ...
            $scope.filename="purQuotAuditReport"+day+""+mon+""+yr+""+n+""+s+""+ms+".xls";
            
            $scope.quotation.filename="purQuotAuditReport"+day+""+mon+""+yr+""+n+""+s+""+ms;
            $http.post($stateParams.tenantid+'/report/auditTrial/quotation/purQuotviewExcel',$scope.quotation).success(function(data) {
                $('#exportXl').remove();
