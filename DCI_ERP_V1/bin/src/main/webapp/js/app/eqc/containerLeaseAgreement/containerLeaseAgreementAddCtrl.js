...
$scope.leaseAgreement={
		...
		fromDate:'',
		toDate:'',
		leaseAgreementParty:'V0014',
		partyAgreementNo:'',
		agreementDate :'',
		leaseterms :'',
		freedays :'',
		leaseAgreementDtl : [] ,
         leaseAgreementdtlNew :[],
}



$scope.tempvoyagedetailList1={
		select:false,
		leaseport:'',
		quota:'',
...
$scope.addCredRow1 = function() {
	  var tmp=angular.copy($scope.tempvoyagedetailList1);
	  $scope.leaseAgreement.leaseAgreementdtlNew.push(tmp);
}
...
$scope.removeCredRow1 =function(){
	var count =0;
		var tmpDelList = [];
		for(var i=$scope.leaseAgreement.leaseAgreementdtlNew.length-1;i>=0;i--){
			if($scope.leaseAgreement.leaseAgreementdtlNew[i].select==true){
				count++;
				tmpDelList.push($scope.leaseAgreement.leaseAgreementdtlNew[i]);
				$scope.leaseAgreement.leaseAgreementdtlNew.splice(i, 1);
			}
		}
		if(count>0){
...
$http.get($stateParams.tenantid+'/app/seaquotation/getiataList').success(function(datas) {
	 debugger
	 $scope.leaseport = datas.commonUtilityBean;
...
$scope.leaseAgreement.agreementDate = dd + '/' + mm + '/'+ yyyy;
...
$scope.getcountryList=function(){
	$http.get($stateParams.tenantid+'/api/onhireReleaseOrder/dropDownList').success(function(datas) {		
    	console.log(datas);
		...
		$.ajax({
            ...
            success : function(response) {
                if(response.portList!=null){
                	...
                	console.log("row.portList",row.portList);
                	row.returnportList=response.portList;
                	console.log("row.returnportList",row.returnportList);
	...
	if (new validationService().checkFormValidity($scope.LeaseAgreementForm)) {
        ...
        $http.post($stateParams.tenantid+'/api/containerLeaseAgreementType/save',leaseAgreement).success(function(data) {
            ...
            if (data.isSuccess) {
                ...
                $state.go('app.eqc.containerLeaseAgreement.list');
            } else {
 ...
 if (agreementRefNo == undefined) {
     ...
     $http.get($stateParams.tenantid+'/api/containerLeaseAgreementType/edit?agreementRefNo=' +agreementRefNo).success(function(result) {
         ...
         if (result.isEdit == false) {
        	 ...
        	 $scope.rowCollection = result.onhireList;
        	 $scope.rowCollection1 = result.offhireList;
        	 if(result.onhireList.length > 0)
        	 $scope.leaseAgreement.leasereference = result.onhireList[0].leasereference;
        	
        	 $http.get($stateParams.tenantid+'/api/onhireReleaseOrder/dropDownList').success(function(data) {
     ...
     if (new validationService().checkFormValidity($scope.LeaseAgreementForm)) {
         ...
         $http.post($stateParams.tenantid+'/api/containerLeaseAgreementType/update', $scope.leaseAgreement).success(function(result) {
             if (result) {
                 ...
                 $state.go('app.eqc.containerLeaseAgreement.list',{tenantid:$stateParams.tenantid});
             } else {
...
//      		$scope.port=data.listPortList;
     ...
     $http.get($stateParams.tenantid+'/app/seaquotation/getCurrencyList').success(function(datas) {	  
			$scope.currency= angular.copy(datas.commonUtilityBean);
		}).error(function(data) {
     ...
     $scope.leasingParty=[ 
                        {id: 'V0014', text: 'MBK - MBK CONTAINER SHIPPING LINE'},
