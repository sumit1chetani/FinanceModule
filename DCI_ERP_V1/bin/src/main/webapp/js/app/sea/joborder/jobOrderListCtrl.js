		...
		.controller(
				...
				function($scope, $timeout, $stateParams, sharedProperties,
					...
					$scope.getQuotationType = function() {
						...
						$scope.modeList.push(data);
						/*
						 * data = {}; data["id"] = "4"; data["text"] = "LINER";
						 * $scope.modeList.push(data);
						 */
						// data = {};
						// data["id"] = "2";
					 ...
					 * '/app/dashboard/checkWhichUser').success(function(data) {
					 * if (data[1].userId=='E0001'|| data[1].userId=='A0001') {
					 * $scope.showExcelButton=true; }
					 * 
					 * }).error(function(data) { logger.logError("Error Please
					 * Try Again"); });
					 */
					...
					$scope.exportExcelnew = function() {
						...
						/*
						 * if($scope.bookingReport.salesPersonId!='' &&
						 * $scope.bookingReport.salesPersonId!=null){
						 */
						$http.post(
								$stateParams.tenantid
										+ '/app/jobOrderSea/ExportExcel',
								$scope.joborder).success(function(response) {
						...
						/*
						 * }else{ logger.logError("Pls select Sales Person"); }
						 */
					...
					$scope.getSearchList = function(joborder) {
						$http.post(
								$stateParams.tenantid
										+ '/app/jobOrderSea/SearchList',
								$scope.joborder).success(function(data) {
							console.log(data);
							if (data.length > 0) {
						...
						$scope.offsetCount = $scope.offsetCount
								+ $scope.limitCount;
					...
					$scope.add = function() {
						...
						;
						$state.go('app.sea.joborder.add', {
							tenantid : $stateParams.tenantid
						});
					};

					/*
					 * $scope.editBooking = function(rowid,status) { if(status ==
					 * 'Fully Allocatted'){
					 ...
					 * else { logger.logError("Not Allowed to Edit ,Trip
					 * Started!"); } }).error(function(data) {
					 * logger.logSuccess("Error in Edit!"); }); } else{
					...
					$scope.editBooking = function(rowid) {
						...
						$http
								.get(
										$stateParams.tenantid
												+ '/jobOrderMonthClose/getJobDate?mode='
												+ $scope.modeType + '&jobId='
												+ rowid)
								.success(
										function(datas) {
											if (datas) {
												logger
														.logError("Job Order Closed Pls Contact IT Support");
											} else {
												$location
														.url($stateParams.tenantid
																+ '/jobOrderSea/edit?rowid='
																+ rowid);
											}
										})
					...
					$scope.viewBooking = function(rowid) {
						...
						$rootScope.joborderlistctrl1 = $('.rounded').val();
						$location.url($stateParams.tenantid
								+ '/salesbooking/add?bookingNo=' + rowid);
					...
					$scope.printQuot = function(rowid) {
						...
						var url = $stateParams.tenantid
								+ '/app/seaquotation/print?rowid=' + rowid;
						var wnd = $window
								.open(
										url,
										'Shipping',
					...
					$scope.excel = function() {
						if ($scope.joborder.branch != '') {
							...
							$http
									.post(
											$stateParams.tenantid
													+ '/app/jobOrderSea/generateExcel',
											$scope.joborder)
									.success(
											function(data) {
												if (data.success == true) {

													$scope.debitNoteFileUrl = data.filePath
															.split("/");
							...
							/*
							 * } else{ if($scope.joborder.status1=='')
				...
				function($scope, $timeout, $stateParams, $filter, $rootScope,
					...
					$scope.addRowTracking = function() {
						...
						$scope.joborder.joborderTrackingDtl
								.push(jobOrderTrackingDtl);
					};
					$scope.specialEmployeeFlag = false;
					$scope.userId = $('#empId').val();

					if ($scope.userId == 'E0002' || $scope.userId == 'E0003'
							|| $scope.userId == 'E0006'
							|| $scope.userId == 'E0004'
							|| $scope.userId == 'E0001'
							|| $scope.userId == 'E0016' || $scope.userId=='E0110') {
							...
							.$watchCollection(
									...
									function(newValue, oldValue) {
										if ($scope.joborder.jobDate != '') {
											...
											if (toDate > frmDate) {
												logger
														.logError("Job Date should be less or equal to current date");
									...
									function(newValue, oldValue) {
										if (newValue != ''
											...
											$http
													.get(
															$stateParams.tenantid
																	+ '/app/jobOrderSea/getbookingListwithMode?mode='
																	+ newValue
																	+ '&jobDate='
					...
					$scope.printRoutingOrder = function(jobId) {
						...
						var url = $stateParams.tenantid
								+ '/app/jobOrderSea/printRoutingOrder?jobId='
								+ jobId;
						var wnd = $window
								.open(
										url,
							...
							$http
									...
									.success(
											function(data) {
												...
												/*
												 * if(!$scope.isEdit){
												 * $scope.joborder.joborderDtl[$scope.trIndex].exRate =
												...
												$scope
														.checkExRate(
																$scope.joborder.joborderDtl[index].exRate,
																index);
												$scope.ratevalues();
											});
						}
					...
					$scope.viewQuotation = function(quotationNo) {
						...
						$location.url($stateParams.tenantid
								+ '/seaQuotation/view?QuotHdId=' + quotationNo);
					...
					$scope.checkExRate = function() {
						if ($scope.joborder.exRate >= fromCurrency
								&& $scope.joborder.exRate <= toCurrency) {
					...
					$scope.printPrealertSea = function(jobId) {
						...
						var url = $stateParams.tenantid
								+ '/app/jobOrderSea/printPreAlert?jobId='
								+ jobId;
						var wnd = $window
								.open(
										url,
					 ...
					 * var buy=0;
					 * 
					 * var buy1 =0; var sell=0; var sell1 =0;
					 * 
					 * var total =0;
					 */
					$scope.selectAll=function(uomSelect,uom){
						if(uomSelect==true && uom!=null && uom!='' && uom!=undefined){
							angular.forEach($scope.joborder.joborderTrackingDtl,
									function(value, key) {
								value.uom=uom;
					...
					$scope.selectAll1=function(cargoSelect,cargoDescription){
						if(cargoSelect==true && cargoDescription!=null && cargoDescription!='' && cargoDescription!=undefined){
							angular.forEach($scope.joborder.joborderTrackingDtl,
									function(value, key) {
								value.cargoDescription=cargoDescription;
					...
					$scope
							.$watch(
									'joborder.joborderTrackingDtl[trIndex1].noOfPackage',
									function(newValue, oldValue) {
										var noOfPcs = 0;
										for (var i = 0; i <= $scope.joborder.joborderTrackingDtl.length; i++) {
											if ($scope.joborder.joborderTrackingDtl[i].noOfPackage == "") {
												...
												noOfPcs = parseInt($scope.joborder.joborderTrackingDtl[i].noOfPackage)
														+ noOfPcs;
					...
					$scope
							.$watch(
									'joborder.joborderTrackingDtl[trIndex1].netWeight',
									function(newValue, oldValue) {
										var noOfNet = 0.0;
										for (var i = 0; i <= $scope.joborder.joborderTrackingDtl.length; i++) {
											if ($scope.joborder.joborderTrackingDtl[i].netWeight == "") {
												$scope.joborder.joborderTrackingDtl[i].netWeight = 0.0;
												noOfNet = parseFloat($scope.joborder.joborderTrackingDtl[i].netWeight)
														+ noOfNet;
					...
					$scope
							.$watch(
									'joborder.joborderTrackingDtl[trIndex1].grossWeight',
									function(newValue, oldValue) {
										var noOfGross = 0.0;
										for (var i = 0; i <= $scope.joborder.joborderTrackingDtl.length - 1; i++) {
											if ($scope.joborder.joborderTrackingDtl[i].grossWeight == "") {
												...
												noOfGross = parseFloat($scope.joborder.joborderTrackingDtl[i].grossWeight)
														+ noOfGross;
						...
						if ($scope.joborder.joborderDtl.length != null
								...
								|| $scope.joborder.joborderDtl.length != '') {
							{
								for (var i = 0; i <= $scope.joborder.joborderDtl.length - 1; i++) {
									...
									$scope.joborder.joborderDtl[i].amount = parseFloat(
											amount1[i]).toFixed(2);
					...
					$scope.salesTypeList = [];
					/*
					 * $scope.getSalesType = function() { var data = {};
					 * data["id"] = "1"; data["text"] = "CROSS TRADE";
					 * $scope.salesTypeList.push(data); data = {}; data["id"] =
					 * "2"; data["text"] = "LOCAL";
					 * $scope.salesTypeList.push(data); data = {}; data["id"] =
					 * "3"; data["text"] = "NOMINATED";
					...
					/*
					 * $scope.getTransactionType = function() { var data = {};
					 * data["id"] = "1"; data["text"] = "BUY";
					 * $scope.transactionTypeList.push(data); data = {};
					 * data["id"] = "2"; data["text"] = "SELL";
					...
					$scope.dropdown = function() {
						...
						$http
								...
								.success(
										function(datas) {
											...
											/*
											 * $scope.customerBuyList =
											 * datas.buyServiceList;
											 * $scope.customerSellList =
											 * datas.sellServiceList;
											 */
											...
											// $scope.sizeTypeList =
											// datas.sizeTypeSelectivityList;
											// $scope.commodityList =
											// datas.commoditySelectivityList;
						...
						$http
								.get(
										$stateParams.tenantid
												+ '/app/commonUtility/getQuoteApproveList')
								.success(function(data) {
									$scope.quotationnoList = data;
								});
						/*
						 * $http.post($stateParams.tenantid+'/app/seaquotation/getServicePartnerList').success(function(datas) {
						 * console.log(datas); $scope.customerDropList =
						 * datas.customerList; $scope.consigneeDropList =
						 * datas.consigneeList; $scope.shipperDropList =
						 * datas.shipperList; $scope.nominatedDropList =
						 * datas.nominatedList; $scope.vendorDropList =
						 * datas.vendorList;
						 ...
						 * $scope.currencylist=
						 * angular.copy(datas.commonUtilityBean);
						 * }).error(function(data) {
						 * 
						 * });
						 * 
						 ...
						 * $scope.uomList = datas.uomList; $scope.salesTypeList =
						 * datas.commonUtilityBean; }).error(function(data) {
						 * 
						 * });
						 * 
						 * $http.get($stateParams.tenantid +
						 * '/app/airquotation/getSalesList')
						 * .success(function(datas) { $scope.salesTypeList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
						 * });
						 ...
						 * .success(function(datas) { $scope.PaymentMethodList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
						 ...
						 * .success(function(datas) { $scope.transactionTypeList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
						 ...
						 * .success(function(datas) { $scope.chargeHeadList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
						 ...
						 * .success(function(datas) { $scope.TermList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
						 ...
						 * .success(function(datas) { $scope.UnitList =
						 * datas.commonUtilityBean;
						 * 
						 * }).error(function(data) {
						 * 
					...
					/*
					 * $http.get( $stateParams.tenantid +
					 * '/app/seaquotation/getuomList') .success(function(datas) {
					 * $scope.uomList = datas.commonUtilityBean;
					 * 
					 * }).error(function(data) {
					 * 
					 * }); $http.get( $stateParams.tenantid +
					 * '/app/jobOrderSea/getsizeTypeList')
					 * .success(function(datas) { $scope.sizeTypeList =
					 * datas.commonUtilityBean;
					 * 
					 * }).error(function(data) {
					 * 
					...
					$http.get(
							$stateParams.tenantid
									+ '/app/commonUtility/getVoyageList')
							.success(function(data) {
					...
					$scope.checkValidation = function() {
						...
						if ($scope.checkundefined($scope.joborder.quotationNo)) {
							msg = msg
									+ "<li>quotation No :Field is required.</li>";
						 ...
						 * $scope.changecolor('commodity'); }else{
						...
						if ($scope.checkundefined($scope.joborder.salesPerson)) {
							msg = msg
									+ "<li>SalesPerson :Field is required.</li>";
						 ...
						 * $scope.changecolor('nominatedBy'); }else{
						...
						/*
						 * angular.forEach($scope.joborder.joborderTrackingDtl,
						 * function(TrackingDtl, index) {
						 * 
						 * if ($scope.checkundefined(TrackingDtl.commodity)) {
						 * msg = msg + "<li>Row :" + (index + 1) + "#
						 * Commodity :Field is required.</li>";
						 * $scope.changecolor('Commodity' + index); } else {
						 * $scope.clearcolor('Commodity' + index); } if
						 * ($scope.checkundefined(TrackingDtl.descriptionGoods)) {
						 * msg = msg + "<li>Row :" + (index + 1) + "#
						 * DescriptionGoods :Field is required.</li>";
						 * $scope.changecolor('descriptionGoods' + index); }
						 * else { $scope.clearcolor('descriptionGoods' + index); }
						 * if ($scope.checkundefined(TrackingDtl.uom)) { msg =
						 * msg + "<li>Row :" + (index + 1) + "# LWH UOM :Field
						 * is required.</li>"; $scope.changecolor('uom' +
						 * index); } else { $scope.clearcolor('uom' + index); }
						 * if ($scope.checkundefined(TrackingDtl.noOfPieces)) {
						 * msg = msg + "<li>Row :" + (index + 1) + "#
						 * NoOfPieces :Field is required.</li>";
						 * $scope.changecolor('noOfPieces' + index); } else {
						 * $scope.clearcolor('noOfPieces' + index); } if
						 * ($scope.checkundefined(TrackingDtl.netWeight)) { msg =
						 * msg + "<li>Row :" + (index + 1) + "# NetWeight
						 * :Field is required.</li>";
						 * $scope.changecolor('netWeight' + index); } else {
						 * $scope.clearcolor('netWeight' + index); } if
						 * ($scope.checkundefined(TrackingDtl.grossWeight)) {
						 * msg = msg + "<li>Row :" + (index + 1) + "#
						 * GrossWeight :Field is required.</li>";
						 * $scope.changecolor('GrossWeight' + index); } else {
						 * $scope.clearcolor('GrossWeight' + index); } if
						 * ($scope.checkundefined(TrackingDtl.amount)) { msg =
						 * msg + "<li>Row :" + (index + 1) + "# Amount :Field
						 * is required.</li>"; $scope.changecolor('Amount' +
						 * index); } else { $scope.clearcolor('Amount' + index); }
						 * 
						 * 
						 * });
						 */
					...
					$scope.removeRowTracking = function() {
						...
						for (var index = len - 1; index < len; index--) {
							if ($scope.joborder.joborderTrackingDtl[index].select == true) {
								...
								$scope.joborder.joborderTrackingDtl.splice(
										index, 1);
							}
						}
					};
					 ...
					 * oldValue) { if (newValue != '' && newValue != undefined) {
					 * 
					 * $http .post( $stateParams.tenantid +
					 * '/app/jobOrderSea/getQuotationList', newValue) .success(
					 * function(data) {
					 * 
					 * $scope.quotationList = data.quotationList;
					 * 
											...
											$http
													...
													.success(
															function(data) {
																...
																if (data.bookingBean.payerName != null
																		&& data.bookingBean.payerName != '') {
																	$scope.joborder.payerName = data.bookingBean.payerName
																			.toString();
																}

																if (data.bookingBean.quotationno != null
																		&& data.bookingBean.quotationno != '') {
																	$scope.joborder.quoteNo = data.bookingBean.quotationno
																			.toString();
																}
																...
																/*
																 * if(data.bookingBean.vessel !=
																 * null &&
																 * data.bookingBean.vessel !=
																 * ''){
																 * $scope.joborder.vessel =
																 * data.bookingBean.vessel; }
																 */
																...
																if (data.bookingBean.branch != null
																	...
																	$scope.joborder.branch = data.bookingBean.branch
																			.toString();
																}
																							...
																							.success(
																									function(
																										...
																										for (var i = 0; i < $scope.joborder.joborderDtl.length; i++) {
																											...
																											if($scope.joborder.joborderDtl[i].conType==null){
																												$scope.joborder.joborderDtl[i].conType='';
																											}else{
																										...
																										$http
																												...
																												.success(
																														function(
																															...
																															$scope.commodityList = data.commonUtilityBean;
																															$scope.compaccList = [];
																															var valArr = $scope.joborder.commodity
																																	.split(',');
																															var i = 0, size = valArr.length;
																															for (i; i < size; i++) {
																																// $("#port").find("option[label="
																																// +
																																// valArr[i]
																																// +
																																// "]").prop("selected",
																											...
																											if (result != null
																												...
																												for (var i = 0; i < $scope.joborder.joborderTrackingDtl.length; i++) {
																													if ($scope.joborder.mode != '5') {
																														$scope.joborder.joborderTrackingDtl[i].containerNo = $scope.joborder.joborderTrackingDtl[i].containerNo
																																.toString();
																														$scope.joborder.joborderTrackingDtl[i].sizeType = $scope.joborder.joborderTrackingDtl[i].containerType
																																.toString();
					...
					$scope.cancel = function() {
						$state.go('app.sea.joborder.list', {
							tenantid : $stateParams.tenantid
						});
					}

					/*
					 * $scope.carrierList = [ {id: '1', text: 'SIMA MARINE
					 * (INDIA) PRIVATE LIMITED'}, ];
					 * 
					 */
					/*
					 * $scope.carrierList = [ {id: '1', text: 'SIMA MARINE
					 * (INDIA) PRIVATE LIMITED'}, {id: '2', text: 'TCI
					 * SEAWAYS'}, {id: '3', text: 'THE SHIPPING CORPORATION OF
					 * INDIA LIMITED'}, {id: '4', text: 'AVANA LOGISTEK
					 * LIMITED'}, {id: '5', text: 'CONTAINER CORPORATION OF
					 * INDIA LIMITED'} ];
					 */
					$http.get(
							$stateParams.tenantid
						...
						if (!$scope.checkundefined(msg)) {
							...
							if ($scope.joborder.joborderDtl.length > 0) {
								if ($scope.joborder.joborderTrackingDtl.length > 0) {
									...
									if ($scope.joborder.remarks1 != null
											&& $scope.joborder.remarks1 != "") {
										$scope.joborder.remarks1 = todate + ' '
												+ time + '     ' + '     '
												+ temp;
									}
									var commodity = '';
									angular.forEach($scope.joborder.commodityL,
											function(item, index) {
												if (commodity != null
														&& commodity != '') {
													commodity = commodity + ','
									...
									var checkPack = true, checkPack1 = true;
									var i = 1, j = 1;
									var msg = '';
									var checkPackcon = true;
									angular
											.forEach(
													$scope.joborder.joborderTrackingDtl,
													function(val, index) {
														if (val.containerNo == undefined
																|| val.containerNo == null
																|| val.containerNo == '') {
															checkPackcon = false;
															msg = msg
																	+ " Row "
														...
														if (val.sizeType == undefined
															...
															msg = msg
																	+ " Row "
																	+ i
																	+ " container size Type is Required! ";
														}
														if (val.cargoDescription == undefined
																|| val.cargoDescription == null
																|| val.cargoDescription == '') {
															checkPackcon = false;
															msg = msg
																	+ " Row "
																	+ i
									...
									if (checkPack == true && checkPack1 == true
											&& checkPackcon == true) {
										$http
												.post(
														$stateParams.tenantid
																+ '/app/jobOrderSea/ckeckConCount',
														$scope.joborder)
												.success(
														function(datas) {
															if (datas.succ == true) {
																...
																//$scope.allow($scope.joborder);
																logger.logError("Container count is not matching with Booking. Please Check! ");
															}
														})
									} else {
										logger.logError(msg);
									...
									logger
											.logError("Atleast One Row Required in Cargo Details");
								}
							} else {
...
app
		.controller(
				...
				function($scope, $timeout, $stateParams, sharedProperties,
					...
					/*
					 * $scope.joborder={ jobId : '', mode:'', service:'',
					 * jobDate:'', branch:'', customer:'', shipper:'', consignee :
					 * '', term : '', aol:'', aod :'', origin:'',
					 * destination:'', commodity:'', status:'', currency:'',
					 * quotationNo:'', salesType:'', salesPerson:'',
					 * nominatedBy:'', vendor:'', carrier:'', vessel:'',
					 * flightDate:'', exRate:'', customsBroker:'', modeName:'',
					 * jbName:'', partyName:'', pkg:'', gross:'', net:'',
					 * vessel:'', container:'', pickUp:'', carting:'',
					 * remarks:'', agent:'', eta:'', nomination:'', exrate:'',
					 * pol:'', pod:'', pickupDate:'', stuffingDate:'',
					 * createdDt:'', sailingDate:'', customsCompletionsDate:'',
					 * etaDestinationDate:'', salesDate:'', purchaseInvoice:''
					 * 
					 * 
					 * $scope.jobOrderDtl = [{ joborderDtlId : '', chargeHead :
					 * '', unit : '', transactionType : '', quantity : '', rate :
					 * '', currency :'', exRate :'', amount : '0', paymentMode:
					 * '', buySellParty : '', Status:'', amount1 :'' }],
					 * joborderTracking : { jobTrackingId : '', jobId : '',
					 * totalPcs : '', totalGrosssWeight : '', totalAmount : '',
					 * totalNetWeight : '', }, joborderTrackingDtl : [{ select :
					 * '', commodity : '', descriptionGoods : '', uom : '',
					 * length : '', width : '', height : '', noOfPieces : '',
					 * netWeight : '', grossWeight : '', chargeableWeight: '',
					 * rate: '', amount : '', remarks : '' }] }
					 */
							...
							.$watch(
									...
									function(newValue, oldValue) {
										if (newValue != ''
											 ...
											 * if(newValue==2) {
											 * $scope.Add=false;
											 * $scope.disable=true; }else{
											 * $scope.Add=true;
											 * $scope.disable=false; }
											 */
											if (newValue == 2
													&& (userid == 'A0001' || userid == 'E0003')) {
												$scope.Add = false;
												$scope.disable = true;
					...
					$scope.printUpdatesRouting = function() {
						...
						angular.forEach($scope.shipperDropList, function(value,
								key) {
						...
						if ($scope.joborder.shpprAddrss2 != null) {
							$scope.prtShpprAddress = $scope.prtShpprAddress
									+ $scope.joborder.shpprAddrss2;
						}
						if ($scope.joborder.shpprAddrss3 != null) {
							$scope.prtShpprAddress = $scope.prtShpprAddress
									+ $scope.joborder.shpprAddrss3;
						}
						if ($scope.joborder.shpprAddrss4 != null) {
							$scope.prtShpprAddress = $scope.prtShpprAddress
									+ $scope.joborder.shpprAddrss4;
						}

						angular.forEach($scope.consigneeDropList, function(
								value, key) {
						...
						// "+$scope.joborder.shpprAddrss2+"
						// "+$scope.joborder.shpprAddrss3+"
						// "+$scope.joborder.shpprAddrss4==null?"":$scope.joborder.shpprAddrss4;

					}

					$scope.printRoutingOrder1 = function() {
						/*
						 * var printVar=false; // This empty methhod to update
						 * values from js to jsp //SEA
						 * $http.post($stateParams.tenantid+
						 * '/app/jobOrderAir/updateValues').success(
						 * function(data) { $scope.printUpdatesRouting(); })
						 */

						var printContents = document
								.getElementById('printableContent').innerHTML;
						var popupWin = window
								.open(
										'',
										'_blank',
										'height=700,width=850,toolbar=no,scrollbars=yes,fullscreen=no,status=yes,menubar=no,location=no,directories=no,resizable=yes');
					...
					$scope.JobSheet = function() {
						...
						var url = $stateParams.tenantid
								+ '/app/jobOrderSea/jobDetailOrder?jobId='
								+ jobId;
						var wnd = $window
								.open(
										url,
					...
					$scope
							.$watch(
									'joborder.joborderDtl1[trIndex].noOfPackage',
									function(newValue, oldValue) {
										var noOfPcs = 0;
										for (var i = 0; i <= $scope.joborder.joborderTrackingDtl.length; i++) {
											if ($scope.joborder.joborderTrackingDtl[i].noOfPackage == "") {
							...
							$http
									...
									.success(
											function(data) {
												 ...
												 */
												/*
												 * $scope.joborder.pickupDate =
												 * data.lJobOrderBean[0].pickupDate
												 * .toString();
												 * $scope.joborder.stuffingDate =
												 * data.lJobOrderBean[0].stuffingDate
												 * .toString();
												 * $scope.joborder.createdDt =
												 * data.lJobOrderBean[0].createdDt
												 * .toString();
												 * $scope.joborder.sailingDate =
												 * data.lJobOrderBean[0].sailingDate
												 * .toString();
												 * $scope.joborder.customsCompletionsDate =
												 * data.lJobOrderBean[0].customsCompletionsDate
												 * .toString();
												 * $scope.joborder.etaDestinationDate =
												 * data.lJobOrderBean[0].etaDestinationDate
												 * .toString();
												 * $scope.joborder.salesInvoice =
												 * data.lJobOrderBean[0].salesInvoice
												 * .toString();
												 * $scope.joborder.salesDate =
												 * data.lJobOrderBean[0].salesDate
												 * .toString();
												 * $scope.joborder.purchaseInvoice =
												 * data.lJobOrderBean[0].purchaseInvoice
												 * .toString();
												 * $scope.joborder.purchaseDate =
												 * data.lJobOrderBean[0].purchaseDate
												 * .toString();
												 */
												...
												for (var i = 0; i < $scope.joborder.joborderTrackingDtl.length; i++) {
													...
													// $scope.joborder.joborderTrackingDtl[i].uom
													// =
													// $scope.joborder.joborderTrackingDtl[i].uom.toString();
												}
											});
								...
								if ($scope.joborder.joborderDtl.length > 0) {
									if ($scope.Add == true) {
										if ($scope.joborder.joborderTrackingDtl.length > 0) {
											...
											if ($scope.joborder.remarks1 != null
													&& $scope.joborder.remarks1 != "") {
												$scope.joborder.remarks1 = localValue
														+ $scope.joborder.remarks1;
											}
										...
										if ($scope.joborder.joborderTrackingDtl.length > 0) {
											...
											&& $scope.Add == false) {
										logger
												.logError(" Some invoices are Pending / Draft ");
									}
					...
					$scope.$watch('joborder.quotationNo', function(newValue,
						...
						if (oldValue != "") {
							...
							$window.open('/#/' + $stateParams.tenantid
									+ '/seaQuotation/view?qkLinkQuon='
									+ qkLinkQuon, '_blank');
						}
					});
					...
					$scope.quickLinkMethd = function(jobId, qulkVal) {
						if (qulkVal != 'Select') {
							$http
									.post(
											$stateParams.tenantid
													+ '/app/jobOrderSea/quickLinkView?category='
													+ qulkVal + '&jobId='
													+ jobId)
									.success(
											function(datas) {
												if (datas.quickLinkId != null) {
													...
													if (qulkVal == "HBL") {
														if (rowid != 0) {
															$location
																	.url($stateParams.tenantid
																			+ '/hbl/edit?rowid='
																			+ rowid);
															// $window.open('#'+$stateParams.tenantid+'/hbl/view?rowid='+rowid,
															// '_blank');
														} else {
															logger
																	.logError("There is no "
																			+ qulkVal);
														}
													} else if (qulkVal == "MBL") {
														if (rowid != 0) {
														...
														if (rowid != 0) {
															$location
																	.url($stateParams.tenantid
																			+ '/invoice/seasalesinvoice/salesInvoiceView'
																			+ rowid);
														...
														if (rowid != 0) {
															$location
																	.url($stateParams.tenantid
																			+ '/invoice/seapurchaseinvoice/PurchaseInvoiceView'
																			+ rowid);
													...
													if (qulkVal == "HBL") {
														// $window.open('#'+$stateParams.tenantid+'/hbl/list?quickLinkIdList='+quickLinkIdList,
														// '_blank');
														$location
																.url($stateParams.tenantid
																		+ '/hbl/list?quickLinkIdList='
																		+ quickLinkIdList);
													} else if (qulkVal == "MBL") {
														$location
																.url($stateParams.tenantid
																		+ '/mabl/list?quickLinkIdList='
																		+ quickLinkIdList);
														// $window.open('#'+$stateParams.tenantid+'/mabl/list?quickLinkIdList='+quickLinkIdList,
														// '_blank');
													} else if (qulkVal == "Delivery Order") {
														$location
																.url($stateParams.tenantid
																		+ '/deliveryorder/list?quickLinkIdList='
																		+ quickLinkIdList);
														// $window.open('#'+$stateParams.tenantid+'/deliveryorder/list?quickLinkIdList='+quickLinkIdList,
														// '_blank');
													} else if (qulkVal == "Sales Invoice") {
														$location
																.url($stateParams.tenantid
																		+ '/invoice/sea/seasalesinvoice/SalesInvoiceList?quickLinkIdList='
																		+ quickLinkIdList);
														// $window.open('#'+$stateParams.tenantid+'/invoice/sea/seasalesinvoice/SalesInvoiceList?quickLinkIdList='+quickLinkIdList,'_blank');
													} else if (qulkVal == "Purchase Invoice") {
														$location
																.url($stateParams.tenantid
																		+ '/invoice/sea/seapurchaseinvoice/PurchaseInvoiceList?quickLinkIdList='
																		+ quickLinkIdList);
													...
													if (qulkVal == "Sales Invoice") {
														$location
																.url($stateParams.tenantid
																		+ '/invoice/sea/salesinvoice/SalesInvoiceAdd');
													}
													if (qulkVal == "Purchase Invoice") {
														$location
																.url($stateParams.tenantid
																		+ '/invoice/sea/purchaseinvoice/PurchaseInvoiceAdd');
													}
												}
...
app
		.controller(
				...
				function($scope, $timeout, $stateParams, sharedProperties,
					...
					/*
					 * $scope.carrierList = [ {id: '1', text: 'SIMA MARINE
					 * (INDIA) PRIVATE LIMITED'}, {id: '2', text: 'TCI SEAWAYS'} ];
					 */
									...
									function(newValue, oldValue) {
										...
										if (newValue != ''
											...
											$http
													.get(
															$stateParams.tenantid
																	+ '/app/jobOrderSea/getDateChange?date='
																	+ oldValue
																	+ '&jobId='
																	+ jobId
																	+ '&mode='
													...
													.success(
															function(datas) {
																...
																if (datas.success) {
																	logger
																			.logError("Already Invoice Raised For this Job No");
																	$scope.joborder.jobDate = '';
																}
															}).error(
															function(data) {
					...
					$scope.dropdown = function() {
						 ...
						 * datas.vendorList;
						 * $scope.serviceParnrDropList=datas.serviceParnrList;
						 * $scope.printUpdatesRouting(); }).error(function(data) {
						 * 
						 * });
						 * 
						 * $http.get( $stateParams.tenantid +
						 ...
						 * .success(function(datas) { $scope.uomList =
						 * datas.commonUtilityBean;
						 * $scope.printUpdatesRouting(); }).error(function(data) {
						 * 
						 * });
					...
					$scope.checkValidation = function() {
						 ...
						 * $scope.changecolor('consignee'); }else{
						 ...
						 * msg = msg + "<li>Customer Broker :Field is
						 * required.</li>";
						 * $scope.changecolor('customsBroker'); }else{
						...
						angular
								.forEach(
										...
										function(chargesDetail, index) {
											...
											/*
											 * if
											 * ($scope.checkundefined(chargesDetail.paymentMode)) {
											 * msg = msg + "<li>Row :" +
											...
											/*
											 * if
											 * ($scope.checkundefined(chargesDetail.amount)) {
											 * msg = msg + "<li>Row :" +
											 * (index + 1) + "# Amount:Field is
											 * required.</li>"; }
											 */
							...
							$http
									...
									.success(
											function(data) {
												...
												if (data.lJobOrderBean[0].remarks1 != ''
														&& data.lJobOrderBean[0].remarks1 != null) {
													$scope.joborder.searemarks = data.lJobOrderBean[0].remarks1
															.toString();
												}
												$scope.joborder.remarks1 = '';
												if (data.lJobOrderBean[0].branch != ''
												...
												// }
												$scope.joborder.jobDate = data.lJobOrderBean[0].jobDate;
												if (data.lJobOrderBean[0].aol != ''
														&& data.lJobOrderBean[0].aol != null) {
													$scope.joborder.aol = data.lJobOrderBean[0].aol
															.toString();
												}
												if (data.lJobOrderBean[0].aod != ''
														&& data.lJobOrderBean[0].aod != null) {
													$scope.joborder.aod = data.lJobOrderBean[0].aod
															.toString();
												}
												if (data.lJobOrderBean[0].term != ''
														&& data.lJobOrderBean[0].term != null) {
													$scope.joborder.term = data.lJobOrderBean[0].term
															.toString();
												}
												if (data.lJobOrderBean[0].origin != ''
														&& data.lJobOrderBean[0].origin != null) {
													$scope.joborder.origin = data.lJobOrderBean[0].origin
															.toString();
												}
												if (data.lJobOrderBean[0].destination != ''
														&& data.lJobOrderBean[0].destination != null) {
													$scope.joborder.destination = data.lJobOrderBean[0].destination
															.toString();
												}
												if (data.lJobOrderBean[0].customer != ''
														&& data.lJobOrderBean[0].customer != null) {
													$scope.joborder.customer = data.lJobOrderBean[0].customer
															.toString();
												}
												if (data.lJobOrderBean[0].shipper != ''
														&& data.lJobOrderBean[0].shipper != null) {
												...
												if (data.lJobOrderBean[0].currency != ''
														&& data.lJobOrderBean[0].currency != null) {
													$scope.joborder.currency = data.lJobOrderBean[0].currency
															.toString();
												}
												if (data.lJobOrderBean[0].quotationnum != ''
														&& data.lJobOrderBean[0].quotationnum != null) {
													$scope.joborder.quotationnum = data.lJobOrderBean[0].quotationnum
															.toString();
												}
												if (data.lJobOrderBean[0].mode
														.toString() != ''
														&& data.lJobOrderBean[0].mode
																.toString() != null) {
													$scope.joborder.mode = data.lJobOrderBean[0].mode
												...
												if (data.lJobOrderBean[0].salesType != ''
														&& data.lJobOrderBean[0].salesType != null) {
													$scope.joborder.salesType = data.lJobOrderBean[0].salesType
															.toString();
												}
												if (data.lJobOrderBean[0].vessel != ''
														&& data.lJobOrderBean[0].vessel != null) {
													$scope.joborder.vessel = data.lJobOrderBean[0].vessel
															.toString();
												}
												if (data.lJobOrderBean[0].consignee != ''
														&& data.lJobOrderBean[0].consignee != null) {
													$scope.joborder.consignee = data.lJobOrderBean[0].consignee
															.toString();
												}
												if (data.lJobOrderBean[0].vendor != ''
														&& data.lJobOrderBean[0].vendor != null) {
												...
												/*
												 * for (var i = 0; i <
												 * $scope.joborder.joborderDtl1.length;
												 * i++) {
												 * 
												 * $scope.joborder.joborderDtl1[i].chargeHead =
												 * $scope.joborder.joborderDtl1[i].chargeHead.toString();
												 * $scope.joborder.joborderDtl1[i].unit =
												 * $scope.joborder.joborderDtl1[i].unit
												 * .toString();
												 * $scope.joborder.joborderDtl1[i].currency =
												 * $scope.joborder.joborderDtl1[i].currency
												 * .toString();
												 * $scope.joborder.joborderDtl1[i].paymentMode=
												 * $scope.joborder.joborderDtl1[i].paymentMode
												 * .toString();
												 * $scope.joborder.joborderDtl1[i].transactionType =
												 * $scope.joborder.joborderDtl1[i].transactionType
												 * .toString();
												 * $scope.joborder.joborderDtl1[i].buySellParty =
												 * $scope.joborder.joborderDtl1[i].buySellParty
												 * .toString();
												 * $scope.joborder.joborderDtl1[i].status =
												 * $scope.joborder.joborderDtl1[i].status.toString();
												 * $scope.joborder.joborderDtl1[i].color =
												 * $scope.joborder.joborderDtl1[i].color;
												 *  }
												 */
												...
												$scope.joborder.joborderTrackingDtl = data.lJobOrderBean[0].joborderTrackingDtl;
												for (var i = 0; i < $scope.joborder.joborderTrackingDtl.length; i++) {
													$scope.joborder.joborderTrackingDtl[i].uom = $scope.joborder.joborderTrackingDtl[i].uom
															.toString();
													$scope.joborder.joborderTrackingDtl[i].sizeType = $scope.joborder.joborderTrackingDtl[i].sizeType
															.toString();
					...
					//			                
					//			                
					// if($scope.joborder.joborderDtl1.length != null
					// ||$scope.joborder.joborderDtl1.length != undefined
					// ||$scope.joborder.joborderDtl1.length != ""
					// ||$scope.joborder.joborderDt11.length != ''){
					// {
					// for( var
					// i=0;i<=$scope.joborder.joborderDtl1.length-1;i++)
					// {
					//			
					//
					// var amount2=[];
					// var amount=[];
					//			
					// if(($scope.joborder.joborderDtl1[i].transactionType !=
					// null ) ||
					// ($scope.joborder.joborderDtl1[i].transactionType !=
					// undefined )
					// ||($scope.joborder.joborderDtl1[i].transactionType != ""
					...
					// {
					//					
					// if(($scope.joborder.joborderDtl1[i].quantity != null ||
					// $scope.joborder.joborderDtl1[i].quantity != undefined
					// ||$scope.joborder.joborderDtl1[i].quantity !=""
					// ||$scope.joborder.joborderDtl1[i].quantity != '') &&
					// ($scope.joborder.joborderDtl1[i].rate != null ||
					// $scope.joborder.joborderDtl1[i].rate != undefined
					// ||$scope.joborder.joborderDtl[i].rate != " "
					// ||$scope.joborder.joborderDtl1[i].rate != '') &&
					// ($scope.joborder.joborderDtl[i].exRate != null ||
					// $scope.joborder.joborderDtl1[i].exRate == undefined
					// ||$scope.joborder.joborderDtl1[i].exRate !=""
					// ||$scope.joborder.joborderDtl1[i].exRate != '') )
					// {
					// amount2[i] =
					// (parseFloat(($scope.joborder.joborderDtl1[i].quantity)) *
					// parseFloat(($scope.joborder.joborderDtl1[i].rate))*parseFloat(($scope.joborder.joborderDtl1[i].exRate)));
					...
					// ||$scope.joborder.joborderDtl1[i].rate != " "
					// ||$scope.joborder.joborderDtl1[i].rate != '') &&
					// ($scope.joborder.joborderDtl1[i].exRate != null ||
					// $scope.joborder.joborderDtl1[i].exRate == undefined
					// ||$scope.joborder.joborderDtl1[i].exRate !=""
					// ||$scope.joborder.joborderDtl1[i].exRate != '') )
					// {
...
app.controller('remarksCtrl', function($scope, $rootScope, sharedProperties,
		logger, $http, $filter, $location, $stateParams, validationService,
		toaster, $window, purchaseObject, ngDialog, $timeout, accountHeadCode,
		jobNo) {
...
app
		.controller(
				...
				function($scope, $http, $filter, logger, $stateParams,
					...
					$scope
							.$watch(
									'joborder.joborderDtl[trIndex].transactionType',
									function(newValue, oldValue) {
										;
										if (newValue != ''
												&& newValue != undefined) {
											...
											$scope.joborder.sell2 = sell1
													.toFixed(2);
											$scope.joborder.buy2 = buy1
													.toFixed(2);
											$scope.joborder.total2 = total
													.toFixed(2);
					...
					$scope
							.$watch(
									'joborder.joborderDtl1[trIndex].transactionType',
									function(newValue, oldValue) {
										;
										if (newValue != ''
												&& newValue != undefined) {
											...
											if ($scope.joborder.joborderDtl1.length != null
													...
													|| $scope.joborder.joborderDtl1.length != '') {
												{
													for (var i = 0; i <= $scope.joborder.joborderDtl1.length - 1; i++) {
														...
														var amount1 = [];
														var amount = [];
														if (($scope.joborder.joborderDtl1[i].transactionType != null)
																|| ($scope.joborder.joborderDtl1[i].transactionType != undefined)
																|| ($scope.joborder.joborderDtl1[i].transactionType != "")
																|| ($scope.joborder.joborderDtl1[i].transactionType != '')) {
															if ($scope.joborder.joborderDtl1[i].transactionType == "1") {
																...
																if (($scope.joborder.joborderDtl1[i].quantity != null
																		|| $scope.joborder.joborderDtl1[i].quantity != undefined
																		|| $scope.joborder.joborderDtl1[i].quantity != "" || $scope.joborder.joborderDtl1[i].quantity != '')
																		&& ($scope.joborder.joborderDtl1[i].rate != null
																				|| $scope.joborder.joborderDtl1[i].rate != undefined
																				|| $scope.joborder.joborderDtl1[i].rate != " " || $scope.joborder.joborderDtl1[i].rate != '')
																		&& ($scope.joborder.joborderDtl1[i].exRate != null
																				|| $scope.joborder.joborderDtl1[i].exRate == undefined
																				|| $scope.joborder.joborderDtl1[i].exRate != "" || $scope.joborder.joborderDtl1[i].exRate != '')) {
																	amount1[i] = (parseFloat(($scope.joborder.joborderDtl1[i].quantity))
																			* parseFloat(($scope.joborder.joborderDtl1[i].rate)) * parseFloat(($scope.joborder.joborderDtl1[i].exRate)));
																	$scope.joborder.joborderDtl1[i].amount = parseFloat(amount1[i]);

																	buy1 = parseFloat(buy1)
																			+ parseFloat($scope.joborder.joborderDtl1[i].amount);
																	console
																			.log("i:"
																					+ i
																					+ " buy1:"
														...
														$scope.joborder.joborderDtl1[i].amount = parseFloat(
																amount1[i])
																.toFixed(2);
											...
											$scope.joborder.sell2 = parseFloat(sell1
													.toFixed(2))
													+ parseFloat($scope.joborder.sell1);
											$scope.joborder.buy2 = parseFloat(buy1
													.toFixed(2))
													+ parseFloat($scope.joborder.buy1);
											$scope.joborder.total2 = parseFloat(total
													.toFixed(2))
													+ parseFloat($scope.joborder.total);
					...
					$scope
							.$watch(
									'joborder.joborderDtl[trIndex].currency',
									function(newValue, oldValue) {
										if (newValue != ''
												&& newValue != undefined) {
					...
					$scope
							.$watchCollection(
									'[joborder.joborderTrackingDtl[trIndex].sizeType,joborder.joborderDtl[trIndex].unit,joborder.joborderDtl[trIndex].conType]',
									function(newValue, oldValue) {
...
app.controller('allowCtrl', function($stateParams, $scope, $http, $location,
		logger, $state, $window, ngDialog) {
	debugger;
	...
	$scope.allowSaveJob = function(joborder) {
		...
		$http.post($stateParams.tenantid + '/app/jobOrderSea/save', joborder)
				.success(
						function(datas) {
	...
	$scope.allowUpdateJob = function(joborder) {
		$http.post($stateParams.tenantid + '/app/jobOrderSea/update',
				$scope.joborder).success(
				function(datas) {
					...
					if (datas.success == true) {
						...
						ngDialog.close();
						$location.url($stateParams.tenantid
								+ '/jobOrderSea/edit?rowid=' + datas.jobHdrId);
						$scope.getEdit();
