//define([ 'hospital/purchase/purchase' ], function(module) {
    ...
    'use strict';
    app.controller('purchaseOrderAddCtrl', function($scope, $state, $http, ngDialog, logger, $location, $controller, $injector, sharedProperties, toaster, $rootScope, $stateParams, $filter, validationService) {
...
//            if(1==1)
                {
                ngDialog.open({
                    template : 'views/hospital/purchase/purchaseOrder/PurchaseOrderBudgetView',
                    scope :$scope
                });
                }else{
...
//        END         
                ...
                if ($scope.purchaseOrder.vendorId != undefined && $scope.purchaseOrder.vendorId != null && $scope.purchaseOrder.vendorId != '') {
                    if ($scope.purchaseOrder.reqType != undefined && $scope.purchaseOrder.reqType != null && $scope.purchaseOrder.reqType != '') {
                        if ($scope.purchaseOrder.potype != undefined && $scope.purchaseOrder.potype != null && $scope.purchaseOrder.potype != '') {
                            ...
                            $scope.getCompanydropdowm($scope.purchaseOrder.companyId);
                            $scope.showPurchaseQuotationDetailDialog($scope, 0, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, purchaseOrder.purchaseOrderNum);
                        } else {
        ...
        $scope.callDialog = function($scope, requestCode, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope) {
            ...
            ngDialog.open({
                scope : $scope,
                template : 'views/hospital/purchase/purchaseOrder/PurchaseOrderRequestAdd',
        ...
        $scope.showPurchaseQuotationDetailDialog = function($scope, requestCode, $http, ngDialog, logger, $injector, sharedProperties, toaster, $rootScope, poNumber) {
            requestCode = angular.copy($scope.vendor, requestCode);
            ngDialog.open({
                scope : $scope,
                template : 'views/hospital/purchase/purchaseOrder/PurchaseQuotationRequestAdd',
...
//                    $scope.purchaseTypeList = response.purchaseTypeList;
    ...
    app.controller('purchaseOrderRequestAddCtrl', function($scope, $http, ngDialog, logger, requestCode, $injector, sharedProperties, toaster, $rootScope, validationService) {
