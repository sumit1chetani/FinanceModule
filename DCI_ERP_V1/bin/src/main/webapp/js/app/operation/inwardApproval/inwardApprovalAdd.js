...
app.controller('bladdingAddCtrl1Approval', function($scope, $rootScope, $http, $location, logger, ngDialog,
    	...
    	/*if($scope.fromdraft == "draft")
    		{
		   	 $state.go('app.documentation.bldraft.bldraftlist',{tenantid:$stateParams.tenantid});
    		   	...
    		   	// $state.go('app.documentation.inwardbladdingApproval.inWardlistApproval',{tenantid:$stateParams.tenantid});
$location.url($stateParams.tenantid+'/documentation/inwardBlApproval/list?tenantid='+$stateParams.tenantid);   
			...
			{	 
            	...
            	$scope.blNoData.shipperCode=$scope.blNoData.shipperCode1;
             	$scope.blNoData.consigneeCode=$scope.blNoData.consigneeCode;
					  ...
					  $http.post($stateParams.tenantid+'/api/inWard/updateApprove', $scope.blNoData).success(function(result) {
				        console.log("result" + result);
				        if (result.isSuccess) {
