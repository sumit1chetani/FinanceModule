...
app.controller('inwardbLDraftAddCtrl', function($scope, $rootScope, $http, $location, logger, ngDialog,
 ...
 $scope.rowCollectionFollowup=[];
    ...
    $scope.blNoData = {
	  ...
	  $scope.blNoData.removeCntrPckg=[];
		...
		//$scope.blNoData.issueDate = dd + '/' + mm + '/'+ yyyy;
		 ...
		 /*$scope.$watch('blNoData.carrier', function(newValue,
				...
				if (newValue != '' && newValue != undefined  ) {
					 ...
					 $http.get($stateParams.tenantid+'/api/inWard/bookingNumberlist?carrier='+newValue).success(function(datas) {
		...
		$scope.bookingNoList=[];
		/*$http.get($stateParams.tenantid+'/api/inWard/getBookingList').success(function(datas) {
		    ...
		    $scope.$watch('blNoData.bookingNo', function(newValue, oldValue) {
		    	console.log("bookingNo",newValue);
			      if(newValue!=null && newValue!=undefined && newValue != '' &&  $scope.isEdit == false){
			    	  $http.get($stateParams.tenantid+'/api/inWard/getJobList?bookingNo='+newValue).success(function(datas) {
			    	         $scope.jobList = datas;
				...
				{	 
			 	  ...
			 	  $http.post($stateParams.tenantid+'/api/inWard/create', $scope.blNoData).success(function(result) {
			          ...
			          if (result.isSuccess) {
			             ...
			             $state.go('app.documentation.inwardbldraft.inwardbldraftlist',{tenantid:$stateParams.tenantid});
			          } else {
