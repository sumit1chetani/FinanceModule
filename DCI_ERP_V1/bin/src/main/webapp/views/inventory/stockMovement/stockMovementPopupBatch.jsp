         						...
         						<thead class="dataTables-Main-Head">
									 ...
									 <th class="width_1 table-heading text-center">
							            <label class="i-checks m-b-none">
							             <input type="checkbox" ng-model="selectedAll" ng-change="checkBatchAll(selectedAll)">
							             <i></i>
							            </label>
							           </th>
							           ...
							           <th class="sorting text-center padding-both-side-2 width_8" >Available Qty</th>
							            <th class="sorting text-center padding-both-side-2 width_2" >Transfer Qty</th>
         					...
         					<tbody class="dataTables-Main-Body">
              					<tr data-ng-class="$index % 2 == 0? 'even' : 'odd'" ng-repeat="batchData in displayedCollectionBatch">
								   <td class="width_1 text-center"><label class="i-checks m-b-none"> 
								   	<input type="checkbox" ng-model="batchData.select"
										id="section{{trIndex}}"><i></i></label>
									</td>
					               <td>{{batchData.dtlItemName}}</td>
					               <td ng-if="itemBatch">{{batchData.lotNo}}</td>
					               <td>{{batchData.batchQty}}</td>
					               <td><input type="text" ng-pattern-restrict="^[0-9]*$"  ng-model="batchData.transferQty" ng-blur="checkQty(batchData)"></td>
					              <!-- 	<td ng-if="itemMrp">{{batchData.mrp}}</td>
