	...
	<tbody style="line-height: 5px;">
		<tr>
			<td class="fntSiz10px" colspan="13" rowspan="6"
				style="width: 57%; vertical-align: baseline; padding-top: 15px;"><b>Company:</b>
...
<%-- 			</c:if> --%>
     ...
     border-top: 1px solid; font-size: 9px; vertical-align: baseline; padding-top: 5px;">
				<c:forEach var="ob" items="${middleList}">
					<span style="font-weight: bold;line-height:13px"><c:out
							value=" ${ob.chargeTypeName}" /></span>
					<br>
...
<%-- 			<c:if test="${hdr.currency == 'AED'}">  --%>
			<td class=""
				...
				<c:if test="${ob.uom != 'PER BL'}"> 
						...
						<c:if test="${ob.tax > 0}">
							<c:set var="taxableamountBC"
								value="${taxableamountBC + (ob.amount)}" />
...
<c:set var="subTotalexchangeBC"
					...
					<br>
						...
						<c:if test="${ob.uom != 'PER BL'}"> 
					<c:set var="subTotalBC" value="${subTotalBC + ob.amount}" />
					</c:if>
					<c:if test="${ob.uom == 'PER BL'}"> 
					<c:set var="subTotalBC" value="${subTotalBC + ob.rate}" />
					</c:if>
				</c:forEach>
			</td>
<%-- </c:if> --%>
		...
		<%-- <tr>
			...
			<td class="fntSiz10px"
				...
				<fmt:formatNumber type="number" maxIntegerDigits="3"
					maxFractionDigits="2" value="${taxpercent}" />%
					</c:if>
					<c:if test="${hdr.showIgst == 'No'}">
					<fmt:formatNumber type="number" maxIntegerDigits="3"
					maxFractionDigits="2" value="${taxpercent/2}" />%
					</c:if>
					</c:if>
					<c:if test="${hdr.currency != 'INR'}">
					<fmt:formatNumber type="number" maxIntegerDigits="3"
 ...
 --%>
		...
		<%-- <tr>
			...
			<td class="fntSiz10px"
				...
				<c:if test="${hdr.currency == 'INR'}">
				<c:if test="${hdr.showIgst == 'Yes'}">
				<c:set var="taxAmountBC" value="${((taxpercent)/100)*taxableamountBC}" /> 
				</c:if>
				<c:if test="${hdr.showIgst == 'No'}">
				<c:set var="taxAmountBC" value="${((taxpercent/2)/100)*taxableamountBC}" /> 
				</c:if>
				</c:if>
				<c:if test="${hdr.currency != 'INR'}">
				<c:set var="taxAmountBC" value="${((taxpercent)/100)*taxableamountBC}" /> 
				...
				<fmt:formatNumber
					...
					value="${fn:substringBefore(taxAmountBC, '.')}" />
			</td>
			<td class="fntSiz10px"
				style="height: 25px; border-left: 1px dotted; border-top: 1px dotted; font-weight: bold; text-align: right;">
				${hdr.currency} 
				<c:if test="${hdr.currency == 'INR'}">
				<c:if test="${hdr.showIgst == 'Yes'}">
				<c:set var="taxAmount" value="${((taxpercent)/100)*taxableamount}" /> 
				</c:if>
				<c:if test="${hdr.showIgst == 'No'}">
				<c:set var="taxAmount" value="${((taxpercent/2)/100)*taxableamount}" /> 
				</c:if>
				</c:if>
				<c:if test="${hdr.currency != 'INR'}">
				<c:set var="taxAmount" value="${((taxpercent)/100)*taxableamount}" /> 
				...
				<fmt:formatNumber
					type="number" groupingUsed="true" maxFractionDigits="0"
					value="${fn:substringBefore(taxAmount, '.')}" />
			</td>
		</tr>
 --%>
		<%-- <c:if test="${hdr.currency == 'INR'}">
			<c:if test="${hdr.showIgst == 'No'}">
				<tr>
					...
					<td class="fntSiz10px"
						style="height: 25px; border-left: 1px dotted; border-top: 1px dotted; font-weight: bold; text-align: right;">
						USD <c:set var="taxAmountBC"
							value="${((taxpercent/2)/100)*taxableamountBC}" /> <fmt:formatNumber
							type="number" groupingUsed="true" 
							value="${taxAmountBC}" />
					</td>
					<td class="fntSiz10px"
						style="height: 25px; border-left: 1px dotted; border-top: 1px dotted; font-weight: bold; text-align: right;">
						${hdr.currency} <c:set var="taxAmount"
							value="${((taxpercent/2)/100)*taxableamount}" /> <fmt:formatNumber
							type="number" groupingUsed="true" 
							value="${taxAmount}" />
					</td>
				</tr>
		...
		<tr>
         ...
         groupingUsed = "false" value = "${subTotal}" /> --%>
			...
			<td class="fntSiz10px"
				style="height: 25px; border-left: 1px dotted; border-top: 1px dotted; font-weight: bold; text-align: right;">
				<c:set var="totaltaxAmountBC" value="${((taxpercent)/100)*taxableamountBC}" /> 
				<c:set var="finalAmtBC" value="${subTotalexchangeBC + totaltaxAmountBC}" /> <fmt:formatNumber
					type="number" groupingUsed="true" 
					value="${finalAmtBC}" />
	...
	<tbody style="line-height: 5px;">
		...
		<tr>
			<td class="fntSiz10px" colspan='3'
				...
				Name</td>
			<td class="fntSiz10px" colspan='6'
				style="height: 15px; border-top: 1px solid; border-right: 1px solid; text-align: center;font-size:10px;">${bank[0].benficiaryName}</td>
			<td class="fntSiz10px" colspan='5' rowspan='10'
				...
				<br> <br>
				<div style="line-height: 15px;font-size:10px;">For Cordelia Container
					Shipping Line, Cyprus</div>
			</td>
	...
	<tbody style="line-height: 5px;">
		...
		<tr>
			...
			<td class="fntSiz10px" colspan='5' rowspan='10'
				...
				<br> <br>
				<div style="line-height: 15px;">For Cordelia Container
					Shipping Line, Cyprus</div>
			</td>
	...
	<tbody style="line-height: 5px;">
		...
		<tr>
			...
			<td class="fntSiz10px" colspan='3'
				style="height: 15px;font-size:10px; border-top: 1px solid; border-right: 1px solid;"></td>
			<td class="fntSiz10px" colspan='6'
				style="height: 15px;font-size:10px; border-top: 1px solid; border-right: 1px solid; text-align: left;"><b>Account Details	
				</b></td>
			</tr>
...
<table class="width_95">
	<tr>
		<td class="normal">***This is a computer-generated invoice and does not require signature/seal*****</td>
