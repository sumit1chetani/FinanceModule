	    ...
	    <table id="dt_basic" class="table table-striped table-bordered table-hover dataTable no-footer" width="100%" role="grid" aria-describedby="dt_basic_info">
	     <thead class="dataTables-Main-Head">
	      <tr role="row">
	       ...
	       <th class="sorting text-center width_10" st-sort="">Date</th>
	       <th class="sorting text-center width_10" st-sort="">Vessel</th>
	       <th class="sorting text-center width_10" st-sort="">Voyage</th>
	       <th class="sorting text-center width_10" st-sort="">BL No.</th>
	       <th class="sorting text-center width_15" st-sort="">Payer</th>
	       <th class="sorting text-center width_15" st-sort="">Cust.</th>
	     ...
	     <tbody class="dataTables-Main-Body">
	      <tr data-ng-class="$index % 2 == 0? 'even' : 'odd'" ng-repeat="objListItem in displayedCollection">
	       ...
	       <td class="width_8">
	       		<span>
			     	<a ng-click="viewPHCInvoice(objListItem.phcinvoiceCode)">
			             <span tooltip="{{objListItem.phcinvoiceCode}}" class="tool-tip-span font-blue" ng-bind="objListItem.phcinvoiceCode"></span>
			        </a></span>		         
	       </td>
	       <td class="width_10" ng-bind="objListItem.phcinvoiceDate"></td>
	       <td class="width_10" ng-bind="objListItem.vesselName"></td>
	       
	        <td class="width_10" ng-bind="objListItem.voyage"></td>
	       
	       <td class="width_10" ng-bind="objListItem.blNo"></td>
	       <td class="width_15" ng-bind="objListItem.accountName"></td>
	       <td class="width_15" ng-bind="objListItem.mloName"></td>
	       <td class="width_15">
			    ...
			    <span class="cursor-pointer padding-left-10 font-blue"><i class="icon-envelope red text-success" data-ng-click="sendPHCInvoiceMail(objListItem.phcinvoiceCode,objListItem.printList)"></i></span>
			    <%-- </security:authorize> --%>
			    <%-- <security:authorize access="hasRole('${form_code}_${print}')"> --%> 
			    <span data-ng-click="printPHCInvoice(objListItem.phcinvoiceCode,objListItem.printList)"
					id="{{objListItem.phcinvoiceCode}}"
					class=" glyphicon glyphicon-print cursor-pointer font-grey padding-left-10"></span>
				<%-- </security:authorize> --%>			
				
				 <security:authorize access="hasRole('${form_code}_${delete}')">
					<span class="cursor-pointer  padding-left-10"> <i class="fa fa-trash-o text-danger-dker text" data-ng-click="deletePHCInvoice(objListItem.phcinvoiceCode,$index)"></i>
					</span>
				 </security:authorize>
			</td>
	       <%-- <td class="td-actions text-center">
	        ...
	        <span class="width_15">
	         <i class="fa fa-pencil text-success text" data-ng-click="editRowBtn(objListItem.phcinvoiceCode)" tooltip="Edit"></i>
	        </span>
	        </security:authorize>
	        <security:authorize access="hasRole('${form_code}_${delete}')">
	        <span class="width_15">
	         <i class="fa fa-trash-o text-danger-dker text" data-ng-click="deleteRow(objListItem.phcinvoiceCode,$index)" tooltip="Delete"></i>
	        </span>
	        </security:authorize>
	       </td> --%>
	      </tr>
