...
<div class="breadcrumb-wrapper ng-scope">
	<div class="panel panel-default panel-default-list"
		st-table="salesInvoiceList" st-safe-src="draftList">
					...
					<table class="table table-striped table-hover dataTable no-footer">
						...
						<tbody class="dataTables-Main-Body">
							<tr data-ng-class="$index % 2 == 0? 'even' : 'odd'"
								ng-repeat="purchase in salesInvoiceList">
								...
								<td class=" td-actions text-center">
									<span> <i
										...
										data-ng-click="saveDraft(purchase.salesSNO)"></i>
									</span>
									<security:authorize access="hasRole('${form_code}_${delete}')">
									<span> <i
										class="fa fa-trash-o text-danger-dker text" data-toggle="tooltip" title="Delete"
										data-ng-click="deleteInvoice(purchase.salesSNO)"></i>
									</span>
								</security:authorize>
	       </td>
...
<div class="panel panel-default panel-default-list"
		st-table="purchaseInvoiceList" st-safe-src="rowCollection">
			...
			<div class="panel panel-default">
				...
				<div class="table-responsive" style=" border: 1px solid #CCC;">
					<table class="table table-striped table-hover dataTable no-footer">
						...
						<tbody class="dataTables-Main-Body">
							<tr data-ng-class="$index % 2 == 0? 'even' : 'odd'"
								ng-repeat="InvoiceList in purchaseInvoiceList">

								<td><a ng-click="view(InvoiceList.salesSNO)"><security:authorize access="hasRole('${form_code}_${view}')"> <span
										tooltip="{{InvoiceList.salesSNO}}"
										class="tool-tip-span font-blue">{{InvoiceList.puchaseInvoiceNo}}</span></security:authorize>
										
								</a></td>
								<!-- <td class="sorting ">{{purchaseInvoiceList.puchaseInvoiceNo}}</td> -->
								<td class="sorting ">{{InvoiceList.puchaseInvoiceDate}}</td>
								<td class="sorting ">{{InvoiceList.jobName}}</td>
								<td class="sorting ">{{InvoiceList.party}}</td>
								<td class="sorting ">{{InvoiceList.aolName}}</td>
								<td class="sorting ">{{InvoiceList.aodName}}</td>
								<td class="sorting ">{{InvoiceList.totalAmount}}</td>
								<td class="sorting ">{{InvoiceList.createdBy}}</td>
								<td class="sorting ">{{InvoiceList.createdOn}}</td>
